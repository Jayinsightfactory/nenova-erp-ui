...
assert.ok(pageSource.includes('transform: showAbove ? \'translateY(-100%)\' : \'none\''), '검색 결과는 아래 공간이 부족하면 입력칸 위쪽으로 배치해야 한다.');
assert.ok(pageSource.includes('focusAction(`related-${index}-customer`)'), '수량 Enter는 동일업체 추가 버튼을 먼저 포커스해야 한다.');
