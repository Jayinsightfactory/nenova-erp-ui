...
(async () => {
  const { sortCustomerProducts, sortMyCustomersFirst, compactCustomerProductLabel, quantitiesMatch, buildForwardOrderWeeks, productAlphabetInitial } = await import('../lib/myCustomerOrderEntry.js');
  ...
  assert.match(page,/기존 주문 · 추가 입력 · 최종 수량/); assert.match(page,/placeholder="\+ 수량"/); assert.match(page,/최종 \$\{Number\(p\.CurrentQty\|\|0\)\+Number\(converted\|\|0\)\}/);
