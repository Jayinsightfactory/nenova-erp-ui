...
async function main() {
  ...
  const cells = read('components/raum/CombinedPurchaseCostCell.js');
  const pnlPage = read('pages/raum/pnl.js');
  const layout = read('components/Layout.js');
  assert.match(page, /라움·초이문 공통/, 'title/summary must state the shared-cost screen explicitly');
  assert.match(page, /공통 저장/);
  assert.match(page, /buildRaumPnlCombinedPurchaseCostMatrix/);
  assert.match(comparisonLib, /const shared = buildRaumPnlSharedPurchaseCostMatrix/, 'combined display retains the original two-partner matrix');
  assert.doesNotMatch(page, /changeScope|setPartnerCode|PNL_PARTNERS\)\.map/, 'partner toggle buttons/state must be removed');
  assert.match(page, /partner=raum/);
  assert.match(page, /partner=choimun/);
  assert.match(cells, /PartnerDetails label="라움"/);
  assert.match(cells, /PartnerDetails label="초이문"/, 'each cell must render separate Raum/Choimun sections');
  assert.match(cells, /단가 다름/);
  assert.match(cells, /costPrices/, 'mismatch cells visibly compare each partner stored cost');
  assert.match(cells, /저장 단가/);
  assert.match(cells, /맞추기 필요/);
  assert.match(cells, /동일 적용/);
