...
let html=render();assert.match(html,/농장 미지정 · 장미/);assert.match(html,/피드백 필요/);assert.doesNotMatch(html,/NEW ·|피드백 이슈로 처리/);
closed.cases[0].recentEvents=[{Body:'이전 코멘트'},{Body:'최신 농장 답변 확인'}];
html=render();assert.match(html,/title="최신 농장 답변 확인"/);assert.match(html,/<small class="latest-comment">최신 농장 답변 확인<\/small>/);assert.doesNotMatch(html,/>이전 코멘트</);
