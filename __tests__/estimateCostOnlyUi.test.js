...
async function main() {
  ...
  assert.match(applyAllEditsSrc, /additionalRows = pendingAdds/, '일반 품목추가는 기존 초안을 기본값으로 사용한다.');
  assert.match(applyAllEditsSrc, /const usesFixCycle = additionalRows\.length > 0/, '운임을 포함해 실제 추가 품목 확정 사이클 여부를 기준으로 구분해야 한다.');
