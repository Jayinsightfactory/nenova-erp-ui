...
assert.match(source, /const MAX_MESSAGES = 1000/);
assert.match(source, /sizeLimit: '1mb'/);
...
compiledModule.exports({ method: 'POST', headers: { origin: 'https://board.example', host: 'board.example' }, user: { accountActive: true }, body: { year: 2026, week: '37-01', from: '2026-09-10', to: '2026-09-11', messages: [{ identity: 'm5', message: '라움 화이트 2박스 추가', created_at: '2026-09-10T09:00:00+09:00' }] } }, res).then(async () => {
  ...
  assert.equal(maximumBatch.body.items.length, 1000);
  const oversizedBatch = cacheResponse();
  await compiledModule.exports(cacheRequest({ messages: [...maximumMessages, { identity: 'batch-1000', message: '라움 화이트 1단 추가' }] }), oversizedBatch);
  assert.equal(oversizedBatch.statusCode, 400, 'more than 1000 messages must be rejected instead of silently truncated');
