// __tests__/workReplay.test.js — 화면 기록(rrweb) 저장 규칙: 기록은 화이트리스트 계정만, 열람은 nenovaSS3 만, 파일만 씀.
...
const src = fs.readFileSync(path.join(cwd, 'lib', 'workReplay.js'), 'utf8')
  ...
  + '\nmodule.exports = { RECORD_USER_IDS, canRecord, canViewReplay, appendEvents, listSessions, readSession };';
...
// 기록 대상·열람 권한
assert.strictEqual(wr.canRecord(owner), true);
assert.strictEqual(wr.canRecord(staff), false, '화이트리스트 밖 계정은 기록 안 함');
assert.strictEqual(wr.canViewReplay(staff), false);

// 직원 계정의 추가 요청은 403, 파일도 안 생김
assert.strictEqual(wr.appendEvents(staff, 'sabcdefgh-1', [ev(4, 1)]).status, 403);
assert.strictEqual(fs.existsSync(path.join(tmp, 'data', 'replay')), false);

// sessionId 검증(경로 탈출 차단)
assert.strictEqual(wr.appendEvents(owner, '../../etc/passwd', [ev(4, 1)]).status, 400);
assert.strictEqual(wr.appendEvents(owner, 'sabcdefgh-1', []).status, 400);

// 정상 추가 + 단계 요약(route/click/input)
const t = 1789700000000;
assert.strictEqual(wr.appendEvents(owner, 'sabcdefgh-1', [
  ev(4, t), ev(2, t + 1),
...
assert.strictEqual(wr.appendEvents(owner, 'sabcdefgh-1', [ev(3, t + 9)]).ok, true);
...
const s = wr.readSession('nenovaSS3', 'sabcdefgh-1');
assert.strictEqual(s.events.length, 6);
assert.strictEqual(s.meta.steps[2].value, '37-1');
assert.strictEqual(wr.readSession('nenovaSS3', '../x'), null);
...
process.chdir(cwd);
console.log('workReplay tests passed: 화이트리스트 기록, 직원 403, 경로 탈출 차단, 단계 요약, 파일 전용 저장');
