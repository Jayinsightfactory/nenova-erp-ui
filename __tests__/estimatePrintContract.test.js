...
assert.match(pageSource, /displayShips = sortEstimateShipmentsForList\(\w+\)/,
  '좌측 업체 목록 렌더링 직전에 매출순 정렬을 적용해야 한다.');
// 담당자별 출력 모달과 좌측 목록이 같은 후보(최근 차수 필터 적용본)를 봐야 한다.
assert.match(pageSource, /sortEstimateShipmentsForList\(visibleShipments\)/,
  '좌측 목록은 visibleShipments(최근 차수 필터 적용)를 정렬해야 한다.');
