...
assert.match(automaticLiveHistoryRead,/messages\.slice\(0,LIVE_HISTORY_MESSAGE_LIMIT\)\.map/);assert.match(automaticLiveHistoryRead,/liveHistoryScopeEpoch/);assert.match(automaticLiveHistoryRead,/liveHistoryBatchKeyRef/);assert.match(automaticLiveHistoryRead,/30000/);
...
assert.match(ui,/label:inLiveRange\?'미확인':'대조 범위 밖'/);
...
otherMessageWarning.products[0].reasonCodes=['NO_SHIPMENT_HISTORY','INVALID_DISTRIBUTION_TOTAL'];
assert.deepEqual(comparisonForIdentity(otherMessageWarning,'chat-1',{includeConsistent:true}).visible[0].productReasonCodes,['INVALID_DISTRIBUTION_TOTAL'],'other messages request warnings must not leak into this request; shared stock/distribution warnings remain');
