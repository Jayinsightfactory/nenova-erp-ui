...
const poolSql = fs.readFileSync(path.join(root, 'docs', 'migrations', '2026-08-19_web_colombia_alloc_pool.sql'), 'utf8');
...
assert.match(poolSql, /SET XACT_ABORT ON;[\s\S]*BEGIN TRANSACTION;[\s\S]*COMMIT TRANSACTION;/i);
assert.match(poolSql, /IncludeHydrangea/);
