...
async function main() {
  ...
  const buffer = await buildRaumPnlWorkbook(records);
  ...
  const withHistory = await buildRaumPnlWorkbook([{
    ...records[0],
    items: records[0].items.map(item => ({
      ...item,
      history: [{ orderYear: 2025, major: 29, costPrice: 99999, partnerCode: 'choimun' }],
      historicalCosts: [111, 222],
    })),
  }]);
  const historicalWb = new ExcelJS.Workbook();
  await historicalWb.xlsx.load(withHistory);
  assert.deepEqual(
    historicalWb.worksheets.map(ws => ws.getSheetValues()),
    wb.worksheets.map(ws => ws.getSheetValues()),
    'historical comparison fields must not alter the export workbook',
  );
