...
function packingRowsForCell(review, cellKey) {
  const rows = (review?.packingRows || []).filter(row => row.mappingStatus === 'MATCHED');
  const exact = rows.filter(row => (row.distributions || []).some(item => String(item.cellKey) === String(cellKey)));
  if (exact.length) return { rows: exact, sameProductFallback: false };
  const productKey = String(cellKey || '').split(':')[1];
  const sameProduct = rows.filter(row => (row.distributions || []).some(item => String(item.cellKey || '').split(':')[1] === productKey));
  return { rows: sameProduct, sameProductFallback: sameProduct.length > 0 };
}

function InvoiceEditActions({ review, cellKey }) {
  const { rows, sameProductFallback } = packingRowsForCell(review, cellKey);
  if (!rows.length) return <button className="primary" onClick={() => tellOpener({ action: 'OPEN_CELL', cellKey })}>물량표에서 확인</button>;
  return <div className="invoice-edit-actions">{sameProductFallback && <small>같은 품목이 다른 업체에 배정된 박스 후보</small>}{rows.map(row => <div key={row.sourceRow}>
    <span>원본 {row.sourceRow}행 · 박스 {row.sourceBoxText || '번호 없음'} · {fmt(row.quantity)}</span>
    <button onClick={() => editMatching(row.sourceRow)}>매칭 수정</button>
    <button className="primary" onClick={() => editDistribution(row.sourceRow)}>박스 분배</button>
  </div>)}</div>;
}
