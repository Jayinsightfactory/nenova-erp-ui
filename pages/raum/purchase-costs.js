...
export default function RaumPurchaseCostsPage() {
  ...
  const [hotels, setHotels] = useState([{ code: 'shilla', label: '신라호텔' }]);
  const [hotelCode, setHotelCode] = useState('shilla');
  ...
  useEffect(() => {
    ...
    const requestedHotel = String(router.query.partner || '').trim().toLowerCase();
    if (requestedHotel && !['raum', 'choimun'].includes(requestedHotel)) setHotelCode(requestedHotel);
    setReady(true);
  }, [router.isReady, router.query.year, router.query.partner, ready]);

  useEffect(() => {
    if (!ready) return;
    let active = true;
    fetch('/api/raum/pnl-hotels').then(response => response.json()).then(result => {
      if (!active || result?.success !== true) return;
      const isolated = (result.partners || []).filter(partner => !['raum', 'choimun'].includes(partner.code));
      if (isolated.length) {
        setHotels(isolated);
        if (!isolated.some(partner => partner.code === hotelCode)) setHotelCode(isolated[0].code);
      }
    }).catch(() => {});
    return () => { active = false; };
  }, [ready, hotelCode]);

  const selectedHotel = hotels.find(hotel => hotel.code === hotelCode) || { code: hotelCode, label: hotelCode === 'shilla' ? '신라호텔' : '선택 호텔' };
  ...
  const load = useCallback(async () => {
    ...
    const loadShilla = async () => {
      try {
        const response = await fetch(`/api/raum/hotel-purchase-costs?year=${encodeURIComponent(orderYear)}&partner=${encodeURIComponent(hotelCode)}`);
        const result = await response.json();
        if (!response.ok || !result.success) throw new Error(result.error || `${selectedHotel.label} 매입단가를 불러오지 못했습니다.`);
        if (!checkCurrent()) return;
        ...
        if (checkCurrent()) {
          setShillaRows([]);
          setShillaState(current => ({ ...current, error: messageOf(cause, `${selectedHotel.label} 매입단가를 불러오지 못했습니다.`) }));
        }
      } finally {
  ...
  }, [ready, orderYear, hotelCode, selectedHotel.label]);
  ...
  const matrix = useMemo(() => buildRaumPnlCombinedPurchaseCostMatrix(sharedRows, shillaRows, { orderYear, partnerCode: hotelCode, partnerLabel: selectedHotel.label }), [sharedRows, shillaRows, orderYear, hotelCode, selectedHotel.label]);
  ...
  const changeYear = next => {
    if (busy) return;
    if (dirtyCount && !window.confirm(`저장하지 않은 공통 또는 ${selectedHotel.label} 단가 변경이 있습니다. 버리고 조회 연도를 바꿀까요?`)) return;
  ...
  const changeHotel = nextCode => {
    if (busy || nextCode === hotelCode) return;
    if (dirtyCount && !window.confirm(`저장하지 않은 공통 또는 ${selectedHotel.label} 단가 변경이 있습니다. 버리고 호텔을 바꿀까요?`)) return;
    requestSequence.current += 1;
    setHotelCode(nextCode);
    setShillaRows([]); setShillaDrafts({}); setShillaState({ loading: false, error: '' });
    setMessage('');
    router.replace({ pathname: router.pathname, query: { ...router.query, year: orderYear, partner: nextCode } }, undefined, { shallow: true });
  };
  ...
  const saveShilla = async () => {
    ...
    if (!entries.length || busy) return;
    if (invalidEntries(entries)) { setShillaState(current => ({ ...current, error: `${selectedHotel.label} 매입단가는 0 이상의 숫자만 입력할 수 있습니다.` })); return; }
    if (entries.some(entry => !String(entry.value ?? '').trim()) && !window.confirm('빈칸으로 저장한 품목은 해당 차수 손익 합계에서 제외됩니다. 계속할까요?')) return;
    setShillaSaving(true); setShillaState(current => ({ ...current, error: '' })); setMessage('');
    try {
      const response = await fetch('/api/raum/hotel-purchase-costs', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ orderYear, partnerCode: hotelCode, updates: entries.map(({ cell, value }) => ({ pnlKey: cell.pnlKey, major: cell.major, identity: cell.identity, expected: cell.snapshot, costPrice: String(value ?? '').trim() ? Number(String(value).replace(/,/g, '')) : null })) }),
      });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.error || `${selectedHotel.label} 매입단가 저장에 실패했습니다.`);
      setShillaRows(Array.isArray(result.shillaRows) ? result.shillaRows : (Array.isArray(result.rows) ? result.rows : []));
      setShillaDrafts({});
      setMessage(`${result.changedCells ?? entries.length}개 ${selectedHotel.label} 별도 단가를 저장했습니다.`);
    } catch (cause) { setShillaState(current => ({ ...current, error: messageOf(cause, `${selectedHotel.label} 매입단가 저장에 실패했습니다.`) })); } finally { setShillaSaving(false); }
  };
  ...
  return <div style={{ padding: 8, color: '#1e293b', fontFamily: 'Malgun Gothic, sans-serif', fontSize: 12 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginBottom: 6 }}>
      ...
      <button type="button" style={{ ...shillaPrimary, opacity: shillaDirtyCount && !busy ? 1 : .55 }} onClick={saveShilla} disabled={!shillaDirtyCount || busy}>{shillaSaving ? '저장 중…' : `${selectedHotel.label} 저장 (${shillaDirtyCount})`}</button>
    </div>
    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }} aria-label="개별 호텔 선택">
      {hotels.map(hotel => <button key={hotel.code} type="button" disabled={busy} onClick={() => changeHotel(hotel.code)} style={{ ...btn, height: 25, background: hotel.code === hotelCode ? '#0f766e' : '#fff', color: hotel.code === hotelCode ? '#fff' : '#334155', borderColor: hotel.code === hotelCode ? '#0f766e' : '#cbd5e1', fontWeight: hotel.code === hotelCode ? 700 : 400 }}>{hotel.label}</button>)}
    </div>
    <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', minHeight: 26, padding: '4px 7px', marginBottom: 6, background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 4 }}>
      <b>{orderYear}년 · {PNL_PARTNERS.raum.label}+{PNL_PARTNERS.choimun.label} 공통 / {selectedHotel.label} 별도</b><span>대차수 {matrix.weeks.length}개</span><span>품목 {matrix.items.length}개</span>
      ...
      <span style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}><a href={`/raum/pnl?partner=raum&year=${encodeURIComponent(orderYear)}`} style={{ color: '#1d4ed8', fontWeight: 700 }}>← 라움</a><a href={`/raum/pnl?partner=choimun&year=${encodeURIComponent(orderYear)}`} style={{ color: '#1d4ed8', fontWeight: 700 }}>← 초이문</a><a href={`/raum/pnl?partner=${encodeURIComponent(hotelCode)}&year=${encodeURIComponent(orderYear)}`} style={{ color: '#0f766e', fontWeight: 700 }}>← {selectedHotel.label}</a></span>
    </div>
    ...
    {shillaState.error ? <div role="alert" style={{ padding: '6px 8px', marginBottom: 5, background: '#fef2f2', border: '1px solid #fecaca', color: '#b91c1c', borderRadius: 4 }}>{selectedHotel.label} 조회/저장 실패: {shillaState.error}</div> : null}
    {matrixConflicts.length ? <div role="alert" style={{ padding: '6px 8px', marginBottom: 5, background: '#fff7ed', border: '1px solid #fdba74', color: '#9a3412', borderRadius: 4 }}>{selectedHotel.label} 중복 차수는 안전하게 제외되어 저장할 수 없습니다. {matrixConflicts.join(' · ')}</div> : null}
