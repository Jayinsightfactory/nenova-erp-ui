// pages/sales/customs-clearance.js — 그외통관비 입력 (팝업 전용, 사이드바 없음)
// 국가별(백상창고료/관세/선율/월드운송료/한국방역) + 콜롬비아 4품목 무게배분 공유값(그외통관비+포워딩 공용) 입력.
// H(그외통관비) 자동값의 소스 — 주차별 매출이익 보고서에서 [📦 그외통관비 입력]으로 연결.
...
// 첫 조회가 나가는 경쟁상태가 생김(2026-07-10 발견). SSR 안전(NO_LAYOUT 클라이언트 전용 페이지).
...
export default function CustomsClearancePage() {
  ...
  const [countryEdits, setCountryEdits] = useState({});     // category -> {field: value}
  const [colombiaEdits, setColombiaEdits] = useState({});   // orderWeek -> {field: value}
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/sales/customs-clearance?week=${encodeURIComponent(weekInput.value)}`, { credentials: 'same-origin' });
  ...
  const saveCountry = async (row) => {
    ...
    try {
      ...
      const r = await fetch('/api/sales/customs-clearance', {
        ...
        body: JSON.stringify({ week: weekInput.value, action: 'saveCountry', category: row.category, row: out }),
      });
      const d = await r.json();
  ...
  const saveColombia = async (c) => {
    ...
    try {
      ...
      const r = await fetch('/api/sales/customs-clearance', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin',
        body: JSON.stringify({ week: weekInput.value, action: 'saveColombia', orderWeek: c.orderWeek, row: out }),
      });
      const d = await r.json();
      if (!d.success) throw new Error(d.error);
      setMessage(`${c.orderWeek} 콜롬비아 배분값 저장 완료`);
      await load();
    } catch (e) { setError(e.message); } finally { setSaving(''); }
  };
  ...
  const saveRates = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/sales/customs-clearance', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin',
        body: JSON.stringify({ week: weekInput.value, action: 'saveRates', rates: merged }),
      });
      const d = await r.json();
      if (!d.success) throw new Error(d.error);
  ...
  return (
    <>
      <Head><title>📦 그외통관비 입력 - nenova ERP</title></Head>
      <div style={st.page}>
        <div style={st.bar}>
          <h1 style={st.h1}>📦 그외통관비 입력{data ? ` — ${data.major}차 (${data.orderYear})` : ''}</h1>
        ...
        <div style={st.hint}>
          ...
          저장값 없으면 <b style={{ color: '#e65100' }}>전차수 값</b>이 기본으로 채워집니다 — 확인 후 저장하세요.
        </div>
                ...
                return (
                  <div key={c.orderWeek} style={{ padding: 12, borderBottom: '1px solid #eef2f7', background: carried ? '#fff7ed' : '#fff' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      ...
                      <button style={{ ...st.tinyBtn, marginLeft: 'auto' }} onClick={() => saveColombia(c)} disabled={saving === c.orderWeek}>
