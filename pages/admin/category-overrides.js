// pages/admin/category-overrides.js — 운송기준원가 카테고리 웹 오버라이드 관리
// DB(Product.FlowerName)는 건드리지 않고 웹에서만 카테고리 재분류하는 항목 전용 관리
...
export default function CategoryOverridesAdmin() {
  ...
  const addOverride = async () => {
    if (!selectedProd || !newCategory.trim()) { alert('품목 + 카테고리를 선택/입력하세요.'); return; }
  ...
  return (
    <Layout title="카테고리 오버라이드 관리">
      <Head><title>카테고리 오버라이드 관리</title></Head>
      <div style={{ padding: '16px 20px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#1a237e' }}>🌐 카테고리 오버라이드 관리</h2>
          <span style={{ fontSize: 11, color: '#888' }}>
            (웹 전용, Product.FlowerName 미변경) · 현재 {list.length}개
          </span>
        ...
        <div style={{ border: '1px solid #c5cae9', borderRadius: 6, padding: 12, marginBottom: 14, background: '#f3f4ff' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#1a237e', marginBottom: 8 }}>➕ 새 오버라이드 추가</div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <input type="text" placeholder="카테고리 (예: 줄맨드라미, 기타-녹색)" value={newCategory}
        ...
        <div style={{ marginBottom: 8 }}>
          <input type="text" placeholder="목록 검색 (품목명/카테고리/DB값)"
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ background: '#e8eaf6' }}>
                  ...
                  <th style={{ ...th, background: '#c5e1a5' }}>오버라이드</th>
                  <th style={th}>메모</th>
                  <th style={th}>저장일시</th>
                  <th style={th}>동작</th>
                </tr>
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr><td colSpan={9} style={{ padding: 30, textAlign: 'center', color: '#888' }}>
                    오버라이드 없음. 위에서 추가하거나 운송기준원가 페이지 카테고리 셀 클릭으로 등록하세요.
                  </td></tr>
                )}
                {filtered.map(x => (
