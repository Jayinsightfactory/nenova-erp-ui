...
function requestMessages(body) {
  if (!body || typeof body !== 'object' || Array.isArray(body) || !Array.isArray(body.messages) || body.messages.length > MAX_MESSAGES || !body.messages.every(validMessage)) {
    throw new TypeError('같은 차수 원문이 1,000건을 초과하거나 형식이 잘못되어 전체 대조하지 못했습니다. 날짜 범위를 줄여 주세요. 일부만 잘라 완료 판정하지 않습니다.');
  }
  const characters = body.messages.reduce((sum, message) => sum + message.message.length, 0);
  if (characters > MAX_MESSAGE_CHARS || JSON.stringify(body).length > 900000) throw new TypeError('원문 용량을 초과해 전체 대조하지 못했습니다. 날짜 범위를 줄여 주세요. (최대 200,000자)');
...
function loadScopedFacts(scope) {
  ...
  const balanceRead = loadDistributionRequestBalanceFacts(queryLiveHistory, sql, scope)
    .then(balanceFacts => ({ balanceFacts, balanceUnavailable: false }))
    .catch(() => ({ balanceFacts: null, balanceUnavailable: true }));
  entry.promise = Promise.all([loadLiveHistoryFacts(queryLiveHistory, sql, scope), balanceRead])
    .then(([facts, { balanceFacts, balanceUnavailable }]) => {
      // A supplemental comparison must not hide the pre-existing live history.
      // Do not retain this partial response: the next call can retry balance facts.
      entry.resolvedAt = Date.now();
      const result = { facts, balanceFacts, balanceUnavailable, asOf: new Date(entry.resolvedAt).toISOString() };
      if (balanceUnavailable && factsByScope.get(cacheKey) === entry) factsByScope.delete(cacheKey);
      return result;
    })
    .catch(error => {
...
export default withAuth(async function handler(req, res) {
  ...
  const warnings = [
    ...
    ...(items.some(item => item.requests.some(request => request.status === 'PRODUCT_HISTORY_CANDIDATE')) ? ['정확 품목 별칭이 없어도 같은 업체에서 요청 이후 같은 수량·방향으로 변경된 분배 이력이 한 품목뿐이면 확인 후보로 표시하며 적용 완료로 판정하지 않습니다.'] : []),
