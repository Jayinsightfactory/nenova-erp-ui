...
export default withAuth(async function handler(req, res) {
  ...
  try {
    ...
    if (customerResult.status === 'rejected') warnings.push('거래처 주문코드를 불러오지 못했습니다.');
    const rows = enrichPivotExeRows(
      ...
      customerResult.status === 'fulfilled' ? customerResult.value.recordset : [],
    );
    return res.status(200).json({ success: true, rows, range, warnings, source: 'nenova.exe FormQuantityPivot + 웹 분배단가/도착원가/거래처 주문코드' });
  } catch (err) {
