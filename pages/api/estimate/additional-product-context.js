...
async function handler(req, res) {
  ...
  try {
    ...
    const prices = await query(
      `SELECT TOP 20 sd.SdetailKey, c.CustName, sm.OrderYear, sm.OrderWeek,
    ...
    const officialSource = official.recordset[0]
      ? [{
          id: 'CUSTPROD',
          kind: 'CUSTPROD',
          customerName: official.recordset[0].CustName,
          year: year,
          week: '업체단가',
          shipmentDate: '',
          cost: Number(official.recordset[0].Cost),
          vatIncluded: true,
          customerPriority: 2,
          selectedCount: 99,
        }]
      : [];
    const historical = rankReferenceCosts(prices.recordset).map(r => ({
      id: `SDETAIL:${r.SdetailKey}`,
      kind: 'SDETAIL',
      ...
      cost: Number(r.Cost),
      vatIncluded: true,
      customerPriority: Number(r.CustomerPriority),
      selectedCount: Number(r.SelectedCount),
    }));
    return res.json({
      success: true,
      year,
