...
export default withAuth(async function handler(req,res) {
  ...
  const cursor=String(req.query.afterId??'0');
  if(!/^\d{1,15}$/.test(cursor)) return res.status(400).json({error:'대화 조회 위치가 올바르지 않습니다.'});
  const url=new URL('https://mindmap-viewer-production-adb2.up.railway.app/api/kakao/nenova-sales-feed');
  Object.entries({...bounds,afterId:cursor,limit:'200'}).forEach(([k,v])=>url.searchParams.set(k,v));
  ...
  try {
    ...
    if(data.ok!==true||!Array.isArray(data.messages)||data.messages.length>200||typeof data.hasMore!=='boolean') throw new Error('contract');
    if(data.messages.some(r=>r.chatroom!=='영업방'||r.chat_id!==roomId||r.source!=='nenovakakao'||!r.external_message_id)) throw new Error('scope');
    return res.json({ok:true,messages:data.messages,hasMore:data.hasMore,nextAfterId:data.nextAfterId});
