...
export default function MyCustomerOrders() {
  ...
  return <>
    ...
    <main className={`my-order-page${focusMode?' is-focus':''}`}>
      ...
      <div className="title-row"><div><h1>내 업체 주문등록</h1><p>차수와 업체를 선택하면 품종·품목 입력 화면만 표시됩니다.</p><nav className="entry-tabs" aria-label="주문 입력 방식"><span aria-current="page">수량 직접 입력</span><Link href="/orders/sales-paste">붙여서 주문등록</Link></nav></div><button onClick={()=>load({confirmDraft:true})} disabled={busy||submitting}>최신 다시불러오기</button></div>
