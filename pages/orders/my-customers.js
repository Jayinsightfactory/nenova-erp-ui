...
import { customerOrderHighlight, sortCustomerProductsAlphabetically, buildForwardOrderWeeks, compactCustomerProductLabel, productAlphabetInitial, sortMyCustomersFirst, customerOrderDraft, customerOrderApprovalFingerprint } from '../../lib/myCustomerOrderEntry';
...
function ProductName({ name }) {
  ...
  return parts.map((part, index) => <Fragment key={index}><span data-product-name-part style={{display:'inline-block',maxWidth:'100%',whiteSpace:'normal',overflowWrap:'anywhere',verticalAlign:'top'}}>{part.trim()}{index < parts.length - 1 ? ' /' : ''}</span>{index < parts.length - 1 && <>{' '}<wbr/></>}</Fragment>);
}
...
export default function MyCustomerOrders() {
  ...
  const productGroups = useMemo(() => {
    ...
    return [...groups].map(([flowerName, groupProducts]) => ({ flowerName, products: sortCustomerProductsAlphabetically(groupProducts) }));
  }, [products]);
  ...
  const jumpToFlower = flowerName => {
    ...
    setTimeout(() => groupRefs.current[flowerName]?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'start' }), 0);
  };
  ...
  return <>
    ...
    <main className="my-order-page">
      ...
      <aside className="side-tools"><div className="live-order" aria-live="polite"><strong>주문 품목 {enteredRows.length}개</strong><p>기존 주문 포함 · 우측 수량은 최종 주문수량</p><small>추가 입력 품목 {changed.length}개 · 최종 수정 {Object.keys(finalQty).length}개</small>{enteredRows.length>0?<div>{enteredRows.map(p=><div className={`live-order-item order-${customerOrderHighlight(draftFor(p))}`} key={p.ProdKey}>
        <span className="live-order-name" title={label(p)}><strong><ProductName name={displayLabel(p)}/></strong><small>{({existing:'기존 주문',changed:'수량 변경',new:'신규 추가',invalid:'입력 확인',none:''})[customerOrderHighlight(draftFor(p))]} · 기존 {Number(p.CurrentQty||0)}{p.OutUnit}{draftFor(p).touched ? ` → 최종 ${draftFor(p).finalQty ?? '확인필요'}${p.OutUnit}` : ''}</small></span>
...
{showExecutionLog&&<div className="execution-backdrop"><section className="execution-log" role="dialog" aria-modal="true" aria-label="주문 실행 로그">
  ...
  {executionLog.status==='awaiting-approval'&&<div className="approval-actions"><p>표에 있는 품목만 반영합니다. 승인 전에는 저장하지 않습니다.<br/>출고 연결·동시 수정 여부는 등록 시 서버에서 다시 확인합니다.</p><button type="button" className="approve-order" disabled={submitting||needsReload||loadedScope!==scopeKey||executionLog.scope!==scopeKey} onClick={()=>submit(executionLog.mode,executionLog.fingerprint)}>승인하고 주문등록 시작 ({executionLog.rows.length}개)</button></div>}
    ...
    <style jsx>{`
      ...
      .my-order-page{display:grid;grid-template-columns:224px minmax(0,1fr);gap:8px;align-items:start;padding-top:32px}
      .title-row{grid-column:1/-1;grid-row:1}.pinned-favorites{grid-column:1/-1;grid-row:2;position:static}.templates{grid-column:1/-1;grid-row:3}
      .filters{grid-column:1;grid-row:4;min-width:0;max-height:calc(100vh - 190px);overflow:auto;box-sizing:border-box;border:1px solid #dce3ec}
      .filters .choice-buttons{flex-direction:column;flex-wrap:nowrap;max-height:210px;overflow:auto;padding:3px}
      .filters .choice-buttons button{flex:none;width:100%;text-align:left;min-height:29px}
      .filters .customers{max-height:360px}.filters .customer-tools{flex-direction:column}.filters .customer-tools input{min-width:0;width:100%;box-sizing:border-box}.filters .pick-group>small{font-size:11px}.filters .customers button span{flex-wrap:wrap}
      .entry-layout{grid-column:2;grid-row:4;min-width:0;grid-template-columns:minmax(0,1fr) 300px}.submit{grid-column:1/-1;grid-row:5}
      .product-scroll{max-height:none;min-height:0;overflow:hidden}.board-guide{font-size:12px;color:#475467;padding:5px 8px;background:#f8fafc}
      .product-list{display:flex;gap:8px;overflow-x:auto;overflow-y:hidden;align-items:flex-start;padding:4px 4px 12px;scrollbar-width:auto;scroll-snap-type:x proximity}
      .product-list:focus-visible{outline:2px solid #155eef;outline-offset:-2px}
      .product-list .flower-group{flex:0 0 410px;min-width:0;margin:0;scroll-snap-align:start;box-sizing:border-box}
      .product-list .flower-products{display:flex;flex-direction:column;align-items:stretch;max-height:calc(100vh - 460px);min-height:120px;overflow-y:auto;overflow-x:hidden}
      .product-list .flower-toggle{position:static;min-height:35px}.product-list .product-row{flex:none;min-height:35px}.side-tools{top:8px;margin-top:0}
      .product-filter input{min-width:0}.board-guide{white-space:normal}
      @media(max-width:1250px){.entry-layout{grid-template-columns:minmax(0,1fr) 270px}.my-order-page{grid-template-columns:190px minmax(0,1fr)}.side-tools{position:static;order:initial}.live-order>div{display:flex}.live-order{max-height:none}}
      @media(max-width:950px){.my-order-page{grid-template-columns:170px minmax(0,1fr)}.entry-layout{grid-template-columns:minmax(0,1fr)}.side-tools{max-height:360px;overflow:auto}.product-list .flower-products{max-height:480px}}
      @media(max-width:600px){.my-order-page{display:flex;flex-direction:column}.title-row,.filters,.entry-layout,.templates,.pinned-favorites{width:100%;box-sizing:border-box}.filters{max-height:280px}.filters .choice-buttons{max-height:120px}.entry-layout{display:grid}.product-list .flower-group{flex-basis:min(410px,100%)}.product-list .product-row{grid-template-columns:minmax(0,1fr) max-content 48px 48px max-content;gap:2px;padding:3px}.product-list .flower-toggle span{flex-direction:row;align-items:baseline}}
      .product-row.has-existing,.product-row.has-existing:hover{background:#ecfdf3;border-color:#75d5a5;box-shadow:inset 4px 0 #12b76a}
      .product-row .current-positive{color:#05603a;font-weight:800;background:#d1fadf;border-radius:4px;padding:2px 3px}
      .product-row.has-input,.product-row.has-input:hover{background:#eff8ff;border-color:#2e90fa;box-shadow:inset 4px 0 #1570ef}
      .product-row.order-existing,.product-row.order-existing:hover,.live-order-item.order-existing{background:#ecfdf3;border-color:#75d5a5;box-shadow:inset 4px 0 #12b76a}
      .product-row.order-changed,.product-row.order-changed:hover,.live-order-item.order-changed{background:#fff4e5;border-color:#f79009;box-shadow:inset 4px 0 #f79009}
      .product-row.order-new,.product-row.order-new:hover,.live-order-item.order-new{background:#eff8ff;border-color:#2e90fa;box-shadow:inset 4px 0 #1570ef}
      .product-row.order-invalid,.product-row.order-invalid:hover,.live-order-item.order-invalid{background:#fef3f2;border-color:#f04438;box-shadow:inset 4px 0 #f04438}
      .board-guide span{display:inline-block;border-radius:4px;padding:2px 6px;margin-left:5px;font-weight:700}.legend-existing{background:#d1fadf;color:#05603a}.legend-changed{background:#ffead5;color:#93370d}.legend-new{background:#d1e9ff;color:#175cd3}
    `}</style>
  </>;
}
