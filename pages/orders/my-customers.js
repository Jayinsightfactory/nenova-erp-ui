...
export default function MyCustomerOrders() {
  ...
  return <>
    ...
    <main className="my-order-page">
      <div className="title-row"><div><h1>내 업체 주문등록</h1><p>차수와 업체를 선택하면 아래에서 바로 품목과 수량을 입력합니다.</p><nav className="entry-tabs" aria-label="주문 입력 방식"><span aria-current="page">수량 직접 입력</span><Link href="/orders/sales-paste">붙여서 주문등록</Link></nav></div><div className="page-actions"><button onClick={loadTemplates} disabled={busy||templateBusy||submitting}>{templateBusy?'불러오는 중':'고정주문 불러오기'}</button><button onClick={()=>load({confirmDraft:true})} disabled={busy||submitting}>최신 다시불러오기</button><button type="button" onClick={()=>setShowExecutionLog(true)}>실행 로그</button></div></div>
      ...
      {searchRows.length>0 && <div className="results">{searchRows.map(p=><button key={p.ProdKey} onClick={()=>addProduct(p)}><b>{groupLabel(p)}</b> {displayLabel(p)} <span>{p.CounName} · {p.OutUnit}</span></button>)}</div>}
      ...
      <div ref={productAreaRef} className="grid-head"><span>품종 / 품목</span><span>현재 주문</span><span>입력수량</span><span>단위</span><span>입력 후</span></div>
