...
export default function PasteOrderPage() {
  ...
  const buildStockNotePayload = (baseWeek) => ({
    baseWeek,
    orderWeek: week,
    pasteText,
    baseStockText,
  ...
  const stockNoteName = (baseWeek, mode = 'save') => {
    const now = new Date();
    const hh = String(now.getHours()).padStart(2, '0');
    const mm = String(now.getMinutes()).padStart(2, '0');
    return mode === 'add'
      ? `기존재고 ${formatWeekDisplay(baseWeek)} ${hh}:${mm}`
      : `기존재고 ${formatWeekDisplay(baseWeek)}`;
  };

  const saveStockNote = async ({ forceCreate = false } = {}) => {
  ...
  const deleteStockNote = async () => {
    const baseWeek = stockBaseWeek || week;
    const existingNote = savedStockNote?.data?.baseWeek === baseWeek ? savedStockNote : null;
    if (!existingNote?.FavoriteKey) {
      alert('삭제할 기존재고 저장본이 없습니다.');
      return;
    }
    if (!confirm(`${formatWeekDisplay(baseWeek)} 기존재고 저장본을 삭제하시겠습니까?\n입력창의 내용은 지우지 않고, 저장본만 삭제합니다.`)) return;
    setStockNoteSaving(true);
    try {
      await apiDelete('/api/favorites', { favoriteKey: existingNote.FavoriteKey });
      setSavedStockNote(null);
      const next = await loadStockNote(baseWeek, { apply: false });
      setStockNoteStatus(next ? `삭제 완료: ${formatWeekDisplay(baseWeek)} 이전 저장본 있음` : `삭제 완료: ${formatWeekDisplay(baseWeek)}`);
    } catch (e) {
      alert(`기존재고 삭제 실패: ${e.message}`);
    } finally {
      setStockNoteSaving(false);
    }
  };
  ...
  const handleAllMixedDistribute = async () => {
    ...
    try {
      ...
      if (!response.ok || result.success !== true || result.verified !== true) {
        ...
        rollbackError.failedIndex = Number.isInteger(result.failedIndex)
          ...
          : Number.isInteger(result.failedEntry?.inputIndex)
            ? result.failedEntry.inputIndex
            : result.failedEntry?.executionIndex;
        rollbackError.rolledBack = result.rolledBack !== false;
        throw rollbackError;
      }

      const serverResults = Array.isArray(result.results) ? result.results : [];
      const serverResultsByEntryId = new Map(serverResults.filter(row => row?.entryId).map(row => [row.entryId, row]));
      const serverResultsByInputIndex = new Map(serverResults.filter(row => Number.isInteger(row?.inputIndex)).map(row => [row.inputIndex, row]));
      ...
      const shipUpdates = {};
      details.filter(x => Number.isFinite(Number(x.outQtyAfter))).forEach(x => {
  ...
  const currentStockNote = savedStockNote?.data?.baseWeek === activeStockBaseWeek ? savedStockNote : null;
  const hasStockNoteText = !!(pasteText.trim() || baseStockText.trim() || remainStockText.trim());
                ...
                {[
                  ...
                  return <section key={group.type} style={{ border: `1px solid ${group.color}66`, borderRadius: 8, overflow: 'hidden', background: '#fff' }}>
                    ...
                    {rows.map((row, idx) => <div key={`${row.orderId}-${row.prodKey}-${idx}`} style={{ display: 'grid', gridTemplateColumns: 'minmax(95px,.7fr) minmax(130px,1.3fr) 80px 95px', gap: 6, padding: '5px 8px', borderTop: '1px solid #eee', fontSize: 11, alignItems: 'center' }}>
                      ...
                      <span style={{ color: row.ok ? '#2e7d32' : '#c62828', fontWeight: 800 }}>{row.ok ? '처리완료' : '전체 롤백'}</span>
