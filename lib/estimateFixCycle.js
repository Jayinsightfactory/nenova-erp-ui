...
/** 확정 차수 저장 로그 — 사용자가 직접 확정취소할 필요가 없음을 분명히 한다. */
export function formatAutoUnfixSaveLog(weeks, extra = '') {
  const weekLabel = sortWeeksAsc(weeks).join(', ') || '해당 차수';
  return `확정된 차수(${weekLabel})라 자동으로 해제 후 다시 저장합니다.${extra}`;
}
