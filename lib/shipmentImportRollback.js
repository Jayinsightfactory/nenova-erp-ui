...
export async function listShipmentImportHistory({ limit = 30 } = {}) {
  ...
  const result = await query(
    `SELECT TOP (@limit) a.AuditKey,a.CreatedDtm,a.CompletedDtm,a.ActorUserId,a.ActorName,a.SourceFileName,a.OrderYear,a.OrderWeek,
...
export async function getShipmentImportAuditRows({ auditKey, prodKey = null } = {}) {
  await assertShipmentImportSnapshotSchema(query);
  const params = { auditKey: pInt(auditKey) };
  let prodFilter = '';
  if (prodKey != null && prodKey !== '') {
    params.prodKey = pInt(prodKey);
    prodFilter = ' AND r.ProdKey=@prodKey';
  }
  const result = await query(
    `SELECT r.AuditRowKey,r.AuditKey,r.RowNo,r.CustKey,r.ProdKey,r.CustomerName,r.ProductName,
            r.SourceType,r.SourceCells,r.ExcelQty,r.NormalizedQty,
            r.OrderBeforeQty,r.OrderAfterQty,r.ShipmentBeforeQty,r.ShipmentAfterQty,
            r.OrderAction,r.ShipmentAction,r.RowStatus,r.VerificationStatus,r.VerificationReason
       FROM ShipmentImportAuditRow r
      WHERE r.AuditKey=@auditKey${prodFilter}
      ORDER BY r.RowNo,r.AuditRowKey`,
    params,
  );
  return result.recordset || [];
}
