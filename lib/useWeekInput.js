...
// 주차(WW) 단위 이동 — 시퀀스 고정, 주차만 ±1 (15-01 → 16-01)
function stepWeekBig(current, direction) {
  ...
  if (week < 1) week = 1;
  return `${String(week).padStart(2, '0')}-${seqStr}`;
}
...
export function WeekSpinInput({ weekInput, style, label }) {
  ...
  // 주차(앞단위) 스핀
  const spinWeekUp   = () => { const w = Math.max(1, (parseInt(week)||1)+1); weekInput.setValue(`${String(w).padStart(2,'0')}-${seq}`); };
  const spinWeekDown = () => { const w = Math.max(1, (parseInt(week)||1)-1); weekInput.setValue(`${String(w).padStart(2,'0')}-${seq}`); };

  // 차수(뒷단위) 스핀 — 01~04 순환, 04 → 01 (주차는 고정)
  const spinSeqUp = () => {
    const s = (parseInt(seq)||1) % MAX_SEQ + 1;
    weekInput.setValue(`${week}-${String(s).padStart(2,'0')}`);
  };
  const spinSeqDown = () => {
    let s = (parseInt(seq)||1) - 1;
    if (s < 1) s = MAX_SEQ;
    weekInput.setValue(`${week}-${String(s).padStart(2,'0')}`);
  };
