...
// 기본 연도 (새 입력의 기준 연도) — 시스템 현재 연도 기준 동적 계산
// (이전 하드코딩 2026 → 매년 1월 1일 자동 갱신, 코드 수정 불필요)
const DEFAULT_YEAR = new Date().getFullYear();

// ── 내부 파싱: "WW-SS" 또는 "YYYY-WW-SS" 모두 처리
// WW-SS 2-part 형식은 연도 정보가 없으므로 현재 연도로 가정 (DB OrderYear 와 별도로 사용)
// 실제 구 데이터는 OrderYear 컬럼으로 구분 — 페이지에서 OrderYear+OrderWeek 합쳐서 'YYYY-WW-SS' 로 정규화 권장
function parseWeekParts(val) {
  ...
  if (parts.length === 3) {
    ...
    if (week >= 1 && week <= 52 && seq >= 1 && seq <= 9)
      return { year: DEFAULT_YEAR, week, seq, isOld: true }; // 연도 미지정 → 현재 연도
  }
  return null;
}
...
export function useWeekInput(initial, { deferDefault = false } = {}) {
  // SSR 페이지는 서버와 브라우저의 시간대가 다를 수 있으므로 첫 렌더에서
  // 현재 차수를 계산하지 않고, mount 뒤 화면이 명시적으로 적용할 수 있다.
  const defaultVal = deferDefault
    ? String(initial || '')
    : ((initial === undefined || initial === '') ? getCurrentWeek() : initial);
  ...
  return {
    ...
    props: {
      ...
      onBlur: handleBlur,
      placeholder: deferDefault ? 'YYYY-WW-SS' : getCurrentWeek(),
    },
