...
WITH ShipmentList AS (
  ...
  SELECT sm.OrderYearWeek,
    ...
    LEFT JOIN PeriodDay pd ON e.EstimateDtm = pd.BaseYmd
    LEFT JOIN CodeInfo ci ON e.EstimateType = ci.DetailCode AND ci.Category = N'EstimateType'
    JOIN Customer c ON sm.CustKey = c.CustKey
   WHERE sm.OrderYearWeek = @orderYearWeek
     ${custFilter}
      AND (pd.WeekDay IN (${weekDayIn}) OR pd.WeekDay IS NULL)
)
SELECT sl.OrderYearWeek,
 ...
 GROUP BY sl.OrderYearWeek, sl.CustKey, sl.CustName, sl.Manager, sl.MinShipmentKey
 ORDER BY CASE WHEN NULLIF(LTRIM(RTRIM(sl.Manager)), N'') IS NULL THEN 1 ELSE 0 END,
          sl.Manager, sl.CustName, sl.CustKey`;
}
...
WITH list AS (
  ...
  SELECT e.ProdKey,
         ...
         1 AS Sort,
         COALESCE(NULLIF(ci.Descr2, N''), NULLIF(e.EstimateType, N''), N'기타') AS EstimateType,
...
SELECT ROW_NUMBER() OVER (
       ...
       l.Descr,
       sd.SdetailKey,
       sd.ShipmentQuantity AS DateShipQty,
       p.OutUnit,
       p.BunchOf1Box,
       p.SteamOf1Box,
       COALESCE(e.ShipmentKey, sdt.ShipmentKey) AS ShipmentKey,
       sm.OrderWeek,
  ...
  FROM list l
   LEFT JOIN PeriodDay pd ON l.ShipmentDtm = pd.BaseYmd
   LEFT JOIN CodeInfo ci ON ci.Category = N'WeekDay' AND ci.DetailCode = pd.WeekDay
  ...
  LEFT JOIN ShipmentDetail sdt ON sd.SdetailKey = sdt.SdetailKey
  LEFT JOIN ShipmentMaster sm ON COALESCE(e.ShipmentKey, sdt.ShipmentKey) = sm.ShipmentKey
 ORDER BY l.Sort,
          l.EstimateType,
...
WITH list AS (
  ...
  SELECT e.ProdKey,
    ...
    FROM Estimate e
    JOIN ShipmentMaster sm ON e.ShipmentKey = sm.ShipmentKey
    -- 정상 출고의 요일 필터는 ShipmentDate에 적용하지만,
    -- Estimate(불량/검역/단가차감·판매요청 등)는 별도 등록행이다.
    -- EXE 원장의 유형 코드·일시가 오래된 행도 인쇄 전에 조용히 탈락하지
    -- 않도록 CodeInfo/PeriodDay는 보조 조회로 둔다. 거래처·차수 범위는
    -- ShipmentMaster가 보장한다.
    LEFT JOIN CodeInfo ci ON e.EstimateType = ci.DetailCode AND ci.Category = N'EstimateType'
   WHERE sm.OrderYearWeek = @orderYearWeek
     AND sm.CustKey = @custKey
),
...
export function filterItemsByExeWeekDay(items, activeWdKr) {
  ...
  return (items || []).filter((item) => {
    // 등록된 Estimate 차감행은 요일/코드 보조정보가 없어도 항상 표시한다.
    // 정상출고만 EXE WeekDay 필터를 적용한다.
    if (item?.EstimateKey != null || (item?.SdetailKey == null && String(item?.EstimateType || '').trim())) return true;
...
// Deletion compares stored values, never rounded EXE display quantities or
// Product.EstUnit. The date-only value is produced by SQL to avoid timezone drift.
export function mapEstimateDeleteSnapshot(row) {
  if (!(Number(row.EstimateKey) > 0) || row.DeleteTypeRaw == null) return null;
  return {
    quantity: Number(row.DeleteQuantityRaw ?? 0),
    cost: Number(row.Cost ?? 0),
    amount: Number(row.Amount ?? 0),
    vat: Number(row.Vat ?? 0),
    unit: String(row.DeleteUnitRaw ?? ''),
    estimateType: String(row.DeleteTypeRaw),
    descr: String(row.Descr ?? ''),
    estimateDate: row.DeleteDateRaw == null ? null : String(row.DeleteDateRaw),
  };
}
...
export function mapExeDetailRowToWebItem(row) {
  ...
  return {
    ...
    DeleteSnapshot: isDeduction ? mapEstimateDeleteSnapshot(row) : null,
    ...
    // Stale checks must use the persisted date price, not a display fallback.
    DateCost: sort === 0 ? Number(row.Cost ?? 0) : null,
    DateShipQty: sort === 0 && row.DateShipQty != null ? Number(row.DateShipQty) : null,
    OutUnit: row.OutUnit,
