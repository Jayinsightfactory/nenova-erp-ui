...
import { distributeUnits } from './distributeUnits.js';

/** @deprecated distributeUnits(outQty, product).estQty 사용 */
export function estimateQuantityFromShipmentUnits(units, product = {}) {
  const outQty = Number(units?.outQ ?? units?.outQty ?? units?.box ?? 0);
  return distributeUnits(outQty, product).estQty;
}
 ...
 * @param {number} [a.steamOf1Bunch] SteamOf1Bunch
 * @param {string} [a.estUnit] Product.EstUnit (nenova.exe distributeUnits)
...
export function computeShipmentAdjustUnits({
  ...
  bunchOf1Box = 0, steamOf1Box = 0, steamOf1Bunch = 0, estUnit,
}) {
  ...
  const product = {
    OutUnit: prodOutUnit,
    EstUnit: estUnit || prodOutUnit,
    BunchOf1Box: b1b,
    SteamOf1Bunch: Number(steamOf1Bunch || 0),
    SteamOf1Box: s1b,
  };
  const missing = [];
  if (userUnit !== prodOutUnit) {
    if ((userUnit === '박스' && prodOutUnit === '단') || (userUnit === '단' && prodOutUnit === '박스')) {
      if (!(b1b > 0)) missing.push('BunchOf1Box');
    }
    if ((userUnit === '박스' && prodOutUnit === '송이') || (userUnit === '송이' && prodOutUnit === '박스')) {
      if (!(s1b > 0)) missing.push('SteamOf1Box');
    }
    if ((userUnit === '단' && prodOutUnit === '송이') || (userUnit === '송이' && prodOutUnit === '단')) {
      if (!(b1b > 0)) missing.push('BunchOf1Box');
      if (!(s1b > 0)) missing.push('SteamOf1Box');
    }
  }
  if (missing.length) {
    const error = new Error(`품목 단위 환산정보(${[...new Set(missing)].join(', ')})가 0 또는 미설정입니다. 품목 마스터를 확인한 뒤 다시 시도하세요.`);
    error.code = 'PRODUCT_UNIT_CONVERSION_MISSING';
    throw error;
  }
  // delta(userUnit) → OutUnit 기준. userUnit==OutUnit 이면 그대로(무해), 단→박스면 /B1B.
  const deltaOut = toOrderUnits(Number(delta || 0), userUnit, product).outQty;
  const before = Number(curOut || 0);
  const qtyAfter = type === 'CANCEL' ? before - deltaOut : before + deltaOut;
  const distributed = distributeUnits(qtyAfter, product);
  const units = {
    box: distributed.box,
    bunch: distributed.bunch,
    steam: distributed.steam,
    outQ: distributed.outQty,
  };
  const estQty = distributed.estQty;
