...
// 2026-07-10 완성본(22차/23차 재고수정본) 실셀 역분석 결과, DB 자동연동 가능한 소스가 전혀 없음
// (SERVICE FEE 인보이스가 WarehouseDetail에 없고, FreightCost 스냅샷도 해당 주차엔 0건) — 전량 웹 수기입력.
//
// 구조:
...
// - 포워딩(S)도 같은 반차수 데이터를 공유: 항공료단가(AirRateUSD, USD/kg) × 배분비율 — 단, 포워딩만
//   GW≈CW면 무게비율, 아니면 CBM비율(박스당CBM×박스수량)로 전환(엑셀 F21 IF문 그대로). 그외통관비는 전환 없음.
// - 콜롬비아 수국·네덜란드·중국·에콰도르·태국은 포워딩도 국가별 USD 직접입력(SERVICE FEE 인보이스/freightwise 조회값).
...
const RATE_DEFAULTS = {
  ...
  BoxWeight_콜롬비아장미: 8, BoxCBM_콜롬비아장미: 10,
  BoxWeight_콜롬비아카네이션: 11, BoxCBM_콜롬비아카네이션: 9,
      ...
      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name='WebColombiaWeekly')
       BEGIN
         CREATE TABLE WebColombiaWeekly (
           ...
           AirRateUSD FLOAT,  -- 포워딩(항공료 단가 USD/kg)
           UpdatedBy NVARCHAR(50), UpdatedAt DATETIME DEFAULT GETDATE()
         );
...
export function computeColombiaAllocation(colWeeklyRow, boxQty, rates) {
  ...
  const airRate = n0(colWeeklyRow?.AirRateUSD);
  const out = {};
  for (const cat of COLOMBIA_ALLOC_CATEGORIES) {
    out[cat] = {
      H: total * weightRatio[cat],                                          // 그외통관비 — 항상 무게비율
      S: airRate * (useWeight ? weightRatio[cat] : cbmRatio[cat]),          // 포워딩(USD) — GW=CW 여부로 전환
    };
  }
  return out;
}
...
export async function computeCustomsAndForwarding(major, orderYear) {
  await ensureCustomsTables();
  const [rates, countryRows, fwdRows, subWeeks] = await Promise.all([
    getRateConfig(),
  ...
  const H = {}, S = {};
  for (const cat of COUNTRY_CATEGORIES) H[cat] = computeCountryCustomsTotal(countryRows[cat], rates);
  for (const cat of FORWARDING_DIRECT_CATEGORIES) S[cat] = fwdRows[cat] != null ? Number(fwdRows[cat]) : 0;
  ...
  for (const wk of subWeeks) {
    ...
    if (!colRow) continue;
    const alloc = computeColombiaAllocation(colRow, boxQty, rates);
