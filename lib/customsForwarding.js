...
export async function computeCustomsAndForwarding(major, orderYear) {
  ...
  for (const cat of COUNTRY_CATEGORIES) {
    ...
    const eff = mergeCountryGw(countryRows[cat], gwDef);
  ...
  for (const wk of subWeeks) {
    ...
    const alloc = computeColombiaAllocation({ ...mergeColombiaGw(colRow, gwDef), AirRateUSD: effectiveAirTotal }, boxQty, rates);
    for (const cat of COLOMBIA_ALLOC_CATEGORIES) { H[cat] += alloc[cat].H; S[cat] += alloc[cat].S; }
  }
  const hColombiaSource = colombiaSavedCount === subWeeks.length ? 'saved'
    : colombiaSavedCount > 0 ? 'partial'
