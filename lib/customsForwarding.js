...
export function effectiveCountryWorldFreight(row, gwDef, rates = RATE_DEFAULTS) {
  ...
  const out = { ...effective };
  if (!hasExplicitValue(row, 'WorldFreight1') && first.amount > 0) out.WorldFreight1 = first.amount;
  if (!hasExplicitValue(row, 'WorldFreight2') && second.amount > 0) out.WorldFreight2 = second.amount;
  return {
    row: out,
    auto,
    source: {
      WorldFreight1: hasExplicitValue(row, 'WorldFreight1') ? 'manual_override' : (first.amount > 0 ? first.source : 'missing'),
      WorldFreight2: hasExplicitValue(row, 'WorldFreight2') ? 'manual_override' : (second.amount > 0 ? second.source : 'missing'),
    },
  };
}
