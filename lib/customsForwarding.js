...
//   같은 APOLLO에 수국이 실리면 수국 박스수량·무게를 풀에 넣고 수국 국가행 GW와 이중 계산하지 않는다.
...
export {
  COLOMBIA_ALLOC_CATEGORIES, effectiveRatesForWeek, colombiaUsesWeightRatio,
  computeCountryCustomsTotal, computeColombiaCustomsTotal, computeColombiaRatios,
  computeColombiaAllocation, computeColombiaAllocationFromTotal,
} from './customsForwardingCalc.js';
...
export async function loadWarehouseGw(major, orderYear) {
  ...
  for (const entry of entries.values()) {
    ...
    if (cat === '콜롬비아 4품목' || cat === '콜롬비아 공유APOLLO') {
      ...
      c.GW += gw; c.CW += cw;
      if (cat === '콜롬비아 공유APOLLO') c.includeHydrangea = true;
      continue;
    }
    if (!cat) continue;
...
export async function computeCustomsAndForwarding(major, orderYear) {
  ...
  for (const wk of subWeeks) {
    ...
    if (gwDef?.includeHydrangea && Number(hydrangeaBoxes) > 0) {
      boxQty[COLOMBIA_HYDRANGEA_CATEGORY] = Number(hydrangeaBoxes);
    }
    ...
    const resolved = resolveColombiaCustomsAllocation({
      orderWeek: wk, orderYear, major, colRow, boxQty, gwDef, airTotal: effectiveAirTotal, rates,
    });
