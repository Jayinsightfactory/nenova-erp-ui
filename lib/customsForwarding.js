...
// 2026-07-10 완성본(22차/23차/26차) 실셀 역분석 결과, 그외통관비(H)는 DB 자동연동 가능한 소스가 없어 전량 웹 수기입력.
//
...
//
// 구조:
// - 국가별(콜롬비아 수국 포함 11개 카테고리): 백상창고료(GW×단가)+관세(리터럴)+선율(리터럴)+월드운송료(리터럴)+한국방역(리터럴)
//   = 그외통관비 총액. 국가레벨의 선율·월드운송료는 트럭대수 공식이 아니라 그냥 매주 합산된 리터럴 금액(중국 월드운송료가
//   610640+135590+489330처럼 임의합인 것으로 확인 — 콜롬비아1차/2차만 트럭대수 공식). VAT 처리는 엑셀 그대로 혼합
      ...
      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name='WebCustomsWeekly')
       BEGIN
         CREATE TABLE WebCustomsWeekly (
           ...
           WorldFreight1 FLOAT, WorldFreight2 FLOAT, -- 월드운송료, 1차/2차 (리터럴 — 트럭공식 아님)
...
// 2026-07-17 검증: 22~27차 사용자 엑셀 51항목 중 39건 정확일치(포워더↔국가 매핑 확정).
// 콜수국=FREIGHTWISE/Apollo(인보이스 '수국' 태그), 네덜란드=Holex(+EZ Flower), 태국=EXCEL,
// 중국=Cloudland+Yunnan Melody, 에콰도르=Freightwise Ecuador. 콜카장(4품목)은 반차수 GW/CW.
// 호주·베트남 등은 입고에 GW 라인이 없을 수 있음 → 화면에서 '확인 필요' 표시만.
export async function loadWarehouseGw(major, orderYear) {
  const r = await query(
    `SELECT wm.OrderWeek, LTRIM(RTRIM(ISNULL(wm.FarmName, N''))) AS farm,
            LTRIM(RTRIM(ISNULL(wm.InvoiceNo, N''))) AS inv,
            SUM(CASE WHEN p.ProdName LIKE N'%ross%weight%' THEN ISNULL(wd.OutQuantity,0) ELSE 0 END) AS gw,
            SUM(CASE WHEN p.ProdName LIKE N'%hargeable%weight%' THEN ISNULL(wd.OutQuantity,0) ELSE 0 END) AS cw
      ...
      WHERE ISNULL(wm.isDeleted,0)=0 AND wm.OrderWeek LIKE @pfx
        ...
        AND (p.ProdName LIKE N'%ross%weight%' OR p.ProdName LIKE N'%hargeable%weight%')
      GROUP BY wm.OrderWeek, LTRIM(RTRIM(ISNULL(wm.FarmName, N''))), LTRIM(RTRIM(ISNULL(wm.InvoiceNo, N'')))`,
  ...
  for (const row of r.recordset) {
    const half = /-0?2$/.test(row.OrderWeek) ? 2 : 1;
    const f = row.farm.toLowerCase();
    let cat = null;
    if (/^freightwise ecuador/.test(f)) cat = '에콰도르';
    else if (/^(freightwise|apollo)/.test(f)) {
      if (/수국/.test(row.inv)) cat = '콜롬비아 수국';
      else {
        const c = colombia[row.OrderWeek] || (colombia[row.OrderWeek] = { GW: 0, CW: 0 });
        c.GW += Number(row.gw || 0); c.CW += Number(row.cw || 0);
        continue;
      }
    } else if (/^holex|^ez flower/.test(f)) cat = '네덜란드';
    else if (/^excel$/.test(f)) cat = '태국';
    else if (/^cloudland|^yunnan/.test(f)) cat = '중국';
    if (!cat) continue;
    const e = countries[cat] || (countries[cat] = { GW1: 0, GW2: 0 });
    e[half === 1 ? 'GW1' : 'GW2'] += Number(row.gw || 0);
  }
    ...
    `WITH freight AS (
     ...
     SELECT f.OrderWeek, f.InvoiceNo, f.TPrice,
       CASE
         ...
         WHEN f.FarmName LIKE N'%Ecuador%' OR f.FarmName LIKE N'%에콰도르%' THEN N'에콰도르'
         WHEN f.FarmName LIKE N'%FREIGHTWISE%' OR f.InvoiceNo LIKE N'%콜수국%' OR f.InvoiceNo LIKE N'%콜카장%' THEN N'콜롬비아'
...
export async function loadCustomsWeekly(major, orderYear) {
  ...
  return Object.fromEntries(r.recordset.map((x) => [x.Category, x]));
}
export async function saveCustomsWeekly(major, orderYear, category, row, actor) {
  await ensureCustomsTables();
  const cols = ['GW1', 'GW2', 'Customs1', 'Customs2', 'SunYul1', 'SunYul2', 'WorldFreight1', 'WorldFreight2', 'Quarantine1', 'Quarantine2'];
  const scopeKey = `${major}|${category}`;
  const existing = (await loadCustomsWeekly(major, orderYear))[category] || {};
  const params = {
    yr: { type: sql.NVarChar, value: String(orderYear) }, mw: { type: sql.NVarChar, value: major },
  ...
  await withTransaction(async (tQ) => {
    for (const c of cols) await logHistory(tQ, orderYear, 'Country', scopeKey, c, existing[c], row[c], actor);
    await tQ(
...
export function computeCountryCustomsTotal(row, rates, category = '') {
  ...
  const customs = n0(row.Customs1) + n0(row.Customs2);                      // 그대로
  const sunYulGross = n0(row.SunYul1) + n0(row.SunYul2);
...
export async function computeCustomsAndForwarding(major, orderYear) {
  await ensureCustomsTables();
  const [rates, countryRows, fwdRows, subWeeks, autoFwd] = await Promise.all([
    getRateConfig(),
  ...
  for (const cat of COUNTRY_CATEGORIES) {
    ...
    H[cat] = computeCountryCustomsTotal(countryRows[cat], rates, cat);
    HSource[cat] = saved ? 'saved' : 'missing';
  }
  ...
  for (const wk of subWeeks) {
    ...
    const alloc = computeColombiaAllocation({ ...(colRow || {}), AirRateUSD: effectiveAirTotal }, boxQty, rates);
    for (const cat of COLOMBIA_ALLOC_CATEGORIES) { H[cat] += alloc[cat].H; S[cat] += alloc[cat].S; }
  }
  const hColombiaSource = colombiaSavedCount === 0 ? 'missing' : colombiaSavedCount === subWeeks.length ? 'saved' : 'partial';
