...
// - 콜롬비아 카장알루는 반차수 단위로 합산 BILL의 그외통관비 TOTAL을 계산한 뒤
//   APOLLO CW=GW이면 박스당무게 비율, CW>GW이면 박스당CBM 비율로 H와 S를 함께 배분한다.
//   APOLLO는 포워딩 업체다. 배분 풀 기본값은 콜카장알루(장미/카네이션/알스트로/루스커스)이고,
//   그외통관비 화면에서 콜카장알루수국을 고른 반차수만 수국 박스를 풀에 넣는다.
...
import {
  isColombiaSharedApolloShipment, isColombiaSharedApolloContext, COLOMBIA_HYDRANGEA_CATEGORY,
  isColombiaHydrangeaPool, applyColombiaHydrangeaBoxes, colombiaAllocPoolLabel,
} from './colombiaFlowerClassification.js';
...
import {
  COLOMBIA_ALLOC_CATEGORIES, effectiveRatesForWeek, colombiaRatioMode,
...
export {
  COLOMBIA_ALLOC_CATEGORIES, effectiveRatesForWeek, colombiaUsesWeightRatio, colombiaRatioMode,
  computeCountryCustomsTotal, computeColombiaCustomsTotal, computeColombiaRatios,
  computeColombiaAllocation, computeColombiaAllocationFromTotal,
} from './customsForwardingCalc.js';
...
export {
  classifyColombiaAllocCategory, COLOMBIA_ALLOC_TOKEN_MAP,
  isColombiaSharedApolloShipment, isColombiaSharedApolloContext, COLOMBIA_HYDRANGEA_CATEGORY,
  isColombiaHydrangeaPool, applyColombiaHydrangeaBoxes, colombiaAllocPoolLabel,
} from './colombiaFlowerClassification.js';
...
function resolveForwardingCategorySet(categories) {
  ...
  if (isColombiaSharedApolloContext(values)) return '콜롬비아 4품목';
...
function inferForwardingCategoryFromText(row, contextCategories = new Set()) {
  ...
  if (knownCountry === '콜롬비아') {
    if (isColombiaSharedApolloShipment({
      farmName: farm, invoiceNo: invoice,
      flowerNames: [...contextCategories],
      prodNames: [product],
    })) {
      return '콜롬비아 4품목';
    }
...
export const RATE_DEFAULTS = {
  ...
  // 수국 5.6kg / 7CBM — 2026 31차 원본 콜롬비아 1차 시트 K41/P41 실측값(2026-08-19 정정, 이전 10/12는 시드 추정치)
...
export function ensureCustomsTables() {
  ...
  _ensured = assertWebSchemaContract('profit-report-customs@4', [
    ...
    { table: 'WebColombiaWeekly', columns: ['OrderYear', 'OrderWeek', 'GW', 'CW', 'HandlingFee', 'ItemCount', 'Truck1t', 'Truck2_5t', 'Truck5t', 'CustomsFee', 'DisinfectFee', 'QuarantineDeductFee', 'AirRateUSD', 'BakSangRateApplied', 'IncludeHydrangea', 'UpdatedBy', 'UpdatedAt'] },
...
export async function loadWarehouseGw(major, orderYear) {
  ...
  const inferCategory = (entry) => {
    ...
    if (knownCountry === '콜롬비아' || /apollo|freightwise/.test(farm)) {
      if (isColombiaSharedApolloShipment({
        farmName: entry.farm, invoiceNo: entry.inv,
        flowerNames: flowers, prodNames: [entry.prodName],
      })) {
        return '콜롬비아 공유APOLLO';
      }
  ...
  for (const entry of entries.values()) {
    ...
    if (cat === '콜롬비아 4품목' || cat === '콜롬비아 공유APOLLO') {
      const c = colombia[entry.orderWeek] || (colombia[entry.orderWeek] = { GW: 0, CW: 0, includeHydrangea: false });
      c.GW += gw; c.CW += cw;
      if (cat === '콜롬비아 공유APOLLO') c.includeHydrangea = true; // 화면 힌트. 배분 풀은 IncludeHydrangea 저장값으로만 켠다.
      continue;
    }
    if (!cat) continue;
...
export async function colombiaHydrangeaBoxQty(orderWeek, orderYear) {
  const r = await query(
    `SELECT wm.WarehouseKey, wm.OrderNo AS AWB, wm.FarmName, wm.InvoiceNo,
  ...
  const normalizeAwb = (value) => String(value || '').trim().toLowerCase().replace(/[\s-]/g, '');
  const awbFlowers = new Map();
  const warehouses = new Map();
  for (const row of r.recordset) {
    const awb = normalizeAwb(row.AWB);
    if (awb) {
      const names = awbFlowers.get(awb) || [];
      names.push(row.FlowerName, row.ProdName);
      awbFlowers.set(awb, names);
    }
    const entry = warehouses.get(row.WarehouseKey) || {
      farm: row.FarmName, inv: row.InvoiceNo, awb: row.AWB, flowers: [], hydBoxes: 0,
    };
    entry.flowers.push(row.FlowerName, row.ProdName);
    if (/수국|hydrangea/i.test(`${row.FlowerName} ${row.ProdName}`)) {
      entry.hydBoxes += Number(row.BoxQuantity || 0);
    }
    warehouses.set(row.WarehouseKey, entry);
  }
  let total = 0;
  for (const entry of warehouses.values()) {
    const flowers = [...entry.flowers, ...(awbFlowers.get(normalizeAwb(entry.awb)) || [])];
    if (isColombiaSharedApolloShipment({
      farmName: entry.farm, invoiceNo: entry.inv, flowerNames: flowers,
    })) {
      total += entry.hydBoxes;
    }
  }
  return total;
}
...
export async function saveColombiaWeekly(orderWeek, orderYear, row, actor) {
  await ensureCustomsTables();
  const cols = ['GW', 'CW', 'HandlingFee', 'ItemCount', 'Truck1t', 'Truck2_5t', 'Truck5t', 'CustomsFee', 'DisinfectFee', 'QuarantineDeductFee', 'AirRateUSD', 'BakSangRateApplied', 'IncludeHydrangea'];
...
/**
 * 입고 Gross Weight → 월드 운송료 **추천** 차량/금액(등급표, lib/colombiaTruck.js deriveTruckPlan).
 * 저장된 실제 WorldFreight 금액이 없을 때만 이 추천값을 사용하며, 추천값 자체를 DB에 수기값으로
 * 저장하지 않는다. 2.5t 초과~5t는 5t 1대(3,342kg → 275,000원)다.
 */
 ...
 * (5t 1대)으로 덮이지 않는다. Manual 표시가 없는 레거시 리터럴만 추천값으로 대체하며,
...
export function resolveColombiaCustomsAllocation({ orderWeek, orderYear, major, colRow, boxQty, hydrangeaBoxes = 0, gwDef, airTotal, rates }) {
  ...
  const includeHydrangea = historical ? false : isColombiaHydrangeaPool(colRow?.IncludeHydrangea);
  const effectiveRow = historical
    ? { ...historical.row }
    : { ...mergeColombiaTruck(mergeColombiaGw(colRow, gwDef), gwDef), IncludeHydrangea: includeHydrangea ? 1 : 0 };
  const effectiveBoxQty = historical
    ? historical.boxQty
    : applyColombiaHydrangeaBoxes(boxQty, hydrangeaBoxes, includeHydrangea);
...
export async function computeCustomsAndForwarding(major, orderYear) {
  ...
  for (const wk of subWeeks) {
    const [colRow, boxQtyLive, hydrangeaBoxes] = await Promise.all([
    ...
    const resolved = resolveColombiaCustomsAllocation({
      orderWeek: wk, orderYear, major, colRow, boxQty, hydrangeaBoxes, gwDef, airTotal: effectiveAirTotal, rates,
    });
    ...
    for (const cat of COLOMBIA_ALLOC_CATEGORIES) { H[cat] += resolved.allocation[cat]?.H || 0; S[cat] += resolved.allocation[cat]?.S || 0; }
    if (resolved.allocation?.[COLOMBIA_HYDRANGEA_CATEGORY]) {
      H[COLOMBIA_HYDRANGEA_CATEGORY] = (H[COLOMBIA_HYDRANGEA_CATEGORY] || 0) + resolved.allocation[COLOMBIA_HYDRANGEA_CATEGORY].H;
      S[COLOMBIA_HYDRANGEA_CATEGORY] = (S[COLOMBIA_HYDRANGEA_CATEGORY] || 0) + resolved.allocation[COLOMBIA_HYDRANGEA_CATEGORY].S;
    }
  }
