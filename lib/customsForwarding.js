...
import {
  AUDITED_BASELINE_YEAR, getAuditedBakSangRate, getAuditedCountryH, getAuditedColombiaWeekly,
} from './profitReportAuditedBaseline.js';
import {
...
// 2026 22~27차 감사 기준값(그외통관비 H) 단일 진실 소스 — 프로덕션/화면(customs-clearance API)이
// 함께 재사용한다. 테스트 fixture(__tests__/fixtures/profit-report-22-27.json)는 참조하지 않는다.
export {
  AUDITED_BASELINE_YEAR, getAuditedBakSangRate, getAuditedCountryH, getAuditedColombiaWeekly,
} from './profitReportAuditedBaseline.js';
...
export async function ensureCustomsTables() {
  ...
  _ensured = (async () => {
    ...
    // 저장 시점 유효 백상 창고료 요율 스냅샷 — 기존 운영 DB에도 무중단으로 추가한다.
    await query(
      `IF COL_LENGTH('dbo.WebCustomsWeekly', 'BakSangRateApplied') IS NULL
       ALTER TABLE dbo.WebCustomsWeekly ADD [BakSangRateApplied] FLOAT NULL`, {}
    );
    await query(
      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name='WebColombiaWeekly')
       BEGIN
         CREATE TABLE WebColombiaWeekly (
           ...
           BakSangRateApplied FLOAT NULL, -- 저장 시점 유효 백상 창고료 요율 스냅샷
           UpdatedBy NVARCHAR(50), UpdatedAt DATETIME DEFAULT GETDATE()
         );
    ...
    await query(
      `IF COL_LENGTH('dbo.WebColombiaWeekly', 'BakSangRateApplied') IS NULL
       ALTER TABLE dbo.WebColombiaWeekly ADD [BakSangRateApplied] FLOAT NULL`, {}
    );
    await query(
...
/** GET/조회 전용 — 스키마를 만들지 않고 필수 웹 전용 테이블 존재만 확인한다.
 * 2026-08-11 결함수정 7: computeCustomsAndForwarding()이 profit-report GET 경로에서
 * ensureCustomsTables()(CREATE TABLE/ALTER TABLE)를 실행하고 있었다 — GET은 SELECT 전용이어야
 * 한다(CLAUDE.md 규칙 1). 스키마 생성은 저장(POST) 경로의 ensureCustomsTables()에서만 수행한다. */
export async function assertCustomsReadSchema() {
  const result = await query(
    `SELECT v.TableName
       FROM (VALUES (N'WebCustomsRateConfig'), (N'WebCustomsWeekly'), (N'WebColombiaWeekly'), (N'WebForwardingWeekly'), (N'WebCustomsHistory')) v(TableName)
      WHERE OBJECT_ID(N'dbo.' + v.TableName, N'U') IS NULL`,
    {},
  );
  const missing = result.recordset.map((row) => row.TableName);
  if (missing.length) {
    const error = new Error(`그외통관비/포워딩 스키마가 설치되지 않았습니다: ${missing.join(', ')}. 배포 마이그레이션 또는 최초 저장을 먼저 실행하세요.`);
    error.code = 'CUSTOMS_SCHEMA_MISSING';
    ...
    `WITH freight AS (
     ...
     SELECT f.OrderWeek, f.InvoiceNo, f.TPrice,
       CASE
         ...
         WHEN UPPER(REPLACE(ISNULL(f.FarmName,N''),N' ',N'')) LIKE N'%FREIGHTWISEECUADOR%'
           OR f.FarmName LIKE N'%에콰도르%'
           OR f.InvoiceNo LIKE N'%에콰%'
           OR UPPER(f.InvoiceNo) LIKE N'%ECUADOR%' THEN N'에콰도르'
...
export async function saveCustomsWeeklyBatch(major, orderYear, entries, actor) {
  ...
  const currentRates = await getRateConfig();
  const effectiveBakSangRate = getAuditedBakSangRate(orderYear, major) ?? currentRates.BakSangRate;
  await withTransaction(async (tQ) => {
 ...
 * 월드운송료는 1차/2차를 각각 별도 트럭으로 계산하지 않고, 그 대차수 국가의 GW1+GW2 합산
 * 중량으로 트럭 1대를 선정해 1차 칸에 전액 반영한다(2차=0). 콜롬비아 4품목(장미/카네이션/
 * 알스트로/루스커스)처럼 반차수마다 실제 별도 출고되는 품목이 아니라, 국가 단위 그외통관비는
 * 원본 엑셀이 그 대차수 전체 물량을 트럭 1대로 계산했기 때문이다(2026-08-12 결함수정 —
 * 근거: 26차 콜롬비아 수국 GW1 2779+GW2 1444=4223kg→5t 1대. 반차수 각각 트럭을 잡으면
 * 1444kg도 별도 2.5t로 계산돼 26차에 약 +350,000원 과다계상됐다).
 * 1차/2차 중 한쪽이라도 명시적 수기 override(WorldFreight{1,2}Manual=1)면 그 값을 그대로
 * 보존한다. override가 없는 칸은, 결합 GW로 자동값을 낼 수 있는 한(combined.amount>0) 항상 0으로
 * 둔다 — 1차에 이미 트럭 1대분 전액을 반영했으므로 2차에 남은 레거시 리터럴을 그대로 더하면
 * 이중계상된다(2026-08-12 결함수정: WorldFreight2Manual이 없는 레거시 리터럴이 결합 자동값과
 * 함께 합산되던 잔여 이중계상 경로). 레거시 리터럴은 결합 GW 자동값 자체를 낼 수 없을 때
 * (GW가 전혀 없는 구형 데이터)만 조용히 지우지 않고 보존한다. */
...
/** 국가(콜롬비아 수국 포함) 1건의 유효 H — 저장행이 전혀 없고 감사 기준값(2026 22~27차)이 있으면
 * 그 값을 그대로 쓴다("explicit saved manual row values > audited Excel baseline > current auto
 * source > global defaults" 우선순위). profit-report의 실계산(computeCustomsAndForwarding)과
 * 그외통관비 입력화면 미리보기(customs-clearance API)가 이 함수 하나만 써야 두 화면이 항상
 * 일치한다. */
export function resolveCountryCustomsTotal({ row, gwDef, rates, category, orderYear, major }) {
  const effectiveRates = effectiveRatesForWeek(rates, orderYear, major);
  const world = effectiveCountryWorldFreight(row, gwDef, effectiveRates);
  const auditedH = !row ? getAuditedCountryH(orderYear, major, category) : null;
  const hasGwAuto = Number(gwDef?.GW1) > 0 || Number(gwDef?.GW2) > 0;
  if (auditedH != null) {
    return { total: auditedH, source: 'audited_baseline', world, effectiveRates };
  }
  return {
    total: computeCountryCustomsTotal(row || hasGwAuto ? world.row : null, effectiveRates, category),
    source: row ? 'saved' : hasGwAuto ? 'gw_auto' : 'missing',
    world,
    effectiveRates,
  };
}

/** 콜롬비아 4품목 반차수 1건의 유효 배분값 — 저장행이 전혀 없고 감사 기준값이 있으면 박스수량·GW도
 * 감사값을 쓴다("historical Colombia box quantities can differ from current warehouse state"
 * — 현재 창고 DB 상태가 감사 시점과 다를 수 있으므로, 저장행이 없는 감사 대상 반차수는 현재 입고
 * 데이터가 아니라 감사 기준값을 쓴다). resolveCountryCustomsTotal과 마찬가지로 profit-report
 * 실계산과 그외통관비 입력화면 미리보기가 이 함수 하나만 공유한다. */
export function resolveColombiaCustomsAllocation({ orderWeek, orderYear, major, colRow, boxQty, gwDef, airTotal, rates }) {
  ...
  const effectiveRates = effectiveRatesForWeek(rates, orderYear, resolvedMajor);
  const audited = !colRow ? getAuditedColombiaWeekly(orderYear, orderWeek) : null;
  if (audited) {
    return {
      allocation: computeColombiaAllocationFromTotal(audited.total, airTotal, audited.boxQty, effectiveRates),
      total: audited.total,
      boxQty: audited.boxQty,
      gw: audited.gw,
      truckAuto: deriveColombiaTruckAllocation(audited.gw),
  ...
  return {
    allocation: computeColombiaAllocation({ ...effectiveRow, AirRateUSD: airTotal }, boxQty, effectiveRates),
    total: computeColombiaCustomsTotal(effectiveRow, effectiveRates),
    boxQty,
    gw: Number(effectiveRow.GW) || 0,
    truckAuto: Number(effectiveRow.GW) > 0 ? deriveColombiaTruckAllocation(effectiveRow.GW) : null,
    source: colRow ? 'saved' : (Number(effectiveRow.GW) > 0 ? 'gw_auto' : 'missing'),
    effectiveRates,
  };
}
...
export async function computeCustomsAndForwarding(major, orderYear) {
  ...
  for (const wk of subWeeks) {
    ...
    if (resolved.source === 'audited_baseline') colombiaAuditedCount += 1;
  ...
  const hColombiaSource = colombiaSavedCount === subWeeks.length
    ? 'saved'
    : colombiaAuditedCount > 0 && (colombiaSavedCount + colombiaAuditedCount) === subWeeks.length
      ? 'audited_baseline'
  ...
  return {
    ...
    sources: {
      H: HSource,
      S: SSource,
      colombia: { expectedWeeks: subWeeks.length, customsSavedWeeks: colombiaSavedCount, auditedWeeks: colombiaAuditedCount, forwardingDetectedWeeks: colombiaForwardingCount },
    },
  };
}
