...
export function effectiveCountryWorldFreight(row, gwDef, rates = RATE_DEFAULTS) {
  ...
  // API/화면의 자동 표시값은 부가세 제외 공급가. 계산에 사용하는 row는
  // 기존 H 공식(월드운송료 ÷ 1.1)을 보존하기 위해 포함가를 유지한다.
  const auto = { WorldFreight1: vatInclusiveToNet(first.amount), WorldFreight2: vatInclusiveToNet(second.amount) };
  const out = { ...effective };
