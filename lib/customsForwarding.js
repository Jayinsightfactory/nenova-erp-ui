...
import { classifyCategory, EXTRA_CATEGORY, CNF_CATEGORIES, isNonInventoryCostItem, isNonValueWeightItem } from './profitReportClassification.js';
...
export function buildForwardingLedger(rows = [], options = {}) {
  ...
  for (const row of rows) {
    ...
    // CNF(운임포함, 호주·베트남)는 운송료 전표가 원래 없으므로 항공료 기대범위에서 제외한다
    // (2026-08-26 사장님 확인). 전표 분류 컨텍스트에는 남겨 혹시 존재하는 행은 정상 분류한다.
    if (!CNF_CATEGORIES.includes(category)) addContext(expectedByWeek, week, category);
  }
...
export async function loadWarehouseGw(major, orderYear) {
  ...
  const inboundWeeks = {}; // 카테고리 → 실제 입고 반차수. GW가 0이어도 꽃 입고 존재는 남긴다.
  ...
  for (const entry of entries.values()) {
    ...
    if (cat) {
      const inboundKey = cat === '콜롬비아 4품목' ? '콜롬비아 4품목' : cat;
      const weeks = inboundWeeks[inboundKey] || (inboundWeeks[inboundKey] = new Set());
      weeks.add(entry.orderWeek);
    }
  ...
  return {
    countries,
    colombia,
    inboundWeeks: Object.fromEntries(
      Object.entries(inboundWeeks).map(([category, weeks]) => [category, [...weeks].sort()]),
    ),
  };
}

// 장미 인보이스는 BoxQuantity=0 + BunchQuantity(단)로만 입고되는 행이 많아(Esperance/Maxiflores 등)
// BoxQuantity 합계만 쓰면 장미 박스가 절반 이하로 잡혀 무게배분이 카네이션 쪽으로 쏠린다
// (28-01 실측: Box합 86 vs 원본 배분표 190박스, 환산 적용 시 191). BunchOf1Box(장미 10단/박스)로 환산한다.
const COLOMBIA_ALLOC_BOX_QTY_SQL = `
  CASE
    WHEN ISNULL(wd.BoxQuantity,0) > 0 THEN CAST(wd.BoxQuantity AS FLOAT)
    WHEN ISNULL(wd.BunchQuantity,0) > 0 AND ISNULL(p.BunchOf1Box,0) > 0
      THEN CAST(wd.BunchQuantity AS FLOAT) / p.BunchOf1Box
    ELSE 0
  END`;
...
export async function colombiaBoxQtyByCategory(orderWeek, orderYear) {
  const r = await query(
    `SELECT ${CASE_COLOMBIA_ALLOC} AS Category, SUM(${COLOMBIA_ALLOC_BOX_QTY_SQL}) AS q
...
export async function computeCustomsAndForwarding(major, orderYear) {
  ...
  for (const wk of subWeeks) {
    ...
    colombiaWeekDetails.push({
      ...
      forwardingDetected: autoDetected || colRow?.AirRateUSD != null,
  ...
  return {
    ...
    sources: {
      ...
      countryInbound: autoGw.inboundWeeks || {},
    },
  };
}
