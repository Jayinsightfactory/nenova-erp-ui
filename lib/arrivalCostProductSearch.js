...
export function arrivalCountryFlowerKey(value = {}) {
  const cf = String(value.countryFlower || value.CountryFlower || '').trim();
  if (cf) return cf;
  const coun = String(value.counName || value.CounName || '').trim();
  const flower = String(value.flowerName || value.FlowerName || '').trim();
  if (coun || flower) return `${coun}${flower}`;
  return '기타';
}

export function countryFlowerGroupsForProductMatch(query, mappings = {}) {
  const raw = effectiveArrivalProductQuery(query);
  if (!raw) return [];
  const compactTerms = arrivalQueryCompactTerms(raw);
  const wantedToken = normalizeToken(raw);
  const names = new Set();
  const fuzzy = findMappingFuzzy(raw, mappings);
  if (fuzzy?.value) names.add(arrivalCountryFlowerKey(fuzzy.value));
  for (const [alias, value] of Object.entries(mappings || {})) {
    const aliasToken = normalizeToken(alias);
    const hit = mappingHitsQuery(alias, value, compactTerms)
      || (wantedToken && aliasToken && aliasToken === wantedToken);
    if (hit) names.add(arrivalCountryFlowerKey(value));
  }
  return [...names].filter((name) => name && name !== '기타').sort((a, b) => a.localeCompare(b, 'ko'));
}
