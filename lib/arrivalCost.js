...
export async function listArrivalCost({
  ...
  const [rowsRes, importsRes] = await Promise.all([
    query(`${LINE_SELECT} WHERE ${where.join(' AND ')} ORDER BY l.OrderWeek DESC, l.CountryName, l.FlowerNameRaw, l.ProductNameRaw, l.ArrivalLineKey OFFSET @offset ROWS FETCH NEXT @pageSize ROWS ONLY`, params),
  ...
  const total = Number(rowsRes.recordset[0]?.TotalCount || 0);
  return {
    rows,
