...
import { calculateArrivalCost, arrivalFlowerLikeTerms, arrivalFlowerInferSql, arrivalFlowersFromTexts } from './arrivalCostExcel.js';
...
function uniqueArrivalFlowers(names) {
  return arrivalFlowersFromTexts(names);
}
...
export async function listArrivalCost({
  ...
  if (flower) {
    ...
    const flowerSql = flowerTerms.map((_, index) => (
      `l.FlowerNameRaw LIKE @flower${index} OR p.FlowerName LIKE @flower${index}`
      + ` OR l.ProductNameRaw LIKE @flower${index} OR p.ProdName LIKE @flower${index} OR p.DisplayName LIKE @flower${index}`
    )).join(' OR ');
    ...
    hasProductSearch ? query(`
      SELECT DISTINCT TOP 80 FlowerName FROM (
        ...
        SELECT NULLIF(LTRIM(RTRIM(p.FlowerName)), N'')
         ...
         WHERE ${scopeWhere.join(' AND ')}
        UNION
        SELECT ${arrivalFlowerInferSql('l.ProductNameRaw')}
          FROM dbo.WebArrivalCostLine l
          LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
         WHERE ${scopeWhere.join(' AND ')}
        UNION
        SELECT ${arrivalFlowerInferSql('p.ProdName')}
          FROM dbo.WebArrivalCostLine l
          LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
         WHERE ${scopeWhere.join(' AND ')}
        UNION
        SELECT ${arrivalFlowerInferSql('p.DisplayName')}
          FROM dbo.WebArrivalCostLine l
          LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
         WHERE ${scopeWhere.join(' AND ')}
      ) flowers
  ...
  const extraFlowerSql = flower
    ? arrivalFlowerLikeTerms(flower).map((_, index) => (
      ` OR p.FlowerName LIKE @flower${index} OR p.ProdName LIKE @flower${index} OR p.DisplayName LIKE @flower${index}`
    )).join('')
    : '';
  ...
  return {
    ...
    matchedVarieties: uniqueArrivalFlowers([
      ...
      ...rows.flatMap((row) => [row.flowerNameRaw, row.dbFlowerName, row.productNameRaw, row.prodName, row.displayName]),
      ...missingRows.flatMap((row) => [row.flowerNameRaw, row.dbFlowerName, row.productNameRaw, row.prodName, row.displayName]),
