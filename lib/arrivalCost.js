...
import { arrivalWeekPredicate } from './arrivalImportPolicy.js';
import { calculateArrivalCost } from './arrivalCostExcel.js';
import { loadMappings } from './parseMappings.js';
import { bindArrivalCostProductMatch, buildArrivalCostProductMatch, arrivalCountryFlowerKey } from './arrivalCostProductSearch.js';
...
const ARRIVAL_LINE_INSERT_COLUMNS = `ImportKey,OrderYear,OrderWeek,CountryName,FlowerNameRaw,ProductNameRaw,FarmNameRaw,ProdKey,FarmKey,Unit,Quantity,FobUSD,FreightPerUnitUSD,CustomsPerUnitKRW,OtherPerUnitKRW,SourceArrivalCostKRW,SourceArrivalCostVatKRW,SelectedArrivalCostKRW,ExchangeRate,GrossWeight,ChargeableWeight,FreightUSD,InvoiceUSD,WeightMetricShare,VolumeMetricShare,ValueMetricShare,AllocationBasis,MatchStatus,SourceFileName,SheetName,SourceRow,RawJson,CreatedBy,UpdatedBy`;
const ARRIVAL_LINE_INSERT_CHUNK = 40;

function arrivalLineInsertParams(row, {
  importKey, year, fileName, actorId, idx,
}) {
  const k = (name) => `${name}_${idx}`;
  return {
    [k('importKey')]: { type: sql.Int, value: importKey },
    [k('year')]: { type: sql.NVarChar, value: year },
    [k('week')]: { type: sql.NVarChar, value: nv(row.orderWeek, 10) },
    [k('country')]: { type: sql.NVarChar, value: nv(row.countryName, 100) },
    [k('flower')]: { type: sql.NVarChar, value: nv(row.flowerNameRaw, 200) },
    [k('product')]: { type: sql.NVarChar, value: nv(row.productNameRaw, 300) },
    [k('farmRaw')]: { type: sql.NVarChar, value: nv(row.farmNameRaw, 200) },
    [k('prodKey')]: { type: sql.Int, value: n(row.prodKey) },
    [k('farmKey')]: { type: sql.Int, value: n(row.farmKey) },
    [k('unit')]: { type: sql.NVarChar, value: nv(row.unit, 40) },
    [k('qty')]: sqlDecimal(row.quantity),
    [k('fob')]: { type: sql.Decimal, value: n(row.fobUSD), precision: 18, scale: 6 },
    [k('freightUnit')]: { type: sql.Decimal, value: n(row.freightPerUnitUSD), precision: 18, scale: 6 },
    [k('customs')]: sqlDecimal(row.customsPerUnitKRW),
    [k('other')]: sqlDecimal(row.otherPerUnitKRW),
    [k('sourceCost')]: sqlDecimal(row.sourceArrivalCostKRW),
    [k('sourceVat')]: sqlDecimal(row.sourceArrivalCostVatKRW),
    [k('selectedCost')]: sqlDecimal(row.selectedArrivalCostKRW),
    [k('fx')]: { type: sql.Decimal, value: n(row.exchangeRate), precision: 18, scale: 6 },
    [k('gw')]: sqlDecimal(row.grossWeight),
    [k('cw')]: sqlDecimal(row.chargeableWeight),
    [k('freight')]: { type: sql.Decimal, value: n(row.freightUSD), precision: 18, scale: 6 },
    [k('invoice')]: { type: sql.Decimal, value: n(row.invoiceUSD), precision: 18, scale: 6 },
    [k('weightShare')]: { type: sql.Decimal, value: n(row.weightMetricShare), precision: 18, scale: 8 },
    [k('volumeShare')]: { type: sql.Decimal, value: n(row.volumeMetricShare), precision: 18, scale: 8 },
    [k('valueShare')]: { type: sql.Decimal, value: n(row.valueMetricShare), precision: 18, scale: 8 },
    [k('basis')]: { type: sql.NVarChar, value: 'SOURCE' },
    [k('status')]: { type: sql.NVarChar, value: nv(row.matchStatus, 30) },
    [k('file')]: { type: sql.NVarChar, value: nv(fileName, 260) },
    [k('sheet')]: { type: sql.NVarChar, value: nv(row.sheetName, 120) },
    [k('sourceRow')]: { type: sql.Int, value: n(row.sourceRow) },
    [k('raw')]: { type: sql.NVarChar, value: nv(row.rawJson, 20000) },
    [k('by')]: { type: sql.NVarChar, value: actorId },
  };
}

function arrivalLineInsertTuple(idx) {
  return `(@importKey_${idx},@year_${idx},@week_${idx},@country_${idx},@flower_${idx},@product_${idx},@farmRaw_${idx},@prodKey_${idx},@farmKey_${idx},@unit_${idx},@qty_${idx},@fob_${idx},@freightUnit_${idx},@customs_${idx},@other_${idx},@sourceCost_${idx},@sourceVat_${idx},@selectedCost_${idx},@fx_${idx},@gw_${idx},@cw_${idx},@freight_${idx},@invoice_${idx},@weightShare_${idx},@volumeShare_${idx},@valueShare_${idx},@basis_${idx},@status_${idx},@file_${idx},@sheet_${idx},@sourceRow_${idx},@raw_${idx},@by_${idx},@by_${idx})`;
}

async function insertArrivalCostLines(tQuery, rows, context) {
  for (let offset = 0; offset < rows.length; offset += ARRIVAL_LINE_INSERT_CHUNK) {
    const chunk = rows.slice(offset, offset + ARRIVAL_LINE_INSERT_CHUNK);
    const params = {};
    const values = chunk.map((row, i) => {
      Object.assign(params, arrivalLineInsertParams(row, { ...context, idx: i }));
      return arrivalLineInsertTuple(i);
    }).join(',');
    await tQuery(
      `INSERT INTO dbo.WebArrivalCostLine (${ARRIVAL_LINE_INSERT_COLUMNS}) VALUES ${values}`,
      params,
    );
  }
}
...
function serializeLine(row) {
  return {
    ...
    countryFlower: row.ProdKey == null
      ? '기타'
      : arrivalCountryFlowerKey({
        CountryFlower: row.CountryFlower,
        CounName: row.DbCountryName,
        FlowerName: row.DbFlowerName,
      }),
...
const LINE_CORE_COLUMNS = `
  l.ArrivalLineKey, l.ImportKey, l.OrderYear, l.OrderWeek, l.CountryName,
  l.FlowerNameRaw, l.ProductNameRaw, l.FarmNameRaw, l.ProdKey, l.FarmKey,
  l.Unit, l.Quantity, l.FobUSD, l.FreightPerUnitUSD, l.CustomsPerUnitKRW,
  l.OtherPerUnitKRW, l.SourceArrivalCostKRW, l.SourceArrivalCostVatKRW,
  l.SelectedArrivalCostKRW, l.ExchangeRate, l.GrossWeight, l.ChargeableWeight,
  l.FreightUSD, l.InvoiceUSD, l.WeightMetricShare, l.VolumeMetricShare,
  l.ValueMetricShare, l.AllocationBasis, l.MatchStatus, l.SourceFileName,
  l.SheetName, l.SourceRow, l.Notes, l.IsCurrent, l.CreatedAt, l.UpdatedAt
`;

const AUTO_FARM_APPLY = `
    OUTER APPLY (
      SELECT TOP 1 f2.FarmKey, f2.FarmName
        FROM dbo.Farm f2
       WHERE ISNULL(f2.isDeleted,0)=0
         AND l.FarmKey IS NULL
         AND REPLACE(REPLACE(REPLACE(REPLACE(LOWER(ISNULL(f2.FarmName,N'')),N' ',N''),N'-',N''),N'.',N''),N'''',N'') =
             REPLACE(REPLACE(REPLACE(REPLACE(LOWER(ISNULL(l.FarmNameRaw,N'')),N' ',N''),N'-',N''),N'.',N''),N'''',N'')
       ORDER BY f2.FarmKey
    ) autoFarm
`;

function usageApplySql(includeUsage) {
  if (!includeUsage) {
    return `
    OUTER APPLY (SELECT CAST(0 AS INT) AS UsageCount) usageStats
    OUTER APPLY (SELECT CAST(0 AS INT) AS MatchCount) matchStats`;
  }
  return `
    OUTER APPLY (SELECT CAST(COUNT_BIG(*) AS INT) AS UsageCount FROM dbo.OrderDetail od WHERE od.ProdKey=l.ProdKey AND ISNULL(od.isDeleted,0)=0) usageStats
    OUTER APPLY (SELECT CAST(COUNT_BIG(*) AS INT) AS MatchCount FROM dbo.WebArrivalCostLine ml WHERE ml.ProdKey=l.ProdKey AND ml.IsCurrent=1) matchStats`;
}

function lineSelectSql({ includeUsage = true } = {}) {
  return `
  SELECT ${LINE_CORE_COLUMNS}, CAST(COUNT_BIG(*) OVER() AS INT) AS TotalCount,
    ...
    ${usageApplySql(includeUsage)}
    ${AUTO_FARM_APPLY}
`;
}

const LINE_SELECT = lineSelectSql({ includeUsage: true });
...
const COUNTRY_FLOWER_SQL = `ISNULL(NULLIF(LTRIM(RTRIM(p.CountryFlower)), N''), ISNULL(p.CounName,N'') + ISNULL(p.FlowerName,N''))`;

function weekSortSql(weekOrder) {
  const dir = String(weekOrder).toLowerCase() === 'asc' ? 'ASC' : 'DESC';
  return `TRY_CONVERT(int, PARSENAME(REPLACE(l.OrderWeek, N'-', N'.'), 2)) ${dir},
          TRY_CONVERT(int, PARSENAME(REPLACE(l.OrderWeek, N'-', N'.'), 1)) ${dir},
          l.OrderWeek ${dir}`;
}
...
export async function listArrivalCostVarieties({ orderYear, orderWeek } = {}) {
  ...
  const lineWeekSql = week ? `AND ${arrivalWeekPredicate('l.OrderWeek')}` : '';
  if (week) params.week = { type: sql.NVarChar, value: nv(week, 10) };
  const warehouseUnion = week ? `
      UNION
      SELECT ${COUNTRY_FLOWER_SQL}
       ...
       WHERE CONVERT(NVARCHAR(4),wm.OrderYear)=@year AND ${arrivalWeekPredicate('wm.OrderWeek')}
         AND ISNULL(wm.isDeleted,0)=0 AND ISNULL(p.isDeleted,0)=0` : '';
  const result = await query(`
    WITH varieties AS (
      SELECT ${COUNTRY_FLOWER_SQL} AS CountryFlower
        FROM dbo.WebArrivalCostLine l
        LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
       WHERE l.OrderYear=@year ${lineWeekSql} AND l.IsCurrent=1 AND l.ProdKey IS NOT NULL
      ${warehouseUnion}
      UNION
      SELECT N'기타'
        WHERE EXISTS (
          SELECT 1 FROM dbo.WebArrivalCostLine l
           WHERE l.OrderYear=@year ${lineWeekSql} AND l.IsCurrent=1 AND l.ProdKey IS NULL
        )
    )
    SELECT CountryFlower FROM varieties
     WHERE NULLIF(LTRIM(RTRIM(CountryFlower)), N'') IS NOT NULL
     ORDER BY CASE WHEN CountryFlower=N'기타' THEN 1 ELSE 0 END, CountryFlower`, params);
  return { varieties: result.recordset.map((row) => row.CountryFlower).filter(Boolean) };
}
...
export async function listArrivalCost({
  ...
  const weekDir = String(weekOrder).toLowerCase() === 'asc' ? 'asc' : 'desc';
  if (!hasProductSearch && !week && !flower) throw new Error('품목명으로 검색하거나 품종을 선택하세요. 전체보기는 차수가 필요합니다.');
  if (!hasProductSearch && !flower && !allVarieties) throw new Error('품종을 선택하거나 전체보기를 명시하세요.');
  if (!hasProductSearch && allVarieties && !week) throw new Error('전체보기는 차수를 입력하세요.');
  const where = ['l.OrderYear=@year'];
  const params = {
    year: { type: sql.NVarChar, value: year },
  };
  if (week) {
    where.push(arrivalWeekPredicate('l.OrderWeek'));
  ...
  if (flower === '기타') {
    where.push('l.ProdKey IS NULL');
  } else if (flower) {
    where.push(`${COUNTRY_FLOWER_SQL} = @flower`);
    params.flower = { type: sql.NVarChar, value: nv(flower, 200) };
  }
  void country;
  ...
  const skipUsageScan = Boolean(hasProductSearch || farm);
  const [rowsRes, importsRes, varietyRes] = await Promise.all([
    query(`${lineSelectSql({ includeUsage: !skipUsageScan })} WHERE ${where.join(' AND ')} ORDER BY
      ${weekSortSql(weekDir)},
      ISNULL(l.CountryName, p.CounName),
      ${COUNTRY_FLOWER_SQL},
      ISNULL(NULLIF(LTRIM(RTRIM(p.DisplayName)), N''), ISNULL(p.ProdName, l.ProductNameRaw)),
      ISNULL(f.FarmName, l.FarmNameRaw),
      l.ArrivalLineKey
      OFFSET @offset ROWS FETCH NEXT @pageSize ROWS ONLY`, {
      ...params,
      hasSearch: { type: sql.Int, value: skipUsageScan ? 1 : 0 },
    }),
    ...
    hasProductSearch ? query(`
      SELECT TOP 80 CountryFlower FROM (
        SELECT ${COUNTRY_FLOWER_SQL} AS CountryFlower
          FROM dbo.WebArrivalCostLine l
          LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
         WHERE ${scopeWhere.join(' AND ')}
           AND l.ProdKey IS NOT NULL
        UNION
        SELECT N'기타'
          WHERE EXISTS (
            SELECT 1 FROM dbo.WebArrivalCostLine l
            LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
             WHERE ${scopeWhere.join(' AND ')} AND l.ProdKey IS NULL
          )
      ) flowers
       WHERE NULLIF(LTRIM(RTRIM(CountryFlower)), N'') IS NOT NULL
       ORDER BY CASE WHEN CountryFlower=N'기타' THEN 1 ELSE 0 END, CountryFlower`, varietyParams) : Promise.resolve({ recordset: [] }),
  ]);
  ...
  const missingUsageSql = skipUsageScan
    ? `OUTER APPLY (SELECT CAST(0 AS INT) AS UsageCount) usageStats
    OUTER APPLY (SELECT CAST(0 AS INT) AS MatchCount) matchStats`
    : `OUTER APPLY (SELECT CAST(COUNT_BIG(*) AS INT) AS UsageCount FROM dbo.OrderDetail od WHERE od.ProdKey=p.ProdKey AND ISNULL(od.isDeleted,0)=0) usageStats
    OUTER APPLY (SELECT CAST(COUNT_BIG(*) AS INT) AS MatchCount FROM dbo.WebArrivalCostLine ml WHERE ml.ProdKey=p.ProdKey AND ml.IsCurrent=1) matchStats`;
  const missingRes = safePage === 1 && week ? await query(`
    SELECT TOP (@pageSize)
      ...
      p.ProdName, p.DisplayName, p.FlowerName AS DbFlowerName, p.CounName AS DbCountryName,
      p.CountryFlower, NULL AS FarmName, NULL AS AutoFarmKey, NULL AS AutoFarmName, p.OutUnit AS Unit,
    ...
    WHERE CONVERT(NVARCHAR(4),wm.OrderYear)=@year AND ${arrivalWeekPredicate('wm.OrderWeek')}
      AND ISNULL(wm.isDeleted,0)=0 AND ISNULL(p.isDeleted,0)=0
      AND (@flower=N'' OR (@flower<>N'기타' AND ${COUNTRY_FLOWER_SQL}=@flower))
      AND (
        @product=N''
      ...
      AND NOT EXISTS (SELECT 1 FROM dbo.WebArrivalCostLine x WHERE x.OrderYear=@year AND ${arrivalWeekPredicate('x.OrderWeek')} AND x.IsCurrent=1 AND x.ProdKey=p.ProdKey)
    GROUP BY p.ProdKey,p.CounName,p.FlowerName,p.CountryFlower,p.ProdName,p.DisplayName,p.OutUnit,usageStats.UsageCount,matchStats.MatchCount
    ORDER BY ISNULL(usageStats.UsageCount,0)+ISNULL(matchStats.MatchCount,0)*2 DESC,p.FlowerName,p.CounName,p.ProdName,p.ProdKey`, {
      ...
      ...Object.fromEntries(Object.entries(params).filter(([key]) => key.startsWith('mapPk') || key.startsWith('product'))),
    }) : { recordset: [] };
  ...
  const matchedVarieties = [...new Set([
    ...(varietyRes.recordset || []).map((row) => row.CountryFlower),
    ...missingRows.map((row) => row.countryFlower),
  ].filter(Boolean))].sort((a, b) => Number(a === '기타') - Number(b === '기타') || a.localeCompare(b, 'ko'));
  return {
...
export function arrivalCostSupersedeScopes(rows = []) {
  const scopes = new Set();
  const weeks = new Set();
  for (const row of rows) {
    const week = String(row.orderWeek || '');
    const country = String(row.countryName || '');
    scopes.add(`${week}|${country}`);
    weeks.add(week);
  }
  for (const week of weeks) scopes.add(`${week}|`);
  return [...scopes];
}

export async function createArrivalCostImport({ parsed, fileName, user, orderYear, driveSource = null }) {
  await ensureArrivalCostTables();
  const a = actor(user);
  ...
  const scopes = arrivalCostSupersedeScopes(parsed.rows);
  return withTransaction(async (tQuery) => {
    // Manual and automatic uploads share a DB-owned lock: no duplicate revision or partial replacement.
    await tQuery(`DECLARE @rc INT; EXEC @rc=sys.sp_getapplock @Resource=N'WebArrivalCostImport', @LockMode='Exclusive', @LockOwner='Transaction', @LockTimeout=10000; IF @rc<0 THROW 51000,N'도착원가 등록 잠금 대기 중입니다',1;`);
    if (driveSource) {
      const sourceParams = { year: { type: sql.NVarChar, value: year },
        week: { type: sql.NVarChar, value: driveSource.week }, country: { type: sql.NVarChar, value: driveSource.country },
        paddedWeek: { type: sql.NVarChar, value: driveSource.week.split('-').map(v => v.padStart(2, '0')).join('-') },
        sha: { type: sql.NVarChar, value: driveSource.sha } };
      const previous = await tQuery(`SELECT h.ImportKey,h.AfterJson FROM dbo.WebArrivalCostHistory h
        JOIN dbo.WebArrivalCostImport i ON i.ImportKey=h.ImportKey
        WHERE h.ActionType=N'DRIVE_IMPORT' AND i.OrderYear=@year
          AND JSON_VALUE(h.AfterJson,'$.week')=@week AND JSON_VALUE(h.AfterJson,'$.sha')=@sha`, sourceParams);
      if (previous.recordset.length) return { duplicate: true, importKey: previous.recordset[0].ImportKey };
      const current = await tQuery(`SELECT DISTINCT l.ImportKey,h.AfterJson FROM dbo.WebArrivalCostLine l
        LEFT JOIN dbo.WebArrivalCostHistory h ON h.ImportKey=l.ImportKey AND h.ActionType=N'DRIVE_IMPORT'
        WHERE l.OrderYear=@year AND l.OrderWeek IN (@week,@paddedWeek) AND (l.CountryName=@country OR l.CountryName=N'') AND l.IsCurrent=1`, sourceParams);
      for (const row of current.recordset) {
        const before = row.AfterJson ? JSON.parse(row.AfterJson) : null;
        if (!before || before.family !== driveSource.family) throw new Error('수동 등록 또는 다른 원가 파일군의 현재본이 있어 자동 교체를 보류했습니다');
        if (before.modified >= driveSource.modified) throw new Error('이미 반영된 원본보다 이전이거나 동일한 수정시각입니다');
      }
      const edited = await tQuery(`SELECT TOP 1 h.HistoryKey FROM dbo.WebArrivalCostHistory h
        JOIN dbo.WebArrivalCostLine l ON l.ArrivalLineKey=h.ArrivalLineKey
        WHERE l.OrderYear=@year AND l.OrderWeek=@week AND l.CountryName=@country AND l.IsCurrent=1
          AND h.ActionType IN (N'MATCH',N'BASIS_CHANGE')`, sourceParams);
      if (edited.recordset.length) throw new Error('현재 원가에 수동 매칭·배분기준 수정 이력이 있어 자동 교체를 보류했습니다');
    }
    ...
    // 국가가 비어 있던 이전 차수 현재본(16-1B→33-2 유령 행)도 같은 차수에서 내린다.
    // 행마다 이력을 넣지 않고 집합 INSERT로 남겨, 2천 행 재업로드가 nginx 502가 나지 않게 한다.
    for (const scope of scopes) {
      const [week, country] = scope.split('|');
      const scopeParams = {
        year: { type: sql.NVarChar, value: year },
        week: { type: sql.NVarChar, value: nv(week, 10) },
        country: { type: sql.NVarChar, value: nv(country, 100) },
        importKey: { type: sql.Int, value: importKey },
        after: { type: sql.NVarChar, value: JSON.stringify({ importKey }) },
        by: { type: sql.NVarChar, value: a.id },
        name: { type: sql.NVarChar, value: a.name },
      };
      await tQuery(
        `INSERT INTO dbo.WebArrivalCostHistory (ArrivalLineKey,ImportKey,ActionType,BeforeJson,AfterJson,ChangedBy,ChangedByName)
         SELECT ArrivalLineKey,@importKey,N'SUPERSEDE',N'',@after,@by,@name
           FROM dbo.WebArrivalCostLine
    ...
    await insertArrivalCostLines(tQuery, parsed.rows, {
      importKey,
      year,
      fileName,
      actorId: a.id,
    ...
    if (driveSource) {
      await tQuery(`INSERT INTO dbo.WebArrivalCostHistory (ImportKey,ActionType,AfterJson,ChangedBy,ChangedByName)
        VALUES (@key,N'DRIVE_IMPORT',@source,@by,@name)`, {
        key: { type: sql.Int, value: importKey }, source: { type: sql.NVarChar, value: JSON.stringify(driveSource) },
        by: { type: sql.NVarChar, value: a.id }, name: { type: sql.NVarChar, value: a.name },
      });
      const verify = await tQuery(`SELECT COUNT(*) AS cnt FROM dbo.WebArrivalCostLine WHERE ImportKey=@key AND IsCurrent=1`, { key: { type: sql.Int, value: importKey } });
      if (Number(verify.recordset[0]?.cnt) !== parsed.rows.length) throw new Error('도착원가 등록 후 행수 검증 실패');
    }
