...
import { bindArrivalCostProductMatch, buildArrivalCostProductMatch } from './arrivalCostProductSearch.js';
...
export async function listArrivalCost({
  ...
  if (flower) {
    where.push('(l.FlowerNameRaw LIKE @flower OR p.FlowerName LIKE @flower)');
    params.flower = { type: sql.NVarChar, value: `%${nv(flower, 200)}%` };
  }
  void country;
  const varietyParams = Object.fromEntries(Object.entries(params).filter(([key]) => !['offset', 'pageSize', 'flower'].includes(key)));
  const [rowsRes, importsRes, varietyRes] = await Promise.all([
    ...
    hasProductSearch ? query(`
      SELECT DISTINCT TOP 40 NULLIF(LTRIM(RTRIM(COALESCE(p.FlowerName, l.FlowerNameRaw))), N'') AS FlowerName
        FROM dbo.WebArrivalCostLine l
        LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
       WHERE ${scopeWhere.join(' AND ')}
         AND NULLIF(LTRIM(RTRIM(COALESCE(p.FlowerName, l.FlowerNameRaw))), N'') IS NOT NULL
  ...
  const missingRes = safePage === 1 && week ? await query(`
    ...
    ORDER BY ISNULL(usageStats.UsageCount,0)+ISNULL(matchStats.MatchCount,0)*2 DESC,p.FlowerName,p.CounName,p.ProdName,p.ProdKey`, {
      ...
      ...Object.fromEntries(Object.entries(params).filter(([key]) => key.startsWith('mapPk'))),
    }) : { recordset: [] };
  ...
  return {
    ...
    products: [],
    farms: [],
    matchedVarieties: (varietyRes.recordset || []).map((row) => row.FlowerName).filter(Boolean),
    total,
    page: safePage,
