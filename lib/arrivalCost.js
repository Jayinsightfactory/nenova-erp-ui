...
export async function listArrivalCostVarieties({ orderYear, orderWeek } = {}) {
  ...
  const result = await query(`
    ...
    SELECT FlowerName FROM varieties WHERE FlowerName IS NOT NULL ORDER BY FlowerName`, scope);
...
export async function listArrivalCost({
  ...
  await ensureArrivalCostTables();
  const scope = requiredScope(orderYear, orderWeek);
  if (!flower && !allVarieties) throw new Error('품종을 선택하거나 전체보기를 명시하세요.');
  const where = ['l.OrderYear=@year', 'l.OrderWeek=@week'];
  const params = {
  ...
  if (product) {
    const compactProduct = String(product).toLocaleLowerCase('ko-KR').replace(/[^\p{L}\p{N}]+/gu, '');
    where.push(`(
    ...
    params.product = { type: sql.NVarChar, value: `%${nv(product, 300)}%` };
    params.productCompact = { type: sql.NVarChar, value: `%${nv(compactProduct, 300)}%` };
  }
  ...
  const [rowsRes, importsRes] = await Promise.all([
    query(`${LINE_SELECT} WHERE ${where.join(' AND ')} ORDER BY
      ...
      l.FlowerNameRaw, l.CountryName, l.ProductNameRaw, l.ArrivalLineKey
      OFFSET @offset ROWS FETCH NEXT @pageSize ROWS ONLY`, {
      ...params,
      hasSearch: { type: sql.Int, value: product || farm || country ? 1 : 0 },
    }),
  ...
  const missingRes = safePage === 1 ? await query(`
    SELECT TOP (@pageSize)
    ...
    WHERE CONVERT(NVARCHAR(4),wm.OrderYear)=@year AND wm.OrderWeek=@week
      AND ISNULL(wm.isDeleted,0)=0 AND ISNULL(p.isDeleted,0)=0
      AND (@flower=N'' OR p.FlowerName=@flower)
      AND (@country=N'' OR p.CounName LIKE @country)
      AND (@product=N'' OR p.ProdName LIKE @product OR p.DisplayName LIKE @product)
    ...
    ORDER BY ISNULL(usageStats.UsageCount,0)+ISNULL(matchStats.MatchCount,0)*2 DESC,p.FlowerName,p.CounName,p.ProdName,p.ProdKey`, {
      year: params.year, week: params.week, pageSize: params.pageSize,
      flower: { type: sql.NVarChar, value: nv(flower || '', 200) },
      country: { type: sql.NVarChar, value: country ? `%${nv(country, 100)}%` : '' },
      product: { type: sql.NVarChar, value: `%${nv(product || '', 300)}%` },
