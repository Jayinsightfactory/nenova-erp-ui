...
function sqlDecimal(value, precision = 18, scale = 4) {
  // db.query/withTransaction pass only type/value to Request.input. Precision
  // must be on the MSSQL type, not sibling properties (which default to scale0).
  return { type: sql.Decimal(precision, scale), value: n(value) };
}
...
function arrivalLineInsertParams(row, {
  ...
  return {
    ...
    [k('fob')]: sqlDecimal(row.fobUSD, 18, 6),
    [k('freightUnit')]: sqlDecimal(row.freightPerUnitUSD, 18, 6),
    ...
    [k('fx')]: sqlDecimal(row.exchangeRate, 18, 6),
    [k('gw')]: sqlDecimal(row.grossWeight),
    [k('cw')]: sqlDecimal(row.chargeableWeight),
    [k('freight')]: sqlDecimal(row.freightUSD, 18, 6),
    [k('invoice')]: sqlDecimal(row.invoiceUSD, 18, 6),
    [k('weightShare')]: sqlDecimal(row.weightMetricShare, 18, 8),
    [k('volumeShare')]: sqlDecimal(row.volumeMetricShare, 18, 8),
    [k('valueShare')]: sqlDecimal(row.valueMetricShare, 18, 8),
