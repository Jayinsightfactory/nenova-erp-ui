  ...
  const result = await query(`
    WITH varieties AS (
      SELECT ${COUNTRY_FLOWER_SQL} AS CountryFlower
        FROM dbo.WebArrivalCostLine l
        LEFT JOIN dbo.Product p ON p.ProdKey=l.ProdKey
       WHERE l.OrderYear=@year AND l.OrderWeek=@week AND l.IsCurrent=1 AND l.ProdKey IS NOT NULL
      UNION
      ...
      SELECT N'기타'
        WHERE EXISTS (
          SELECT 1 FROM dbo.WebArrivalCostLine l
           WHERE l.OrderYear=@year AND l.OrderWeek=@week AND l.IsCurrent=1 AND l.ProdKey IS NULL
        )
    )
