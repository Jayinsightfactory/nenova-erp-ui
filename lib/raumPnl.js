...
export async function prepareRaumPnlImportPreview(batches) {
  ...
  for (const batch of batches || []) {
    ...
    prepared.push({ ...batch, importedItems: batch.items, items: merged.items, preservation: merged.preservation, itemFingerprint: canonicalItemFingerprint(merged.items) });
  }
