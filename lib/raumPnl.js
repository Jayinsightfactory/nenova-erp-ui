...
export async function ensureRaumPnlTables() {
  ...
  _ensured = query(
    `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name='WebRaumPnl')
     ...
     -- 2026-08-25 라움/초이문 거래처 구분. 기존 행은 라움.
     IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('WebRaumPnl') AND name = 'PartnerCode')
       ALTER TABLE WebRaumPnl ADD PartnerCode NVARCHAR(20) NOT NULL CONSTRAINT DF_WebRaumPnl_PartnerCode DEFAULT N'raum';
