...
export async function lookupErpRefPrices(quoteItems, major, orderYear) {
  ...
  // 도착원가 — 해당 차수부터 과거로 내려가며 품목별 가장 최근 값 (카탈로그/차수피벗과 동일 엔진)
  let arrivalMap = {};
  let arrivalErr = null;
  try {
    const arr = await getArrivalCostsWithFallback({
      orderYear: String(orderYear),
      anchorWeek: `${String(major).padStart(2, '0')}-02`,
      maxWeeks: 26,
    });
    arrivalMap = arr.map || {};
  } catch (e) {
    ...
    for (const it of items || []) {
      ...
      await tQuery(
        `INSERT INTO WebRaumPnlItem
           (PnlKey, Seq, ItemName, Unit, Qty, BranchJson, SalePrice, SaleAmount, CostPrice, RefPrice, RefSource, ErpSalePrice, ProdKey, Remark, IsCustom, CostSource, IsConsigned)
         VALUES (@key, @seq, @name, @unit, @qty, @bj, @sp, @sa, @cp, @rp, @rs, @esp, @pk, @rm, @ic, @cs, @icon)`,
        {
