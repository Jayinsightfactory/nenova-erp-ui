...
import { matchRaumPnlPreservationRows } from './raumPnlPreservationMatch';
import { learnManualRaumCost, saveRaumConsignedRecord, saveRaumItemMapRecord } from './raumPnlCost';
import { fillConsignedCostsFromOrdinary } from './raumPnlConsignedCost';
import { applyShillaHotelAutoMatches } from './shillaPnlHotelMatch';
import {
  defaultPnlTitle, resolvePnlPartner,
} from './raumPnlPartner';
import { requirePnlPartner } from './pnlHotelRegistry';
import { evaluateRaumPnlImportReview, raumPnlImportSaveError } from './raumPnlImportReview';
...
async function requireServerPnlPartner(code, tQuery = query) {
  return requirePnlPartner(code, tQuery);
}

function assertExactPartnerDescriptor(code, descriptor) {
  const requested = String(code == null || code === '' ? 'raum' : code).trim().toLowerCase();
  if (!descriptor || String(descriptor.code || '').trim().toLowerCase() !== requested) {
    throw new Error('선택한 호텔 정보가 달라졌습니다. 호텔을 다시 선택한 뒤 파일을 올려주세요.');
  }
  return descriptor;
}

export function requireExplicitShillaNenovaPct(value) {
  if (value == null || (typeof value === 'string' && value.trim() === '')) {
    const error = new Error('신라호텔 네노바 비율은 원본에서 명시된 0~100 값이 필요합니다.');
    error.code = 'SHILLA_PNL_RATIO_REQUIRED';
    throw error;
  }
  const pct = Number(value);
  if (!Number.isFinite(pct) || pct < 0 || pct > 100) {
    const error = new Error('신라호텔 네노바 비율은 0~100 사이의 명시적 숫자여야 합니다.');
    error.code = 'SHILLA_PNL_RATIO_INVALID';
    throw error;
  }
  return pct;
}

/** API의 shillaSettlementItem을 우회한 raw parser 행은 저장 계층에서 거부한다. */
export function assertCanonicalShillaImportItems(items) {
  for (const [index, item] of (items || []).entries()) {
    const missing = ['price', 'supply', 'costPrice'].filter(field => !Object.prototype.hasOwnProperty.call(item || {}, field));
    if (missing.length) {
      const error = new Error(`신라호텔 ${index + 1}번 품목이 저장 형식으로 변환되지 않았습니다: ${missing.join(', ')}`);
      error.code = 'SHILLA_PNL_CANONICAL_ITEM_REQUIRED';
      throw error;
    }
  }
  return items || [];
...
export function shouldLearnRaumPnlManualCost(partnerCode, item, costPrice, partnerDescriptor) {
  const partner = resolvePnlPartner(partnerCode, partnerDescriptor);
  return partner.code !== 'shilla'
    && partner.customHotel !== true
    && costPrice != null
    && !item?.isCustom
...
export async function lookupErpRefPrices(quoteItems, major, orderYear, partnerCode) {
  ...
  const partner = await requireServerPnlPartner(partnerCode);
  if (partner.erpSync === false) return {};
...
export async function saveRaumPnl({ orderYear, major, title, quoteDate, nenovaPct, note, sourceFile, images, items, verification, actor, partnerCode, confirmGangnamMerge = false }) {
  const partner = await requireServerPnlPartner(partnerCode);
  assertStrictCustomHotelOrderYear(orderYear, partner);
  if (partner.code === 'shilla') {
    const error = new Error('신라호텔 결산은 일반 전체 저장을 사용할 수 없습니다. 검증된 엑셀 가져오기 또는 차수별 매입단가 저장을 사용하세요.');
    error.code = 'SHILLA_PNL_WHOLE_DOCUMENT_SAVE_BLOCKED';
    throw error;
  }
  const review = evaluateRaumPnlImportReview([{ partnerCode: partner.code, verification }], confirmGangnamMerge);
  const reviewError = raumPnlImportSaveError(review);
  ...
  return withTransaction(async (tQuery) => {
    // Registry activation is checked again while the settlement write lock is held.
    await requireServerPnlPartner(partner.code, tQuery);
    ...
    const common = {
      title: { type: sql.NVarChar, value: partner.customHotel === true
        ? defaultPnlTitle(partner.code, mj, '', partner)
        : (title || defaultPnlTitle(partner.code, mj, '', partner)) },
    ...
    // 저장 직전 재보장 — 클라이언트가 보낸 빈 사입 단가도 같은 품목명+단위 일반행 단가로 채운다.
    const filledItems = partner.customHotel === true ? (items || []) : fillConsignedCostsFromOrdinary(items || []);
    for (const it of filledItems) {
      ...
      if (shouldLearnRaumPnlManualCost(partner.code, it, cp, partner)) {
...
// 단가 행으로 이동하지 않는다.
const importItemKey = (it, partnerCode, partnerDescriptor) => {
  const base = costKey(it?.name ?? it?.ItemName);
  const unit = resolvePnlPartner(partnerCode, partnerDescriptor).code === 'shilla'
    ? `|${normSpace(it?.unit ?? it?.Unit)}`
    : '';
  return `${base}${unit}|${Number(it?.price ?? it?.SalePrice ?? 0).toFixed(2)}|${Number(it?.consigned ?? it?.IsConsigned ?? 0) ? 'C' : ''}`;
};
const importBatchKey = (orderYear, major, partnerCode, partnerDescriptor) => `${resolvePnlPartner(partnerCode, partnerDescriptor).code}:${String(orderYear)}-${String(major).padStart(2, '0')}`;

function requiresExplicitImportYear(partner) {
  return partner.code === 'shilla' || partner.customHotel === true;
}

function assertStrictCustomHotelOrderYear(orderYear, partner) {
  if (partner.customHotel === true && !/^\d{4}$/.test(String(orderYear || '').trim())) {
    const error = new Error(`${partner.label} 결산 연도는 네 자리 숫자로 입력하세요.`);
    error.code = 'PNL_CUSTOM_HOTEL_YEAR_REQUIRED';
    error.statusCode = 400;
    throw error;
  }
}

function assertExplicitImportYear(batch, partner) {
  if (requiresExplicitImportYear(partner) && !/^\d{4}$/.test(String(batch.orderYear || '').trim())) {
    const error = new Error(`${partner.label} 원본의 결산 연도를 선택하세요.`);
    error.code = 'PNL_IMPORT_YEAR_REQUIRED';
    error.statusCode = 400;
    throw error;
  }
}

async function requireImportBatchPartners(batches, tQuery = query) {
  const partners = [];
  for (const batch of batches || []) {
    const partner = assertExactPartnerDescriptor(batch?.partnerCode, await requireServerPnlPartner(batch?.partnerCode, tQuery));
    assertExplicitImportYear(batch || {}, partner);
...
export async function loadRaumPnlImportSnapshots(batches) {
  const partners = await requireImportBatchPartners(batches);
  await ensureRaumPnlTables();
  const out = {};
  for (const [index, batch] of (batches || []).entries()) {
    ...
    out[importBatchKey(orderYear, major, batch.partnerCode, partner)] = { pnlKey: row?.PnlKey || null, version: pnlVersion(row) };
  }
  return out;
}
...
// Both preview and save use this exact candidate shape.  Product is joined only
// when active, so a deleted/inactive mapping can never teach a new upload.
export const SHILLA_HOTEL_IMPORT_MATCH_SQL = {
  preview: `SELECT m.OrderYear, i.ItemName, i.Unit, i.ProdKey, ISNULL(i.IsCustom, 0) AS IsCustom
              FROM WebRaumPnl AS m
              INNER JOIN WebRaumPnlItem AS i ON i.PnlKey=m.PnlKey
              INNER JOIN Product AS p ON p.ProdKey=i.ProdKey AND p.isDeleted=0
  ...
  masters: `SELECT m.PnlKey
              FROM WebRaumPnl AS m WITH (UPDLOCK, HOLDLOCK)
             WHERE m.OrderYear=@yr AND m.PartnerCode='shilla' AND ISNULL(m.isDeleted, 0)=0
             ORDER BY m.PnlKey`,
  items: `SELECT i.ItemName, i.Unit, i.ProdKey, ISNULL(i.IsCustom, 0) AS IsCustom
            FROM WebRaumPnlItem AS i WITH (UPDLOCK, HOLDLOCK)
            INNER JOIN WebRaumPnl AS m ON m.PnlKey=i.PnlKey
           WHERE m.OrderYear=@yr AND m.PartnerCode='shilla' AND ISNULL(m.isDeleted, 0)=0
           ORDER BY m.PnlKey, i.ItemKey`,
  product: `SELECT p.ProdKey FROM Product AS p WITH (HOLDLOCK)
             WHERE p.ProdKey=@prodKey AND p.isDeleted=0`,
};

async function loadShillaImportAutoMatchCandidates(years, runQuery) {
  const byYear = new Map();
  for (const year of years) {
    const result = await runQuery(SHILLA_HOTEL_IMPORT_MATCH_SQL.preview, {
      yr: { type: sql.NVarChar, value: year },
    });
    byYear.set(year, result.recordset || []);
  }
  return byYear;
}

/** Transaction form: all callers acquire years in sort order, then master/item/Product locks. */
async function lockShillaImportAutoMatchCandidates(years, tQuery) {
  const byYear = new Map();
  for (const year of years) {
    const yearParam = { yr: { type: sql.NVarChar, value: year } };
    const lock = await tQuery(SHILLA_HOTEL_IMPORT_MATCH_SQL.yearLock, {
      resource: { type: sql.NVarChar, value: `shilla-pnl-year:${year}` },
    });
    if (Number(lock.recordset?.[0]?.LockResult) < 0) {
      const error = new Error('같은 신라호텔 결산 연도가 다른 작업에서 사용 중입니다. 잠시 후 다시 시도하세요.');
      error.code = 'SHILLA_YEAR_LOCK_UNAVAILABLE';
      error.statusCode = 409;
      throw error;
    }
    await tQuery(SHILLA_HOTEL_IMPORT_MATCH_SQL.masters, yearParam);
    const rows = (await tQuery(SHILLA_HOTEL_IMPORT_MATCH_SQL.items, yearParam)).recordset || [];
    const active = new Set();
    for (const prodKey of [...new Set(rows.map(row => Number(row.ProdKey)).filter(Number.isInteger).filter(key => key > 0))].sort((left, right) => left - right)) {
      const product = await tQuery(SHILLA_HOTEL_IMPORT_MATCH_SQL.product, {
        prodKey: { type: sql.Int, value: prodKey },
      });
      if (product.recordset?.[0]) active.add(prodKey);
    }
    byYear.set(year, rows.filter(row => active.has(Number(row.ProdKey))));
  }
  return byYear;
}

export function mergePnlImportedItems(imported, existingRows, partnerCode, options = {}) {
  const partner = resolvePnlPartner(partnerCode, options.partnerDescriptor || options.partner);
  const incoming = partner.code === 'shilla' ? assertCanonicalShillaImportItems(imported) : (imported || []);
  const custom = [];
  const matched = matchRaumPnlPreservationRows(incoming, existingRows, partner.code);
  const orderYear = /^\d{4}$/.test(String(options.orderYear || '').trim()) ? String(options.orderYear).trim() : null;
  const majorDigits = String(options.major || '').replace(/[^0-9]/g, '');
  const major = majorDigits ? String(Number(majorDigits)) : null;
  const mergeContext = `${partner.label || partner.code} · ${orderYear ? `${orderYear}년` : '결산 연도 미상'} · ${major ? `${major}차` : '차수 미상'}`;
  const collisionDetails = matched.collisions.map(collision => {
    const item = incoming[collision.incomingIndex] || {};
    const originalSource = normSpace(item.sourceRemark || item.remark || item.Remark) || '원본 위치를 알 수 없음';
    const itemName = normSpace(item.name || item.ItemName) || null;
    // `??` deliberately preserves a source price of 0 for structured clients.
    const salePrice = item.price ?? item.SalePrice ?? null;
    return {
      ...collision,
      context: mergeContext,
      originalSource,
      location: {
        hotel: partner.label || partner.code || null,
  ...
  const preservation = {
    manualCost: 0, prodKey: 0, custom: 0, fallback: matched.fallbackCount,
    collisions: [...new Set(collisionDetails.map(collision => `${mergeContext} · 원본 ${collision.originalSource}: ${collision.message}`))],
  };
  ...
  const merged = incoming.map((it, index) => {
    const old = matched.matches[index];
  ...
  if (preservation.collisions.length) {
    const err = new Error(`기존 매입단가·품목 연결을 보존하려면 행 확인이 필요합니다. ${preservation.collisions.join('; ')} 기존 저장본은 변경하지 않았습니다.`);
    err.code = 'PRESERVATION_COLLISION';
    err.details = collisionDetails;
    throw err;
  }
  const combined = [...merged, ...custom.map((it, index) => ({ ...it, seq: merged.length + index + 1 }))];
  // 일반행 매입단가 → 같은 품목명(사입 suffix 제거)+단위의 빈 사입행 즉시 연결 (미리보기·저장 동일 결과 보장).
  return { items: partner.customHotel === true ? combined : fillConsignedCostsFromOrdinary(combined), preservation };
}
...
export async function prepareRaumPnlImportPreview(batches) {
  const partners = await requireImportBatchPartners(batches);
  for (const [index, batch] of (batches || []).entries()) {
    if (partners[index].code === 'shilla') {
      requireExplicitShillaNenovaPct(batch.nenovaPct);
      assertCanonicalShillaImportItems(batch.items);
    }
  }
  const snapshots = await loadRaumPnlImportSnapshots(batches);
  const shillaMatchesByYear = await loadShillaImportAutoMatchCandidates(
    (batches || []).filter((_, index) => partners[index].code === 'shilla').map(batch => String(batch.orderYear)).filter((value, index, all) => all.indexOf(value) === index).sort(), query,
  );
  const prepared = [];
  for (const [index, batch] of (batches || []).entries()) {
    const partner = partners[index];
    const pnlKey = snapshots[importBatchKey(batch.orderYear, batch.major, batch.partnerCode, partner)]?.pnlKey;
    const oldItems = pnlKey
      ...
      : [];
    const importedItems = partner.code === 'shilla'
      ? assertCanonicalShillaImportItems(batch.items)
      : batch.items;
    const merged = mergePnlImportedItems(importedItems, oldItems, batch.partnerCode, {
      partnerDescriptor: partner, orderYear: batch.orderYear, major: batch.major,
    });
    const autoMatch = partner.code === 'shilla'
      ? applyShillaHotelAutoMatches(merged.items, shillaMatchesByYear.get(String(batch.orderYear)) || [])
      : { items: merged.items, autoMatchedCount: 0 };
    const existingImportedItems = oldItems.filter(row => !row.IsCustom).map(dbItemToDraft);
    const existingDiff = pnlKey ? diffRaumPnlUpload(existingImportedItems, importedItems) : null;
    prepared.push({ ...batch, importedItems, items: autoMatch.items, preservation: merged.preservation,
      autoMatchedCount: autoMatch.autoMatchedCount, existingDiff, itemFingerprint: canonicalItemFingerprint(autoMatch.items) });
  }
  return { batches: prepared, snapshots };
}

async function assertImportBatches(batches, confirmGangnamMerge = false) {
  ...
  for (const [index, batch] of batches.entries()) {
    ...
    if (partner.code === 'shilla') {
      requireExplicitShillaNenovaPct(batch.nenovaPct);
      assertCanonicalShillaImportItems(batch.importedItems || batch.items);
    }
    const identity = importBatchKey(batch.orderYear, major, partner.code, partner);
    if (identities.has(identity)) throw new Error(`중복 차수: ${identity}`);
    identities.add(identity);
    if (!Array.isArray(batch.verification) || batch.verification.some(c => c?.ok !== true)) {
      throw new Error(`${Number(major)}차 검증에 실패해 전체 저장을 중단했습니다.`);
    }
  }
  const review = evaluateRaumPnlImportReview(batches, confirmGangnamMerge);
  const reviewError = raumPnlImportSaveError(review);
  if (reviewError) throw new Error(reviewError);
  return partners;
}
 ...
 */
export async function saveRaumPnlImportBatch({ batches, sourceFile, actor, expectedSnapshots = {}, confirmGangnamMerge = false }) {
  await assertImportBatches(batches, confirmGangnamMerge);
  await ensureRaumPnlTables();
  return withTransaction(async (tQuery) => {
    const transactionPartners = await requireImportBatchPartners(batches, tQuery);
    const shillaMatchesByYear = await lockShillaImportAutoMatchCandidates(
      (batches || []).filter((_, index) => transactionPartners[index].code === 'shilla').map(batch => String(batch.orderYear)).filter((value, index, all) => all.indexOf(value) === index).sort(), tQuery,
    );
    const results = [];
    for (const [index, batch] of batches.entries()) {
      const orderYear = String(batch.orderYear);
      const major = String(batch.major).padStart(2, '0');
      const partner = transactionPartners[index];
      const identity = importBatchKey(orderYear, major, partner.code, partner);
      ...
      const oldItems = pnlKey
        ...
        : [];
      const liveMerged = mergePnlImportedItems(batch.importedItems || batch.items, oldItems, partner.code, {
        partnerDescriptor: partner, orderYear, major,
      });
      const autoMatch = partner.code === 'shilla'
        ? applyShillaHotelAutoMatches(liveMerged.items, shillaMatchesByYear.get(orderYear) || [])
        : { items: liveMerged.items, autoMatchedCount: 0 };
      if (batch.itemFingerprint && batch.itemFingerprint !== canonicalItemFingerprint(autoMatch.items)) {
        throw new Error(`${Number(major)}차의 수기 원가·품목 매칭 보존 결과가 미리보기 이후 변경되었습니다. 다시 미리보기하세요.`);
      }
      const items = autoMatch.items;
      ...
      if (pnlKey) {
        ...
        const ins = await tQuery(
           ...
           VALUES (@yr, @mj, @pc, @title, @qd, @pct, @note, @src, @vj, @actor)`,
          {
            ...
            pc: { type: sql.NVarChar, value: partner.code },
            title: { type: sql.NVarChar, value: defaultPnlTitle(partner.code, major, '', partner) }, qd: { type: sql.Date, value: qd },
            pct: { type: sql.Float, value: partner.code === 'shilla' ? requireExplicitShillaNenovaPct(batch.nenovaPct) : DEFAULT_NENOVA_PCT },
            note: { type: sql.NVarChar, value: partner.code === 'shilla' ? (batch.warnings || []).join('\n') : '' },
            src: { type: sql.NVarChar, value: sourceFile || '' }, vj: { type: sql.NVarChar, value: verify },
            actor: { type: sql.NVarChar, value: actor || 'user' },
          }
        );
      ...
      results.push({ orderYear, major, pnlKey, itemCount: items.length, autoMatchedCount: autoMatch.autoMatchedCount });
    }
    return results;
  });
}
...
export async function loadRaumPnlList(partnerCode) {
  ...
  const r = await query(
    `SELECT m.PnlKey, m.OrderYear, m.MajorWeek, m.PartnerCode, m.Title, m.QuoteDate, m.AssignedMonth, m.NenovaPct, m.Note, m.SourceFile,
            ...
            SUM(CASE WHEN i.ProdKey IS NULL AND ISNULL(i.IsCustom,0)=0 AND ISNULL(i.IsConsigned,0)=0 THEN 1 ELSE 0 END) AS MissingMatch
       FROM WebRaumPnl m
...
const CUSTOM_HOTEL_CODE_RE = /^hotel_[0-9a-f]{12}$/;

async function assertCustomMasterPartner(master, partner, tQuery = query) {
  const actualCode = String(master?.PartnerCode || '').trim().toLowerCase();
  // Do not let a built-in/default request reach a custom master by PnlKey.
  // Reading the actual descriptor also rejects a hotel deactivated after the
  // list/detail view was opened; writes make that read under their lock.
  if (partner?.customHotel === true || CUSTOM_HOTEL_CODE_RE.test(actualCode)) {
    await requireServerPnlPartner(actualCode, tQuery);
  }
  if (actualCode !== partner.code) {
    const error = new Error('선택한 호텔의 결산이 아닙니다.');
    error.code = 'PNL_CUSTOM_HOTEL_MASTER_MISMATCH';
    throw error;
  }
}

export async function loadRaumPnlDetail(pnlKey, { partnerCode } = {}) {
  const partner = await requireServerPnlPartner(partnerCode);
  await ensureRaumPnlTables();
  const m = await query(
    `SELECT * FROM WebRaumPnl WHERE PnlKey=@key AND isDeleted=0`,
    { key: { type: sql.Int, value: Number(pnlKey) } },
  );
  const master = m.recordset[0];
  if (!master) return null;
  await assertCustomMasterPartner(master, partner);
  const i = await query(
    `SELECT i.*, p.ProdName AS MatchedProdName
       FROM WebRaumPnlItem i
       LEFT JOIN Product p ON p.ProdKey = i.ProdKey AND p.isDeleted=0
      WHERE i.PnlKey=@key
      ORDER BY i.Seq, i.ItemKey`,
    { key: { type: sql.Int, value: Number(pnlKey) } },
  );
  ...
  return {
    ...
    items: (i.recordset || []).map(row => ({
      ...
      prodName: row.MatchedProdName || null,
      remark: row.Remark || '',
...
export async function assignRaumPnlMonth(pnlKey, assignedMonth, actor, { partnerCode } = {}) {
  const requestedPartner = await requireServerPnlPartner(partnerCode);
  await ensureRaumPnlTables();
  return withTransaction(async (tQuery) => {
    const partner = await requireServerPnlPartner(requestedPartner.code, tQuery);
    const current = await tQuery(
      `SELECT TOP 1 PnlKey, OrderYear, PartnerCode FROM WebRaumPnl WITH (UPDLOCK, HOLDLOCK) WHERE PnlKey=@key AND isDeleted=0`,
      { key: { type: sql.Int, value: Number(pnlKey) } }
    );
    const row = current.recordset?.[0];
    if (!row) throw new Error('해당 손익계산서가 없습니다.');
    await assertCustomMasterPartner(row, partner, tQuery);
    ...
    await tQuery(
      ...
      {
        key: { type: sql.Int, value: Number(pnlKey) }, month: { type: sql.Char, value: normalized },
        actor: { type: sql.NVarChar, value: actor || 'user' },
      }
    );
...
export async function deleteRaumPnl(pnlKey, actor, { partnerCode } = {}) {
  const requestedPartner = await requireServerPnlPartner(partnerCode);
  await ensureRaumPnlTables();
  await withTransaction(async (tQuery) => {
    const partner = await requireServerPnlPartner(requestedPartner.code, tQuery);
    const current = await tQuery(
      `SELECT TOP 1 PnlKey, PartnerCode FROM WebRaumPnl WITH (UPDLOCK, HOLDLOCK) WHERE PnlKey=@key AND isDeleted=0`,
      { key: { type: sql.Int, value: Number(pnlKey) } }
    );
    const row = current.recordset?.[0];
    if (!row) throw new Error('해당 손익계산서가 없습니다.');
