 ...
 */
export function parseRaumQuoteWorkbookGroups(XLSX, workbook) {
  const warnings = [];
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    if (!parsed.major) {
      warnings.push(`${sheetName}: 시트명에서 차수를 찾지 못해 제외했습니다. (예: 27차강남양식)`);
      continue;
    }
    // 라움 통합 견적서는 강남/건대 두 지점만 같은 차수 안에서 합산한다. 그 밖의 표는
    // 비슷한 헤더를 가진 참고 시트일 수 있으므로 조용히 섞지 않고 명시적으로 제외한다.
    if (parsed.branch !== '강남' && parsed.branch !== '건대') {
      warnings.push(`${sheetName}: 강남/건대 라움 시트가 아니어서 제외했습니다.`);
      continue;
    }
    const major = String(parsed.major).padStart(2, '0');
    if (!byMajor.has(major)) byMajor.set(major, []);
    byMajor.get(major).push({ ...parsed, parsedSupply: parsed.items.reduce((a, it) => a + it.supply, 0) });
  }
  ...
  if (!batches.length) warnings.push('강남/건대 라움 견적서 시트를 찾지 못했습니다.');
...
export async function saveRaumPnl({ orderYear, major, title, quoteDate, nenovaPct, note, sourceFile, images, items, verification, actor }) {
  await ensureRaumPnlTables();
  const mj = String(major).padStart(2, '0');
  return withTransaction(async (tQuery) => {
    ...
    if (pnlKey) {
      ...
      const ins = await tQuery(
        `INSERT INTO WebRaumPnl (OrderYear, MajorWeek, Title, QuoteDate, NenovaPct, Note, SourceFile, ImageJson, VerifyJson, CreatedBy)
         OUTPUT INSERTED.PnlKey
         VALUES (@yr, @mj, @title, @qd, @pct, @note, @src, @ij, @vj, @actor)`,
        {
          ...common,
...
const importBatchKey = (orderYear, major) => `${String(orderYear)}-${String(major).padStart(2, '0')}`;
...
export async function loadRaumPnlImportSnapshots(batches) {
  ...
  for (const batch of batches || []) {
    ...
    out[importBatchKey(orderYear, major)] = { pnlKey: row?.PnlKey || null, version: pnlVersion(row) };
  }
  return out;
}
...
export async function prepareRaumPnlImportPreview(batches) {
  ...
  const prepared = [];
  for (const batch of batches || []) {
    const pnlKey = snapshots[importBatchKey(batch.orderYear, batch.major)]?.pnlKey;
...
function assertImportBatches(batches) {
  ...
  for (const batch of batches) {
    ...
    const identity = importBatchKey(batch.orderYear, major);
...
export async function saveRaumPnlImportBatch({ batches, sourceFile, actor, expectedSnapshots = {} }) {
  ...
  return withTransaction(async (tQuery) => {
    ...
    for (const batch of batches) {
      const orderYear = String(batch.orderYear);
      const major = String(batch.major).padStart(2, '0');
      const identity = importBatchKey(orderYear, major);
      ...
      if (pnlKey) {
        ...
        const ins = await tQuery(
          ...
          {
            yr: { type: sql.NVarChar, value: orderYear }, mj: { type: sql.NVarChar, value: major },
            title: { type: sql.NVarChar, value: `라움 ${Number(major)}차` }, qd: { type: sql.Date, value: qd },
...
export async function loadRaumPnlList() {
  await ensureRaumPnlTables();
  const r = await query(
    `SELECT m.PnlKey, m.OrderYear, m.MajorWeek, m.Title, m.QuoteDate, m.AssignedMonth, m.NenovaPct, m.Note, m.SourceFile,
      ...
      WHERE m.isDeleted = 0
      GROUP BY m.PnlKey, m.OrderYear, m.MajorWeek, m.Title, m.QuoteDate, m.AssignedMonth, m.NenovaPct, m.Note, m.SourceFile,
               m.CreatedBy, m.CreatedAt, m.UpdatedBy, m.UpdatedAt
