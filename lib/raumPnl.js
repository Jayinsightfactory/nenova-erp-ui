    ...
    for (const it of items || []) {
      ...
      await tQuery(
        `INSERT INTO WebRaumPnlItem
           (PnlKey, Seq, ItemName, Unit, Qty, BranchJson, SalePrice, SaleAmount, CostPrice, RefPrice, RefSource, ErpSalePrice, ProdKey, Remark, IsCustom)
         VALUES (@key, @seq, @name, @unit, @qty, @bj, @sp, @sa, @cp, @rp, @rs, @esp, @pk, @rm, @ic)`,
        {
      ...
      // 매입단가 학습 — 견적서 품목만 (수동 행 '손실' 등은 제외), 마지막 입력값이 이김
      if (cp != null && !it.isCustom && costKey(it.name)) {
        await tQuery(
