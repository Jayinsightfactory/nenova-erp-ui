...
function parseSheet(sheetName, aoa) {
  ...
  return { sheetName, major, branch, quoteDate: detectQuoteDate(aoa), items, summarySupply, summaryTotal };
}
...
export function parseRaumQuoteWorkbook(XLSX, workbook) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    const sum = parsed.items.reduce((a, it) => a + it.supply, 0);
    if (parsed.summarySupply != null && Math.abs(sum - parsed.summarySupply) > 5) {
      warnings.push(
        `${sheetName}: 파싱 합계 ${Math.round(sum).toLocaleString()}원 ≠ 시트 공급가액 ${Math.round(parsed.summarySupply).toLocaleString()}원 — 행 누락/추가 여부 확인 필요`
      );
    }
    sheets.push({ ...parsed, parsedSupply: sum });
  }
  if (sheets.length === 0) {
  ...
  return { sheets, items, major, quoteDate, warnings };
}
...
export async function saveRaumPnl({ orderYear, major, title, quoteDate, nenovaPct, note, sourceFile, items, actor }) {
    ...
    if (pnlKey) {
      await tQuery(
        `UPDATE WebRaumPnl SET Title=@title, QuoteDate=@qd, NenovaPct=@pct, Note=@note, SourceFile=@src,
                UpdatedBy=@actor, UpdatedAt=GETDATE() WHERE PnlKey=@key`,
      ...
      const ins = await tQuery(
        `INSERT INTO WebRaumPnl (OrderYear, MajorWeek, Title, QuoteDate, NenovaPct, Note, SourceFile, CreatedBy)
         OUTPUT INSERTED.PnlKey
         VALUES (@yr, @mj, @title, @qd, @pct, @note, @src, @actor)`,
        {
          ...common,
