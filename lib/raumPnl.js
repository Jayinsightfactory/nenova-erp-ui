...
export function shouldLearnRaumPnlManualCost(partnerCode, item, costPrice) {
  return resolvePnlPartner(partnerCode).code !== 'shilla'
    && costPrice != null
    && !item?.isCustom
...
export async function saveRaumPnl({ orderYear, major, title, quoteDate, nenovaPct, note, sourceFile, images, items, verification, actor, partnerCode, confirmGangnamMerge = false }) {
  ...
  return withTransaction(async (tQuery) => {
    ...
    const filledItems = fillConsignedCostsFromOrdinary(items || []);
    for (const it of filledItems) {
      ...
      if (shouldLearnRaumPnlManualCost(partner.code, it, cp)) {
...
// 단가 행으로 이동하지 않는다.
const importItemKey = (it, partnerCode) => {
  const base = costKey(it?.name ?? it?.ItemName);
  const unit = resolvePnlPartner(partnerCode).code === 'shilla'
...
function shillaImportYears(batches) {
  return [...new Set((batches || [])
    .filter(batch => resolvePnlPartner(batch.partnerCode).code === 'shilla')
    .map(batch => String(batch.orderYear)))].sort();
}
...
export function mergePnlImportedItems(imported, existingRows, partnerCode) {
  const partner = resolvePnlPartner(partnerCode);
  ...
  for (const row of existingRows || []) {
    ...
    const key = importItemKey(row, partner.code);
  ...
  const merged = incoming.map((it, index) => {
    const exact = oldByKey.get(importItemKey(it, partner.code)) || [];
  ...
  return { items: fillConsignedCostsFromOrdinary(combined), preservation };
}

/** 미리보기와 실제 저장이 같은 수기 데이터 병합 결과를 사용한다. */
export async function prepareRaumPnlImportPreview(batches) {
  for (const batch of batches || []) {
    if (resolvePnlPartner(batch.partnerCode).code === 'shilla') {
  ...
  const shillaMatchesByYear = await loadShillaImportAutoMatchCandidates(shillaImportYears(batches), query);
  const prepared = [];
  for (const batch of batches || []) {
    ...
    const oldItems = pnlKey
      ...
      : [];
    const importedItems = resolvePnlPartner(batch.partnerCode).code === 'shilla'
      ? assertCanonicalShillaImportItems(batch.items)
      : batch.items;
    const merged = mergePnlImportedItems(importedItems, oldItems, batch.partnerCode);
    const autoMatch = resolvePnlPartner(batch.partnerCode).code === 'shilla'
...
function assertImportBatches(batches, confirmGangnamMerge = false) {
  ...
  for (const batch of batches) {
    ...
    const identity = importBatchKey(batch.orderYear, major, partner.code);
...
export async function saveRaumPnlImportBatch({ batches, sourceFile, actor, expectedSnapshots = {}, confirmGangnamMerge = false }) {
  assertImportBatches(batches, confirmGangnamMerge);
  await ensureRaumPnlTables();
  return withTransaction(async (tQuery) => {
    const shillaMatchesByYear = await lockShillaImportAutoMatchCandidates(shillaImportYears(batches), tQuery);
    const results = [];
    for (const batch of batches) {
      const orderYear = String(batch.orderYear);
      const major = String(batch.major).padStart(2, '0');
...
export async function loadRaumPnlDetail(pnlKey) {
  ...
  const [m, i] = await Promise.all([
    ...
    query(`SELECT i.*, p.ProdName AS MatchedProdName
             FROM WebRaumPnlItem i
             LEFT JOIN Product p ON p.ProdKey = i.ProdKey AND p.isDeleted=0
            WHERE i.PnlKey=@key
            ORDER BY i.Seq, i.ItemKey`, { key: { type: sql.Int, value: Number(pnlKey) } }),
  ]);
