...
export async function prepareRaumPnlImportPreview(batches) {
  ...
  for (const batch of batches || []) {
    ...
    prepared.push({ ...batch, importedItems, items: merged.items, preservation: merged.preservation, existingDiff, itemFingerprint: canonicalItemFingerprint(merged.items) });
  }
...
export async function loadRaumPnlDetail(pnlKey) {
  ...
  const [m, i] = await Promise.all([
    ...
    query(`SELECT i.*, p.ProdName AS MatchedProdName
             ...
             LEFT JOIN Product p ON p.ProdKey = i.ProdKey AND ISNULL(p.isDeleted,0)=0
            WHERE i.PnlKey=@key
