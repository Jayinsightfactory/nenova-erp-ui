...
export async function lookupErpRefPrices(quoteItems, major, orderYear) {
  // 호텔(라움) 차수 규칙 (사장님 재확정 2026-07-17 v2): 호텔 "N차" 견적 = 전산 N-01 + N-02 분배량.
  // (N+1)-01 에 입력된 라움 분배는 잘못 들어간 것 — ⚖ 전산 일괄수정의 "차수 이동"으로 N-02 로 옮긴다.
  const mj = String(major).padStart(2, '0');
  const w1 = `${mj}-01`;
  const w2 = `${mj}-02`;
  const yw1 = `${orderYear}${mj}%`;
  const yw2 = `${orderYear}${mj}%`;
  // 도착원가 — 호텔 차수 마지막 전산 주(N-02)부터 과거로 내려가며 품목별 가장 최근 값
  let arrivalMap = {};
  let arrivalErr = null;
  try {
