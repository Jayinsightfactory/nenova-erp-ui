...
export function mergePnlImportedItems(imported, existingRows, partnerCode, options = {}) {
  ...
  const mergeContext = `${partner.label || partner.code} · ${/^\d{4}$/.test(String(options.orderYear || '').trim()) ? `${String(options.orderYear).trim()}년` : '결산 연도 미상'} · ${String(options.major || '').replace(/[^0-9]/g, '') ? `${Number(String(options.major).replace(/[^0-9]/g, ''))}차` : '차수 미상'}`;
  const collisionDetails = matched.collisions.map(collision => {
    ...
    return { ...collision, context: mergeContext, originalSource };
  });
  const preservation = {
