...
export async function lookupErpRefPrices(quoteItems, major, orderYear) {
  // 호텔(라움) 차수 규칙 (사장님 확정 2026-07-17): 회사 차수는 수~익주 화요일, 호텔 입고는 익주 월~금 —
  // 따라서 호텔 "N차" 견적 = 전산 N-02 + (N+1)-01 분배량 기준으로 대조한다.
  ...
  // 도착원가 — 호텔 차수의 마지막 전산 주((N+1)-01)부터 과거로 내려가며 품목별 가장 최근 값
  let arrivalMap = {};
  let arrivalErr = null;
  try {
