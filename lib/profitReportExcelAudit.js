...
export function classifyDiffCause({ col, category, webRow, dExcelMinusWeb, pair }) {
  ...
  if (col === 'S') {
    ...
    if (srcS === 'cnf_included_in_purchase' || ['호주', '베트남'].includes(category)) {
      return { cause: 'cnf_included', severity: 'expected', label: 'CNF 거래(운임이 매입단가에 포함) — 포워딩 0이 정상' };
    }
