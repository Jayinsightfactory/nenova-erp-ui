...
export const PIVOT_FIELDS = [
  ...
  { id: 'arrivalCostVat', label: '도착원가(부가세포)', dataKey: 'arrivalCost', zone: 'data', kind: 'measure', fixedCol: true },
...
// columnZone → showSections 파생 (섹션 필드 sec* 기준 — custName/farmName 단독으로 섹션 켜지 않음)
export function sectionsFromColumnZone(columnZone) {
  const cz = columnZone || [];
  return {
    prev:     cz.includes('secPrev'),
    order:    cz.includes('secOrder'),
    incoming: cz.includes('secIncoming'),
...
export function canDropInZone(fieldId, zoneId) {
  ...
  if (zoneId === 'filter') {
    return f.kind !== 'measure' && !!(f.filterable || f.dataKey || fieldId === 'custName' || fieldId === 'farmName');
  }
  if (zoneId === 'row') {
