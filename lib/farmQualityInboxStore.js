...
import {assertQualityRequestEditor} from './farmQualityRequest.js';
...
async function readState(q,scope,locked=false){
 ...
 p.FlowerName,COALESCE(NULLIF(p.DisplayName,N''),p.ProdName,d.ProdName) ProductName FROM dbo.WebSalesDefectDeduction d${hint}
...
async function mutate(input,user,action){
 ...
 const editing=kind==='REQUEST_EDIT';
 const requestEventKey=editing?String(input.requestEventKey??''):null;
 if(editing&&(!/^[1-9]\d*$/.test(requestEventKey)||BigInt(requestEventKey)>9223372036854775807n))throw inboxError('INBOX_INVALID','원본 요청 번호를 확인하세요.');
 if(editing&&(evidenceKeys.length||input.eventDate||input.dueDate||input.appliedWeek!=null))throw inboxError('INBOX_INVALID','요청 수정은 본문만 변경할 수 있습니다.');
 ...
 if(editing){
  const schema=await query("SELECT COL_LENGTH(N'dbo.WebFarmQualityEvent',N'RequestEventKey') AS requestEventLength");
  if(schema.recordset[0]?.requestEventLength==null)throw inboxError('INBOX_NOT_READY','요청 수정 저장소 설치가 필요합니다.');
 }
 return withTransaction(async q=>{
  ...
  if(previous.recordset[0]){
   ...
   return {replayed:true,inbox,caseKey:lower(e.CaseKey),caseVersion:Number(c?.Version),caseStatus:c?.Status,eventKey:String(e.EventKey),event:{...publicEvent(e),RequestEventKey:e.RequestEventKey==null?null:String(e.RequestEventKey),CanEditRequest:e.Kind==='REQUEST'&&e.AuthorId===user.userId}};
  }
  ...
  if(editing){
   if(!current)throw inboxError('INBOX_INVALID','원본 요청이 있는 피드백을 선택하세요.');
   const original=await q(`SELECT e.* FROM dbo.WebFarmQualityEvent e WITH(UPDLOCK,HOLDLOCK)
    JOIN dbo.WebFarmQualityCase c ON c.CaseKey=e.CaseKey
    WHERE e.EventKey=@requestEvent AND e.CaseKey=@key AND c.OrderYear=@year`,{requestEvent:big(requestEventKey),key:uid(current.CaseKey),year:num(scope.year)});
   assertQualityRequestEditor(original.recordset[0],user.userId);
  }
  ...
  }else if(!editing&&!current.InboxKey){
  ...
  const status=action==='event'&&!editing?transitionQuality(current.Status,kind,kind==='REQUEST'||policy.manage):current.Status;
  const inserted=await q(`INSERT dbo.WebFarmQualityEvent(CaseKey,RequestKey,PayloadHash,Kind,Body,AuthorId,AuthorName,Department,BeforeStatus,AfterStatus,EventDate,DueDate,AppliedWeek${editing?',RequestEventKey':''})
   OUTPUT INSERTED.* VALUES(@key,@req,@hash,@kind,@body,@actor,@name,@dept,@before,@after,@date,@due,@applied${editing?',@requestEvent':''})`,{key:uid(current.CaseKey),req:uid(requestId),hash:nv(hash),kind:nv(kind),body:nv(body),actor:nv(user.userId),name:nv(user.userName||user.userId),dept:nv(user.deptName||'부서 미지정'),before:nv(current.Status),after:nv(status),date:nv(eventDate),due:nv(dueDate),applied:num(appliedWeek),...(editing?{requestEvent:big(requestEventKey)}:{})});
  ...
  const newSourceKeys=editing?[]:resolved.newSourceKeys;
  for(const sourceKey of newSourceKeys){
  ...
  for(let offset=0;offset<newSourceKeys.length;offset+=200){
   const chunk=newSourceKeys.slice(offset,offset+200),params={year:num(scope.year),inbox:uid(inboxKey),event:big(event.EventKey)};
  ...
  if(action==='event'&&inboxKey)await q('UPDATE dbo.WebFarmQualityInbox SET Excluded=CASE WHEN @reactivate=1 THEN 0 ELSE Excluded END,Version=Version+1,UpdatedAt=SYSUTCDATETIME() WHERE InboxKey=@inbox AND OrderYear=@year',{reactivate:num(!editing&&component.newSourceKeys.length>0?1:0),inbox:uid(inboxKey),year:num(scope.year)});
  else if(action!=='event')await q(`UPDATE dbo.WebFarmQualityInbox SET Excluded=@excluded,ExclusionReason=@reason,ExcludedAt=SYSUTCDATETIME(),ExcludedBy=@actor,Version=Version+1,UpdatedAt=SYSUTCDATETIME() WHERE InboxKey=@inbox AND OrderYear=@year`,{excluded:num(action==='exclude'?1:0),reason:nv(body),actor:nv(user.userId),inbox:uid(inboxKey),year:num(scope.year)});
  ...
  return {inbox,caseKey:lower(current.CaseKey),caseVersion:Number(savedCase.Version),caseStatus:savedCase.Status,inboxKey,inboxVersion:inboxKey?Number(updated.inboxes.find(r=>lower(r.InboxKey)===inboxKey).Version):null,eventKey:String(event.EventKey),event:{...publicEvent(event),RequestEventKey:requestEventKey,CanEditRequest:kind==='REQUEST'&&event.AuthorId===user.userId}};
 });
}
