...
import {projectQualityRequestEvents} from './farmQualityRequest.js';
import {loadQualityInbox,lockQualityInboxYear} from './farmQualityInboxStore.js';
export {loadQualityInbox,saveQualityInboxEvent,setQualityInboxExclusion} from './farmQualityInboxStore.js';
...
export async function loadQuality(input){
 ...
 result.inbox=await loadQualityInbox(input);
...
export async function qualityEvents(caseKey,year,user){
 ...
 return projectQualityRequestEvents(r.recordset,user?.userId).map((event,index)=>{
  const {PayloadHash,RequestKey,AuthorId,...publicFields}=event;
  return {...publicFields,EventKey:String(event.EventKey),RequestEventKey:event.RequestEventKey==null?null:String(event.RequestEventKey),EventNo:index+1,Evidence:byEvent.get(String(event.EventKey))||[]};
 });
}
export async function deleteQualityCase(input,user){
 if(!canDeleteFarmQuality(user)||user.accountActive===false)throw Object.assign(new Error('피드백 삭제는 nenovaSS3 관리자만 가능합니다.'),{code:'FARM_QUALITY_DELETE_ADMIN_ONLY'});
 ...
 return withTransaction(async q=>{
  ...
  const locked=await q(`SELECT CaseKey,Version,Title,InboxKey FROM dbo.WebFarmQualityCase WITH(UPDLOCK,HOLDLOCK)
  ...
  if(current.InboxKey){
   const others=await q('SELECT CaseKey FROM dbo.WebFarmQualityCase WITH(UPDLOCK,HOLDLOCK) WHERE OrderYear=@year AND InboxKey=@inbox AND CaseKey<>@key',{year:num(scope.year),inbox:uid(current.InboxKey),key:uid(input.caseKey)});
   if(others.recordset.length)throw new Error('다른 이력과 저장 범위를 공유하고 있어 자동 초기화할 수 없습니다. 다른 이력은 변경하지 않았습니다.');
  }
  ...
  for(const event of events.recordset)await q('DELETE FROM dbo.WebFarmQualityInboxSource WHERE OrderYear=@year AND LinkedEventKey=@event',{year:num(scope.year),event:big(event.EventKey)});
  for(const event of events.recordset)await q('DELETE FROM dbo.WebFarmQualityEvidence WHERE EventKey=@event AND OrderYear=@year',{event:big(event.EventKey),year:num(scope.year)});
  await q('DELETE FROM dbo.WebFarmQualityEvent WHERE CaseKey=@key',{key:uid(input.caseKey)});
  const removed=await q(`UPDATE dbo.WebFarmQualityCase SET Status=N'NEW',DueDate=NULL,AppliedWeek=NULL,Version=Version+1,UpdatedAt=SYSUTCDATETIME()
   WHERE CaseKey=@key AND OrderYear=@year AND Version=@version`,{key:uid(input.caseKey),year:num(scope.year),version:num(expectedVersion)});
  if(Number(removed.rowsAffected?.[0]||0)!==1)throw Object.assign(new Error('삭제 직전에 내용이 변경되었습니다. 새로고침 후 다시 확인하세요.'),{code:'QUALITY_STALE'});
  if(current.InboxKey)await q(`UPDATE dbo.WebFarmQualityInbox SET Excluded=0,ExclusionReason=NULL,ExcludedAt=NULL,ExcludedBy=NULL,Version=Version+1,UpdatedAt=SYSUTCDATETIME()
   WHERE InboxKey=@inbox AND OrderYear=@year`,{inbox:uid(current.InboxKey),year:num(scope.year)});
  ...
  return {deleted:true,reset:true,caseStatus:'NEW',caseVersion:expectedVersion+1,caseKey:String(input.caseKey).toLowerCase(),eventCount:events.recordset.length};
 });
}
export async function saveQuality(input,user){
 ...
 return withTransaction(async q=>{
  ...
  if(create){
   ...
   if(!current)throw new Error('피드백을 찾지 못했습니다.');
   const ownership=await q(`SELECT InboxKey FROM dbo.WebFarmQualityInboxSource WITH(UPDLOCK,HOLDLOCK)
    WHERE OrderYear=@year AND SourceKey=@source`,{year:num(scope.year),source:num(current.SourceKey)});
   if(current.InboxKey||ownership.recordset.length)throw Object.assign(new Error('통합 피드백에서 다시 열어 기록하세요.'),{code:'INBOX_REQUIRED'});
