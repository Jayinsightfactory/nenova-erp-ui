...
import {qualityScope,qualityGroups,transitionQuality} from './farmQuality.js';
...
export async function loadQuality(input){
 ...
 const [sources,cases]=await Promise.all([query(sourceSQL,{year:num(scope.year)}),query(`SELECT c.*, latest.Body LatestBody, latest.AuthorName LatestAuthor,
 ...
 return {...result,cases:cases.recordset.map(c=>({...c,DueDate:c.DueDate?.toISOString().slice(0,10)||null,UpdatedAt:c.UpdatedAt?.toISOString(),suspectedRecurrence:c.AppliedWeek!=null&&['OBSERVING','CLOSED'].includes(c.Status)&&all.some(g=>g.prodKey===c.ProdKey&&g.farmName===c.FarmName&&Object.keys(g.weeks).some(w=>Number(w)>=c.AppliedWeek))})),scope};
}
...
export async function saveQuality(input,user){
 ...
 return withTransaction(async q=>{
  ...
  if(previous.recordset[0]){if(previous.recordset[0].PayloadHash!==hash)throw new Error('같은 저장 요청의 내용이 변경되었습니다.');return {caseKey:previous.recordset[0].CaseKey,replayed:true};}
  let current;
  if(create){
   ...
   if(kind!=='COMMENT'&&Number(input.version)!==current.Version)throw new Error('다른 담당자가 상태를 변경했습니다. 새로고침 후 다시 확인하세요.');
  }
