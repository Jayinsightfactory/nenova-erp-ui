...
export async function loadQuality(input){
 ...
 return {...result,...analytics,signals,cases:cases.recordset.map(c=>({...c,CaseKey:String(c.CaseKey).toLowerCase(),DueDate:c.DueDate?.toISOString().slice(0,10)||null,UpdatedAt:c.UpdatedAt?.toISOString(),suspectedRecurrence:c.AppliedWeek!=null&&['OBSERVING','CLOSED'].includes(c.Status)&&all.some(g=>g.prodKey===c.ProdKey&&g.farmName===c.FarmName&&Object.keys(g.weeks).some(w=>Number(w)>=c.AppliedWeek))})),scope};
}
export async function qualityEvents(caseKey,year){
 ...
 return r.recordset.map(event=>({...event,Evidence:byEvent.get(String(event.EventKey))||[]}));
}
