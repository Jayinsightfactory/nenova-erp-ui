export function sortCustomerProducts(rows = []) {
  const flowerUsage = new Map();
  for (const row of rows) {
    const flower = String(row.CountryFlower || row.FlowerName || '기타');
    flowerUsage.set(flower, (flowerUsage.get(flower) || 0) + Number(row.UsageCount || 0));
  }
  return [...rows].sort((a, b) => {
    const af = String(a.CountryFlower || a.FlowerName || '기타');
    const bf = String(b.CountryFlower || b.FlowerName || '기타');
    return (flowerUsage.get(bf) || 0) - (flowerUsage.get(af) || 0)
      || af.localeCompare(bf, 'ko')
      || Number(b.UsageCount || 0) - Number(a.UsageCount || 0)
      || Number(b.LastOrderYear || 0) - Number(a.LastOrderYear || 0)
      || String(b.LastOrderWeek || '').localeCompare(String(a.LastOrderWeek || ''))
      || String(a.DisplayName || a.ProdName || '').localeCompare(String(b.DisplayName || b.ProdName || ''), 'ko');
  });
}

/** API의 최근 거래순을 유지하되 로그인 담당자의 업체 묶음을 항상 앞에 둔다. */
export function sortMyCustomersFirst(rows = []) {
  return rows.map((row, index) => ({ row, index })).sort((a, b) =>
    Number(b.row.IsMine || 0) - Number(a.row.IsMine || 0) || a.index - b.index
  ).map(({ row }) => row);
}

/**
 * 품종 헤더가 이미 있는 주문입력 표에서는 영문 품종 접두를 한 번 더 반복하지 않는다.
 * 원본 품목명은 저장·검색·등록 payload에 그대로 두고 화면용 텍스트만 압축한다.
 */
export function compactCustomerProductLabel(product = {}) {
  const raw = String(product.DisplayName || product.ProdName || '').trim();
  const group = String(product.CountryFlower || product.FlowerName || '').toLowerCase();
  if (!raw) return raw;
  // 목록의 품종 헤더가 이미 분류를 보여주므로, 품명 맨 앞의 분류 접두만 숨긴다.
  // 반드시 `/` 구분자가 있는 경우만 처리해 실제 품종명(예: Roseate)을 건드리지 않는다.
  const withoutCatalogPrefix = raw.replace(/^\s*(?:(?:rose\s+china|china|rose|tulip)\s*\/\s*)/i, '').trim();
  if (/카네이션/.test(group)) {
    return withoutCatalogPrefix.replace(/^\s*carnation\s*(?:\/\s*)?/i, '').trim() || raw;
  }
  return withoutCatalogPrefix || raw;
}

export function quantitiesMatch(expected, actual, epsilon = 0.0001) {
  return Math.abs(Number(expected || 0) - Number(actual || 0)) <= epsilon;
}

// Existing orders are display data, never implicit write targets.
export function customerOrderDraft(product, additional, replacement, convert) {
  const current = Number(product.CurrentQty || 0);
  const hasReplacement = replacement !== undefined;
  const hasAdditional = additional !== undefined && String(additional).trim() !== '';
  const raw = hasReplacement ? replacement : additional;
  const numeric = Number(raw);
  const invalid = (hasReplacement || hasAdditional) && (String(raw).trim() === '' || !Number.isFinite(numeric) || numeric < 0);
  const delta = !hasReplacement && hasAdditional && !invalid ? convert(numeric) : 0;
  const finalQty = invalid || delta === null ? null : hasReplacement ? numeric : current + delta;
  return { current, finalQty, delta, hasReplacement, hasAdditional,
    touched: hasReplacement || hasAdditional,
    changed: finalQty !== null && !quantitiesMatch(current, finalQty) };
}

/** Approval is valid only for the exact selected scope, mode and write payload. */
export function customerOrderApprovalFingerprint(scope, mode, items) {
  return JSON.stringify({ scope, mode, items: items.map(item => ({
    prodKey: item.prodKey, qty: item.qty, unit: item.unit, expectedCurrentQty: item.expectedCurrentQty,
  })) });
}

/** Shared display state for the product board and current-order sidebar. */
export function customerOrderHighlight(draft) {
  if (draft.touched && draft.finalQty === null) return 'invalid';
  if (draft.changed) return draft.current > 0 ? 'changed' : 'new';
  return draft.current > 0 ? 'existing' : 'none';
}

const FLOWER_PREFIXES = new Set(['rose','carnation','hydrangea','chrysanthemum','lily','tulip','gerbera','lisianthus','ranunculus','peony','orchid','anthurium','gypsophila','alstroemeria','ruscus']);
const COUNTRY_PREFIXES = new Set(['china','colombia','colombian','ecuador','netherlands','holland','ethiopia','australia','kenya','vietnam','thailand','korea','japan']);
const KOREAN_SKIP = new Set(['장미','카네이션','수국','백합','튤립','루스커스','중국','콜롬비아','에콰도르','네덜란드','호주','태국','베트남']);

function skippableAlphabetToken(token) {
  const trimmed = String(token || '').trim();
  if (!trimmed || /^\[[^\]]*\]$/.test(trimmed)) return true;
  if (/^\d+(?:cm)?$/i.test(trimmed)) return true;
  const latin = trimmed.replace(/[^A-Za-z]/g, '').toLowerCase();
  const hangul = trimmed.replace(/[^가-힣]/g, '');
  if (/^(mq|hq|cm)$/i.test(latin)) return true;
  if (latin && (FLOWER_PREFIXES.has(latin) || COUNTRY_PREFIXES.has(latin))) return true;
  if (hangul && KOREAN_SKIP.has(hangul) && !latin) return true;
  return false;
}

function latinInitialFrom(text) {
  const match = String(text || '').match(/[A-Za-z]/);
  return match ? match[0].toUpperCase() : '';
}

export function productAlphabetInitial(product = {}) {
  const names = [product.DisplayName, product.ProdName].map(value => String(value || '').trim()).filter(Boolean);
  for (const source of names) {
    const tokens = source.replace(/\[[^\]]*\]/g, ' ').replace(/[\/|·_-]+/g, ' ').split(/\s+/).filter(Boolean);
    const variety = tokens.find(token => !skippableAlphabetToken(token)) || '';
    const initial = latinInitialFrom(variety);
    if (initial) return initial;
  }
  return '#';
}

/** Display-only sorting: preserve the history-derived candidate set and ERP identity. */
export function sortCustomerProductsAlphabetically(rows = []) {
  const key = product => {
    for (const source of [product.DisplayName, product.ProdName]) {
      const tokens = String(source || '').replace(/\[[^\]]*\]/g, ' ').replace(/[\/|·_-]+/g, ' ').split(/\s+/).filter(Boolean);
      const start = tokens.findIndex(token => !skippableAlphabetToken(token));
      const name = tokens.slice(Math.max(0, start)).join(' ');
      const latin = name.match(/[A-Za-z].*/);
      if (latin) return latin[0];
    }
    return compactCustomerProductLabel(product);
  };
  return [...rows].sort((a, b) => {
    const ai = productAlphabetInitial(a), bi = productAlphabetInitial(b);
    return Number(ai === '#') - Number(bi === '#')
      || key(a).localeCompare(key(b), 'en', { numeric: true, sensitivity: 'base' })
      || Number(a.ProdKey) - Number(b.ProdKey);
  });
}

function calendarMajorWeek(now = new Date()) {
  const year = now.getFullYear();
  const start = new Date(year, 0, 1);
  const dayOfYear = Math.floor((now - start) / 86400000) + 1;
  return { year, major: Math.min(Math.ceil(dayOfYear / 7), 52) };
}

function shiftMajorWeek(year, major, delta) {
  let nextYear = Number(year);
  let nextMajor = Number(major) + Number(delta);
  while (nextMajor > 52) { nextMajor -= 52; nextYear += 1; }
  while (nextMajor < 1) { nextMajor += 52; nextYear -= 1; }
  return { year: nextYear, major: nextMajor };
}

/** 등록 차수 선택지: 현재 차수 -backOffset 부터, 기본 선택은 현재 +majorOffset, 이후 majorCount개까지. */
export function buildForwardOrderWeeks(now = new Date(), majorOffset = 2, majorCount = 6, backOffset = 2) {
  const current = calendarMajorWeek(now);
  const startDelta = -Number(backOffset);
  const weekCount = Number(backOffset) + Number(majorOffset) + Number(majorCount);
  const defaultDelta = Number(majorOffset);
  const choices = [];
  for (let i = 0; i < weekCount; i += 1) {
    const delta = startDelta + i;
    const { year, major } = shiftMajorWeek(current.year, current.major, delta);
    for (const seq of [1, 2]) {
      choices.push({
        year: String(year),
        week: `${String(major).padStart(2, '0')}-${String(seq).padStart(2, '0')}`,
        label: `${major}-${seq}`,
        default: delta === defaultDelta && seq === 1,
      });
    }
  }
  return choices;
}
