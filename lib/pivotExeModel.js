...
import { createPivotValueOrderComparator } from './pivotExeValueOrder.js';
...
export const EXE_FIELDS = [
  ...
  { id: 'CustOrderCode', label: '거래처 주문코드', numeric: false },
  { id: 'ShipmentDtm', label: '출고일', numeric: false },
  { id: 'UPrice', label: '입고단가', numeric: true },
  { id: 'TPrice', label: '입고총단가', numeric: true },
  { id: 'DistCost', label: '분배단가', numeric: true, defaultSummary: 'weightedavg', weightField: 'Quantity' },
  { id: 'ArrivalCost', label: '도착원가', numeric: true, defaultSummary: 'max' },
...
export const EXE_DEFAULT_LAYOUT = Object.freeze({
  ...
  filter: ['CountryFlower', 'CustArea', 'CustOrderCode', 'ShipmentDtm', 'UPrice', 'TPrice', 'DistCost', 'ArrivalCost', 'OrderNo', 'CustDescr'],
  data: ['Quantity'],
});
...
function makeTree(rows, fields, sort, valueComparators) {
  ...
  const visit = node => {
    ...
    const comparator = valueComparators.get(fields[node.depth + 1]);
    if (comparator) items.sort((a, b) => comparator(a.value, b.value) || a.order - b.order);
    else if (direction === 'asc' || direction === 'desc') items.sort((a, b) => (direction === 'desc' ? -1 : 1) * (compareValues(a.value, b.value) || a.order - b.order));
    node.items = items;
    items.forEach(visit);
  };
  visit(root);
...
function defaultSummary(field) {
  const item = FIELD_BY_ID.get(field);
  return item?.defaultSummary || (item?.numeric ? 'sum' : 'count');
}
...
function normalizeSummaries(layout, summaryTypes) {
  ...
  for (const field of layout.data) {
    ...
    const values = asArray(configured ?? defaultSummary(field));
    const summaries = [...new Set(values.map(value => String(value || '').toLowerCase()).map(value => value === 'average' ? 'avg' : value)
      .filter(value => ['sum', 'avg', 'weightedavg', 'min', 'max', 'count'].includes(value)))];
    for (const summary of (summaries.length ? summaries : [defaultSummary(field)])) output.push({ field, summary, key: `${field}:${summary}` });
  }
  return output;
}
...
function addMeasureValue(state, row, measure) {
  ...
  const accumulator = state.measures[measure.key] || (state.measures[measure.key] = { sum: 0, count: 0, weightedSum: 0, weight: 0, min: null, max: null });
  ...
  if (measure.summary === 'weightedavg') {
    const weight = finiteNumber(row?.[FIELD_BY_ID.get(measure.field)?.weightField]);
    if (weight === null || weight <= 0) return;
    accumulator.weightedSum += numericValue * weight;
...
function finishMeasure(state, measure) {
  ...
  if (measure.summary === 'weightedavg') return accumulator.weight > 0 ? accumulator.weightedSum / accumulator.weight : null;
...
export function buildPivotModel(rows, options = {}) {
  ...
  // Build each rank map once, shared by all sibling groups on both axes.
  const valueComparators = new Map();
  for (const field of new Set([...layout.row, ...layout.column])) {
    const order = options.valueOrders?.[field];
    if (Array.isArray(order) && order.length) valueComparators.set(field,
      createPivotValueOrderComparator(order, String(sortDirection(options.sort, field) || 'asc').toLowerCase()));
  }
  const rowTree = makeTree(filteredRows, layout.row, options.sort, valueComparators);
  const columnTree = makeTree(filteredRows, layout.column, options.sort, valueComparators);
