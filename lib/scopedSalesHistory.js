...
async function readScopedSalesHistory(body,read,validateComparison) {
  ...
  for(const group of groupSalesHistoryMessages(body.messages,body.year,body.week)) {
    // Keep a large inbox from holding one SQL request open for every message.
    // Facts are cached server-side by scope, so bounded chunks reuse the same
    // read-only snapshot while allowing the UI to receive progress promptly.
    const chunks=[];
    for(let i=0;i<group.messages.length;i+=50) chunks.push(group.messages.slice(i,i+50));
    for(const chunk of chunks) {
    const {response,data}=await read({...body,...group,messages:chunk});
