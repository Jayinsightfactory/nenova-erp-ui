...
const NON_RETRYABLE_ESTIMATE_SAVE_ERROR_CODES = new Set([
  'STALE_DATA',
  'ERP_SCOPE_MISMATCH',
  'ERP_EDIT_LOCKED',
  'ERP_EDIT_STALE',
  'ERP_EDIT_GUARD_INVALID',
  'DIRECTIONAL_YEAR_INVALID',
  'STOCK_SHORTAGE',
  'FUTURE_STOCK_SNAPSHOT_EXISTS',
...
export const ESTIMATE_SAVE_RECOVERY_MAX_REQUESTS = 4;
export const ESTIMATE_INFLIGHT_OBSERVATION_DELAYS_MS = [1_000, 2_000, 3_000, 5_000, 8_000];
export const ESTIMATE_SAVE_STOCK_BUSY_DELAYS_MS = [1_000, 2_000, 3_000, 4_000, 5_000];
export const ESTIMATE_SAVE_STOCK_BUSY_MAX_RETRIES = 5;
export const ESTIMATE_SAVE_STOCK_BUSY_MAX_TOTAL_DELAY_MS = 15_000;
...
function isKnownNoWriteStockGateBusy(error) {
  return error?.code === 'STOCK_GATE_BUSY'
    && error?.data?.retryable === true
    && error?.data?.saved === false;
}

function isNonRetryableEstimateSaveError(error) {
  return NON_RETRYABLE_ESTIMATE_SAVE_ERROR_CODES.has(String(error?.code || ''));
}

export function boundedStockBusyDelayMs(requestedDelayMs, totalDelayMs = 0) {
  const requested = Math.max(0, Number(requestedDelayMs) || 0);
  const remaining = Math.max(0, ESTIMATE_SAVE_STOCK_BUSY_MAX_TOTAL_DELAY_MS - (Number(totalDelayMs) || 0));
...
function mayHaveInFlightCommit(error) {
  const status = Number(error?.status || error?.statusCode || error?.response?.status || 0);
  return status === 504
    || error?.name === 'AbortError'
    || error?.name === 'TypeError'
    || ['ECONNRESET', 'EPIPE', 'ETIMEDOUT'].includes(String(error?.code || ''));
}

async function observeInFlightCommit({ reconcile, error, delays, onState }) {
  let snapshot = await reconcile();
  if (!mayHaveInFlightCommit(error) || snapshot?.status !== 'unchanged') return snapshot;
  for (let index = 0; index < delays.length; index += 1) {
    const delayMs = Math.max(0, Number(delays[index]) || 0);
    onState({ phase: 'observing-commit', attempt: index + 1, delayMs });
    if (delayMs > 0) await new Promise(resolve => setTimeout(resolve, delayMs));
    snapshot = await reconcile();
    if (snapshot?.status !== 'unchanged') return snapshot;
  }
  return snapshot;
}
...
export async function runRecoverableEstimateSave({
  ...
  commitObservationDelays = ESTIMATE_INFLIGHT_OBSERVATION_DELAYS_MS,
  maxRequests = ESTIMATE_SAVE_RECOVERY_MAX_REQUESTS,
  stockBusyDelays = ESTIMATE_SAVE_STOCK_BUSY_DELAYS_MS,
  maxStockBusyRetries = ESTIMATE_SAVE_STOCK_BUSY_MAX_RETRIES,
} = {}) {
  let lastTransientError = null;
  let requestNo = 1;
  let stockBusyRetries = 0;
  let stockBusyDelayTotalMs = 0;
  const stockBusyRetryLimit = Math.max(0, Math.min(
    ESTIMATE_SAVE_STOCK_BUSY_MAX_RETRIES,
    Math.floor(Number(maxStockBusyRetries) || 0),
  ));
  while (requestNo <= maxRequests) {
    try {
      return {
        data: await request(requestNo),
        recovered: requestNo > 1 || stockBusyRetries > 0,
        alreadyApplied: false,
      };
    } catch (error) {
      if (isKnownNoWriteStockGateBusy(error)) {
        if (stockBusyRetries >= stockBusyRetryLimit) {
          onState({
            phase: 'stockbusy',
            waitAttempt: stockBusyRetries,
            maxRetries: stockBusyRetryLimit,
            totalDelayMs: stockBusyDelayTotalMs,
            preservesInput: true,
            error,
          });
          throw recoveryError(
            'STOCK_GATE_BUSY',
            '재고 계산이 계속 진행 중이라 저장하지 않았습니다. 입력값은 그대로 보관했습니다. 잠시 후 다시 저장하세요.',
            error,
          );
        }
        const delayMs = boundedStockBusyDelayMs(stockBusyDelays?.[stockBusyRetries], stockBusyDelayTotalMs);
        stockBusyRetries += 1;
        stockBusyDelayTotalMs += delayMs;
        onState({
          phase: 'stock-wait',
          waitAttempt: stockBusyRetries,
          delayMs,
      ...
      if (isNonRetryableEstimateSaveError(error)) throw error;
      ...
      for (let reconcileAttempt = 1; reconcileAttempt <= 3; reconcileAttempt += 1) {
        try {
          snapshot = await observeInFlightCommit({ reconcile, error, delays: commitObservationDelays, onState });
          break;
      ...
      if (requestNo < maxRequests) onState({
        phase: 'retrying',
