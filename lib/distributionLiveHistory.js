...
function normalizedProductPhrase(line, customers = []) {
  ...
  return value
    .replace(/\d+(?:\.\d+)?\s*(박스|box|단|bunch|송이|stem|stems)/gi, ' ')
...
function extractQuantity(text) {
  const matches = [...String(text || '').matchAll(/(\d+(?:\.\d+)?)\s*(박스|box|단|bunch|송이|stem|stems)/gi)];
