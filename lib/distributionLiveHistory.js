'use strict';
const {combinedHistoryEvidence}=require('./combinedHistoryEvidence');
...
// Exact bilingual catalog phrases that are already used interchangeably by the
// sales team and the ERP master. Keep this deliberately small and exact: this
// is evidence pairing, not fuzzy product matching.
const PRODUCT_EQUIVALENT_PHRASES = Object.freeze({
  '플라야블랑카': ['playa blanca'],
});
...
function canonicalUnit(value) {
  ...
  if (unit === '송이' || unit === '대' || unit === 'stem' || unit === 'stems') return '송이';
  return null;
}
...
// Catalog names repeat for every line in a 200-message batch. Cache only pure
// string transforms, never matching decisions or mutable catalog identities.
const keyCache=new Map(),phraseCache=new Map(),exactRegexCache=new Map();
function memoString(cache,input,compute) {
...
function key(value) {
  return memoString(keyCache,String(value||''),input=>input.trim().toLocaleLowerCase('ko-KR'));
}
...
function containsExactPhrase(text, value) {
  ...
  const expression=memoString(exactRegexCache,candidate,input=>new RegExp(`(?:^|\\s|[,.:;()])${escapeRegex(input).replace(/\s+/g,'\\s+')}(?=$|\\s|[,.:;()])`,'u'));
  return expression.test(key(text));
}
...
function exactCustomerLabelMentions(text, customers) {
  const requested=phrase(text).replace(/^[-•·]\s*/, '');
  if(!requested)return [];
  return (customers||[]).filter(customer=>{
    const source=String(customer?.CustName||'').replace(/（/g,'(').replace(/）/g,')');
    const labels=[source,...[...source.matchAll(/\(([^()]+)\)/g)].map(match=>match[1])];
    return labels.some(label=>phrase(label)===requested || (requested.length>=2 && phrase(label).replace(/(?:꽃상사|꽃도매|플라워|원예|농원)$/,'')===requested));
  });
}
...
function exactAliasMentions(text, mappings, idName, rows) {
  ...
  for (const [alias, mapping] of Object.entries(mappings || {})) {
    ...
    const shortAlias=idName==='custKey'?String(alias).replace(/원예$/, ''):'';
    if ((containsExactPhrase(text, alias)||(shortAlias.length>=2&&key(label)===key(shortAlias))) && Number.isInteger(id) && id > 0) ids.add(id);
  }
  const rowKey = idName === 'custKey' ? 'CustKey' : 'ProdKey';
  return (rows || []).filter(row => ids.has(Number(row?.[rowKey])));
}

function normalizedProductPhrase(line, customers = []) {
  let value = phrase(line);
  for (const customer of customers || []) {
    const name = phrase(customer?.CustName);
    if (name && (value === name || value.startsWith(`${name} `) || value.startsWith(`${name}:`))) {
      value = value.slice(name.length).replace(/^[:\s]+/, ' ');
      break;
    }
  }
  return value
    .replace(/(?:[+-]\s*)?\d+(?:\.\d+)?\s*(박스|box|단|bunch|송이|대|stems?)/gi, ' ')
    .replace(/(?:^|\s)[+-]\s*\d+(?:\.\d+)?(?=\s|$)/g, ' ')
    .replace(/\((?:잔량분|검역표기|오늘\s*출고|화요일\s*출고)\)|<\d+번>/g, ' ')
    .replace(/(추가|증가|늘려|더\s*해|플러스|취소|감소|빼|마이너스|차감|변동\s*없|변경\s*없|그대로|유지|삭제)/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function isFeeTarget(mapping, product) {
  return /(?:운송료|배송비|수수료|\b(?:expense|freight|fee)\b)/i.test(`${mapping?.prodName || ''} ${product?.ProdName || ''} ${product?.DisplayName || ''}`);
}

function feeAliasReason(line, facts, aliases) {
  const requested = normalizedProductPhrase(line, facts.customers);
  for (const [alias, mapping] of Object.entries(aliases.products || {})) {
    if (phrase(alias) !== requested) continue;
    const product = (facts.products || []).find(row => Number(row.ProdKey) === Number(mapping?.prodKey));
    if (product && isFeeTarget(mapping, product)) return '운송료·배송비·수수료 등 비물리 품목 별칭은 수량 변경 이력과 연결하지 않습니다.';
  }
  return null;
}

function exactProductPhrase(line, facts, aliases) {
  const requested = normalizedProductPhrase(line, facts.customers);
  if (!requested) return null;
  const ids = new Set();
  if(requested==='휘슬러')for(const product of facts.products||[])if(phrase(product.ProdName)==='alstromeria whistler')ids.add(Number(product.ProdKey));
  for (const product of facts.products || []) {
    if ([product.ProdName, product.DisplayName].some(name => phrase(name) === requested) && !isFeeTarget(null, product)) ids.add(Number(product.ProdKey));
  }
  for (const equivalent of PRODUCT_EQUIVALENT_PHRASES[requested] || []) {
    for (const product of facts.products || []) {
      if ([product.ProdName, product.DisplayName].some(name => containsExactPhrase(name, equivalent)) && !isFeeTarget(null, product)) {
        ids.add(Number(product.ProdKey));
      }
    }
  }
  for (const [alias, mapping] of Object.entries(aliases.products || {})) {
    const id = Number(mapping?.prodKey);
    if (phrase(alias) === requested && Number.isInteger(id) && id > 0) {
      const product = (facts.products || []).find(row => Number(row.ProdKey) === id);
      if (product && !isFeeTarget(mapping, product)) ids.add(id);
    }
  }
...
function extractAction(text) {
  ...
  const cancel = /(취소|감소|빼|마이너스|차감)|(?:^|\s)-\s*\d/.test(value);
...
function extractQuantity(text) {
  const matches = [...String(text || '').matchAll(/(\d+(?:\.\d+)?)\s*(박스|box|단|bunch|송이|대|stems?)/gi)];
  if (!matches.length) {
    const signed=[...String(text||'').matchAll(/(?:^|\s)[+-]\s*(\d+(?:\.\d+)?)(?=\s|$)/g)];
    if(signed.length===1)return {hasQuantity:true,qty:Number(signed[0][1]),unit:null,inferUnit:true};
  }
  ...
  const qty = finiteNumber(matches[0][1]);
  const unit = canonicalUnit(matches[0][2]);
...
function convertToOutUnit(qty, unit, product) {
  ...
  const stemsPerBunch = finiteNumber(product?.SteamOf1Bunch);
  if (Number.isFinite(stemsPerBunch) && stemsPerBunch > 0) {
    if (inputUnit === '단' && outputUnit === '송이') {
      return { qty: qty * stemsPerBunch, unit: product.OutUnit, reason: 'Product의 1단당 송이수로 OutUnit 수량을 계산했습니다.' };
    }
    if (inputUnit === '송이' && outputUnit === '단') {
      return { qty: qty / stemsPerBunch, unit: product.OutUnit, reason: 'Product의 1단당 송이수로 OutUnit 수량을 계산했습니다.' };
    }
  }
  const factors = {
...
function unresolvedProduct(identity, quote, reason, index, customer, action, quantity, message, scope, facts) {
  ...
  return {
    id: `${identity}:${index + 1}:product-unresolved`, quote,
    customerText: customer.CustName, productText: normalizedProductPhrase(quote, facts.customers),
    custKey: Number(customer.CustKey), prodKey: null, action,
    inputQty: quantity.qty, inputUnit: quantity.unit, qty: quantity.qty, unit: null,
    year: scope.year, week: scope.weeks[0], sourceAt, timestamp_approximate: approximate,
    status: sourceAt && !approximate ? 'PRODUCT_UNRESOLVED' : 'AMBIGUOUS', reason,
    orderEvents: [], shipmentEvents: [],
  };
}

function phrase(value) {
  return memoString(phraseCache,key(value),input=>input.replace(/\s+/g,' ').trim());
}

function exactProductInLine(line, facts, aliases) {
  return exactProductPhrase(line, facts, aliases);
}
...
function exactProductFromFamily(family, line, facts, aliases) {
  ...
  const ids = new Set();
  if(/알스트로(?:메리아)?$/.test(familyKey)&&normalizedProductPhrase(line,facts.customers)==='휘슬러')for(const product of facts.products||[])if(phrase(product.ProdName)==='alstromeria whistler')ids.add(Number(product.ProdKey));
  for (const [alias, mapping] of Object.entries(aliases.products || {})) {
    const aliasKey = phrase(alias);
    const suffix = aliasKey.startsWith(`${familyKey} `) ? aliasKey.slice(familyKey.length + 1) : '';
    const id = Number(mapping?.prodKey);
    const product = (facts.products || []).find(row => Number(row.ProdKey) === id);
    if (suffix && normalizedProductPhrase(lineKey, facts.customers) === suffix && product && !isFeeTarget(mapping, product) && Number.isInteger(id) && id > 0) ids.add(id);
  }
  return unique((facts.products || []).filter(product => ids.has(Number(product.ProdKey))));
}
...
function normalizeFamilyHeader(line) {
  return phrase(line)
    .replace(/(?:^|\s)\d{1,2}\s*-\s*\d{1,2}(?:차)?(?=\s|$)/g, ' ')
...
function isFamilyHeader(line, aliases) {
  const family = normalizeFamilyHeader(line);
  return Boolean(family) && Object.keys(aliases.products || {}).some(alias => phrase(alias).startsWith(`${family} `));
}
...
function parseOneMessage(message, facts, aliases, scope) {
  ...
  for (let index = 0; index < inputLines.length; index += 1) {
    ...
    if (!quantity.hasQuantity && !action.action && /^(?:->|→)?\s*(?:오늘|내일|월요일|화요일|수요일|목요일|금요일|토요일|일요일).*출고/.test(line)) continue;
    if (requests.length && /^(?:->|→)?\s*총\s*\d/.test(line) && !action.action) continue;
    if (!quantity.hasQuantity && action.action && !action.invalid && /^\d{1,2}(?:\s*-\s*\d{1,2})?차?\s/.test(line)) {
      titleAction=action.action;
      if(isFamilyHeader(line,aliases))currentFamily=normalizeFamilyHeader(line);
    }
    const customerCandidates = [
      ...exactMentions(line, facts.customers, ['CustName']),
      ...exactCustomerLabelMentions(line, facts.customers),
      ...exactAliasMentions(line, aliases.customers, 'custKey', facts.customers),
    ];
    const lineCustomer = unique(customerCandidates);
    // An explicit, unmatched "customer : item" must not inherit the previous
    // customer's identity when several customers share one message.
    if(quantity.hasQuantity&&/[:：]/.test(line)&&!customerCandidates.length){currentCustomer=null;currentAction=null;customerAmbiguous=false;}
    if (customerCandidates.length) {
      ...
      currentAction = titleAction;
    }
    const productLine=lineCustomer&&/[:：]/.test(line)?line.split(/[:：]/).slice(1).join(':').trim():line;
    const lineProduct = exactProductInLine(productLine, facts, aliases);
    if (!quantity.hasQuantity) {
      ...
      if (!lineProduct && !lineCustomer && !action.action && !action.invalid && line.length <= 120) {
        if (isFamilyHeader(line, aliases)) currentFamily = normalizeFamilyHeader(line);
        else {
          // An unrecognised bare header is never allowed to inherit the prior
          // customer; this prevents a following item from being assigned to it.
    ...
    // A present family header is authoritative. A generic exact alias (for
    // example 블루) must not override a different family-specific alias.
    const product = currentFamily
      ? exactProductFromFamily(currentFamily, productLine, facts, aliases)
      : lineProduct;
    if(quantity.inferUnit){quantity.unit=canonicalUnit(product?.OutUnit);if(!quantity.unit||!(quantity.qty>0))quantity.error='부호 수량의 단위를 전산 품목에서 확정하지 못했습니다.';}
    const effectiveAction = action.action || currentAction;
    if (!sourceScopeError && !customerAmbiguous && currentCustomer && !product && !feeAliasReason(line, facts, aliases) && !action.invalid && !(action.missing && !effectiveAction) && !quantity.error && effectiveAction) {
      requests.push(unresolvedProduct(identity, line, '저장된 정확 품목 별칭은 없지만 같은 업체·수량·방향의 전산 분배 이력을 후보로 확인합니다.', index, currentCustomer, effectiveAction, quantity, message, scope, facts));
      continue;
    }
    if (sourceScopeError || customerAmbiguous || !currentCustomer || !product || action.invalid || (action.missing && !effectiveAction) || quantity.error || !effectiveAction) {
      const reason = sourceScopeError || (customerAmbiguous ? '현재 줄의 업체 별칭이 둘 이상의 전산 업체와 연결됩니다.' : null) || (action.invalid ? action.error : null) || (action.missing && !effectiveAction ? action.error : null) || quantity.error || (!product
        ? feeAliasReason(line, facts, aliases) || '현재 줄의 전산 품목명 또는 저장된 정확 별칭을 하나로 찾지 못했습니다.'
    ...
    if (!converted) {
      ...
      requests.push({
        ...
        inputQty: quantity.qty, inputUnit: quantity.unit, qty: quantity.qty, unit: product.OutUnit,
        unitComparisonMode: 'NUMERIC_ONLY',
        conversionReason: `Product 환산계수가 없어 입력 ${quantity.qty}${quantity.unit}와 전산 ${product.OutUnit} 이력의 숫자만 후보 대조합니다.`,
        year: scope.year, week: scope.weeks[0], sourceAt, timestamp_approximate: approximate,
        status: sourceAt && !approximate ? 'UNIT_UNRESOLVED' : 'AMBIGUOUS',
        reason: sourceAt ? (approximate ? '원문 시각이 근사값이어서 단위 미확정 이력 후보도 연결하지 않습니다.' : '') : '원문 작성 시각이 없어 단위 미확정 이력 후보도 연결하지 않습니다.',
        orderEvents: [], shipmentEvents: [],
      });
      continue;
    }
...
function pairRequests(items, facts, scope) {
  const combined=combinedHistoryEvidence(items.flatMap(item=>item.requests),facts.shipmentEvents,scope,facts.queryTruncated);
  const claims = new Map();
  for (const item of items) {
    for (const request of item.requests) {
      if (request.status === 'PRODUCT_UNRESOLVED') {
        const eventEligible = (event, isShipment) => event && event.year === scope.year && event.week === scope.weeks[0]
          && event.custKey === request.custKey && validTimestamp(request.sourceAt) && validTimestamp(event.changeAt)
          && Date.parse(event.changeAt) >= Date.parse(request.sourceAt) && (!isShipment || !event.multiDate);
        const deltaMatches = event => Math.abs((event.after - event.before) - (request.action === 'ADD' ? request.inputQty : -request.inputQty)) <= EPSILON;
        const orders = facts.orderEvents.filter(event => eventEligible(event, false) && deltaMatches(event));
        const shipments = facts.shipmentEvents.filter(event => eventEligible(event, true) && deltaMatches(event));
        const productIds = new Set([...orders, ...shipments].map(event => event.prodKey));
        request.orderEvents = orders;
        request.shipmentEvents = shipments;
        if (!facts.queryTruncated && shipments.length === 1 && orders.length <= 1 && productIds.size === 1) {
          const event = shipments[0];
          request.prodKey = event.prodKey; request.productText = event.prodName; request.unit = event.unit;
          request.status = 'PRODUCT_HISTORY_CANDIDATE'; request.matchState = 'NUMERIC_HISTORY_CANDIDATE';
          request.reason = `정확 품목 별칭은 없지만 같은 업체에서 요청 이후 같은 수량·방향으로 변경된 분배 이력이 한 품목만 있습니다. ${request.quote} → ${event.prodName} 후보이며 적용 확정이 아닙니다.`;
          for (const matched of [...orders, ...shipments]) {
            const kind = shipments.includes(matched) ? 'shipment' : 'order';
            claims.set(`${kind}:${matched.eventId}`, [...(claims.get(`${kind}:${matched.eventId}`) || []), request]);
          }
        } else {
          request.status = 'AMBIGUOUS';
          request.reason = facts.queryTruncated ? '조회 결과가 제한되어 품목 이력 후보를 선택하지 않습니다.' : '정확 품목 별칭이 없고 같은 업체·수량·방향의 분배 이력을 한 품목으로 확정할 수 없습니다.';
        }
        continue;
      }
      if (request.status === 'AMBIGUOUS') continue;
      if(combined.has(request.id)) {
        const group=combined.get(request.id);
        request.orderEvents=[];request.shipmentEvents=[group.event];request.status='DISTRIBUTION_EVIDENCE';request.matchState='MATCHING_HISTORY';
        request.reason=`동일 업체·품목 ${group.count}건 요청 합계 ${group.sum}${request.unit}가 하나의 전산 분배 변동과 일치합니다. 개별 원문 완료를 직접 기록한 로그는 아닌 합산 수량 근거입니다.`;
        continue;
      }
      ...
      if (request.unitComparisonMode === 'NUMERIC_ONLY' && (matchingOrders.length === 1 || matchingShipments.length === 1)
        && matchingOrders.length <= 1 && matchingShipments.length <= 1) {
        request.status = 'UNIT_HISTORY_CANDIDATE';
        request.reason = `입력 ${request.inputQty}${request.inputUnit}와 전산 ${request.unit}의 환산계수는 없지만, 같은 수량·방향의 ${matchingShipments.length ? '분배' : '주문'} 이력 후보가 있습니다. 적용 확정이 아니라 확인 후보입니다.`;
...
async function loadLiveHistoryFacts(query, sql, scope) {
  ...
  const [customers, products, orders, shipments] = await Promise.all([
    ...
    query('SELECT ProdKey,ProdName,DisplayName,OutUnit,BunchOf1Box,SteamOf1Bunch,SteamOf1Box FROM Product WHERE ISNULL(isDeleted,0)=0'),
