...
function pnlHttpError(message, status, code, details) {
...
function preservationCollisionResponse(data) {
  return data?.code === 'PRESERVATION_COLLISION'
    ? { code: 'PRESERVATION_COLLISION', details: data.details }
    : null;
}
...
export async function readRaumPnlJsonResponse(response, { operation = 'preview' } = {}) {
  ...
  if (!response.ok) {
    const collision = preservationCollisionResponse(data);
    throw pnlHttpError(
      responseFailureMessage(response.status, operation, data),
      response.status,
      collision?.code || 'RAUM_PNL_HTTP_ERROR',
      collision?.details,
    );
  }
  if (!data || typeof data !== 'object' || Array.isArray(data) || data.success !== true) {
    ...
    const collision = preservationCollisionResponse(data);
    throw pnlHttpError(message, response.status, collision?.code || 'RAUM_PNL_INVALID_RESPONSE', collision?.details);
  }
  return data;
}
