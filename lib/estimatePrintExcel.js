...
export function buildEstimatePrintWorksheet({
  ...
  aoa.push([`수신: ${custName || ''}`, `차수: ${week || ''}차`, `출력일자: ${printDate || ''}`, serialNo ? `일련번호: ${serialNo}` : '']);
  if (bigoLabel) aoa.push([`비고: ${bigoLabel}`]);
  aoa.push([`금액: ${Number(totals.total || 0).toLocaleString()}원 (공급가 ${Number(totals.supply || 0).toLocaleString()} + 세액 ${Number(totals.vat || 0).toLocaleString()}) / VAT 포함`]);
  aoa.push([]);
  ...
  printRows.forEach((r, idx) => {
    ...
    if (statementFormat) {
      ...
      setCell(ws, row, 1, `${estimateTypeLabel(r.EstimateType)}${getStatementProductName(r)}`, STYLES.text);
    ...
    setCell(ws, row, col++, `${estimateTypeLabel(r.EstimateType)}${r.ProdName || ''}`, STYLES.text);
