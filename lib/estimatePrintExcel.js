...
export function buildEstimatePrintWorksheet({
  ...
  // ReportEstimate.cs 일반 견적서와 동일한 7열:
  // 순번·품목명[규격]·수량(단위 포함)·단가·공급가액·부가세·적요.
  const colCount = statementFormat ? 10 : 7;
  ...
  aoa.push([
    isSupplyPrintFormat(printFormat)
      ? `금액: ${Number(totals.total || 0).toLocaleString()}원 (공급가 ${Number(totals.supply || 0).toLocaleString()} + 세액 ${Number(totals.vat || 0).toLocaleString()}) / 분배단가=공급가액`
  ...
  if (statementFormat) {
    ...
    aoa.push(["순번", "품목명[규격]", "수량", "단가", "공급가액", "부가세", "적요"]);
  }
  ...
  printRows.forEach((r, i) => {
    ...
    const line = [
      i + 1,
      r._exePrint ? (r.ProdName || "") : `${estimateTypeLabel(r.EstimateType)}${r.ProdName || ""}`,
      r.UnitQuantity || `${Number(r.Quantity) || 0}${r.Unit || ""}`,
    ];
    line.push(
      Number(r.Cost) || 0,
      Number(r.Amount) || 0,
  ...
  if (statementFormat) {
    ...
    aoa.push(["공급가액", totals.supply, "VAT", totals.vat, "합계", totals.total, ""]);
  }
  ...
  printRows.forEach((r, idx) => {
    ...
    setCell(ws, row, col++, r.UnitQuantity || `${Number(r.Quantity) || 0}${r.Unit || ""}`, STYLES.text);
  ...
  if (statementFormat) {
    ...
    setCell(ws, footRow, 0, "공급가액", STYLES.foot);
    setCell(ws, footRow, 1, totals.supply, STYLES.foot);
    setCell(ws, footRow, 2, "VAT", STYLES.foot);
    setCell(ws, footRow, 3, totals.vat, STYLES.foot);
    setCell(ws, footRow, 4, "합계", STYLES.foot);
    setCell(ws, footRow, 5, totals.total, STYLES.footTotal);
    setCell(ws, footRow, 6, "", STYLES.foot);
  }
