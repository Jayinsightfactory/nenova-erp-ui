...
//
// 관세청 자동조회 기준일은 WarehouseMaster.InputDate 하나만 사용한다. ArrivalDtm/UploadDtm은
// 실제 수입신고일이라는 근거가 없으므로 대체값으로 쓰지 않는다. InputDate가 없는 매입은
// date:null 진단행으로 남겨 해당 카테고리 전체를 자동 적용 불가로 처리한다.
//
...
export function mapCategoryDateRowsToWeights(rawRows) {
  ...
  for (const row of Array.isArray(rawRows) ? rawRows : []) {
    ...
    // InputDate가 없는 상품 매입도 버리지 않는다. date:null로 남겨 상위 KCS 오케스트레이터가
    // 그 카테고리 전체를 자동 적용 불가로 판정하게 한다(일부 날짜만 평균하는 오류 방지).
    out.push({ category, currency, date, weight, missingDeclarationDate: !date });
  }
  return out.sort((a, b) =>
