...
export function resolveImportCustomer(inputName, customers = [], {
  ...
  if (!customer && normalizedInput) {
    ...
    candidates.sort((a, b) => (
      ...
      || customerUsage(usageByCustKey, b.CustKey) - customerUsage(usageByCustKey, a.CustKey)
...
function customerUsage(usageByCustKey, custKey) {
  if (!usageByCustKey) return 0;
  const raw = usageByCustKey instanceof Map
    ? usageByCustKey.get(Number(custKey))
    : usageByCustKey[Number(custKey)] || usageByCustKey[String(custKey)];
  if (raw == null) return 0;
  if (typeof raw === 'number') return raw;
  return (Number(raw.usageCount ?? raw.UsageCount ?? raw.total ?? 0) || 0)
    + (Number(raw.recentUsageCount ?? raw.RecentUsageCount ?? raw.recent ?? 0) || 0) * 2;
}
