...
function syncBodyCells(table, colCount, widthsByIndex) {
  table.querySelectorAll('tbody td, tfoot td').forEach((td) => {
    td.style.boxSizing = 'border-box';
    td.style.padding = '0 1px';
  });
  // colgroup이 열 너비를 강제 — data-col-index 있는 셀만 동기화
  table.querySelectorAll('[data-col-idx]').forEach((cell) => {
    const idx = parseInt(cell.dataset.colIdx, 10);
    const w = widthsByIndex[idx];
    if (!Number.isFinite(idx) || w == null) return;
...
export function useColumnResize(deps = [], options = {}) {
  ...
  useEffect(() => {
    ...
    const leadingColWidth = optionsRef.current.leadingColWidth;
    const hasLeadingCol = leadingColWidth != null;
    ...
    if (hasLeadingCol) {
      let leadCol = colgroup.querySelector('col[data-leading-col]');
      if (!leadCol) {
        leadCol = document.createElement('col');
        leadCol.setAttribute('data-leading-col', '1');
        colgroup.insertBefore(leadCol, colgroup.firstChild);
      }
      const lw = widths['g:prod'] ?? leadingColWidth;
      applyColWidth(leadCol, null, lw);
    }
    ...
    const colOffset = hasLeadingCol ? 1 : 0;

    const applyAt = (idx, w) => {
      widthMap[idx] = w;
      applyColWidth(colgroup.children[idx + colOffset], ths[idx], w);
    };
    ...
    const syncTableWidth = () => {
      ...
      for (let i = 0; i < colgroup.children.length; i++) {
        ...
        const fallback = colgroup.children[i].hasAttribute('data-leading-col')
          ? (leadingColWidth ?? defaultWidth)
          : defaultWidth;
        total += Number.isFinite(w) ? w : fallback;
      }
      table.style.tableLayout = 'fixed';
      table.style.width = `${total}px`;
      table.style.minWidth = `${total}px`;
      table.style.maxWidth = `${total}px`;
      syncBodyCells(table, colgroup.children.length, widthMap);
    };
