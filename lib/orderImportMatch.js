...
export function buildProductSuggestions(inputName, allProducts, {
  ...
  // 구분 가능한 별칭이 하나라도 직접 포함된 후보가 있으면, `light`처럼
  // 일부 토큰만 겹치는 후보는 제외한다. 직접 후보가 전혀 없을 때만
  // 기존 퍼지 점수 결과를 사용해 오타 검색을 보존한다.
  const directScored = scored.filter((x) => hasDirectProductAlias(input, x.prod));
  const ranked = directScored.length ? directScored : scored;
  const out = [];
