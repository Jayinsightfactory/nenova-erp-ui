...
import { scoreMatch, getDisplayName, suggestDisplayName, getEnglishOf, tokenizeForMatch } from './displayName.js';
import { scoreProductSearchOptions } from './productSearchRanking.js';
...
import { inputFlowerFamily, isFlowerFamilyMismatch } from './pasteFlowerContext.js';
export { isFlowerFamilyMismatch } from './pasteFlowerContext.js';
import {
...
function isGenericProductAlias(value) {
  return /^(?:rose|carnation|hydrangea|lily|tulip|ruscus|lisianthus|alstro|alstroemeria|gerbera|장미|카네이션|수국|백합|튤립|루스커스|리시안셔스|알스트로|거베라|colombia|china|netherlands|holland|ecuador|australia|태국|베트남|호주|중국|콜롬비아|네덜란드|에콰도르)$/i.test(String(value || ''));
}

/**
 * 유사도만으로 `문라이트`를 `Candlelight`처럼 같은 단어 일부가 겹치는
 * 품목으로 확정하지 않도록, 검색어의 구분 가능한 별칭이 DB 품목명에
 * 실제로 존재하는지 확인한다. 오타 검색은 direct 후보가 없을 때 기존
 * 점수 기반 후보로 되돌아간다.
 */
function hasDirectProductAlias(inputName, product) {
  const tokens = tokenizeForMatch(inputName);
  const aliasGroups = tokens
    .map((token) => [...new Set([token, getEnglishOf(token)].map(compactAliasText).filter(Boolean))])
    .filter((group) => group.length);
  if (!aliasGroups.length) return true;
  const target = compactAliasText([
    product?.ProdName,
    product?.DisplayName,
    product?.FlowerName,
    product?.CounName,
  ].filter(Boolean).join(' '));
  const strongGroups = aliasGroups.filter((group) => group.some((alias) => !isGenericProductAlias(alias)));
  const required = strongGroups.length ? strongGroups : aliasGroups;
...
export function buildProductMappingStats(mappings = {}) {
  ...
  for (const [input, value] of Object.entries(mappings || {})) {
    ...
    current.count += 1;
    if (input) current.aliases.push(String(input));
...
function resolveRoseCandidate(inputName, chosenProd, allProducts, usageByProdKey = null) {
  ...
  let candidates = allProducts
    ...
    .map(prod => ({
      prod,
      score: scoreMatch(input, prod, '') + customerUsageBoost(usageByProdKey, prod.ProdKey),
      usage: usageRank(usageByProdKey, prod.ProdKey),
    }))
    .filter(x => x.score >= 70)
    .sort((a, b) => b.score - a.score
      || b.usage.usageRank - a.usage.usageRank
...
function customerUsageBoost(usageByProdKey, prodKey) {
  const usage = usageRank(usageByProdKey, prodKey);
  return Math.min(8, Math.log10(usage.usageCount + usage.recentUsageCount * 2 + 1) * 4);
}

function findBestProductCandidate(inputName, allProducts, usageByProdKey = null) {
  const input = String(inputName || '').trim();
  if (!input) return null;
  const requestedCountry = explicitCountry(input);
  const scored = allProducts
    .filter(prod => !isFlowerFamilyMismatch(input, prod))
    .filter(prod => !isMixBoxMismatch(input, prod))
    .filter(prod => !isFreightMismatch(input, prod))
    // 국가가 헤더에서 명시되면 동일한 수국 화이트가 여러 국가에 있어도
    // 해당 국가 후보만 점수 우위가 생기게 한다. 국가가 없는 기존 입력의
    // 자동매칭 순서는 바꾸지 않는다.
    .map(prod => ({
      prod,
      score: scoreMatch(input, prod, '')
        + (requestedCountry ? countryPreference(input, prod) : 0)
        + customerUsageBoost(usageByProdKey, prod.ProdKey),
    }))
    .filter(x => x.score >= 72)
    .sort((a, b) => b.score - a.score);
...
export function buildProductSuggestions(inputName, allProducts, {
  ...
  // 검색 문자열·별칭·사용량·최근 사용량의 기본 정렬은 모든 품목 검색이
  // 공유하는 엔진에 위임한다. 주문등록 전용 업무필터(국가·운송료·믹스박스)
  // 와 이력 매칭 보정만 이 호출부에서 추가한다.
  const ranked = scoreProductSearchOptions(
    input,
    sourceProducts
      .filter(prod => !isFlowerFamilyMismatch(input, prod))
      .filter(prod => !isMixBoxMismatch(input, prod))
      ...
      .map((prod) => {
        ...
        return {
          ...
          RecentUsageCount: usage.recentUsageCount,
          CustomerUsageCount: usage.usageCount,
          CustomerRecentUsageCount: usage.recentUsageCount,
          MappingCount: mapping.count,
          __mappingBoost: mapping.rank,
          __countryBoost: countryPreference(input, prod),
        };
      }),
    {
      minScore,
      exactBonus: 0,
      directAlias: hasDirectProductAlias,
      scoreBoost: ({ product }) => Number(product.__mappingBoost || 0) + Number(product.__countryBoost || 0),
    },
  );
  ...
  for (const x of ranked) {
    ...
    out.push({
      ...
      suggestedDisplayName: suggestDisplayName(prod.ProdName),
      flowerName: prod.FlowerName,
      counName: prod.CounName,
      outUnit: prod.OutUnit,
      bunchOf1Box: Number(prod.BunchOf1Box || 0),
      steamOf1Bunch: Number(prod.SteamOf1Bunch || 0),
      steamOf1Box: Number(prod.SteamOf1Box || 0),
      score: x.matchScore,
      usageCount: usage.usageCount,
      recentUsageCount: usage.recentUsageCount,
      customerUsageCount: usage.usageCount,
 ...
 */
export function matchImportRow(row, { allProducts, productByKey, prodUnitMap, savedMappings, unitCatalog, usageByProdKey = null, mappingRelevanceMode = 'standard' }) {
  ...
  // `블루`, `화이트`처럼 여러 화종에 반복되는 칼라명은 업로드 품명에서
  // 추출한 화종 문맥(`수국 블루`)을 먼저 사용한다.
  const fuzzyMatch = (matchName !== inputName ? findMappingFuzzy(matchName, savedMappings) : null)
    || findMappingFuzzy(inputName, savedMappings);
  ...
  const imageMappingScore = mappingRelevanceMode === 'image' && row.matchName && savedMappedProd
    ? scoreMatch(matchName, savedMappedProd, '')
    : 100;
  const imageMappingIrrelevant = Boolean(mappingRelevanceMode === 'image' && row.matchName && savedMappedProd && imageMappingScore < 72);
  const mappedProd = (
    legacyFallbackMapping ||
    imageMappingIrrelevant ||
    isFlowerFamilyMismatch(matchName, savedMappedProd) ||
  ...
  const preferredProdAllowed = preferredProd
    && !isFlowerFamilyMismatch(matchName, preferredProd)
    && !isMixBoxMismatch(matchName, preferredProd)
    && !isFreightMismatch(matchName, preferredProd)
    ? preferredProd
    : null;
  const scoredProd = !mappedProd
    ? (preferredProdAllowed || findBestProductCandidate(matchName, allProducts, usageByProdKey))
    : null;
  // 사용자가 저장한 정확/유사 매핑은 자동 장미 후보 재정렬보다 우선한다.
  // 기존에는 mappedProd를 찾은 뒤에도 resolveRoseCandidate가 다시 candidates[0]을
  // 선택해, 재업로드하면 수동 변경 전 품목으로 되돌아갈 수 있었다.
  const resolved = mappedProd
    ? { prod: mappedProd, ambiguousCountry: false, reason: null }
    : resolveRoseCandidate(matchName, scoredProd, allProducts, usageByProdKey);
  ...
  const suggestions = buildProductSuggestions(matchName, allProducts, { usageByProdKey });

  return {
    rowNo: row.rowNo,
    inputName,
    detailLabel: row.detailLabel || '',
    detailLabels: Array.isArray(row.detailLabels) ? row.detailLabels : (row.detailLabel ? [row.detailLabel] : []),
    sourceDetails: Array.isArray(row.sourceDetails) ? row.sourceDetails : [],
    qty: row.qty,
    ...
    mappingScope: mappedProd ? (savedMap?.mappingScope || 'global') : null,
    customerUsageCount: usageRank(usageByProdKey, prod?.ProdKey).usageCount,
    customerRecentUsageCount: usageRank(usageByProdKey, prod?.ProdKey).recentUsageCount,
...
export function clearImportProductMatchForName(row = {}, nextInputName = '') {
  const inputName = String(nextInputName || '');
  const previousInput = String(row.inputName || '');
  const previousMatch = String(row.matchName || previousInput);
  const prefix = previousInput && previousMatch.endsWith(previousInput)
    ? previousMatch.slice(0, previousMatch.length - previousInput.length).trim()
    : '';
  return {
    inputName,
    matchName: prefix ? `${prefix} ${inputName}`.trim() : inputName,
    prodKey: null,
    prodName: null,
    displayName: null,
    flowerName: null,
