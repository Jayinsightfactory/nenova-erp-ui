...
function findBestProductCandidate(inputName, allProducts) {
  ...
  const scored = allProducts
    ...
    .map(prod => ({
      prod,
      score: scoreMatch(input, prod, '') + (requestedCountry ? countryPreference(input, prod) : 0),
    }))
    .filter(x => x.score >= 72)
    .sort((a, b) => b.score - a.score);
 ...
 */
export function matchImportRow(row, { allProducts, productByKey, prodUnitMap, savedMappings, unitCatalog, mappingRelevanceMode = 'standard' }) {
