...
function defaultZones() {
  return {
    ...
    values: EXE_DEFAULT_LAYOUT.data.map((id) => ({ id, aggregation: FIELD_BY_ID.get(id)?.numeric ? 'sum' : 'count' })),
  };
}
...
function normalizeZones(input, hidden) {
  ...
  return {
    ...
    values: visible(layout.data).map((id) => {
      ...
      const aggregation = ['sum', 'avg', 'min', 'max', 'count'].includes(requested) && (field?.numeric || requested === 'count')
        ? requested
        : (field?.numeric ? 'sum' : 'count');
...
function isDynamicDataWidthKey(id) {
  const match = typeof id === 'string' && id.match(/^col-[a-f0-9]{64}-([A-Za-z]+):(sum|avg|min|max|count)$/);
