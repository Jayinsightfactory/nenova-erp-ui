...
const DIRECTIONAL_REQUEST_RE = /(?:변화|넣어\s*주세요|넣어줘|(?:^|[\s(])[+-]\s*\d+)/;
const STOCK_RE = /(?:잔량|재고)/;
const QUANTITY_RE = /\d+(?:\.\d+)?\s*(?:박스|box|단|bunch|송이|stem|stems)/i;

function hasRequestSignal(value) {
  return EXPLICIT_ACTION_RE.test(value) || DELIVERY_ACTION_RE.test(value) || DIRECTIONAL_REQUEST_RE.test(value);
}
