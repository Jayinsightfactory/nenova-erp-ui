...
function classifyMessage(message) {
  ...
  // Only actionable quantity changes belong in the work list. Stock annotations
  // and delivery/chat-room references are not additional ERP instructions.
  const workText=text.replace(/메시지가\s*삭제되었습니다\.?/g,'')
    .replace(/[（(][^()（）]*(?:검역|취소분|추가분)[^()（）]*[)）]/g,'')
    .replace(/추가취소방|추가\s*방|취소\s*방/g,'')
    .replace(/취소분|추가분|취소선/g,'');
  const hasQuantity=/\d+(?:\.\d+)?\s*(?:박스|box|단|bunch|송이|대|개|스팀|st(?:ems?)?)(?=\s|$|[가-힣(])|(?:^|\s)[+-]\d+(?:\.\d+)?|[가-힣a-z]\s+\d+(?:\.\d+)?\s*$/im.test(workText);
  const changeAction=/(?:추가|증가|늘려|더\s*해|플러스|취소|감소|빼|마이너스|차감|삭제)/.test(workText)
    || /(?:^|\s)[+-]\d+(?:\.\d+)?/m.test(workText);
  if(hasQuantity&&changeAction)return 'REQUEST';
...
function quantityProcessedRequests(identity, item) {
  ...
  return requests.filter(request => {
    ...
    if (before === null || after === null || direction === 0) return false;
    // Compare the normalized ERP quantity, not the input display unit.
    // A request written as 1박스 can legitimately be stored as 10단 in
    // nenova.exe; requiring the literal unit here incorrectly hid completed
    // work as unconfirmed.
    const normalizedQty = finiteNumber(request.qty) ?? qty;
    return normalizedQty !== null && normalizedQty > 0
      && Math.abs(after - before - direction * normalizedQty) <= 1e-6;
  });
}
