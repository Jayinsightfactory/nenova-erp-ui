...
function detectCustomerColumns(XLSX, sheet, range, logs) {
  ...
  for (let r = 1; r <= Math.min(12, range.e.r + 1); r += 1) {
    let summaryCount = 0;
    const rowSummaryCols = [];
    for (let c = 1; c <= range.e.c + 1; c += 1) {
      const label = String(cellValue(XLSX, sheet, r, c) || '').trim();
      if (SUMMARY_LABELS.has(label)) {
        summaryCount += 1;
        rowSummaryCols.push(c);
      }
    }
    if (summaryCount >= 2) {
      headerRow = r;
      for (const c of rowSummaryCols) summaryCols.add(c);
      break;
    }
  }
  ...
  for (let c = 1; c <= range.e.c + 1; c += 1) {
    ...
    if (!customer || SUMMARY_LABELS.has(customer) || summaryCols.has(c)) continue;
  ...
  logs.push(`엑셀 헤더 감지: R${headerRow}, 업체열 ${columns.length}개, 요약열 ${summaryCols.size}개`);
    ...
    for (const row of targetRows) {
      ...
      if (row.uploadQty <= 0) {
        ...
        if (orderMutation.orderChanged || currentState.dateIssueCount > 0) {
          const log = `[${logStamp()} ${actor}] 엑셀업로드 출고일지정 ${oldQty}`;
          await tQ(
            `UPDATE ShipmentDetail
                SET CustKey=@ck,
                    ShipmentDtm=@dt,
