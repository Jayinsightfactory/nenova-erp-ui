...
export async function applyImportRows({ rawWeek, rawYear, rows, user, ackQtyWarnings = false, shipmentOnly = false, cleanupZeroOrder = false }) {
  const week = normalizeImportWeek(rawWeek);
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      const orderPlan = resolveImportOrderSyncPlan({
        ...
        cleanupZeroOrder: !shipmentOnly && cleanupZeroOrder,
      });
      let orderMutation = {
        orderCreated: false,
        detailCreated: false,
