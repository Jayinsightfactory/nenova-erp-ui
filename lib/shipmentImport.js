...
export async function buildImportPreview({ parsedRows, rawWeek }) {
  ...
  for (const row of parsedRows) {
    ...
    const customer = custMap.get(normText(customerLabel)) ||
    ...
    const product = chooseProduct(row, customer);
    if (!product || !customer) {
      if (row.uploadQty !== 0) {
        unmatched.push({
          ...row,
  ...
  const previewRows = [...merged.values()]
    ...
    .map(r => ({
      ...r,
      changeQty: r.uploadQty - r.currentOutQty,
      orderDiffQty: r.uploadQty - r.orderQty,
      status: !dbMap.has(`${r.custKey}|${r.prodKey}`)
        ? '주문없음'
        : sameQty(r.uploadQty, r.currentOutQty) && sameQty(r.uploadQty, r.orderQty)
          ? '동일'
          : '변경',
    }))
    .sort((a, b) => (a.custName + a.prodName).localeCompare(b.custName + b.prodName, 'ko'));

  const summaryByCustomer = {};
  for (const r of previewRows) {
    if (!summaryByCustomer[r.custName]) {
      summaryByCustomer[r.custName] = { custName: r.custName, orderQty: 0, currentOutQty: 0, uploadQty: 0, changeQty: 0, changedLines: 0 };
    }
  ...
  logs.push(`매칭 완료: 표시 ${previewRows.length}건, 변경 ${changedRows.length}건, 미매칭 ${unmatched.length}건`);
  ...
  return {
    ...
    changedRows,
    unmatched,
    summaryByCustomer: Object.values(summaryByCustomer).sort((a, b) => a.custName.localeCompare(b.custName, 'ko')),
    logs,
  };
}
...
async function syncOrderDetailForShipmentImport({ tQ, row, week, orderYear, uid, actor, customer, product, hasOrderYearWeekColumn }) {
  ...
  if (!orderMasterKey) {
    ...
    orderCreated = true;
    orderMasterKey = await safeNextKey(tQ, 'OrderMaster', 'OrderMasterKey');
    const orderMasterParams = {
      mk: { type: sql.Int, value: orderMasterKey },
      yr: { type: sql.NVarChar, value: String(orderYear) },
      week: { type: sql.NVarChar, value: week },
      ywk: { type: sql.NVarChar, value: String(orderYear) + week.replace('-', '') },
  ...
  const descr = `[${logStamp()} ${actor}] 엑셀업로드 주문 ${orderQtyFromDetail(detail, product)}>${desiredQty}`;
  ...
  const orderDetailKey = await safeNextKey(tQ, 'OrderDetail', 'OrderDetailKey');
  await tQ(
  ...
  await insertOrderHistory(tQ, orderDetailKey, 0, desiredQty, `[${logStamp()} ${actor}] 엑셀업로드 주문 0>${desiredQty}`, uid);
...
export async function applyImportRows({ rawWeek, rawYear, rows, user }) {
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      const prod = await tQ(
        `SELECT TOP 1 p.OutUnit, ISNULL(p.BunchOf1Box,0) AS BunchOf1Box, ISNULL(p.SteamOf1Box,0) AS SteamOf1Box,
      ...
      const units = toOrderUnits(row.uploadQty, productInfo.OutUnit || row.outUnit || '박스', productInfo);
      ...
      if (!shipmentKey && row.uploadQty > 0) {
        shipmentKey = await safeNextKey(tQ, 'ShipmentMaster', 'ShipmentKey');
      ...
      const old = shipmentKey
        ? await tQ(
          `SELECT TOP 1 SdetailKey, ISNULL(OutQuantity,0) AS OutQuantity
      ...
      if (row.uploadQty <= 0) {
        if (oldDetail) {
          const log = `[${logStamp()} ${actor}] 엑셀업로드 ${oldQty}>0`;
        ...
        const log = `[${logStamp()} ${actor}] 엑셀업로드 ${oldQty}>${row.uploadQty}`;
        await tQ(
          `UPDATE ShipmentDetail
              SET ShipmentDtm=@dt, OutQuantity=@outQty, EstQuantity=@estQty,
        ...
        // 수량이 이미 같은 경우에는 주문 수량 동기화만 반영하고 출고분배 레코드는 건드리지 않는다.
      } else {
        const detailKey = await safeNextKey(tQ, 'ShipmentDetail', 'SdetailKey');
        const log = `[${logStamp()} ${actor}] 엑셀업로드 0>${row.uploadQty}`;
        await tQ(
      ...
      if (shipmentChanged || orderMutation.orderChanged) {
        applied.push({ ...row, beforeQty: oldQty, afterQty: units.outQty, shipmentChanged, estimateQty: estQty, ...orderMutation });
      }
    }
    return applied;
  });
  ...
  return {
    ...
    shipmentChangedCount: result.filter(r => r.shipmentChanged).length,
    orderChangedCount: result.filter(r => r.orderChanged).length,
    orderCreatedCount: result.filter(r => r.orderCreated || r.detailCreated).length,
    orderUpdatedCount: result.filter(r => r.detailUpdated).length,
    orderDeletedCount: result.filter(r => r.detailDeleted).length,
    appliedRows: result,
  };
}
