...
export { normalizeUploadQtyForProduct, detectQtyWarnings, hasCriticalQtyWarnings, appendPeerQtyWarnings, appendCustomerPeerQtyWarnings, resolveImportOrderSyncPlan } from './shipmentImportQty.js';
...
export async function buildImportPreview({ parsedRows, rawWeek, customerOverrides }) {
  ...
  for (const row of parsedRows) {
    ...
    const uploadQty = normalizeUploadQtyForProduct(
      { ...row, excelQty, excelUnit, uploadQty: excelQty },
      product
    );
  ...
  const previewRows = [...merged.values()]
    ...
    .map(r => {
      ...
      const excelOrderQty = Number(r.excelOrderQtyRaw || 0) > 0
        ...
        : 0;
      const qtyRow = { ...r, excelOrderQty };
      const qtyWarnings = detectQtyWarnings(qtyRow, prod);
      return {
        ...r,
        excelOrderQty,
        changeQty: r.missingFromExcel ? (0 - r.currentOutQty) : (r.uploadQty - r.orderQty),
        orderDiffQty: r.missingFromExcel ? 0 : (r.uploadQty - r.orderQty),
        shipmentDiffQty: r.missingFromExcel ? (0 - r.currentOutQty) : (r.uploadQty - r.currentOutQty),
...
export async function applyImportRows({ rawWeek, rawYear, rows, user, ackQtyWarnings = false }) {
  const week = normalizeImportWeek(rawWeek);
  ...
  for (const row of targetRows) {
    const prod = await query(
      `SELECT TOP 1 OutUnit, ISNULL(BunchOf1Box,0) AS BunchOf1Box, ISNULL(SteamOf1Box,0) AS SteamOf1Box
    ...
    const state = await loadCurrentImportState(query, row, week, productInfo);
    const checkRow = {
      ...row,
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      const currentState = await loadCurrentImportState(tQ, applyRow, week, productInfo);
