...
async function applyImportRowsCore({ rawWeek, rawYear, rows, user, ackQtyWarnings = false, shipmentOnly = false, onProgress = null }) {
 ...
 * 감사 테이블 장애가 업무 저장을 막지 않도록 감사 저장은 fail-soft 로 처리한다.
 */
export async function applyImportRows(args = {}) {
  ...
  try {
    ...
    auditKey = await createShipmentImportAuditBatch({
      orderYear,
      week,
      rows,
      targetRowCount,
    ...
    console.error('[shipmentImportAudit] 시작 기록 실패:', auditError.message);
  }

  try {
    const result = await applyImportRowsCore(args);
