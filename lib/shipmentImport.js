...
// 전산(nenova.exe) 규약: EstQuantity = OutQuantity (입력 출고수량 그대로).
// 단/송이 환산수량은 BunchQuantity/SteamQuantity 컬럼에 별도 저장되고, EstQuantity 에는
// 출고수량과 동일 값을 넣는다. (실데이터: OutUnit='단' Freedom 품목 모두 Est=Out)
// 과거 버그: bunch 가 0보다 크면 무조건 단(bunch)을 써서 견적수량/금액이 BunchOf1Box 배로 부풀려짐.
