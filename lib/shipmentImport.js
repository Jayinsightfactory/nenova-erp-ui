...
async function applyImportRowsCore({ rawWeek, rawYear, rows, user, ackQtyWarnings = false, shipmentOnly = false, onProgress = null, auditKey = null }) {
  ...
  for (const row of targetRows) {
    ...
    // stale: preview 조회 시점 스냅샷(orderQty/currentOutQty)과 apply 직전 재조회 값이 다르면 중단.
    // UI 가 preview row 그대로 orderQty/currentOutQty 를 함께 보낼 때만 비교(값이 없으면 스킵).
    if (row.orderQty != null && !sameQty(row.orderQty, state.orderQty)) {
      preflightBlockers.push(`${rowLabel}: 주문수량이 검증 이후 변경됨(검증시 ${row.orderQty} → 현재 ${state.orderQty}) — 다시 검증하세요.`);
    }
    if (row.currentOutQty != null && !sameQty(row.currentOutQty, state.currentOutQty)) {
