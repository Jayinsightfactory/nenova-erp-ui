...
export async function buildImportPreview({ parsedRows, rawWeek }) {
  ...
  const previewRows = [...merged.values()]
    ...
    .map(r => ({
      ...
      needsShipmentApply: !sameQty(r.uploadQty, r.currentOutQty),
...
export async function applyImportRows({ rawWeek, rawYear, rows, user }) {
  ...
  const logs = [
    `${week}차 엑셀 검증 적용 완료: 적용 ${result.length}건`,
    `주문 신규/추가 ${result.filter(r => r.orderCreated || r.detailCreated).length}건, 주문수정 ${result.filter(r => r.detailUpdated).length}건, 주문삭제 ${result.filter(r => r.detailDeleted).length}건`,
    `분배 신규 ${result.filter(r => r.shipmentAction === '분배신규').length}건, 분배수정 ${result.filter(r => r.shipmentAction === '분배수정').length}건, 분배삭제 ${result.filter(r => r.shipmentAction === '분배삭제').length}건`,
    ...result.slice(0, 500).map(r => r.log),
  ];
