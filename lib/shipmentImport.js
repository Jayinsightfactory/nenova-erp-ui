...
export async function buildImportPreview({ parsedRows, rawWeek, customerOverrides, productOverrides, custKeysInScope, prodKeysInScope, pairKeysInScope }) {
  ...
  const currentSheetProductKeys = new Map();
  ...
  for (const row of parsedRows) {
    ...
    matchedPairKeys.add(`${Number(customer.CustKey)}|${Number(product.ProdKey)}`);
    const matchedSheetName = String(row.sheetName || '').trim();
    if (matchedSheetName) {
      if (!currentSheetProductKeys.has(matchedSheetName)) currentSheetProductKeys.set(matchedSheetName, new Set());
      currentSheetProductKeys.get(matchedSheetName).add(Number(product.ProdKey));
    }
  ...
  // 이전 성공 업로드의 같은 시트에서 다룬 pair도 이번 최종값 범위로 복원한다. 예를 들어
  // 새 파일에서 업체 열 자체가 사라진 경우, 현재 keymap에는 pair가 없어도 이전 분배를 0으로
  // 명시해야 한다. 품목이 이번 같은 시트에 남아 있을 때만 포함해 다른 시트 오삭제를 막는다.
  let priorImportPairKeys = new Set();
  if (currentSheetProductKeys.size > 0) {
    const auditSchemaResult = await query(
      `SELECT CASE WHEN OBJECT_ID(N'dbo.ShipmentImportAudit',N'U') IS NOT NULL
                     AND OBJECT_ID(N'dbo.ShipmentImportAuditRow',N'U') IS NOT NULL
                   THEN 1 ELSE 0 END AS IsReady`
    );
    let priorAuditRows = [];
    if (Number(auditSchemaResult.recordset?.[0]?.IsReady || 0) === 1) {
      const priorAuditResult = await query(
        `SELECT r.CustKey,r.ProdKey,r.SourceCells
           FROM ShipmentImportAudit a
           JOIN ShipmentImportAuditRow r ON r.AuditKey=a.AuditKey
          WHERE a.OrderYear=@importYear AND a.OrderWeek=@week
            AND a.AuditStatus IN (N'SUCCESS',N'SUCCESS_WITH_WARNINGS')`,
        {
          week: { type: sql.NVarChar, value: week },
          importYear: { type: sql.NVarChar, value: String(orderYear) },
        }
      );
      priorAuditRows = priorAuditResult.recordset || [];
    }
    priorImportPairKeys = buildPriorImportPairKeys(priorAuditRows, currentSheetProductKeys);
    if (priorImportPairKeys.size > 0) logs.push(`이전 동일 시트 업로드 범위 복원: 업체×품목 ${priorImportPairKeys.size}건`);
  }
 ...
 */
async function verifyAppliedShipmentRows(queryFn, week, targets, orderYear = String(new Date().getFullYear())) {
...
async function applyImportRowsCore({ rawWeek, rawYear, rows, user, ackQtyWarnings = false, shipmentOnly = false, onProgress = null, auditKey = null }) {
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    const inTxVerification = await verifyAppliedShipmentRows(tQ, week, inTxTargets, orderYear);
  ...
  try {
    verification = await verifyAppliedShipmentRows(query, week, dedupedVerifyTargets, orderYear);
  } catch (e) {
