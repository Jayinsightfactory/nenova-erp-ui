...
async function loadCurrentImportState(tQ, row, week, product = {}) {
  ...
  const shipmentResult = await tQ(
    `SELECT ISNULL(SUM(ISNULL(sd.OutQuantity,0)),0) AS currentOutQty
...
export async function applyImportRows({ rawWeek, rawYear, rows, user }) {
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      if (sameQty(currentState.orderQty, row.uploadQty) && sameQty(currentState.currentOutQty, units.outQty)) {
