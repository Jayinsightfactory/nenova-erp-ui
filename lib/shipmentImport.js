...
// 전산(nenova.exe) 규약: EstQuantity 는 "단(bunch) 환산수량" (Cost 가 단가이므로 금액=Cost×Est).
//  - 박스 단위 품목: Est = 박스수(OutQuantity) × 박스당 단수(BunchOf1Box)   예) 카네이션 2박스 → 30
//  - 단/송이 단위 품목: 입력수량이 이미 단/송이 → Est = OutQuantity 그대로  예) 장미 10단 → 10
// 과거 버그: 단/송이 품목인데도 bunch(=Out×BunchOf1Box)를 써서 10배로 부풀려짐(장미/알스트로).
//            (박스 품목은 우연히 같은 식이라 정상이었음)
function estimateQuantityFromUnits(units, product = {}) {
  const outUnit = normalizeOrderUnit(product.OutUnit, '박스');
  if (outUnit === '박스') return Number(units.bunch || 0) || Number(units.outQty ?? units.box ?? 0);
  return Number(units.outQty ?? units.box ?? 0);
}
...
export async function applyImportRows({ rawWeek, rawYear, rows, user }) {
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      const estQty = estimateQuantityFromUnits(units, productInfo);
