...
async function syncOrderDetailForShipmentImport({ tQ, row, week, orderYear, uid, userName, customer, product, hasOrderYearWeekColumn, hasOrderDetailDescrColumn }) {
  ...
  if (detail && desiredQty <= 0) {
    // 최종 분배표의 0·빈칸·행누락은 주문 취소가 아니다. 주문 원장을 항상 보존한다.
    return {
      orderCreated,
      detailCreated: false,
      detailUpdated: false,
...
async function applyImportRowsCore({ rawWeek, rawYear, rows, user, ackQtyWarnings = false, shipmentOnly = false, onProgress = null, auditKey = null }) {
  ...
  for (const row of targetRows) {
    ...
    const finalStateStale = evaluateImportFinalStateStale({
      ...
      intendedOrderQty: normalizedQty,
      preserveOrder: normalizedQty <= 0,
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      verifyTargets.push({
        ...
        verifyOrder: !shipmentOnly && normalizedUploadQty > 0,
        intendedOrder: !shipmentOnly && normalizedUploadQty > 0
          ? normalizedUploadQty
          : currentState.orderQty,
      });
      const orderAlreadyMatches = applyRow.uploadQty > 0
        ? sameQty(currentState.orderQty, applyRow.uploadQty)
        : true;
