  ...
  for (const sheetName of sheetNames) {
    ...
    for (let r = dataStartRow; r <= range.e.r + 1; r += 1) {
      for (const cc of customerColumns) {
        ...
        // 빈 셀은 "명시적 0"이 아니라 PRESERVE 대상이다 — 행 자체를 만들지 않는다.
        // 리터럴 0이 입력된 셀만 정상 행으로 생성해 기존 "명시 0→분배삭제·주문보존" 경로를 태운다.
        if (!hasCellValue(XLSX, sheet, r, cc.col)) continue;
...
export async function buildImportPreview({ parsedRows, rawWeek, customerOverrides, productOverrides, custKeysInScope, prodKeysInScope }) {
  ...
  // missingFromExcel(빈칸/누락 행·시트)은 절대 0을 합성해 자동 삭제하지 않는다 — PRESERVE.
  // 화면에는 정보성으로 계속 표시(엑셀누락)하되 apply 대상에서는 제외한다.
  // 삭제는 오직 셀에 리터럴 0이 명시된 행(엑셀누락이 아닌 상태='변경', uploadQty=0)에서만 발생한다.
  const applyRows = previewRows.filter(r => !r.fixBlocked && !r.missingFromExcel && (r.status !== '동일' || r.needsShipmentApply));
  ...
  if (missingFromExcelCount > 0) logs.push(`엑셀에서 빠진 기존 주문/분배: ${missingFromExcelCount}건 (정보성 표시만, 자동 0삭제 대상 아님 — PRESERVE)`);
