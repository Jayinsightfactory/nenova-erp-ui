...
export async function buildImportPreview({ parsedRows, rawWeek }) {
  ...
  for (const row of parsedRows) {
    ...
    if (customerLabel !== row.customerLabel && row.uploadQty !== 0) {
      ...
      aliasHits[aliasKey].qty += Number(row.uploadQty || 0);
    }
...
async function ensureOrderDetailForShipmentImport({ tQ, row, week, orderYear, uid, customer, product }) {
  if (row.status !== '주문없음' || row.uploadQty <= 0) return { orderCreated: false, detailCreated: false };
  ...
  if (existingDetail.recordset.length) return { orderCreated, detailCreated: false, orderMasterKey };

  const units = toOrderUnits(row.uploadQty, row.outUnit, product);
  ...
  return { orderCreated, detailCreated: true, orderMasterKey, orderDetailKey };
}
...
export async function applyImportRows({ rawWeek, rawYear, rows, user }) {
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      const orderMutation = await ensureOrderDetailForShipmentImport({
        tQ,
        row,
        week,
        orderYear,
      ...
      if (row.uploadQty <= 0) {
        ...
        await tQ(
            ...
            WHERE SdetailKey=@dk`,
          {
            ...
            log: { type: sql.NVarChar, value: `\n[${new Date().toISOString().slice(0, 16).replace('T', ' ')}] [${actor}] 엑셀업로드 ${oldQty}>${row.uploadQty}` },
          }
        );
        await tQ(`DELETE FROM ShipmentDate WHERE SdetailKey=@dk`, { dk: { type: sql.Int, value: oldDetail.SdetailKey } });
        await tQ(
        ...
        await tQ(
              ...
              ROUND(@cost * @estQty / 1.1, 0), ROUND(@cost * @estQty / 11, 0),0,@log)`,
          {
            ...
            log: { type: sql.NVarChar, value: `[${new Date().toISOString().slice(0, 16).replace('T', ' ')}] [${actor}] 엑셀업로드 0>${row.uploadQty}` },
          }
        );
        await tQ(
          `INSERT INTO ShipmentDate (SdetailKey, ShipmentDtm, ShipmentQuantity, EstQuantity, Cost, Amount, Vat)
      ...
      if (oldQty !== units.outQty) {
        applied.push({ ...row, beforeQty: oldQty, afterQty: units.outQty, estimateQty: estQty, ...orderMutation });
      }
    }
    return applied;
  });
