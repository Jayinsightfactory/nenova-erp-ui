...
function importMemo(kind, before = null, after = null) {
  const head = `엑셀업로드 ${kind}`.trim();
  if (before == null || after == null) return head;
  return `${head} ${memoQty(before)}>${memoQty(after)}`;
}
...
async function syncOrderDetailForShipmentImport({ tQ, row, week, orderYear, uid, customer, product, hasOrderYearWeekColumn, hasOrderDetailDescrColumn }) {
  ...
  const descr = importMemo('주문', orderQtyFromDetail(detail, product), desiredQty);
  ...
  await insertOrderHistory(tQ, orderDetailKey, 0, desiredQty, importMemo('주문', 0, desiredQty), uid);
...
export async function applyImportRows({ rawWeek, rawYear, rows, user }) {
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      if (row.uploadQty <= 0) {
        if (oldDetail) {
          const log = importMemo('분배', oldQty, 0);
        ...
        const log = importMemo('분배', oldQty, row.uploadQty);
        await tQ(
        ...
        const log = importMemo('분배', 0, row.uploadQty);
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      if (desiredQty <= 0) {
        ...
        const log = importMemo('사전분배', oldQty, 0);
        await insertShipmentHistory(tQ, oldDetail.SdetailKey, oldQty, 0, log, uid);
        await tQ(`DELETE FROM ShipmentDate WHERE SdetailKey=@dk`, { dk: { type: sql.Int, value: oldDetail.SdetailKey } });
        await tQ(`DELETE FROM ShipmentDetail WHERE SdetailKey=@dk`, { dk: { type: sql.Int, value: oldDetail.SdetailKey } });
        shipmentAction = '분배삭제';
        ...
        const log = importMemo('사전분배', oldQty, desiredQty);
        await tQ(
          `UPDATE ShipmentDetail
              SET CustKey=@ck,
                  ShipmentDtm=@dt,
                  ...
                  Amount=ROUND(ISNULL(NULLIF(Cost,0), @fallbackCost) * @estQty / 1.1, 0),
                  Vat=ROUND(ISNULL(NULLIF(Cost,0), @fallbackCost) * @estQty / 11, 0),
                  Descr=CASE WHEN ISNULL(Descr,'')='' THEN @log ELSE ISNULL(Descr,'') + CHAR(10) + @log END
            WHERE SdetailKey=@dk`,
          {
            dk: { type: sql.Int, value: oldDetail.SdetailKey },
            ck: { type: sql.Int, value: Number(row.custKey) },
        ...
        const log = importMemo('사전분배', 0, desiredQty);
        const detailKey = await tryInsertWithRetry(tQ, 'ShipmentDetail', 'SdetailKey', async (newDk) => {
          await tQ(
            `INSERT INTO ShipmentDetail
               (SdetailKey,ShipmentKey,CustKey,ProdKey,ShipmentDtm,OutQuantity,EstQuantity,BoxQuantity,BunchQuantity,SteamQuantity,Cost,Amount,Vat,isFix,Descr)
