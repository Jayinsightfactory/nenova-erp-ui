...
// EstQuantity 는 품목의 출고단위(OutUnit) 기준 수량이어야 전산(nenova.exe)과 일치한다.
// 박스 단위 품목은 EstQuantity = OutQuantity(=box) 여야 하는데, 과거엔 bunch 가 0보다 크면
// 무조건 단(bunch)을 써서 박스품목 견적금액이 BunchOf1Box 배(예: 10배)로 부풀려졌다.
function estimateQuantityFromUnits(units, product = {}) {
  const outUnit = normalizeOrderUnit(product.OutUnit, '박스');
  if (outUnit === '단') return Number(units.bunch || 0) || Number(units.box || 0);
  if (outUnit === '송이') return Number(units.steam || 0) || Number(units.box || 0);
