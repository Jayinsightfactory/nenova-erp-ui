...
// ── 출고분배 환산 — DB 프로시저 usp_DistributeOne 과 100% 동일하게 재현.
//   입력 qty 는 OutUnit 기준 수량(박스 품목=박스수, 단 품목=단수, 송이 품목=송이수).
//   OutUnit 으로 Box/Bunch/Steam 을 만든 뒤, EstUnit 에 해당하는 값을 EstQuantity 로 ROUND.
//   (proc: 박스→Bunch=Out×BunchOf1Box, 단→Bunch=Out 그대로, 송이→Steam=Out 그대로 …
//    EstUnit=단이면 Est=Bunch, 박스면 Est=Box, 송이면 Est=Steam)
//   과거 업로드 버그: 단 품목인데 Bunch=Out×BunchOf1Box 로 10배 부풀림(장미/알스트로) → 본 함수로 해소.
function distributeUnits(qty, product = {}) {
  ...
  // EstUnit 기준 환산값 선택. proc 는 단일값만 쓰지만, 마스터에 환산계수가
  // 비어 있으면(예: 수국 박스품목 BunchOf1Box=0, SteamOf1Box=30, EstUnit=단)
  // proc 산식 그대로는 Est=0 이 되어 견적이 깨진다. 실제 정상 데이터(=30)는
  // SteamOf1Box 경로를 쓰므로, 0 이면 상위 단위로 폴백해 정상값과 일치시킨다.
  let estRaw;
...
export async function applyImportRows({ rawWeek, rawYear, rows, user }) {
  ...
  const result = await withTransaction(async (tQ) => {
    ...
    for (const row of targetRows) {
      ...
      const units = distributeUnits(row.uploadQty, productInfo);
