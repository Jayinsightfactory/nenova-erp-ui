...
export function buildProfitReportAudit(rows = [], context = {}) {
  ...
  for (const row of rows) {
    ...
    if (row.variant !== 'noEnding' && ['category_average_fallback', 'carried_category_unit_cost'].includes(source.E)) {
      add('warning', 'STOCK_BEGIN_APPROXIMATED', row, ['E'],
        '기초재고를 근사치(원본공식 확대 또는 전차수 단가 이월)로 채웠습니다. 재고 매입단가 근거를 입력하면 정확값으로 대체됩니다.');
    }
    if (row.variant !== 'noEnding' && ['category_average_fallback', 'carried_category_unit_cost'].includes(source.F) && nonZero(stock.endQty)) {
    ...
    if (manual.H == null && shouldAuditCustoms(row.category) && (source.H === 'missing' || source.H === 'partial')) {
      if (source.H === 'missing' && !nonZero(auto.Q)) {
        ...
        const missingColombiaGwWeeks = formatWeeks(colombiaWeeks.filter((item) => !(n0(item.gw) > 0)).map((item) => item.orderWeek));
        const message = source.H === 'partial'
          ? `콜롬비아 4품목이 함께 쓰는 그외통관비 원천이 일부 반차수만 저장되었습니다.${missingColombiaGwWeeks ? ` 누락 반차수: ${missingColombiaGwWeeks}.` : ''} 그외통관비 입력 화면에서 해당 반차수 GW/CW와 구성요소를 한 번만 확인하세요.`
          : `콜롬비아 4품목이 함께 쓰는 그외통관비 원천이 없습니다.${missingColombiaGwWeeks ? ` 누락 반차수: ${missingColombiaGwWeeks}.` : ''} 그외통관비 입력 화면에서 누락된 반차수 GW/CW와 구성요소를 한 번만 확인하세요.`;
