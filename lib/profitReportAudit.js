...
export function buildProfitReportAudit(rows = []) {
  ...
  for (const row of rows) {
    ...
    if (row.variant !== 'noEnding' && manual.E == null && source.E === 'missing_finalized_snapshot') {
      add('error', 'STOCK_BEGIN_SNAPSHOT_MISSING', row, ['E'],
        '전차수의 마지막 확정 세부차수 재고 스냅샷이 없어 기초재고를 자동 확정할 수 없습니다. 전차수 재고 마감을 확인해야 합니다.');
    }
    if (row.variant !== 'noEnding' && manual.F == null && source.F === 'missing_finalized_snapshot') {
      add('error', 'STOCK_END_SNAPSHOT_MISSING', row, ['F'],
        '이번 차수의 마지막 확정 세부차수 재고 스냅샷이 없어 기말재고를 자동 확정할 수 없습니다. 해당 차수의 재고 마감을 확인해야 합니다.');
    }
