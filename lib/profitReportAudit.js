...
export function buildProfitReportAudit(rows = [], context = {}) {
  ...
  for (const row of rows) {
    ...
    // 재고 스냅샷은 있는데 품목별 재고평가단가를 하나도 구하지 못한 경우 — 값을 추정하지 않고 표시만 한다.
    if (row.variant !== 'noEnding' && manual.F == null && source.F === 'missing_stock_valuation' && nonZero(stock.endQty)) {
      add('error', 'STOCK_VALUATION_MISSING', row, ['F'],
        '기말 재고수량은 있는데 품목별 재고평가단가를 구할 수 없어 기말재고액을 계산하지 못했습니다. 재고단가표에서 해당 품목 단가를 지정해야 합니다.');
    }
    ...
    // 재고평가단가를 못 구해 품목별 최근 매입단가 폴백으로 F를 계산할 때, 일부 품목이 최근 매입
    // 라인을 못 찾으면 그 수량만큼 0으로 조용히 빠진다. 값을 추정해 채우지 않고 needs_input으로 명시한다.
    const usesRecentCostFallback = row.stockSourceKind?.end
      ? row.stockSourceKind.end !== 'stock_price_table'
      : (Boolean(stock.unitMismatch) || !nonZero(stock.purchQty));
