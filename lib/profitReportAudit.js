...
export function buildProfitReportAudit(rows = [], context = {}) {
  ...
  for (const row of rows) {
    ...
    } else if (row.variant !== 'noEnding' && source.E === 'missing_price_evidence' && row.beginStock?.unitMismatch) {
    ...
    } else if (row.variant !== 'noEnding' && source.F === 'missing_price_evidence' && nonZero(stock.endQty) && stock.unitMismatch) {
