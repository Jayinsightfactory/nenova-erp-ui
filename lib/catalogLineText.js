...
export function buildCatalogCellLines(line, fields) {
  ...
  if (f.showEng && eng) {
    rows.push({ text: eng, kind: 'eng', fontSize: 14, bold: true, color: '222222' });
  }
  if (f.showKor && kor) {
    rows.push({ text: kor, kind: 'kor', fontSize: 14, bold: true, color: '222222' });
  }
  if ((f.showEng || f.showKor) && !eng && !kor) {
    const fallback = line?.catalogName || line?.prodName;
    if (fallback) {
      rows.push({ text: fallback, kind: 'name', fontSize: 14, bold: true, color: '222222' });
    }
  }
  if (f.showPrice) {
    const price = fmtCatalogSalePrice(line);
    if (price) rows.push({ text: price, kind: 'price', fontSize: 12, bold: true, color: 'C0392B' });
  }
  for (const n of [1, 2, 3]) {
    if (f[`showExtra${n}`]) {
      const v = String(line?.[`extra${n}`] || '').trim();
      if (v) rows.push({ text: v, kind: `extra${n}`, fontSize: 10, bold: false, color: '444444' });
    }
  }
  return rows;
}
