...
export function catalogImageRotatePad(rotateDeg, frameAspect = 1) {
  const r = Math.abs(normalizeRotate(rotateDeg));
  if (!r) return 1;
  const rad = (r * Math.PI) / 180;
  const a = Number.isFinite(frameAspect) && frameAspect > 0 ? frameAspect : 1;
  return Math.abs(Math.cos(rad)) + Math.abs(Math.sin(rad)) * Math.max(a, 1 / a);
}

/** 회전용 바깥 래퍼 — overflow hidden 부모 안에서 회전 클리핑 완화 */
export function catalogImageStageStyle(source, extra = {}, frameAspect = 1) {
  const { rotate } = resolveCatalogImageTransform(source);
  const pad = catalogImageRotatePad(rotate, frameAspect);
  return {
    position: 'relative',
    width: `${pad * 100}%`,
    height: `${pad * 100}%`,
...
/** 슬롯/세션에 저장된 수동 위치 여부 (플래그 없어도 비기본값이면 유지) */
export function lineHasStoredImageTransform(line) {
  if (!line) return false;
  if (line.imageManualAdjusted) return true;
  const t = resolveCatalogImageTransform(line);
  return t.posX !== 50 || t.posY !== 50 || t.scale !== 100 || t.rotate !== 0;
}
...
export function mergeLineImageFields(line, img) {
  ...
  if (lineHasStoredImageTransform(line)) {
    const t = resolveCatalogImageTransform(line);
    return {
...
export async function renderCatalogImageDataUri(url, source, size = 800, frameAspect = 1, contentZoom = 1) {
  ...
  const ratio = Number.isFinite(frameAspect) && frameAspect > 0 ? frameAspect : 1;
  const width = Math.round(size * Math.min(1, ratio));
  const height = Math.round(size * Math.min(1, 1 / ratio));
  canvas.width = width;
  ...
  const aspect = bitmap.width / bitmap.height / ratio;
  const { coverW, coverH, left: normLeft, top: normTop } = catalogCoverBox(aspect, scale, posX, posY);
  const drawW = width * coverW;
  const drawH = height * coverH;
  const boxLeft = normLeft * width;
  const boxTop = normTop * height;
  ...
  const pad = catalogImageRotatePad(rotate, ratio);
  const zoom = Number.isFinite(contentZoom) ? Math.max(1, contentZoom) : 1;
  ctx.scale(pad * zoom, pad * zoom);
  ctx.drawImage(bitmap, boxLeft - width / 2, boxTop - height / 2, drawW, drawH);
  ctx.restore();
