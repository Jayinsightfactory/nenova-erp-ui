...
// 카테고리 분류는 본표(salesByCategory)와 동일한 SQL 조각을 재사용한다 — 복제 금지 원칙.
// (lib/profitReport.js CASE_CATEGORY와 같은 빌더. profitReport.js를 직접 import하면
//  ensure* 스키마 함수까지 모듈 그래프에 끌려오므로 원천 빌더를 쓴다.)
 ...
 * }>>}
 */
export async function loadCustomerProductSales(orderYear, major, { prodKeys, withCategory = false } = {}) {
  const params = {
  ...
  const result = await query(
    `WITH agg AS (
       SELECT sm.CustKey AS CustKey, sd.ProdKey AS ProdKey,
              ...
              ${withCategory ? `${CASE_CATEGORY} AS Category,` : ''}
        ...
        WHERE sm.OrderWeek LIKE @pfx
          ...
          ${prodFilterSql}
        GROUP BY sm.CustKey, sd.ProdKey, c.CustName, p.ProdName${withCategory ? `, ${CASE_CATEGORY}` : ''}
     )
     SELECT a.CustKey, a.ProdKey, a.CustName, a.ProductName, ${withCategory ? 'a.Category,' : ''} a.Qty, a.EstQty, a.Amount, a.Vat,
  ...
  return (result.recordset || []).map((row) => {
    ...
    return {
      ...
      ...(withCategory ? { category: row.Category || null } : {}),
      qty,
      estQty,
      amount,
      vat,
...
/**
 * 신규품목 재고단가 후보 — 같은 연도·대차수의 확정 분배를 업체×품목×실제 VAT 포함 Cost로
 * 묶는다. 원본 재고잔량 시트처럼 이산적인 실제 단가를 보존하며 평균단가를 만들지 않는다.
 */
export async function loadConfirmedCustomerProductPrices(orderYear, major, { prodKeys } = {}) {
  const params = {
    pfx: { type: sql.NVarChar, value: `${major}-%` },
    yw: { type: sql.NVarChar, value: `${orderYear}${major}` },
    yr: { type: sql.NVarChar, value: String(orderYear) },
  ...
  const result = await query(
    `SELECT sm.CustKey AS CustKey, sd.ProdKey AS ProdKey, sd.Cost AS GrossCost,
            SUM(ISNULL(sd.EstQuantity,0)) AS EstQty,
      ...
      WHERE sm.OrderWeek LIKE @pfx
        ...
        AND ${NON_VALUE_WEIGHT_SQL}
        ${prodFilterSql}
      GROUP BY sm.CustKey, sd.ProdKey, sd.Cost
      ORDER BY sd.ProdKey, sd.Cost`,
    params,
  );
  return (result.recordset || []).map((row) => ({
    custKey: row.CustKey != null ? Number(row.CustKey) : null,
    prodKey: Number(row.ProdKey),
    grossCost: n0(row.GrossCost),
    estQty: n0(row.EstQty),
    distributionCount: n0(row.DistributionCount),
  }));
}
