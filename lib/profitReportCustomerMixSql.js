...
// 분배단가 조회 (읽기 전용).
//
// 확정 매출 필터는 lib/profitReport.js#salesByCategory 와 **동일**해야 한다 — lib/pivotStats.js 의
// `sd.isFix=1` 이 아니다. 두 필터가 다른 결과를 낸다는 것은 docs/exe-golden/FormProfitReport.md 에
// 문서화된 기존 회귀이며, 드라이버 분석 카운트가 보고서 자체 N과 어긋나지 않으려면 반드시
// profitReport.js 쪽 필터(`sm.OrderWeek LIKE 'major-%' AND sm.OrderYearWeek=yw AND
// ISNULL(sm.isDeleted,0)=0 AND ISNULL(sm.isFix,0)=1 AND ISNULL(sd.OutQuantity,0)<>0` +
...
/**
 * 거래처×품목 매출 조회 — 확정 출고(ShipmentMaster.isFix=1)만, lib/profitReport.js#salesByCategory
