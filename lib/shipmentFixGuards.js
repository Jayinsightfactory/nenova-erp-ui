...
export function formatFixApiErrorMessage(data, week) {
  ...
  if (data?.code === 'LATER_FIXED_EXISTS') {
    const later = data.nextWeek
      ? `${data.nextWeek.orderYear} ${data.nextWeek.orderWeek}`
      : (data.laterWeeks || []).join(', ');
    const names = (data.products || []).map((p) => p.prodName || p.ProdName).filter(Boolean);
    return `${week}: 다음차수(${later})가 확정되어 취소할 수 없습니다. 다음차수를 먼저 취소하세요.${names.length ? `\n(${names.join(', ')})` : ''}`;
  }
  if (data?.code === 'STOCK_CALC_FAILED') {
    return `${week}: 확정취소는 반영됐지만 재고 재계산에 실패했습니다. 잠시 후 같은 차수를 다시 시도하세요.`;
  }
