export function parseNaturalInlineOrderLine(line) {
  const match = String(line || '').trim().match(
    /^(.+?)\s+(?:-|:|：)\s+(.+?)\s*(-?\d+(?:\.\d+)?)?\s*(박\s*스|단|송\s*이(?:\s*\(\s*대\s*\))?|개(?:\s*\(\s*대\s*\))?|스\s*팀(?:\s*\(\s*대\s*\))?|스\s*템(?:\s*\(\s*대\s*\))?|stems?|steam)?\s*(추가|취소)\s*$/i,
  );
...
export function parseNaturalSectionActionLine(line) {
  ...
  const match = value.match(
    /^(.+?)\s*(-?\d+(?:\.\d+)?)?\s*(박\s*스|boxes?|box|bx|단|bunch(?:es)?|bun|송\s*이(?:\s*\(\s*대\s*\))?|개(?:\s*\(\s*대\s*\))?|스\s*팀(?:\s*\(\s*대\s*\))?|스\s*템(?:\s*\(\s*대\s*\))?|stems?|steam)?\s*(추가|취소)\s*$/i,
  );
  if (!match) return null;
  return {
