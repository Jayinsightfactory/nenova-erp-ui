...
import { extractProductsFromPptxSpatial } from './catalogPptxSpatial.js';
import { persistCatalogMatch } from './catalogMatchPersist.js';
...
export async function importCatalogSourceBuffer(buffer, filename, products, uploadedBy, { forceReplace = false } = {}) {
  ...
  if (ext.endsWith('.json')) {
    ...
    if (py?.length) {
      ...
      const spatial = await extractProductsFromPptxSpatial(sourceBuffer);
      if (spatial?.length) {
        extracted = spatial.map(p => ({
          name: p.name || '',
          eng_name: p.eng_name || '',
          code: '',
          label: p.label || `${p.eng_name || ''} ${p.name || ''}`.trim(),
  ...
  for (const item of extracted) {
    ...
    if (!forceReplace && listImages({ prodKey: prod.ProdKey }).length > 0) {
      skipped.push({ prodKey: prod.ProdKey, label: item.label || prod.DisplayName });
      continue;
    }
    try {
      const imported = importBufferForProduct(
        item.buffer,
        prod.ProdKey,
        `${prod.ProdKey}.jpg`,
        'image/jpeg',
