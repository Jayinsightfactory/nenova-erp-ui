...
import { computeProfitRow, computeProfitTotals, calcRevenueRatio, calcPurchaseRatio } from './profitReportCalc';
...
function buildFilteredSheet(wb, { rows, visibleCols, confirmedTotals }) {
...
export function buildProfitReportXlsx({ major, rows, note, audit, visibleCols, confirmedTotals }) {
  ...
  const extraCalc = extra ? (extra.confirmed && extra.calc ? extra.calc : computeProfitRow(extra)) : null;
  ...
  if (Array.isArray(visibleCols) && visibleCols.length) buildFilteredSheet(wb, { rows, visibleCols, confirmedTotals });
