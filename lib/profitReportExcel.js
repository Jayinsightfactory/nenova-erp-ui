...
// 기존 회계 양식의 셀 위치·값·숫자서식은 유지하면서, 사용자가 어느 탭을 열어도
// 디자인이 보이도록 원본 시트에도 화면과 같은 색상 체계를 입힌다.
function styleOriginalSheet(ws) {
  const applyBand = (row, fromCol, toCol, makeStyle) => {
    for (let col = fromCol; col <= toCol; col += 1) {
      const addr = XLSX.utils.encode_cell({ r: row - 1, c: col - 1 });
      const cell = ws[addr] || (ws[addr] = { t: 's', v: '' });
      cell.s = { ...(cell.s || {}), ...makeStyle(col, cell) };
    }
  };
  ...
  applyBand(7, 2, 21, () => ({
    font: { name: '맑은 고딕', sz: 9, bold: true, color: { rgb: DISPLAY_COLORS.white } },
    fill: { patternType: 'solid', fgColor: { rgb: DISPLAY_COLORS.blue } },
    border: thinBorder,
    alignment: { horizontal: 'center', vertical: 'center', wrapText: true },
  }));
  for (let row = 8; row <= 23; row += 1) {
    applyBand(row, 2, 21, (col) => ({
      font: { name: '맑은 고딕', sz: 9, bold: col === 2, color: { rgb: DISPLAY_COLORS.text } },
      fill: { patternType: 'solid', fgColor: { rgb: row % 2 ? DISPLAY_COLORS.stripe : DISPLAY_COLORS.white } },
      border: thinBorder,
      alignment: { horizontal: col === 2 ? 'left' : 'right', vertical: 'center', wrapText: col === 2 },
    }));
  }
  applyBand(24, 2, 21, (col) => ({
    font: { name: '맑은 고딕', sz: 9, bold: true, color: { rgb: DISPLAY_COLORS.text } },
    fill: { patternType: 'solid', fgColor: { rgb: DISPLAY_COLORS.total } },
    border: { ...thinBorder, top: { style: 'medium', color: { rgb: DISPLAY_COLORS.blue } } },
    alignment: { horizontal: col === 2 ? 'left' : 'right', vertical: 'center' },
  }));

  ws['!rows'] = ws['!rows'] || [];
  ws['!rows'][0] = { ...(ws['!rows'][0] || {}), hpt: 30 };
  ws['!rows'][6] = { ...(ws['!rows'][6] || {}), hpt: 34 };
  for (let row = 7; row <= 23; row += 1) ws['!rows'][row] = { ...(ws['!rows'][row] || {}), hpt: 23 };
}
...
export function buildProfitReportXlsx({ major, rows, note, audit, visibleCols, confirmedTotals, monthly, displaySheetFirst = false }) {
  ...
  if (displaySheetFirst) styleOriginalSheet(sheet);
  ...
  if (Array.isArray(visibleCols) && visibleCols.length) {
    ...
    if (displaySheetFirst) {
      ...
      wb.Workbook.Views = [{ ...(wb.Workbook.Views?.[0] || {}), activeTab: 0, firstSheet: 0 }];
    }
  }
