...
export async function loadShipmentProdGroups(q, { week, year } = {}) {
  const params = {};
  let weekFilter = '';
  if (week) {
    weekFilter = `
      AND EXISTS (
        ...
        WHERE od.ProdKey = p.ProdKey
          ...
          AND CAST(om.OrderYear AS NVARCHAR(4)) = @year
          AND ISNULL(om.isDeleted, 0) = 0
          AND ISNULL(od.isDeleted, 0) = 0
      )`;
    params.week = { type: sql.NVarChar, value: week };
    params.year = { type: sql.NVarChar, value: String(year || new Date().getFullYear()) };
  }
