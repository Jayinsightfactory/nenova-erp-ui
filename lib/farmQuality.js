...
export const QUALITY_SIGNAL_KINDS = {
 ...
 UNASSIGNED_ITEM_WEEK:'농장 미지정 · 품목 반복',
 UNASSIGNED_ITEM_PERSISTENT:'농장 미지정 · 업체 품목 반복'
};
...
// Coverage includes every selected-year active source; qualityGroups retains
// its confirmed-and-complete rule and qualitySignals has its own analysis scope.
...
const occurrenceFacts=(rows,scope,counts={})=>{
 const facts=[],seen=new Set();counts.invalidSourceCount=0;counts.invalidWeekCount=0;
 for(const row of rows||[]){
  ...
  if(Number(row.OrderYear)!==scope.year||sourceDeleted(row))continue;
  if(Number.isSafeInteger(sourceKey)&&sourceKey>0){if(seen.has(sourceKey))continue;seen.add(sourceKey);}
  if(!week){counts.invalidWeekCount++;continue;}
  if(week<scope.from||week>scope.to)continue;
  if(!Number.isSafeInteger(sourceKey)||sourceKey<=0||!Number.isSafeInteger(prodKey)||prodKey<=0||!Number.isFinite(quantity)||quantity<=0){counts.invalidSourceCount++;continue;}
  const unit=String(row.SourceUnit||'미지정').trim()||'미지정';
  facts.push({sourceKey,week,orderWeek:String(row.OrderWeek),prodKey,confirmed:row.ImportConfirmed===true||row.ImportConfirmed===1,productName:row.ProductName||row.ProdName||String(prodKey),farmName,farmIdentity:Number(row.FarmKey)>0?`key:${Number(row.FarmKey)}`:`name:${matchText(farmName)}`,unit,unitIdentity:matchText(unit),quantity,customerIdentity:Number.isSafeInteger(Number(row.CustKey))&&Number(row.CustKey)>0?`key:${Number(row.CustKey)}`:'',customerName:snapshotName(row.CustName??row.CustomerName,'업체 미지정')});
 }
 return facts;
};
...
const signal=(kind,facts,extra={})=>{
 ...
 const confirmedSourceCount=facts.filter(f=>f.confirmed).length,unconfirmedSourceCount=facts.length-confirmedSourceCount,missingFarmSourceCount=facts.filter(f=>!f.farmName).length;
 const reviewReasons=[];if(unconfirmedSourceCount)reviewReasons.push('수입부 확인 미완료 원본 포함');if(missingFarmSourceCount)reviewReasons.push('농장 미지정 원본 포함');
 const canCreate=reviewReasons.length===0;
 return {kind,kindLabel:QUALITY_SIGNAL_KINDS[kind],attribution:'FARM',farmName:facts[0].farmName,productName:products.length===1?facts[0].productName:`${products.length}개 품목`,unit:facts[0].unit,sourceCount:facts.length,productCount:products.length,weekCount:weeks.length,quantity:facts.reduce((sum,f)=>sum+f.quantity,0),firstWeek:weeks[0],lastWeek:weeks.at(-1),weeks,sourceKey:canCreate?facts[0].sourceKey:null,sourceKeys:unique(facts.map(f=>f.sourceKey)).sort((a,b)=>a-b),reviewRequired:!canCreate,reviewReasons,confirmedSourceCount,unconfirmedSourceCount,missingFarmSourceCount,canCreate,breakdown:signalBreakdown(facts),...extra};
};
export function qualitySignals(rows,scope){
 const allFacts=occurrenceFacts(rows,scope),facts=allFacts.filter(f=>f.farmName),unassigned=allFacts.filter(f=>!f.farmName),signals=[];
 for(const items of grouped(unassigned,f=>[f.week,f.prodKey,f.unitIdentity])){
  if(unique(items.map(i=>i.customerIdentity).filter(Boolean)).length<2)continue;
  const value=signal('UNASSIGNED_ITEM_WEEK',items,{prodKey:items[0].prodKey,attribution:'PRODUCT'});
  value.key=`${scope.year}|${value.kind}|${value.sourceKeys.join(',')}`;signals.push(value);
 }
 for(const items of grouped(unassigned.filter(f=>f.customerIdentity),f=>[f.customerIdentity,f.prodKey,f.unitIdentity])){
  const weeks=unique(items.map(i=>i.week)).sort((a,b)=>a-b);if(weeks.length<2||!recurring(weeks))continue;
  const value=signal('UNASSIGNED_ITEM_PERSISTENT',items,{prodKey:items[0].prodKey,attribution:'CUSTOMER_PRODUCT'});
  value.key=`${scope.year}|${value.kind}|${value.sourceKeys.join(',')}`;signals.push(value);
 }
...
export function qualitySignalCoverage(rows,scope,signals=qualitySignals(rows,scope)){
 const counts={},facts=occurrenceFacts(rows,scope,counts),keys=new Set(facts.map(f=>f.sourceKey));
 const detected=new Set(signals.flatMap(s=>s.sourceKeys||[]).filter(key=>keys.has(key)));
 // Confirmed means the previous trusted analysis scope (confirmed + known farm).
 const confirmedSourceCount=facts.filter(f=>f.confirmed&&f.farmName).length;
 return {analyzedSourceCount:facts.length,confirmedSourceCount,additionalSourceCount:facts.length-confirmedSourceCount,detectedSourceCount:detected.size,noPatternSourceCount:facts.length-detected.size,reviewSignalCount:signals.filter(s=>s.reviewRequired).length,totalSignalCount:signals.length,...counts};
}
