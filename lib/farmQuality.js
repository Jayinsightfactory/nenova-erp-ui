...
const occurrenceFacts=(rows,scope)=>{
 ...
 for(const row of rows||[]){
  ...
  facts.push({sourceKey,week,orderWeek:String(row.OrderWeek),prodKey,productName:row.ProductName||row.ProdName||String(prodKey),farmName,farmIdentity:Number(row.FarmKey)>0?`key:${Number(row.FarmKey)}`:`name:${matchText(farmName)}`,unit,unitIdentity:matchText(unit),quantity,customerIdentity:Number(row.CustKey)>0?`key:${Number(row.CustKey)}`:''});
 }
 return facts;
};
