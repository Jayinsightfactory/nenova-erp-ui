...
function parseRaumFormatRows(rows, header, sourceName, logs) {
  ...
  for (let i = header.headerRow + 1; i < rows.length; i += 1) {
    ...
    const categoryFamily = extractOrderImportMatchFamily(category);
    if (categoryFamily) lastMatchFamily = categoryFamily;
    ...
    out.push({
      rowNo: i + 1,
      inputName,
      matchName: lastMatchFamily ? `${lastMatchFamily} ${inputName}` : inputName,
      ...
      note: header.priceCol >= 0 ? String(row[header.priceCol + 1] || '').trim() : '',
      color: colorCell,
  ...
  logs.push(`${sourceName}: 라움형 ${out.length}행 → 실제 품목 ${merged.length}건 합산 (칼라 → 품목, 품명 → 세부정보)`);
  return merged;
}
...
export function parseOrderImportWorkbook(XLSX, workbook, { sourceName } = {}) {
  ...
  for (const sheetName of names) {
    ...
    if (parsed.rows.length > best.rows.length) {
      best = { ...parsed, sheetName, metadata: parseOrderImportMetadata(raw) };
    }
  }
