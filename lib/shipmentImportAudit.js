...
// 결과를 별도 테이블에 남긴다. 업로드 복원에 필요한 헤더 완료 상태는 주문/분배 저장과
// 같은 트랜잭션에서 기록하고, 상세 행/후속 요약만 별도 best-effort 로 보강한다.
...
async function ensureAuditTables() {
  await query(`
    ...
    END;
    IF COL_LENGTH('dbo.ShipmentImportAudit','SourceFileName') IS NULL
      ALTER TABLE dbo.ShipmentImportAudit ADD SourceFileName NVARCHAR(260) NULL;
...
export async function createShipmentImportAuditBatch({
  ...
  const result = await query(
    `INSERT INTO dbo.ShipmentImportAudit
       (ActorUserId, ActorName, SourceFileName, OrderYear, OrderWeek, ApplyMode, AuditStatus, InputRowCount, TargetRowCount)
     OUTPUT INSERTED.AuditKey
     VALUES (@uid, @name, @file, @year, @week, @mode, 'STARTED', @inputRows, @targetRows)`,
    {
      ...
      file: { type: sql.NVarChar, value: text(sourceFileName, 260) },
...
/**
 * 주문·분배·스냅샷과 같은 트랜잭션에서 최소 완료 상태를 확정한다.
 * 이 UPDATE가 실패하면 업무 쓰기도 함께 rollback 되어, "데이터는 바뀌었는데 되돌리기
 * 이력이 없음" 상태가 생기지 않는다. 커밋 후 finishShipmentImportAuditBatch가 행별
 * 상세와 최종 요약을 덧씌운다.
 */
export async function markShipmentImportAuditAppliedInTransaction(tQ, {
  auditKey,
  appliedCount = 0,
  ...
  if (!auditKey) throw new Error('엑셀 업로드 안전 이력 번호가 없습니다.');
  const result = await tQ(
    `UPDATE dbo.ShipmentImportAudit
        SET CompletedDtm=GETDATE(), AuditStatus=N'SUCCESS',
            ...
            VerificationMismatchCount=@verifyMismatch, ErrorCode=NULL, ErrorMessage=NULL
      WHERE AuditKey=@auditKey AND AuditStatus=N'STARTED';
     SELECT @@ROWCOUNT AS UpdatedCount;`,
    {
      auditKey: { type: sql.Int, value: Number(auditKey) },
      applied: { type: sql.Int, value: Number(appliedCount || 0) },
      skipNoChange: { type: sql.Int, value: Number(skippedNoChangeCount || 0) },
      skipFixed: { type: sql.Int, value: Number(skippedFixedCount || 0) },
      verifyChecked: { type: sql.Int, value: Number(verification.checked || 0) },
      verifyMatched: { type: sql.Int, value: Number(verification.matched || 0) },
      verifyMismatch: { type: sql.Int, value: Number(verification.mismatchCount || 0) },
    },
  );
  if (Number(result.recordset?.[0]?.UpdatedCount || 0) !== 1) {
    const error = new Error('엑셀 업로드 이력 상태가 변경되어 전체 적용을 중단했습니다. 다시 검증하세요.');
    error.code = 'SHIPMENT_IMPORT_AUDIT_STATE_CHANGED';
