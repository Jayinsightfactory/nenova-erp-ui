...
export async function updateCatalogImagePosition(id, { posX, posY, scale, rotate, autoAdjusted, manualAdjusted }) {
  const body = JSON.stringify({ posX, posY, scale, rotate, autoAdjusted, manualAdjusted });
  const jsonHeaders = { 'Content-Type': 'application/json' };
  try {
    const data = await catalogApiFetch(`/api/catalog/images/${id}`, {
      method: 'PATCH',
      headers: jsonHeaders,
      body,
    });
    ...
    const data = await catalogApiFetch(`/api/catalog/images/${id}?action=position`, {
      method: 'PUT',
      headers: jsonHeaders,
      body,
    });
