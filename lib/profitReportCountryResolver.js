...
export function buildProfitReportCategorySql(alias = 'p') {
  ...
  CASE
    ...
    WHEN ${product} LIKE N'%수국 운송료%' THEN N'콜롬비아 수국'
    ...
    WHEN ${product} LIKE N'%호주 운송료%' THEN N'호주'
    WHEN ${product} LIKE N'%태국 운송료%' THEN N'태국'
    WHEN ${product} LIKE N'%중국 운송료%' THEN N'중국'
    WHEN ${product} LIKE N'%레몬잎 운송료%' THEN N'미국'
...
export function currencyCodeForCategory(category) {
  ...
  // 새 국가/미분류 행에 USD를 추정 적용하면 R(과세환율)과 매입액이 조용히 왜곡된다.
  // 통화가 확인된 카테고리만 결정적으로 반환하고, 나머지는 입력·분류 대상으로 남긴다.
  return CATEGORY_CURRENCY[category] || null;
}
