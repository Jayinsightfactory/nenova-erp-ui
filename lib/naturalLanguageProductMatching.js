...
export const PRODUCT_MATCH_MODEL_VERSION = 'nl-product-v4';
...
export function scoreNaturalLanguageProducts(rawQuery, products = [], options = {}) {
  ...
  const candidates = products.map((product) => {
    ...
    const categoryNormalizedRaw = query.flower && !conflicts.flower
    ...
    const agreement = (query.country && !conflicts.country ? 0.12 : 0) + (query.flower && !conflicts.flower ? 0.12 : 0) + (query.unit && !conflicts.unit ? 0.04 : 0) + spellingAgreement;
    ...
    const score = spellingSimilarity > 0 && spellingSimilarity < 0.5 && !masterNameExact && !aliasExact
      ? Math.min(categoryCappedScore, PRODUCT_MATCH_THRESHOLDS.medium - 0.01)
      : categoryCappedScore;
    const policy = confidencePolicy(score, conflicts);
    const reasons = [exact && 'exact', masterNameExact && 'master-name-exact', categoryNormalizedExact && 'flower-category-normalized', lexical >= 0.7 && 'token-alias', spellingAgreement > 0 && 'spelling-inventory', customer > 0 && 'customer-usage', recent > 0 && 'recent-usage', query.country && !conflicts.country && 'country', query.flower && !conflicts.flower && 'flower', query.unit && !conflicts.unit && 'unit'].filter(Boolean);
    return {
