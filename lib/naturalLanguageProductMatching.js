...
export const PRODUCT_MATCH_MODEL_VERSION = 'nl-product-v2';
...
export function scoreNaturalLanguageProducts(rawQuery, products = [], options = {}) {
  ...
  const candidates = products.map((product) => {
    ...
    const exact = aliasExact || categoryNormalizedExact
      || [product.ProdName, product.DisplayName, product.ProdCode].some((value) => compact(value) === query.normalized);
    const lexical = Math.max(
      Math.max(0, Math.min(100, scoreMatch(query.raw, product, ''))) / 100,
      categoryNormalizedRaw !== query.raw ? Math.max(0, Math.min(100, scoreMatch(categoryNormalizedRaw, product, ''))) / 100 : 0,
    ...
    const reasons = [exact && 'exact', categoryNormalizedExact && 'flower-category-normalized', lexical >= 0.7 && 'token-alias', customer > 0 && 'customer-usage', recent > 0 && 'recent-usage', query.country && !conflicts.country && 'country', query.flower && !conflicts.flower && 'flower', query.unit && !conflicts.unit && 'unit'].filter(Boolean);
    return {
