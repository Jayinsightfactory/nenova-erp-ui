...
export const PRODUCT_MATCH_MODEL_VERSION = 'nl-product-v3';
...
export function scoreNaturalLanguageProducts(rawQuery, products = [], options = {}) {
  ...
  const candidates = products.map((product) => {
    ...
    const score = categoryNormalizedExact && !masterNameExact && !aliasExact
      ? Math.min(computedScore, PRODUCT_MATCH_THRESHOLDS.high - 0.01)
      : computedScore;
    const policy = confidencePolicy(score, conflicts);
    const reasons = [exact && 'exact', masterNameExact && 'master-name-exact', categoryNormalizedExact && 'flower-category-normalized', lexical >= 0.7 && 'token-alias', customer > 0 && 'customer-usage', recent > 0 && 'recent-usage', query.country && !conflicts.country && 'country', query.flower && !conflicts.flower && 'flower', query.unit && !conflicts.unit && 'unit'].filter(Boolean);
    return {
