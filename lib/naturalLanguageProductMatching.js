...
import { getEnglishOf, scoreMatch, spellingInventorySimilarity, tokenizeForMatch } from './displayName.js';

export const PRODUCT_MATCH_MODEL_VERSION = 'nl-product-v5';
...
function bestVarietySpellingSimilarity(raw, product) {
  const flowerTokens = new Set([...FLOWER_ALIASES.values()].flat().flatMap((alias) => tokenizeForMatch(alias).map((token) => String(token).toLowerCase())));
  const queryTokens = tokenizeForMatch(raw).filter((token) => /^[a-z]{3,}$/i.test(token) && !flowerTokens.has(String(token).toLowerCase()));
  const productTokens = tokenizeForMatch([product?.ProdName, product?.DisplayName, product?.ProdCode].filter(Boolean).join(' '))
    .filter((token) => /^[a-z]{3,}$/i.test(token) && !flowerTokens.has(String(token).toLowerCase()));
  if (!queryTokens.length || !productTokens.length) return 0;
  let weakestRequiredToken = 1;
  for (const queryToken of queryTokens) {
    let best = 0;
    for (const productToken of productTokens) best = Math.max(best, spellingInventorySimilarity(queryToken, productToken));
    weakestRequiredToken = Math.min(weakestRequiredToken, best);
  }
  return weakestRequiredToken;
}

function stripMatchedConcept(raw, canonical, catalog) {
  if (!canonical) return String(raw || '');
  const aliases = catalog.get(canonical) || [];
  const aliasTokens = new Set(aliases.flatMap((alias) => tokenizeForMatch(alias).map((token) => String(token).toLowerCase())));
  return tokenizeForMatch(raw)
    .filter((token) => !aliasTokens.has(String(token).toLowerCase()))
    .join(' ');
}
...
export function scoreNaturalLanguageProducts(rawQuery, products = [], options = {}) {
  ...
  const candidates = products.map((product) => {
    ...
    const productNameFlowerConcept = findConcept([product.ProdName, product.DisplayName].filter(Boolean).join(' '), FLOWER_ALIASES);
    const flowerMetadataMatches = Boolean(query.flower && findConcept(flower, FLOWER_ALIASES) === query.flower);
    const flowerTokenIsPartOfProductName = Boolean(query.flower && productNameFlowerConcept === query.flower);
    const conflicts = {
      country: Boolean(query.country && findConcept(country, COUNTRY_ALIASES) !== query.country),
      // `Solomio Rose (Pink)`처럼 ROSE가 화종이 아니라 품종명 일부인 카네이션도 있다.
      // 전산 품목명 자체가 같은 토큰을 갖고 있으면 화종 충돌로 숨기지 않는다.
      flower: Boolean(query.flower && !flowerMetadataMatches && !flowerTokenIsPartOfProductName),
    ...
    // 공급처 문구는 품종 앞뒤에 꽃 종류를 덧붙이는 경우가 있다.
    // 후보의 실제 FlowerName과 일치할 때만 그 종류 토큰을 제외해 품종 문자열을 비교한다.
    // 예: "SOLOMIO ROSE PINK" -> "SOLOMIO PINK" (장미 후보에 한함).
    const categoryNormalizedRaw = query.flower && flowerMetadataMatches
      ? stripMatchedConcept(query.raw, query.flower, FLOWER_ALIASES)
      : query.raw;
    const categoryNormalized = compact(categoryNormalizedRaw);
    const categoryNormalizedExact = Boolean(categoryNormalized)
      && [product.ProdName, product.DisplayName, product.ProdCode].some((value) => compact(value) === categoryNormalized);
    const masterNameExact = [product.ProdName, product.DisplayName, product.ProdCode].some((value) => compact(value) === query.normalized);
    const exact = aliasExact || masterNameExact || categoryNormalizedExact;
    const categoryLexical = categoryNormalizedRaw !== query.raw
      ? Math.min(0.92, Math.max(0, Math.min(100, scoreMatch(categoryNormalizedRaw, product, ''))) / 100)
      : 0;
    ...
    const spellingSimilarity = bestVarietySpellingSimilarity(query.raw, product);
    const spellingAgreement = spellingSimilarity >= 0.85 ? 0.2 : 0;
    const agreement = (query.country && !conflicts.country ? 0.12 : 0) + (flowerMetadataMatches ? 0.12 : 0) + (flowerTokenIsPartOfProductName && !flowerMetadataMatches ? 0.08 : 0) + (query.unit && !conflicts.unit ? 0.04 : 0) + spellingAgreement;
    const penalty = (conflicts.country ? 0.48 : 0) + (conflicts.flower ? 0.42 : 0) + (conflicts.unit ? 0.18 : 0);
    const exactBonus = aliasExact || masterNameExact ? 0.22 : categoryNormalizedExact ? 0.12 : 0;
    const computedScore = Math.max(0, Math.min(1, lexical * 0.7 + exactBonus + usage + customer + recent + alias + agreement - penalty));
    // 화종 토큰을 제거해야만 같아지는 축약 후보는 실제 사용량이 많아도 자동 확정하지 않는다.
    // 전산 전체 명칭 직접 일치가 존재할 수 있으므로 반드시 사용자 확인 후보로 남긴다.
    const categoryCappedScore = categoryNormalizedExact && !masterNameExact && !aliasExact
      ? Math.min(computedScore, PRODUCT_MATCH_THRESHOLDS.high - 0.01)
      : computedScore;
    // 품목명 안의 ROSE를 품종명 근거로 인정해 후보에는 남기되, Product.FlowerName과
    // 불일치하는 상태를 자동 확정까지 올리지는 않는다. 직원이 실제 전산 행을 확인해 선택한다.
    const flowerTokenCappedScore = flowerTokenIsPartOfProductName && !flowerMetadataMatches
      ? Math.min(categoryCappedScore, PRODUCT_MATCH_THRESHOLDS.high - 0.01)
      : categoryCappedScore;
    const score = spellingSimilarity > 0 && spellingSimilarity < 0.5 && !masterNameExact && !aliasExact
      ? Math.min(flowerTokenCappedScore, PRODUCT_MATCH_THRESHOLDS.medium - 0.01)
      : flowerTokenCappedScore;
    const policy = confidencePolicy(score, conflicts);
    const reasons = [exact && 'exact', masterNameExact && 'master-name-exact', categoryNormalizedExact && 'flower-category-normalized', flowerTokenIsPartOfProductName && !flowerMetadataMatches && 'flower-token-in-product-name', lexical >= 0.7 && 'token-alias', spellingAgreement > 0 && 'spelling-inventory', customer > 0 && 'customer-usage', recent > 0 && 'recent-usage', query.country && !conflicts.country && 'country', flowerMetadataMatches && 'flower', query.unit && !conflicts.unit && 'unit'].filter(Boolean);
    return {
      ...
      score: Number(score.toFixed(4)), ...policy, conflicts, reasons, spellingSimilarity: Number(spellingSimilarity.toFixed(4)), modelVersion: PRODUCT_MATCH_MODEL_VERSION,
      source: product,
    };
  }).sort((a, b) => b.score - a.score || a.prodKey - b.prodKey);
  // 전산 마스터 전체 명칭과 정확히 같은 행은 분류 메타데이터가 비어 있거나 오래됐어도
  // 수동 확인 후보에서 숨기지 않는다. 충돌 정책은 그대로라 자동 선택은 되지 않는다.
  const compatible = candidates.filter((item) => (!item.conflicts.country && !item.conflicts.flower) || item.reasons.includes('master-name-exact'));
