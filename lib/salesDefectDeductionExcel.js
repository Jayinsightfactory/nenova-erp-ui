...
export function formatSalesDefectExportRows(records) {
  ...
  for (const item of source) {
    ...
    const key = exportGroupKey(displayCustomerName) || `row-${customerGroups.size}`;
    if (!customerGroups.has(key)) customerGroups.set(key, { customerName: displayCustomerName, rows: [] });
...
export async function buildSalesDefectWorkbook(records, { year, week, managerName = '' } = {}) {
  ...
  // 원본 양식의 품명(F열)은 영문 DB 품명과 한글 표시명을 함께 담아야 하므로
  // 기본 폭의 2.5배(15 → 37.5)로 고정한다. 업체/품종 폭은 기존 양식을 유지한다.
