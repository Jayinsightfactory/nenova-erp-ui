...
export function formatSalesDefectExportRows(records) {
  ...
  for (const group of customerGroups.values()) {
    ...
    const original = group.originalCustomerName;
    if (original && exportGroupKey(original) !== exportGroupKey(group.customerName)) {
      const originalLabel = `기존명: ${original}`;
      if (output.length - groupStart >= 2) output[groupStart + 1].customerName = originalLabel;
      else output.push({ customerOriginalOnly: true, customerName: originalLabel });
    }
  }
  return output;
}
