...
function cloneTemplateWorksheet(wb, source, name) {
  const target = wb.addWorksheet(name, {
    properties: cloneStyle(source.properties),
    pageSetup: cloneStyle(source.pageSetup),
    views: cloneStyle(source.views),
  });
  for (let c = 1; c <= 11; c += 1) {
    target.getColumn(c).width = source.getColumn(c).width;
    target.getColumn(c).hidden = source.getColumn(c).hidden;
  }
  for (let r = 1; r <= 44; r += 1) {
    target.getRow(r).height = source.getRow(r).height;
    for (let c = 1; c <= 11; c += 1) {
      const from = source.getCell(r, c);
      const to = target.getCell(r, c);
      to.value = from.value;
      to.style = cloneStyle(from.style);
    }
  }
  for (const merged of Object.values(source._merges || {})) target.mergeCells(merged.range);
  for (const image of source.getImages()) target.addImage(image.imageId, cloneStyle(image.range));
  return target;
}
...
export async function buildSalesDefectWorkbook(records, { year, week, managerName = '' } = {}) {
  ...
  const worksheets = [templateWs];
  for (let index = 1; index < pages.length; index += 1) {
    worksheets.push(cloneTemplateWorksheet(wb, templateWs, safeSheetName(`${normalizedWeek}차-${index + 1}`)));
  }
  templateWs.name = safeSheetName(`${normalizedWeek}차`);
  const first = 6;
  worksheets.forEach((ws, pageIndex) => {
    if (pageIndex > 0) ws.name = safeSheetName(`${normalizedWeek}차-${pageIndex + 1}`);
    ...
    ws.pageSetup = {
      ...
      fitToWidth: 1, fitToHeight: 1, printArea: 'B1:K44',
    };
    ws.headerFooter = { ...(ws.headerFooter || {}), oddFooter: `&C${pageIndex + 1} / ${pages.length} 페이지` };
    for (let r = first; r <= 44; r += 1) {
      for (const column of ['B', 'D', 'F', 'G', 'H', 'I', 'J']) setMergedCell(ws, `${column}${r}`, null);
      setExportCellLayout(ws, r);
    }
    pages[pageIndex].forEach((item, index) => {
      const r = first + index;
