...
export function buildProductMatcher(products) {
  ...
  for (const p of products) {
    ...
    const code = norm(p.ProdCode);
    if (code) byCode.set(code, p);
    for (const n of [p.DisplayName, p.ProdName]) {
      const k = norm(n);
