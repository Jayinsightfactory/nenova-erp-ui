...
function readiness(request, scope, historyTruncated) {
  ...
  if (request?.unitComparisonMode === 'NUMERIC_ONLY') return 'UNIT_CONVERSION_UNRESOLVED';
...
function compareRequests(rawRequests, shipmentEvents, scope, historyTruncated) {
  const combined=combinedHistoryEvidence(rawRequests,shipmentEvents,scope,historyTruncated);
  ...
  for (const [eventId, claimants] of claims) {
    ...
    if(claimants.every(result=>!result.status&&combined.get(result.request.id)?.event.eventId===eventId)) {
      for(const result of claimants){result.status='CONSISTENT';result.observed=requestDelta(result.request);result.reasonCodes.push('COMBINED_SHIPMENT_HISTORY');}
      continue;
    }
