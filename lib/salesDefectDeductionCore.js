...
/** 차감 원장 원차수가 등록 대상 차수보다 앞선 경우만 이월 등록을 허용한다. */
export function isEarlierOrSameScope(sourceYear, sourceWeek, targetYear, targetWeek) {
  const sy = normalizeYear(sourceYear);
  const sw = normalizeParentWeek(sourceWeek);
  const ty = normalizeYear(targetYear);
  const tw = normalizeParentWeek(targetWeek);
  if (!sy || !sw || !ty || !tw) return false;
  return sy < ty || (sy === ty && sw <= tw);
}

/** 원차수와 적용 대상 차수를 화면/등록 로그에서 동일하게 표현한다. */
export function registrationScope(sourceYear, sourceWeek, targetYear, targetWeek) {
  return {
    sourceYear: normalizeYear(sourceYear),
    sourceWeek: normalizeParentWeek(sourceWeek),
    targetYear: normalizeYear(targetYear),
    targetWeek: normalizeParentWeek(targetWeek),
    isCarryover: isEarlierOrSameScope(sourceYear, sourceWeek, targetYear, targetWeek)
      && (Number(sourceYear) !== Number(targetYear) || Number(sourceWeek) !== Number(targetWeek)),
  };
}
...
function authorityNumber(user = {}) {
  const raw = user?.authority;
  if (raw == null || String(raw).trim() === '') return null;
  const value = Number(raw);
  return Number.isFinite(value) ? value : null;
}

export function isDefectAdmin(user = {}) {
  const authority = authorityNumber(user);
  return authority != null
    ? authority <= 1
    : /admin|관리자|대표/i.test(String(user?.authority || '')) || String(user?.deptName || '') === '대표';
}

export function canUseDefectIncoming(user = {}) {
  const authority = authorityNumber(user);
  return isDefectAdmin(user)
    || authority === 4
    || authority === 7
    || /수입/.test(String(user?.deptName || ''));
}

export function canUseDefectSupport(user = {}) {
  const authority = authorityNumber(user);
  return isDefectAdmin(user)
    || authority === 2
    || /영업지원/.test(String(user?.deptName || ''));
}

export function canUseDefectSales(user = {}) {
  const authority = authorityNumber(user);
  return isDefectAdmin(user)
    || authority === 6
    || (/영업/.test(String(user?.deptName || '')) && !/영업지원/.test(String(user?.deptName || '')));
}

/**
 * 작성자 감사값이 있는 행은 CreatedBy/CreatedByName만 소유권으로 사용한다.
 * 두 값이 모두 비어 있는 레거시 행만 UpdatedBy/UpdatedByName으로 보완한다.
 */
export function isDeductionOwner(row = {}, user = {}) {
  const userAliases = new Set(
    [user?.userId, user?.userName]
      .map((value) => String(value ?? '').trim().toLowerCase())
      .filter(Boolean),
  );
  if (!userAliases.size) return false;
  const createdAliases = [row.CreatedBy ?? row.createdBy, row.CreatedByName ?? row.createdByName]
    .map((value) => String(value ?? '').trim().toLowerCase())
    .filter(Boolean);
  const ownerAliases = createdAliases.length
    ? createdAliases
    : [row.UpdatedBy ?? row.updatedBy, row.UpdatedByName ?? row.updatedByName]
      .map((value) => String(value ?? '').trim().toLowerCase())
      .filter(Boolean);
  return ownerAliases.some((value) => userAliases.has(value));
}

export function canManageDeduction(row = {}, user = {}) {
  return isDefectAdmin(user) || isDeductionOwner(row, user);
}
...
/** 영업수입불량차감 화면/엑셀에서 허용하는 세 가지 표시 단위만 반환한다. */
export function normalizeDefectUnit(value) {
  const normalized = normalizeUnit(value);
  return ['단', '박스', '스팀(대)'].includes(normalized) ? normalized : '';
}
...
export function normalizeDeductionRow(row = {}) {
  return {
    ...
    orderYear: normalizeYear(row.orderYear ?? row.OrderYear),
    orderWeek: normalizeParentWeek(row.orderWeek ?? row.OrderWeek),
    ...
    appliedOrderYear: normalizeYear(row.appliedOrderYear ?? row.AppliedOrderYear),
    appliedOrderWeek: normalizeParentWeek(row.appliedOrderWeek ?? row.AppliedOrderWeek),
    appliedShipmentKey: Number(row.appliedShipmentKey ?? row.AppliedShipmentKey) > 0
      ? Number(row.appliedShipmentKey ?? row.AppliedShipmentKey) : null,
    appliedCostSourceYear: normalizeYear(row.appliedCostSourceYear ?? row.AppliedCostSourceYear),
    appliedCostSourceWeek: normalizeParentWeek(row.appliedCostSourceWeek ?? row.AppliedCostSourceWeek),
...
/** 변경사항이 없는 감사 이력은 화면의 매칭/수정 이력에서 숨긴다. */
export function isNoopDeductionHistory(item = {}) {
  return /변경\s*없음/.test(String(item?.ChangeSummary ?? item?.changeSummary ?? ''));
}
