...
function quoteMonth(row) {
  const assigned = String(row?.AssignedMonth || '').trim();
  if (/^\d{4}-(0[1-9]|1[0-2])$/.test(assigned)) return assigned;
...
export function normalizeRaumAssignedMonth(value, orderYear) {
  if (value == null || value === '') return null;
  const month = String(value).trim();
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(month)) throw new Error('배정 월은 YYYY-MM 형식이어야 합니다.');
  if (String(orderYear || '') !== month.slice(0, 4)) throw new Error('배정 월은 해당 차수 연도와 같아야 합니다.');
  return month;
}
