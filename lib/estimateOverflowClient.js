...
// Remove only the draft values that were actually sent and committed. If the
// user typed a newer value while the request was in flight, that newer draft
// must remain available for the next save.
export function removeMatchingEstimateDrafts(drafts = {}, committedEntries = []) {
  const committed = new Map(committedEntries
    .filter(entry => entry?.editKey)
    .map(entry => [entry.editKey, entry.draftValue]));
  if (committed.size === 0) return drafts;
  let changed = false;
  const next = { ...drafts };
  committed.forEach((draftValue, editKey) => {
    if (Object.prototype.hasOwnProperty.call(next, editKey)
      && String(next[editKey]) === String(draftValue)) {
      delete next[editKey];
      changed = true;
    }
  });
...
export function normalizeEstimateCombinedCostMode(candidate, fallback = 'once') {
  const allowed = new Set(['once', 'fixed', 'weekFav']);
  return allowed.has(candidate) ? candidate : (allowed.has(fallback) ? fallback : 'once');
}
