...
import { allowedProfitClassificationTargets, masterPatchForProfitCategory } from './profitReportClassificationInput.js';
...
export async function classifyUnclassifiedProfitProduct({ major, orderYear, prodKey, category, actor }) {
  ...
  const eligible = (await unclassifiedDetailsByCategory(major, orderYear)).some(row => row.prodKey === key);
  if (!eligible) {
  ...
  return withTransaction(async (tQ) => {
    const locked = await tQ(
      `SELECT ProdKey, ProdName, CounName, FlowerName, isDeleted
         FROM Product WITH (UPDLOCK,HOLDLOCK) WHERE ProdKey=@pk`,
      { pk: { type: sql.Int, value: key } },
    );
    const before = locked.recordset[0];
    if (!before || Number(before.isDeleted || 0) !== 0) {
      const error = new Error('활성 품목을 찾을 수 없습니다.'); error.statusCode = 404; throw error;
    }
    if (classifyCategory(before.CounName, before.FlowerName, before.ProdName) !== EXTRA_CATEGORY) {
      const error = new Error('이미 다른 분류로 변경된 품목입니다. 화면을 새로 조회하세요.'); error.statusCode = 409; throw error;
    }
    const patch = masterPatchForProfitCategory(category, before.FlowerName);
    await tQ(
      `UPDATE Product SET CounName=@country, FlowerName=@flower WHERE ProdKey=@pk`,
      {
        pk: { type: sql.Int, value: key },
        country: { type: sql.NVarChar, value: patch.counName },
        flower: { type: sql.NVarChar, value: patch.flowerName },
      },
    );
    await tQ(
      `IF OBJECT_ID(N'dbo.SystemActionLog',N'U') IS NOT NULL
       ...
       VALUES (@actor,N'PRODUCT_CLASSIFICATION',N'POST',N'/api/sales/profit-report',N'Product',1,
         @payload,N'SUCCESS',@result,N'MEDIUM')`,
      {
        ...
        result: { type: sql.NVarChar, value: `${before.CounName || ''}/${before.FlowerName || ''} → ${patch.counName}/${patch.flowerName}`.slice(0, 1000) },
      },
    );
    return { prodKey: key, prodName: before.ProdName, before: { counName: before.CounName, flowerName: before.FlowerName }, after: patch };
  });
}
