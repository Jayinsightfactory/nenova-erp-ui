...
/** E기초/F기말 상품재고액 자동 평가 — 재고잔량 시트 공식 그대로:
 * Σ( ProductStock.Stock(박스) × 박스당수량(SteamOf1Box→BunchOf1Box→1) × 품목단가(Product.Cost) ) ÷ 1.1
 * ※ 담당자 수기 단가/실사조정과 다를 수 있어 "기본값"으로 채우고 화면에서 수정 가능.
 * 해당 대차수의 마지막 세부차수(…-02 없으면 -01) 스냅샷을 그 차수의 "기말"로 본다. */
export async function stockValueByCategory(major, orderYear) {
  ...
  if (!week) return { week: null, values: {} };
  // 수국 평가단가는 담당자 단가표(재고잔량 시트) 기준 — 사장님 지정(2026-07-09):
  // 화이트 2600, 블루 2700, 그린류 3100, 모히또 2250, 골드피치·노랑·스페셜핑크 3200, 나머지(핑크·라벤더·피치 등) 3300.
  // 그 외 품종은 Product.Cost. 화면에서 셀 수기수정으로 언제든 덮어쓸 수 있음.
  const PRICE_EXPR = `
    CASE WHEN ISNULL(p.CounName,'') LIKE N'%콜롬비아%' AND ISNULL(p.FlowerName,'') LIKE N'%수국%' THEN
      CASE
  ...
  return { week, values: Object.fromEntries(r.recordset.map(x => [x.Category, Number(x.v)])) };
}
