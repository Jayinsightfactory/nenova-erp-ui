...
import {
  distributionMajorityStockPriceSuggestion,
...
function confirmedDistributionRowsByProduct(rows = []) {
...
function confirmedDistributionSamplePeerPrice(category, rows = []) {
  return distributionMajorityStockPriceSuggestion({
    category,
    rows,
...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear) {
  ...
  const [beginWeekDistributionRows, currentWeekDistributionRows] = await Promise.all([
    beginWeek
      ? loadConfirmedCustomerProductPrices(prevOrderYear, prevMajor, { prodKeys: r.recordset.map((row) => row.ProdKey) })
      : [],
    endWeek
      ? loadConfirmedCustomerProductPrices(orderYear, major, { prodKeys: r.recordset.map((row) => row.ProdKey) })
      : [],
  ]);
  const beginDistributionRowsByProduct = confirmedDistributionRowsByProduct(beginWeekDistributionRows);
  const distributionRowsByProduct = confirmedDistributionRowsByProduct(currentWeekDistributionRows);
  const preparedRows = r.recordset.map((row) => {
    ...
    const sample = isSampleInventoryProduct(row);
    const beginSamplePeerPrice = beginPrice ?? (!sample
      ? confirmedDistributionSamplePeerPrice(row.Category, beginDistributionRowsByProduct.get(key) || [])
      : null);
    const endSamplePeerPrice = endPrice ?? (!sample
      ? confirmedDistributionSamplePeerPrice(row.Category, distributionRowsByProduct.get(key) || [])
      : null);
    return {
      row, key, beginFallback, endFallback, beginPrice, endPrice,
      beginSamplePeerPrice, endSamplePeerPrice,
    };
  });
  const beginSampleAverage = sampleInventoryAveragePriceSuggestions(preparedRows.map((item) => ({
    ...
    price: item.row.ConversionStatus === 'VERIFIED' ? item.beginSamplePeerPrice : null,
    priceSource: item.beginPrice != null ? 'VERIFIED_EXACT'
      : item.beginSamplePeerPrice != null ? 'CONFIRMED_DISTRIBUTION' : '',
  })));
  const endSampleAverage = sampleInventoryAveragePriceSuggestions(preparedRows.map((item) => ({
    ...
    price: item.row.ConversionStatus === 'VERIFIED' ? item.endSamplePeerPrice : null,
    priceSource: item.endPrice != null ? 'VERIFIED_EXACT'
      : item.endSamplePeerPrice != null ? 'CONFIRMED_DISTRIBUTION' : '',
  })));
...
export async function stockSnapshotByCategory(major, orderYear) {
  ...
  const confirmedDistributionRows = await loadConfirmedCustomerProductPrices(orderYear, major, {
    prodKeys: r.recordset.map((row) => row.ProdKey),
  });
  const confirmedRowsByProduct = confirmedDistributionRowsByProduct(confirmedDistributionRows);
  const preparedRows = r.recordset.map((row) => {
    ...
    const catalogEvidence = catalogCandidate
      ...
      ? catalogCandidate
      : null;
    const exactFallback = arrival || catalogEvidence;
    const exactPrice = directPrice == null ? exactFallback?.price ?? null : directPrice;
    const samplePeerPrice = exactPrice ?? (!isSampleInventoryProduct(row)
      ? confirmedDistributionSamplePeerPrice(row.Category, confirmedRowsByProduct.get(String(row.ProdKey)) || [])
      : null);
    return {
      row,
      qty,
      ...
      samplePeerPrice: samplePeerPrice == null ? null : Number(samplePeerPrice),
    };
  });
  const sampleAverageByProduct = sampleInventoryAveragePriceSuggestions(preparedRows.map((item) => ({
    ...
    price: item.row.ConversionStatus === 'VERIFIED' ? item.samplePeerPrice : null,
    priceSource: item.exactPrice != null ? 'VERIFIED_EXACT'
      : item.samplePeerPrice != null ? 'CONFIRMED_DISTRIBUTION' : '',
  })));
  const grouped = {};
  for (const prepared of preparedRows) {
    const { row, qty, directPrice, arrival, catalogEvidence, exactPrice } = prepared;
    const category = row.Category;
    if (!grouped[category]) grouped[category] = {
      value: 0, qty: 0, negativeQty: 0, missing: 0,
      direct: 0, arrival: 0, workbookCatalog: 0, sampleAverage: 0, sourceRefs: [],
  ...
  return {
    ...
    priceEvidenceStatus: Object.fromEntries(categories.map(([category, item]) => [category,
      item.missing || item.conversionMissing ? 'INPUT_REQUIRED'
        : [item.direct > 0, item.arrival > 0, item.workbookCatalog > 0, item.sampleAverage > 0].filter(Boolean).length > 1 ? 'VERIFIED_MIXED'
