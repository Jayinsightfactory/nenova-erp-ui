...
// E/F는 확정 ProductStock + 해당 스냅샷 시점 VERIFIED 단가 근거로만 평가하며 최종값 override를 금지한다.
 ...
 * - 보고서 E/F는 StockMaster.isFix=1인 확정 스냅샷만 사용한다.
 *   미확정·NULL·시작재고 마커는 전년도/입출고 추정으로 대체하지 않는다.
...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear) {
  ...
  const r = await query(
    `SELECT p.ProdKey, p.ProdName, ${CASE_CATEGORY} AS Category,
      ...
      WHERE (smk.StockKey=@beginStockKey OR smk.StockKey=@endStockKey)
        AND ISNULL(smk.isFix,0)=1
        AND ISNULL(ps.Stock,0) > 0
        AND ${stockablePurchaseItemSql('p')}
      GROUP BY p.ProdKey, p.ProdName, p.CounName, p.FlowerName, p.SteamOf1Box, p.BunchOf1Box, p.Cost
  ...
  const rows = r.recordset.map((row) => {
    ...
    const beginFallback = row.BeginPrice == null ? beginArrival[key] : null;
    const endFallback = row.SetPrice == null ? endArrival[key] : null;
    ...
    return {
      ...
      AppliedSource: row.SetPrice != null ? 'VERIFIED_EVIDENCE' : endFallback ? 'VERIFIED_ARRIVAL_COST' : 'INPUT_REQUIRED',
      BeginSource: row.BeginPrice != null ? 'VERIFIED_EVIDENCE' : beginFallback ? 'VERIFIED_ARRIVAL_COST' : 'INPUT_REQUIRED',
...
/** 차수말 재고 스냅샷 집계 — 확정 ProductStock 수량과 같은 차수 VERIFIED 단가 증거를 집계한다.
...
export async function stockSnapshotByCategory(major, orderYear) {
  ...
  if (!week) return {
    ...
    selection: 'missing_stock_snapshot', stockMasterIsFix: null,
  };
  await ensureStockPriceTable();
  const [r, arrivalEvidence] = await Promise.all([
    query(
    `SELECT ${CASE_CATEGORY} AS Category,p.ProdKey,
  ...
  for (const row of r.recordset) {
    ...
    if (!grouped[category]) grouped[category] = { value: 0, qty: 0, negativeQty: 0, missing: 0, direct: 0, arrival: 0, sourceRefs: [] };
    ...
    const price = directPrice == null ? arrival?.price ?? null : directPrice;
    item.qty += qty;
  ...
  return {
    ...
    priceEvidenceStatus: Object.fromEntries(categories.map(([category, item]) => [category,
      item.missing ? 'INPUT_REQUIRED' : item.arrival > 0 && item.direct > 0 ? 'VERIFIED_MIXED' : item.arrival > 0 ? 'VERIFIED_ARRIVAL_COST' : 'VERIFIED'])),
