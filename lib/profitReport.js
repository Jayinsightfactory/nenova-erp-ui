...
/**
 * 선택 대차수에서 nenova.exe 재고현황의 마지막 "확정" 세부차수를 찾는다.
 *
 * - 27-01/27-02만 존재한다는 가정 금지: 27-03, 27-04도 숫자 순서로 지원한다.
 * - StockMaster.isFix=1만 확정 스냅샷으로 인정한다. 미확정(0), 시작재고 마커(2),
 *   NULL 행을 보고서의 기말/기초 재고로 조용히 사용하지 않는다.
 * - 같은 세부차수에 중복 StockMaster가 남아 있으면 가장 최근 StockKey를 선택한다.
 */
export async function latestFinalizedStockWeek(major, orderYear) {
  ...
  return row
    ? { week: row.OrderWeek, stockKey: Number(row.StockKey), isFix: Number(row.isFix) === 1 }
    : { week: null, stockKey: null, isFix: false };
}
...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear) {
  ...
  const [endSnapshot, beginSnapshot] = await Promise.all([
    latestFinalizedStockWeek(major, orderYear),
    latestFinalizedStockWeek(prevMajor, prevOrderYear),
  ]);
 ...
 * nenova.exe 재고현황과 동일하게 해당 대차수의 마지막 확정 세부차수 ProductStock.Stock을 기말잔량으로 쓴다.
...
export async function stockSnapshotByCategory(major, orderYear) {
  const snapshot = await latestFinalizedStockWeek(major, orderYear);
  const week = snapshot.week;
  if (!week) return { week: null, stockKey: null, values: {}, qtys: {}, recentCost: {}, negativeQtys: {}, anchored: {}, selection: 'missing_finalized_snapshot' };
  await ensureStockPriceTable();
