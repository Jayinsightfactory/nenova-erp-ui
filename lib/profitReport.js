...
import { productMasterStockPriceSuggestion } from './profitReportCalc.js';
...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear) {
  ...
  const r = await query(
    `SELECT p.ProdKey, p.ProdName, ${CASE_CATEGORY} AS Category,
      ...
      WHERE (smk.StockKey=@beginStockKey OR smk.StockKey=@endStockKey)
        ...
        AND ${stockablePurchaseItemSql('p')}
      GROUP BY p.ProdKey, p.ProdName, p.CounName, p.FlowerName, p.EstUnit, p.SteamOf1Box, p.BunchOf1Box, p.Cost
  ...
  const rows = r.recordset.map((row) => {
    ...
    const productMasterSuggestion = productMasterStockPriceSuggestion({
    ...
    return {
      ...
      SuggestedPrice: endPrice == null ? productMasterSuggestion?.price ?? null : null,
      SuggestedBasis: productMasterSuggestion?.basis || null,
      SuggestedSourceRef: productMasterSuggestion
        ? `erp:Product:${key}:Cost:${productMasterSuggestion.grossCost}:read-at:${suggestionReadAt}`
        : null,
      SuggestedEffectiveAt: productMasterSuggestion ? suggestionReadAt.slice(0, 10) : null,
    };
  });
