...
// 외부 확정값(H 통관비·R 과세환율·S 포워딩·수량 없는 F 선언 기말상품재고액)과 비고만 웹 전용 테이블에 저장한다.
// E는 EXE ProductStock + 검증 단가 근거로만 평가하며 최종값 override를 금지한다.
// F는 재고수량이 있을 때 같은 공식을 쓰고, 수량이 없어도 금액이 있으면 선언 금액으로 보존한다.
...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear) {
  ...
  const [beginArrival, endArrival, beginCarried, endCarried] = await Promise.all([
  ...
  const preparedRows = r.recordset.map((row) => {
    ...
    const beginFallback = row.BeginPrice == null ? beginArrival[key] || validBeginCatalog || beginCarried[key] : null;
    const endFallback = row.SetPrice == null ? endArrival[key] || validEndCatalog || endCarried[key] : null;
...
export async function stockSnapshotByCategory(major, orderYear) {
  ...
  await ensureStockPriceTable();
  const [r, arrivalEvidence, carriedEvidence, unitMismatch] = await Promise.all([
    query(
  ...
  const preparedRows = r.recordset.map((row) => {
    ...
    const exactFallback = arrival || catalogEvidence || carried;
  ...
  for (const prepared of preparedRows) {
    const { row, qty, directPrice, arrival, catalogEvidence, carried, exactPrice } = prepared;
    const category = row.Category;
    if (!grouped[category]) grouped[category] = {
      value: 0, qty: 0, negativeQty: 0, missing: 0,
      direct: 0, arrival: 0, carried: 0, workbookCatalog: 0, sampleAverage: 0, sourceRefs: [],
  ...
  return {
    ...
    priceEvidenceStatus: Object.fromEntries(categories.map(([category, item]) => [category,
      item.missing || item.conversionMissing ? 'INPUT_REQUIRED'
        : [item.direct > 0, item.arrival > 0, item.carried > 0, item.workbookCatalog > 0, item.sampleAverage > 0].filter(Boolean).length > 1 ? 'VERIFIED_MIXED'
  ...
  const r = await query(
    `SELECT Category, ColKey, Value, TextValue, SourceRef, EffectiveAt, ConfirmedBy, ConfirmedAt
      ...
      WHERE OrderYear=@yr AND MajorWeek=@mw
        AND (Category=N'_note' OR (ColKey IN (N'H',N'R',N'S',N'F')
