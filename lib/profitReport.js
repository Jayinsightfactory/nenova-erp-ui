...
export { CATEGORIES, EXTRA_CATEGORY, classifyCategory, isNonValueWeightItem } from './profitReportClassification.js';
  ...
  const r = await query(
    `WITH stockUnits AS (
       SELECT DISTINCT ${CASE_CATEGORY} AS Category, ISNULL(p.EstUnit,N'') AS EstUnit
         FROM ProductStock ps
         JOIN Product p ON ps.ProdKey=p.ProdKey
        WHERE ps.StockKey=@stockKey AND ISNULL(ps.Stock,0) <> 0 AND ${nonValueWeightSql('p')}
     ), purchUnits AS (
       SELECT DISTINCT ${CASE_CATEGORY} AS Category, ISNULL(p.EstUnit,N'') AS EstUnit
         FROM WarehouseDetail wd
         JOIN WarehouseMaster wm ON wd.WarehouseKey=wm.WarehouseKey
