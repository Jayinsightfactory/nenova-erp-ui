...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear) {
  ...
  const preparedRows = r.recordset.map((row) => {
    ...
    return { row, key, beginFallback, endFallback, beginPrice, endPrice };
  });
  ...
  const rows = preparedRows.map(({ row, key, beginFallback, endFallback, beginPrice: exactBeginPrice, endPrice: exactEndPrice }) => {
    ...
    const beginSampleSource = beginSample ? {
      ...
      sourceRefs: [`erp:${prevOrderYear}:${beginWeek}:stock:${beginSnapshot.stockKey}:sample-average:${beginSample.basis}:${beginSample.unit}:peers:${beginSample.peerCount}`],
    } : null;
    const endSampleSource = endSample ? {
      price: endSample.price,
      source: 'VERIFIED_SAMPLE_AVERAGE',
      sourceRefs: [`erp:${orderYear}:${endWeek}:stock:${endSnapshot.stockKey}:sample-average:${endSample.basis}:${endSample.unit}:peers:${endSample.peerCount}`],
    } : null;
    ...
    if (price == null || !Number.isFinite(Number(price))) item.missing += 1;
      ...
      if (directPrice != null) item.direct += 1;
        ...
        if (catalogEvidence) {
          ...
          item.sourceRefs.push(`erp:${orderYear}:${week}:stock:${snapshot.stockKey}:sample-average:${sampleAverage.basis}:${sampleAverage.unit}:peers:${sampleAverage.peerCount}`);
        }
      }
    }
  }
