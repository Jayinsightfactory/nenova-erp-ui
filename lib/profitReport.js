...
import { allowedProfitClassificationTargets } from './profitReportClassificationInput.js';
import { buildProfitOverrideCaseSql, loadProfitCategoryOverrides, saveProfitCategoryOverride } from './profitReportCategoryOverrides.js';
...
import {
  ...
  isSampleInventoryProduct,
  sampleInventoryAveragePriceSuggestions,
  selectCarriedAcquisitionPriceEvidence,
  selectStockPriceEvidence,
} from './profitReportCalc.js';

export { formatUnclassifiedNote, composeProfitReportNote } from './profitReportNotes.js';
export {
  CATEGORIES, EXTRA_CATEGORY, CNF_CATEGORIES, classifyCategory, isNonValueWeightItem,
...
export let CASE_CATEGORY = buildProfitReportCategorySql('p');

let UNCLASSIFIED_CATEGORY_FILTER = `${CASE_CATEGORY} = N'${EXTRA_CATEGORY}'`;

export function refreshProfitCategorySql(force = false) {
  loadProfitCategoryOverrides(force);
  const allowed = [...new Set([...CATEGORIES.filter(item => !item.manualOnly).map(item => item.key), '국내'])];
  CASE_CATEGORY = buildProfitOverrideCaseSql(buildProfitReportCategorySql('p'), 'p', allowed);
  UNCLASSIFIED_CATEGORY_FILTER = `${CASE_CATEGORY} = N'${EXTRA_CATEGORY}'`;
}

refreshProfitCategorySql();
...
export async function unclassifiedDetailsByCategory(major, orderYear) {
  ...
  const [sales, estimates, purchases, forwarding] = await Promise.all([
    query(
      `SELECT N'매출' AS SourceName, p.ProdKey,
        ...
        GROUP BY p.ProdKey, ISNULL(p.CounName,N'(국가 없음)'), ISNULL(p.FlowerName,N'(품종 없음)'), ISNULL(p.ProdName,N'(품명 없음)')`,
      params,
    ),
    query(
      `SELECT N'견적' AS SourceName, p.ProdKey,
              ISNULL(p.CounName,N'(국가 없음)') AS CountryName,
              ISNULL(p.FlowerName,N'(품종 없음)') AS FlowerName,
              ISNULL(p.ProdName,N'(품명 없음)') AS ProdName,
    ...
    query(
      `SELECT N'입고' AS SourceName, p.ProdKey,
              ISNULL(p.CounName,N'(국가 없음)') AS CountryName,
              ISNULL(p.FlowerName,N'(품종 없음)') AS FlowerName,
              ISNULL(p.ProdName,N'(품명 없음)') AS ProdName,
    ...
    query(
      `WITH bill AS (
       ...
       SELECT N'포워딩' AS SourceName, p.ProdKey,
              ISNULL(p.CounName,N'(국가 없음)') AS CountryName,
              ISNULL(p.FlowerName,N'(품종 없음)') AS FlowerName,
              ISNULL(p.ProdName,N'(품명 없음)') AS ProdName,
              CAST(0 AS FLOAT) AS Quantity,
...
export async function classifyUnclassifiedProfitProduct({ major, orderYear, prodKey, category, actor }) {
  const key = Number(prodKey);
  if (!Number.isInteger(key) || key <= 0) {
    const error = new Error('처리할 품목키가 올바르지 않습니다.'); error.statusCode = 400; throw error;
  }
  if (!allowedProfitClassificationTargets(major).includes(category)) {
    const error = new Error('선택한 국가·화종 분류는 이 차수에 사용할 수 없습니다.'); error.statusCode = 400; throw error;
  }
  const eligibleRows = (await unclassifiedDetailsByCategory(major, orderYear)).filter(row => row.prodKey === key);
  if (!eligibleRows.length) {
    const error = new Error('현재 연도·차수의 미분류 거래에서 다시 확인되지 않은 품목입니다.'); error.statusCode = 409; throw error;
  }
  const before = eligibleRows[0];
  const saved = saveProfitCategoryOverride(key, category, { orderYear, majorWeek: major, savedBy: actor });
  refreshProfitCategorySql(true);
  try {
    await query(
      `IF OBJECT_ID(N'dbo.SystemActionLog',N'U') IS NOT NULL
       INSERT INTO SystemActionLog
         (Actor,ActionType,Method,Endpoint,AffectedTable,AffectedCount,Payload,Result,ResultDesc,RiskLevel)
       VALUES (@actor,N'REPORT_PRODUCT_CLASSIFICATION',N'POST',N'/api/sales/profit-report',N'WebProfitCategoryOverride',1,
         @payload,N'SUCCESS',@result,N'MEDIUM')`,
      {
        actor: { type: sql.NVarChar, value: String(actor || 'user').slice(0, 100) },
        payload: { type: sql.NVarChar, value: JSON.stringify({ orderYear: String(orderYear), major, prodKey: key, category }).slice(0, 4000) },
        result: { type: sql.NVarChar, value: `${before.country || ''}/${before.flower || ''} → 보고서 ${category}`.slice(0, 1000) },
      },
    );
  } catch (error) {
    // 파일 자체에 저장자·시각·적용 연도/차수가 남으므로 분류 적용은 보존한다.
    // 보조 SystemActionLog 실패 때문에 사용자에게 저장 실패로 오인시키지 않는다.
    console.error('[profit-report] classification action log failed:', error?.message || error);
  }
  return { prodKey: key, prodName: before.product, before: { counName: before.country, flowerName: before.flower }, after: { category }, savedAt: saved.savedAt };
}
...
/**
 * exact 같은 세부차수의 입고·FreightCost 원장에서 재현한 품목별 도착원가.
 * 운송기준원가/피벗과 같은 read-only 계산 경로를 재사용하며 DB에 근거를 승격하거나 저장하지 않는다.
 * Product.EstUnit 일치는 소비 지점에서 다시 검사한다.
 */
export async function freightArrivalPriceEvidenceByProduct(orderYear, orderWeek) {
  if (!/^\d{4}$/.test(String(orderYear || '')) || !/^\d{1,2}-\d{1,2}$/.test(String(orderWeek || ''))) return {};
  const { getArrivalCostsForWeekRange } = await import('./pivotFreightArrival.js');
  const rows = await getArrivalCostsForWeekRange({
    weekStart: String(orderWeek),
    weekEnd: String(orderWeek),
    orderYear: String(orderYear),
  });
  return Object.fromEntries(Object.entries(rows || {}).flatMap(([prodKey, row]) => {
    const price = Number(row?.arrivalCost || 0);
    const unit = normalizeInventoryUnit(row?.displayUnit);
    if (!(price > 0) || !unit) return [];
    return [[String(prodKey), {
      price,
      unit,
      source: 'VERIFIED_FREIGHT_ARRIVAL_CALC',
      sourceRefs: [`erp-freight-arrival:${orderYear}:${orderWeek}:prod-${prodKey}:${row?.source || 'calculated'}:${unit}`],
    }]];
  }));
}

function missingPriceItemFromRow(row) {
  return {
    prodKey: Number(row.ProdKey),
    prodName: row.ProdName || '',
    displayName: row.DisplayName || '',
    unit: normalizeInventoryUnit(row.EstUnit),
  };
}
...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear, options = {}) {
  ...
  const [beginArrival, endArrival, beginFreightArrival, endFreightArrival, beginCarried, endCarried] = await Promise.all([
    beginWeek ? arrivalPriceEvidenceByProduct(prevOrderYear, beginWeek) : {},
    endWeek ? arrivalPriceEvidenceByProduct(orderYear, endWeek) : {},
    beginWeek ? freightArrivalPriceEvidenceByProduct(prevOrderYear, beginWeek) : {},
    endWeek ? freightArrivalPriceEvidenceByProduct(orderYear, endWeek) : {},
    beginWeek ? carriedAcquisitionPriceEvidenceByProduct(prevOrderYear, beginWeek) : {},
    endWeek ? carriedAcquisitionPriceEvidenceByProduct(orderYear, endWeek) : {},
  ]);
  const beginWorkbookCatalog = beginWeek ? inventoryWorkbookPriceEvidenceByProduct(prevOrderYear, beginWeek, {
    rateEvidenceByCurrency: options.beginRateEvidenceByCurrency || {},
  }) : {};
  const endWorkbookCatalog = endWeek ? inventoryWorkbookPriceEvidenceByProduct(orderYear, endWeek, {
    rateEvidenceByCurrency: options.endRateEvidenceByCurrency || {},
  }) : {};
  const r = await query(
    `SELECT p.ProdKey, p.ProdName, p.DisplayName, ${CASE_CATEGORY} AS Category,
            p.OutUnit, p.EstUnit,
            ...
            SUM(CASE WHEN smk.StockKey = @beginStockKey THEN ps.Stock * (${STOCK_TO_EST_UNIT_EXPR}) ELSE 0 END) AS StockBeginEst,
            SUM(CASE WHEN smk.StockKey = @endStockKey THEN ps.Stock * (${STOCK_TO_EST_UNIT_EXPR}) ELSE 0 END) AS StockEndEst
      ...
      WHERE (smk.StockKey=@beginStockKey OR smk.StockKey=@endStockKey)
        ...
        AND ${stockablePurchaseItemSql('p')}
      GROUP BY p.ProdKey, p.ProdName, p.DisplayName, p.CounName, p.FlowerName, p.OutUnit, p.EstUnit,
               p.SteamOf1Box, p.BunchOf1Box, p.SteamOf1Bunch
  ...
  const [beginUnitMismatch, endUnitMismatch] = await Promise.all([
    beginWeek ? categoryUnitMismatch(prevMajor, prevOrderYear, beginSnapshot.stockKey) : {},
    endWeek ? categoryUnitMismatch(major, orderYear, endSnapshot.stockKey) : {},
  ]);
  const preparedRows = r.recordset.map((row) => {
    ...
    const validBeginFreight = beginFreightArrival[key]?.unit === unit ? beginFreightArrival[key] : null;
    const validEndFreight = endFreightArrival[key]?.unit === unit ? endFreightArrival[key] : null;
    const beginFallback = row.BeginPrice == null ? selectStockPriceEvidence({
      arrival: beginArrival[key], freightArrival: validBeginFreight, catalogEvidence: validBeginCatalog, carried: beginCarried[key],
    }) : null;
    const endFallback = row.SetPrice == null ? selectStockPriceEvidence({
      arrival: endArrival[key], freightArrival: validEndFreight, catalogEvidence: validEndCatalog, carried: endCarried[key],
    }) : null;
    ...
    return {
      row, key, beginFallback, endFallback, beginPrice, endPrice,
    };
  });
  const beginSampleAverage = sampleInventoryAveragePriceSuggestions(preparedRows.map((item) => ({
    rowKey: item.key,
    prodName: item.row.ProdName,
    displayName: item.row.DisplayName,
    scopeKey: `${prevOrderYear}:${beginWeek || 'missing'}:${beginSnapshot.stockKey || 0}`,
    category: item.row.Category,
    unit: item.row.EstUnit,
    qty: item.row.ConversionStatus === 'VERIFIED' ? item.row.StockBeginEst : 0,
    price: item.row.ConversionStatus === 'VERIFIED' ? item.beginPrice : null,
    priceSource: item.beginPrice != null ? 'VERIFIED_EXACT' : '',
  })));
  const endSampleAverage = sampleInventoryAveragePriceSuggestions(preparedRows.map((item) => ({
    rowKey: item.key,
    prodName: item.row.ProdName,
    displayName: item.row.DisplayName,
    scopeKey: `${orderYear}:${endWeek || 'missing'}:${endSnapshot.stockKey || 0}`,
    category: item.row.Category,
    unit: item.row.EstUnit,
    qty: item.row.ConversionStatus === 'VERIFIED' ? item.row.StockEndEst : 0,
    price: item.row.ConversionStatus === 'VERIFIED' ? item.endPrice : null,
    priceSource: item.endPrice != null ? 'VERIFIED_EXACT' : '',
  })));
  const rows = preparedRows.map(({ row, key, beginFallback, endFallback, beginPrice: exactBeginPrice, endPrice: exactEndPrice }) => {
    const beginSample = exactBeginPrice == null ? beginSampleAverage[key] : null;
    const endSample = exactEndPrice == null ? endSampleAverage[key] : null;
    const beginSampleSource = beginSample ? {
      price: beginSample.price,
      source: 'VERIFIED_SAMPLE_AVERAGE',
      sourceRefs: [`erp:${prevOrderYear}:${beginWeek}:stock:${beginSnapshot.stockKey}:sample-average:${beginSample.basis}:${beginSample.unit}:peers:${beginSample.peerCount}:sources:${beginSample.peerSources.join('+') || 'VERIFIED_EXACT'}`],
    } : null;
    const endSampleSource = endSample ? {
      price: endSample.price,
      source: 'VERIFIED_SAMPLE_AVERAGE',
      sourceRefs: [`erp:${orderYear}:${endWeek}:stock:${endSnapshot.stockKey}:sample-average:${endSample.basis}:${endSample.unit}:peers:${endSample.peerCount}:sources:${endSample.peerSources.join('+') || 'VERIFIED_EXACT'}`],
    } : null;
    const resolvedBeginFallback = beginFallback || beginSampleSource;
    const resolvedEndFallback = endFallback || endSampleSource;
    const beginPrice = exactBeginPrice == null ? resolvedBeginFallback?.price ?? null : exactBeginPrice;
    const endPrice = exactEndPrice == null ? resolvedEndFallback?.price ?? null : exactEndPrice;
    const beginCategoryAverageAvailable = CATEGORY_AVERAGE_INVENTORY_KEYS.includes(String(row.Category || ''))
      && !beginUnitMismatch[row.Category];
    const endCategoryAverageAvailable = CATEGORY_AVERAGE_INVENTORY_KEYS.includes(String(row.Category || ''))
      && !endUnitMismatch[row.Category];
    const hasBeginStock = Math.abs(Number(row.StockBeginEst || 0)) > 0.001;
    const hasEndStock = Math.abs(Number(row.StockEndEst || 0)) > 0.001;
    const conversionVerified = row.ConversionStatus === 'VERIFIED';
    const beginRequiresInput = hasBeginStock && beginPrice == null && conversionVerified && !beginCategoryAverageAvailable;
    const endRequiresInput = hasEndStock && endPrice == null && conversionVerified && !endCategoryAverageAvailable;
    const requiresInput = beginRequiresInput || endRequiresInput;
    const inputReason = requiresInput
      ? `${beginRequiresInput ? '기초' : ''}${beginRequiresInput && endRequiresInput ? '·' : ''}${endRequiresInput ? '기말' : ''} 재고의 검증된 매입원가 근거가 없습니다.`
      : !conversionVerified && (hasBeginStock || hasEndStock)
        ? '품목 단위 환산정보를 먼저 수정해야 합니다.'
        : (beginCategoryAverageAvailable || endCategoryAverageAvailable) && (beginPrice == null || endPrice == null)
          ? '엑셀 카테고리 평균매입원가 공식으로 자동 계산됩니다.'
          : null;
    return {
      ...row,
      ...
      AppliedSource: row.SetPrice != null ? 'VERIFIED_EVIDENCE' : resolvedEndFallback?.source || 'INPUT_REQUIRED',
      BeginSource: row.BeginPrice != null ? 'VERIFIED_EVIDENCE' : resolvedBeginFallback?.source || 'INPUT_REQUIRED',
      ArrivalSourceRefs: resolvedEndFallback?.sourceRefs || [],
      SampleAverageBasis: endSample?.basis || null,
      SampleAveragePeerCount: endSample?.peerCount || 0,
      BeginRequiresInput: beginRequiresInput,
      EndRequiresInput: endRequiresInput,
      RequiresInput: requiresInput,
      InputReason: inputReason,
      AutoCompleted: !requiresInput,
    };
  });
  // 판매단가 자동평가·제안(2026-08-27 사장님 확정: "확정 엑셀은 더 없고, 재고 매입단가는
  // 판매단가 기준으로 자동처리") — 26~31차 백테스트에서 확정 엑셀 대비 오차율이 판매단가
  // 7.3% vs 매입원가환산 45.2%로 검증된 기준이다. 검증된 매입근거가 전무한 품목은 이 값이
  // 보고서 계산(stockSnapshotByCategory)에도 자동 적용되며, 여기서는 같은 값을 입력화면에
  // 제안·자동완성 표시로 내려보낸다. 담당자가 실제 근거를 저장하면 언제나 그 값이 우선한다.
  const suggestTargets = rows.filter((row) => row.RequiresInput).map((row) => Number(row.ProdKey));
  if (suggestTargets.length) {
    const endRank = (() => {
      const m = String(endWeek || beginWeek || '').match(/^(\d{1,2})-(\d{1,2})$/);
      return m ? Number(m[1]) * 100 + Number(m[2]) : 9999;
    })();
    const auto = await autoStockUnitPrices([...new Set(suggestTargets)], orderYear, endRank);
    for (const row of rows) {
      if (!row.RequiresInput) continue;
      const resolved = resolveAutoStockPrice(auto, row.ProdKey, currencyCodeForCategory(row.Category));
      if (!resolved) continue;
      row.SuggestedPrice = resolved.price;
      row.SuggestedSource = resolved.source;
      row.SuggestedDetail = resolved.detail;
      row.SuggestedEffectiveAt = resolved.effectiveAt;
      // 자동처리: 제안이 있으면 입력 없이도 보고서 계산에 같은 값이 쓰이므로
      // 입력 필요 목록에서 내리고 자동완성으로 표시한다(수정·근거 저장은 계속 가능).
      row.AppliedPrice = row.AppliedPrice == null ? resolved.price : row.AppliedPrice;
      row.SetPrice = row.SetPrice == null ? resolved.price : row.SetPrice;
      if (row.BeginRequiresInput && row.BeginPrice == null) row.BeginPrice = resolved.price;
      if (row.AppliedSource === 'INPUT_REQUIRED') row.AppliedSource = 'AUTO_SALES_PRICE';
      if (row.BeginSource === 'INPUT_REQUIRED') row.BeginSource = 'AUTO_SALES_PRICE';
      row.BeginRequiresInput = false;
      row.EndRequiresInput = false;
      row.RequiresInput = false;
      row.AutoCompleted = true;
      row.InputReason = `판매단가 기준 자동평가(${resolved.detail}) — 실제 매입근거 저장 시 대체됩니다.`;
    }
  }
  return {
    beginOrderYear: String(prevOrderYear),
    beginWeek,
...
/** 판매단가 기준 자동평가 원천(2026-08-27) — 같은 연도, 대상 차수 이하에서
 * ① 가장 최근 확정 판매의 공급가 단가 ② 가장 최근 매입의 외화 단가를 배치 조회한다.
 * stockSnapshotByCategory(보고서 계산)와 stockPriceRows(입력화면 표시)가 함께 사용한다. */
export async function autoStockUnitPrices(prodKeys, orderYear, uptoRank) {
  const keys = [...new Set((prodKeys || []).map(Number).filter((n) => Number.isInteger(n) && n > 0))];
  if (!keys.length) return { sales: {}, purchases: {}, rateByCode: {} };
  const keyList = keys.join(',');
  const [sales, purchases, rates] = await Promise.all([
    query(
      `SELECT ProdKey, UnitSupply, OrderWeek, LastDate FROM (
         SELECT sd.ProdKey, sm.OrderWeek,
                SUM(sd.Amount) / NULLIF(SUM(sd.EstQuantity), 0) AS UnitSupply,
                MAX(CONVERT(NVARCHAR(10), sd.ShipmentDtm, 120)) AS LastDate,
                ROW_NUMBER() OVER (PARTITION BY sd.ProdKey ORDER BY
                  TRY_CAST(LEFT(sm.OrderWeek,2) AS INT) * 100 + TRY_CAST(RIGHT(sm.OrderWeek,2) AS INT) DESC) AS rn
           FROM ShipmentDetail sd
           JOIN ShipmentMaster sm ON sd.ShipmentKey=sm.ShipmentKey
          WHERE sm.OrderYear=@yr AND ISNULL(sm.isDeleted,0)=0
            AND ISNULL(sm.isFix,0)=1 AND ISNULL(sd.isFix,0)=1
            AND sd.ProdKey IN (${keyList}) AND ISNULL(sd.Amount,0) > 0 AND ISNULL(sd.EstQuantity,0) > 0
            AND TRY_CAST(LEFT(sm.OrderWeek,2) AS INT) * 100 + TRY_CAST(RIGHT(sm.OrderWeek,2) AS INT) <= @rank
          GROUP BY sd.ProdKey, sm.OrderWeek
       ) x WHERE rn = 1`,
      { yr: { type: sql.NVarChar, value: String(orderYear) }, rank: { type: sql.Int, value: Number(uptoRank) || 9999 } },
    ),
    query(
      `SELECT ProdKey, UnitForeign, OrderWeek, LastDate FROM (
         SELECT wd.ProdKey, wm.OrderWeek,
                SUM(wd.TPrice) / NULLIF(SUM(wd.EstQuantity), 0) AS UnitForeign,
                MAX(CONVERT(NVARCHAR(10), wm.InputDate, 120)) AS LastDate,
                ROW_NUMBER() OVER (PARTITION BY wd.ProdKey ORDER BY
                  TRY_CAST(LEFT(wm.OrderWeek,2) AS INT) * 100 + TRY_CAST(RIGHT(wm.OrderWeek,2) AS INT) DESC) AS rn
           FROM WarehouseDetail wd
           JOIN WarehouseMaster wm ON wd.WarehouseKey=wm.WarehouseKey
          WHERE ISNULL(wm.OrderYear,'')=@yr AND ISNULL(wm.isDeleted,0)=0
            AND wd.ProdKey IN (${keyList}) AND ISNULL(wd.TPrice,0) > 0 AND ISNULL(wd.EstQuantity,0) > 0
            AND TRY_CAST(LEFT(wm.OrderWeek,2) AS INT) * 100 + TRY_CAST(RIGHT(wm.OrderWeek,2) AS INT) <= @rank
          GROUP BY wd.ProdKey, wm.OrderWeek
       ) x WHERE rn = 1`,
      { yr: { type: sql.NVarChar, value: String(orderYear) }, rank: { type: sql.Int, value: Number(uptoRank) || 9999 } },
    ),
    currencyRates(),
  ...
  return {
    sales: Object.fromEntries(sales.recordset.map((row) => [String(row.ProdKey), row])),
    purchases: Object.fromEntries(purchases.recordset.map((row) => [String(row.ProdKey), row])),
    rateByCode: Object.fromEntries((rates || []).map((row) => [row.CurrencyCode, Number(row.ExchangeRate)])),
  };
}

/** autoStockUnitPrices 결과에서 품목 1건의 자동평가 단가를 고른다 —
 * 1순위 판매단가(공급가×1.1, 판매현황·원본 재고잔량 시트 관행과 동일 VAT포함),
 * 2순위 최근 매입 외화단가×통화마스터 근사환율(판매기록 없는 신품목만). 없으면 null. */
export function resolveAutoStockPrice(auto, prodKey, currencyCode) {
  const sale = auto?.sales?.[String(prodKey)];
  if (sale && Number(sale.UnitSupply) > 0) {
    return {
      price: Math.round(Number(sale.UnitSupply) * 1.1),
      source: 'recent_sales_price',
      detail: `${Number(String(sale.OrderWeek).slice(0, 2))}차 판매 공급가 ${Math.round(Number(sale.UnitSupply)).toLocaleString()}원 × 1.1(VAT포함)`,
      effectiveAt: sale.LastDate || null,
    };
  }
  const purchase = auto?.purchases?.[String(prodKey)];
  const rate = currencyCode ? Number(auto?.rateByCode?.[currencyCode]) : null;
  if (purchase && Number(purchase.UnitForeign) > 0 && Number(rate) > 0) {
    return {
      price: Math.round(Number(purchase.UnitForeign) * rate),
      source: 'recent_purchase_cost_approx',
      detail: `${Number(String(purchase.OrderWeek).slice(0, 2))}차 매입 ${Number(purchase.UnitForeign).toFixed(2)} ${currencyCode} × 통화마스터 ${Math.round(rate).toLocaleString()}(근사)`,
      effectiveAt: purchase.LastDate || null,
    };
  }
  return null;
}
...
/**
 * 같은 연도의 이전 VERIFIED 매입원가를, 근거 이후 새 매입이 없는 품목에만 이어 쓴다.
 * 판매·분배단가와 workbook catalog는 후보 SQL에 존재하지 않으며 다른 연도도 조회하지 않는다.
 */
export async function carriedAcquisitionPriceEvidenceByProduct(orderYear, targetWeek) {
  await ensureStockPriceTable();
  const hasArrival = await hasArrivalEvidenceSchema();
  const targetMatch = String(targetWeek || '').trim().match(/^(\d{1,2})-(\d{1,2})$/);
  if (!/^\d{4}$/.test(String(orderYear || '')) || !targetMatch) return {};
  const targetRank = Number(targetMatch[1]) * 100 + Number(targetMatch[2]);
  const params = {
    yr: { type: sql.NVarChar, value: String(orderYear) },
    targetRank: { type: sql.Int, value: targetRank },
  };
  const [stockEvidenceResult, arrivalResult, purchaseResult] = await Promise.all([
    query(
      `SELECT spe.OrderYear,spe.OrderWeek,spe.ProdKey,p.EstUnit AS Unit,spe.Price,
              spe.SourceRef,spe.ConfirmedAt
         FROM WebStockPriceEvidence spe
         JOIN Product p ON p.ProdKey=spe.ProdKey AND ISNULL(p.isDeleted,0)=0
        WHERE spe.OrderYear=@yr AND spe.EvidenceStatus=N'VERIFIED'
          AND spe.SourceRef IS NOT NULL AND spe.EffectiveAt IS NOT NULL
          AND spe.ConfirmedBy IS NOT NULL AND spe.ConfirmedAt IS NOT NULL
          AND (TRY_CONVERT(INT,LEFT(${NORMALIZED_ORDER_WEEK_SQL('spe.OrderWeek')},2))*100
             + TRY_CONVERT(INT,RIGHT(${NORMALIZED_ORDER_WEEK_SQL('spe.OrderWeek')},2))) < @targetRank`,
      params,
    ),
    hasArrival ? query(
      `SELECT l.OrderYear,l.OrderWeek,l.ProdKey,l.Unit,l.Quantity,
              CASE WHEN ISNULL(l.SelectedArrivalCostKRW,0)>0 THEN l.SelectedArrivalCostKRW
                   ELSE l.SourceArrivalCostKRW END AS Price,
              l.SourceFileName,l.SheetName,l.SourceRow,i.RevisionNo,
              confirmation.ChangedAt AS ConfirmedAt
         FROM dbo.WebArrivalCostLine l
         JOIN dbo.WebArrivalCostImport i ON i.ImportKey=l.ImportKey AND ISNULL(i.IsDeleted,0)=0
         JOIN dbo.Product p ON p.ProdKey=l.ProdKey AND ISNULL(p.isDeleted,0)=0
         CROSS APPLY (
           SELECT TOP 1 h.ChangedBy,h.ChangedAt
             FROM dbo.WebArrivalCostHistory h
            WHERE h.ArrivalLineKey=l.ArrivalLineKey AND h.ActionType IN (N'MATCH',N'BASIS_CHANGE')
              AND NULLIF(LTRIM(RTRIM(ISNULL(h.ChangedBy,N''))),N'') IS NOT NULL
            ORDER BY h.ChangedAt DESC,h.HistoryKey DESC
         ) confirmation
        WHERE l.OrderYear=@yr AND l.IsCurrent=1 AND l.MatchStatus=N'MATCHED'
          AND ISNULL(l.Quantity,0)>0
          AND (ISNULL(l.SelectedArrivalCostKRW,0)>0 OR ISNULL(l.SourceArrivalCostKRW,0)>0)
          AND NULLIF(LTRIM(RTRIM(ISNULL(l.SourceFileName,N''))),N'') IS NOT NULL
          AND NULLIF(LTRIM(RTRIM(ISNULL(l.SheetName,N''))),N'') IS NOT NULL
          AND ISNULL(l.SourceRow,0)>0 AND ISNULL(i.RevisionNo,0)>0
          AND ${NORMALIZED_UNIT_SQL('l.Unit')}=${NORMALIZED_UNIT_SQL('p.EstUnit')}
          AND (TRY_CONVERT(INT,LEFT(${NORMALIZED_ORDER_WEEK_SQL('l.OrderWeek')},2))*100
             + TRY_CONVERT(INT,RIGHT(${NORMALIZED_ORDER_WEEK_SQL('l.OrderWeek')},2))) < @targetRank`,
      params,
    ) : Promise.resolve({ recordset: [] }),
    query(
      `SELECT wm.OrderYear,wm.OrderWeek,wd.ProdKey
         FROM WarehouseDetail wd
         JOIN WarehouseMaster wm ON wm.WarehouseKey=wd.WarehouseKey
         JOIN Product p ON p.ProdKey=wd.ProdKey
        WHERE ISNULL(wm.isDeleted,0)=0 AND ISNULL(wm.OrderYear,N'')=@yr
          AND (ABS(ISNULL(wd.OutQuantity,0))+ABS(ISNULL(wd.BoxQuantity,0))
             + ABS(ISNULL(wd.BunchQuantity,0))+ABS(ISNULL(wd.SteamQuantity,0))
             + ABS(ISNULL(wd.TPrice,0)))>0
          AND ${stockablePurchaseItemSql('p')}
          AND (TRY_CONVERT(INT,LEFT(${NORMALIZED_ORDER_WEEK_SQL('wm.OrderWeek')},2))*100
             + TRY_CONVERT(INT,RIGHT(${NORMALIZED_ORDER_WEEK_SQL('wm.OrderWeek')},2))) <= @targetRank`,
      params,
    ),
  ]);
  const arrivalGrouped = new Map();
  for (const row of arrivalResult.recordset || []) {
    const key = `${row.ProdKey}:${row.OrderWeek}`;
    const quantity = Number(row.Quantity || 0);
    const price = Number(row.Price || 0);
    if (!(quantity > 0 && price > 0)) continue;
    if (!arrivalGrouped.has(key)) arrivalGrouped.set(key, {
      orderYear: row.OrderYear, orderWeek: row.OrderWeek, prodKey: row.ProdKey,
      unit: row.Unit, weightedValue: 0, quantity: 0, source: 'VERIFIED_ARRIVAL_COST',
      sourceRefs: [], confirmedAt: row.ConfirmedAt,
    });
    const item = arrivalGrouped.get(key);
    item.weightedValue += quantity * price;
    item.quantity += quantity;
    item.sourceRefs.push(`arrival-cost:${row.OrderYear}:${row.OrderWeek}:rev-${row.RevisionNo}:${row.SourceFileName}#${row.SheetName}!row-${row.SourceRow}`);
    if (row.ConfirmedAt && (!item.confirmedAt || new Date(row.ConfirmedAt) > new Date(item.confirmedAt))) item.confirmedAt = row.ConfirmedAt;
  }
  const candidates = [
    ...(stockEvidenceResult.recordset || []).map((row) => ({
      orderYear: row.OrderYear, orderWeek: row.OrderWeek, prodKey: row.ProdKey,
      unit: row.Unit, price: row.Price, source: 'VERIFIED_EVIDENCE',
      sourceRefs: [String(row.SourceRef)], confirmedAt: row.ConfirmedAt,
    })),
    ...[...arrivalGrouped.values()].map((item) => ({
      ...item, price: item.quantity > 0 ? item.weightedValue / item.quantity : null,
    })),
  ];
  const products = [...new Map(candidates.map((row) => [String(row.prodKey), {
    prodKey: row.prodKey, unit: row.unit,
  }])).values()];
  return selectCarriedAcquisitionPriceEvidence({
...
export async function stockSnapshotByCategory(major, orderYear, options = {}) {
  ...
  await ensureStockPriceTable();
  const [r, arrivalEvidence, freightArrivalEvidence, carriedEvidence, unitMismatch] = await Promise.all([
    query(
    `SELECT ${CASE_CATEGORY} AS Category,p.ProdKey,p.ProdName,p.DisplayName,p.OutUnit,p.EstUnit,
    ...
    freightArrivalPriceEvidenceByProduct(orderYear, week),
    carriedAcquisitionPriceEvidenceByProduct(orderYear, week),
  ...
  const workbookCatalog = inventoryWorkbookPriceEvidenceByProduct(orderYear, week, {
    rateEvidenceByCurrency: options.rateEvidenceByCurrency || {},
  });
  const preparedRows = r.recordset.map((row) => {
    ...
    const catalogEvidence = catalogCandidate
      ...
      ? catalogCandidate
      : null;
    const freightCandidate = freightArrivalEvidence[String(row.ProdKey)];
    const freightArrival = freightCandidate
      && freightCandidate.unit === normalizeInventoryUnit(row.EstUnit)
      ? freightCandidate
      : null;
    const carried = carriedEvidence[String(row.ProdKey)];
    const exactFallback = selectStockPriceEvidence({ arrival, freightArrival, catalogEvidence, carried });
    const exactPrice = directPrice == null ? exactFallback?.price ?? null : directPrice;
    return {
      row,
      qty,
      directPrice,
      ...
      freightArrival,
      carried,
      exactPrice: exactPrice == null ? null : Number(exactPrice),
    };
  });
  const sampleAverageByProduct = sampleInventoryAveragePriceSuggestions(preparedRows.map((item) => ({
    rowKey: item.row.ProdKey,
    prodName: item.row.ProdName,
    displayName: item.row.DisplayName,
    scopeKey: `${orderYear}:${week}:${snapshot.stockKey}`,
    category: item.row.Category,
    unit: item.row.EstUnit,
    qty: item.row.ConversionStatus === 'VERIFIED' ? item.qty : 0,
    price: item.row.ConversionStatus === 'VERIFIED' ? item.exactPrice : null,
    priceSource: item.exactPrice != null ? 'VERIFIED_EXACT' : '',
  })));
  // 판매단가 기준 자동평가(2026-08-27 사장님 확정 — "재고 매입단가는 자동처리"):
  // 위의 모든 검증 근거(직접확정·도착원가·카탈로그·이월·샘플평균)가 없는 품목만, 최근 확정
  // 판매단가(공급가×1.1) → 최근 매입 외화단가×근사환율 순으로 자동 평가한다. 26~31차 백테스트
  // 오차율 판매단가 7.3% vs 매입원가환산 45.2%. 근거를 저장하면 언제나 그 값이 우선한다.
  const autoMissingKeys = preparedRows
    .filter((item) => item.row.ConversionStatus === 'VERIFIED'
      && item.exactPrice == null && !sampleAverageByProduct[String(item.row.ProdKey)])
    .map((item) => Number(item.row.ProdKey));
  const weekRankMatch = String(week).match(/^(\d{1,2})-(\d{1,2})$/);
  const weekRank = weekRankMatch ? Number(weekRankMatch[1]) * 100 + Number(weekRankMatch[2]) : 9999;
  const autoPrices = autoMissingKeys.length
    ? await autoStockUnitPrices(autoMissingKeys, orderYear, weekRank)
    : { sales: {}, purchases: {}, rateByCode: {} };
  const grouped = {};
  for (const prepared of preparedRows) {
    const { row, qty, directPrice, arrival, catalogEvidence, freightArrival, carried, exactPrice } = prepared;
    const category = row.Category;
    if (!grouped[category]) grouped[category] = {
      value: 0, qty: 0, negativeQty: 0, missing: 0, missingItems: [],
      direct: 0, arrival: 0, freightArrival: 0, carried: 0, workbookCatalog: 0, sampleAverage: 0, salesAuto: 0, sourceRefs: [],
    ...
    if (row.ConversionStatus !== 'VERIFIED') {
      ...
      item.missingItems.push(missingPriceItemFromRow(row));
      continue;
    }
    const sampleAverage = exactPrice == null ? sampleAverageByProduct[String(row.ProdKey)] : null;
    const autoResolved = exactPrice == null && !sampleAverage
      ? resolveAutoStockPrice(autoPrices, row.ProdKey, resolveCurrencyCodeForCategory(category))
      : null;
    const price = exactPrice == null ? (sampleAverage?.price ?? autoResolved?.price ?? null) : exactPrice;
    item.qty += qty;
    ...
    if (price == null || !Number.isFinite(Number(price))) {
      ...
      if (autoResolved) {
        item.salesAuto += 1;
        item.sourceRefs.push(`auto-stock-price:${orderYear}:${week}:prod-${row.ProdKey}:${autoResolved.source}:${autoResolved.detail}`);
      } else if (directPrice != null) item.direct += 1;
      else if (arrival) {
        item.arrival += 1;
        item.sourceRefs.push(...(arrival?.sourceRefs || []));
      } else {
        if (freightArrival) {
          item.freightArrival += 1;
          item.sourceRefs.push(...(freightArrival?.sourceRefs || []));
        } else if (catalogEvidence) {
          item.workbookCatalog += 1;
          item.sourceRefs.push(...(catalogEvidence?.sourceRefs || []));
        } else if (carried) {
          item.carried += 1;
          item.sourceRefs.push(...(carried?.sourceRefs || []));
        } else if (sampleAverage && isSampleInventoryProduct(row)) {
          item.sampleAverage += 1;
          item.sourceRefs.push(`erp:${orderYear}:${week}:stock:${snapshot.stockKey}:sample-average:${sampleAverage.basis}:${sampleAverage.unit}:peers:${sampleAverage.peerCount}:sources:${sampleAverage.peerSources.join('+') || 'VERIFIED_EXACT'}`);
        }
      }
    }
  }
  ...
  return {
    ...
    priceEvidenceStatus: Object.fromEntries(categories.map(([category, item]) => [category,
      item.missing || item.conversionMissing ? 'INPUT_REQUIRED'
        // 판매단가 자동평가가 한 품목이라도 섞이면 카테고리 전체를 자동평가 상태로 표시해
        // 감사·화면이 "정확 근거 아님"을 놓치지 않게 한다(값은 이미 합산에 반영됨).
        : item.salesAuto > 0 ? 'VERIFIED_SALES_PRICE_AUTO'
          : [item.direct > 0, item.arrival > 0, item.freightArrival > 0, item.carried > 0, item.workbookCatalog > 0, item.sampleAverage > 0].filter(Boolean).length > 1 ? 'VERIFIED_MIXED'
            : item.arrival > 0 ? 'VERIFIED_ARRIVAL_COST'
              : item.freightArrival > 0 ? 'VERIFIED_FREIGHT_ARRIVAL_CALC'
                : item.workbookCatalog > 0 ? 'VERIFIED_WORKBOOK_CATALOG'
                  : item.carried > 0 ? 'VERIFIED_CARRIED_ACQUISITION'
                    : item.sampleAverage > 0 ? 'VERIFIED_SAMPLE_AVERAGE'
                      : 'VERIFIED'])),
    ...
    missingPriceItems: Object.fromEntries(categories.map(([category, item]) => [category, item.missingItems])),
  ...
  const r = await query(
    `SELECT Category, ColKey, Value, TextValue, SourceRef, EffectiveAt, ConfirmedBy, ConfirmedAt
      ...
      WHERE OrderYear=@yr AND MajorWeek=@mw
        AND (Category=N'_note' OR (ColKey IN (N'H',N'R',N'S',N'AC',N'AJ')
...
export async function saveManual(major, orderYear, values, note, actor, evidenceByCategory = {}) {
  ...
  for (const [category, cols] of Object.entries(values || {})) {
    for (const [colKey, value] of Object.entries(cols || {})) {
      // AC(매출조정)/AJ(이익조정) — 차수귀속을 원본 엑셀처럼 수동 반영하는 열(2026-08-26).
      // E/F 등 자동계산 열은 계속 직접입력을 거부한다.
      if (!['H', 'R', 'S', 'AC', 'AJ'].includes(colKey)) {
