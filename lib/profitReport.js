...
export async function stockPriceRows(major, prevMajor, orderYear, prevOrderYear = orderYear, options = {}) {
  ...
  // 판매단가 자동제안(2026-08-27 사장님 지시) — 26~31차 백테스트에서 확정 엑셀 대비 오차율이
  // 판매단가 7.3% vs 매입원가환산 45.2%로, 회사 확정 기준(엑셀 담당자 관행)과 정합한 판매단가를
  // 1순위 제안으로 채택했다. 판매기록이 없는 품목만 최근 매입 외화단가×통화마스터 근사환율을
  // 2순위로 제안한다. 어디까지나 "제안"이며, 저장 시 제안 출처가 근거(sourceRef)로 기록되고
  // 담당자가 수정한 값은 기존 근거 입력 흐름 그대로다.
  const suggestTargets = rows.filter((row) => row.RequiresInput).map((row) => Number(row.ProdKey));
  if (suggestTargets.length) {
    const keyList = [...new Set(suggestTargets)].join(',');
    const endRank = (() => {
    ...
    const [sales, purchases, rates] = await Promise.all([
      query(
        ...
        { yr: { type: sql.NVarChar, value: String(orderYear) }, rank: { type: sql.Int, value: endRank } },
      ),
      query(
    ...
    const salesByKey = Object.fromEntries(sales.recordset.map((row) => [String(row.ProdKey), row]));
    const purchaseByKey = Object.fromEntries(purchases.recordset.map((row) => [String(row.ProdKey), row]));
    for (const row of rows) {
      if (!row.RequiresInput) continue;
      const sale = salesByKey[String(row.ProdKey)];
      if (sale && Number(sale.UnitSupply) > 0) {
        // 판매현황·재고잔량 시트 관행과 동일하게 VAT 포함 단가를 제안한다(기존 26~34차 백필과 같은 기준).
        row.SuggestedPrice = Math.round(Number(sale.UnitSupply) * 1.1);
        row.SuggestedSource = 'recent_sales_price';
        row.SuggestedDetail = `${Number(String(sale.OrderWeek).slice(0, 2))}차 판매 공급가 ${Math.round(Number(sale.UnitSupply)).toLocaleString()}원 × 1.1(VAT포함)`;
        row.SuggestedEffectiveAt = sale.LastDate || null;
        continue;
      }
      const purchase = purchaseByKey[String(row.ProdKey)];
      const currency = currencyCodeForCategory(row.Category);
      const rate = currency ? rateByCode[currency] : null;
      if (purchase && Number(purchase.UnitForeign) > 0 && Number(rate) > 0) {
        row.SuggestedPrice = Math.round(Number(purchase.UnitForeign) * Number(rate));
        row.SuggestedSource = 'recent_purchase_cost_approx';
        row.SuggestedDetail = `${Number(String(purchase.OrderWeek).slice(0, 2))}차 매입 ${Number(purchase.UnitForeign).toFixed(2)} ${currency} × 통화마스터 ${Math.round(Number(rate)).toLocaleString()}(근사)`;
        row.SuggestedEffectiveAt = purchase.LastDate || null;
      }
    }
  }
  return {
...
export async function stockSnapshotByCategory(major, orderYear, options = {}) {
  ...
  for (const prepared of preparedRows) {
    ...
    if (!grouped[category]) grouped[category] = {
      ...
      direct: 0, arrival: 0, freightArrival: 0, carried: 0, workbookCatalog: 0, sampleAverage: 0, sourceRefs: [],
    ...
    const price = exactPrice == null ? sampleAverage?.price ?? null : exactPrice;
    item.qty += qty;
