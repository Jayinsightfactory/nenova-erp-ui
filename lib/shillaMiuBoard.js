...
/**
 * 전체 탭과 업체 탭이 공유하는 단일 표의 행 묶음.
 *
 * 화면 열은 두 탭 모두 업체 · 품목명 · 단위 · 예상물량 · 현재분배 · 업체최종분배 ·
 * 업체잔량 · 미우이관 · 완료 · 세부 로 같다. 이 함수는 그 표에 들어갈 행의 순서와
 * 묶음만 만들고, 수량 계산식은 buildGroupRows/buildOverviewSections 결과를 그대로 쓴다.
 *
 * - 같은 품목이 여러 업체에 있으면 업체별 행을 각각 유지한다(업체 수량 합산 금지).
 * - 미우 자체수량과 미우 총수량은 사라지지 않도록 같은 열 구조의 작은 합계 행 하나로 보존한다.
 * - 업체 탭(overview 없음)은 receiver 가 null 이라 합계 행 없이 같은 열만 쓴다.
 * - 수령업체가 여럿이면 blockKey 에 수령 CustKey 가 들어가 서로 섞이지 않는다.
 *
 * @returns {Array<{blockKey,receiverCustKey,receiverCustName,prodKey,prodName,unit,country,
 *   items:Array<{itemKey,group,row,week}>, receiver:null|{receiverCustKey,receiverCustName,selfQty,selfExpected}}>}
 */
export function buildUnifiedBlocks({ week, boards = [], overview = [] } = {}) {
  const target = normalizeMajorWeek(week);
  const blocks = new Map();

  const blockOf = (receiverCustKey, receiverCustName, src) => {
    const key = `${Number(receiverCustKey) || 0}|${Number(src.prodKey)}|${src.unit || ""}`;
    if (!blocks.has(key))
      blocks.set(key, {
        blockKey: key,
        receiverCustKey: Number(receiverCustKey) || 0,
        receiverCustName: receiverCustName || "",
        prodKey: Number(src.prodKey),
    ...
    if (!block.prodName && src.prodName) block.prodName = src.prodName;
    if (!block.country && src.country) block.country = src.country;
    if (!block.receiverCustName && receiverCustName)
      block.receiverCustName = receiverCustName;
    return block;
  };
  ...
  for (const board of boards) {
    ...
    for (const row of board.rows || []) {
      const w = row.weeks?.[target];
      if (!w) continue;
      const block = blockOf(group.receiverCustKey, group.receiverCustName, row);
      block.items.push({
        itemKey: `${Number(group.groupKey) || 0}|${Number(row.prodKey)}`,
        group,
        row,
        week: w,
      });
  ...
  // 미우 자체수량은 원천업체 행이 하나도 없는 품목에서도 표에 남아야 한다.
  for (const section of overview || []) {
    for (const row of section.rows || []) {
      const block = blockOf(
        section.receiverCustKey,
        section.receiverCustName,
        row,
      );
      block.receiver = {
        receiverCustKey: Number(section.receiverCustKey) || 0,
        receiverCustName: section.receiverCustName || "",
        selfQty: roundQty(row.receiverSelfQty),
        selfExpected: roundQty(row.receiverSelfExpected),
      };
    }
  }
  ...
  return [...blocks.values()].sort(
    (a, b) =>
      a.receiverCustKey - b.receiverCustKey ||
      `${a.country}${a.prodName}`.localeCompare(
        `${b.country}${b.prodName}`,
        "ko",
      ) ||
      String(a.unit).localeCompare(String(b.unit), "ko"),
  );
}
