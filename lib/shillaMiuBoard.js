...
export function buildBoardRows({ weeks, orders = [], incoming = [], shipments = [], allocations = [], openingStocks = [] }) {
  ...
  for (const record of orders) {
    ...
    ensureWeek(ensureRow(rows, record), record.week).orderQty += n(record.qty);
  }
  ...
  return [...rows.values()]
    ...
    .filter((row) => orderedWeeks.some((week) => {
      ...
      return n(w.orderQty) || n(w.incomingQty) || n(w.erp.shilla) || n(w.erp.raum) || n(w.erp.miu) || n(w.web.RAUM.qty) || n(w.web.MIU.qty);
    }))
