...
function publicLease(lease, viewer = null) {
  ...
  if (!lease) return {
    active: false, ownedByMe: false, ownerName: '', pageCode: '', expiresAt: null,
  };
  return {
    active: true,
    ownedByMe,
...
export async function readErpEditSnapshot(executor, rawScope, { lock = false } = {}) {
  ...
  return { scope, digest, rowCount: rows.length };
}
...
export async function acquireErpEditLease(tQ, rawScope, user = {}, rawClient = {}) {
  ...
  const token = existingActive ? existing.leaseToken : crypto.randomUUID();
  const baseline = existingActive ? existing.baselineDigest || snapshot.digest : snapshot.digest;
