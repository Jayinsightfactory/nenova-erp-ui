...
import { isAdminUser } from './userAccess.js';
...
function leaseStamp(lease) {
  if (!lease) return '';
  return crypto.createHash('sha256').update(JSON.stringify([
    lease.orderYear, lease.orderWeek, lease.custKey, lease.leaseToken, lease.revision,
  ])).digest('hex');
}
...
export async function getErpEditStatus(executor, rawScope, viewer = null) {
  const scope = normalizeEditScope(rawScope);
  // GET must remain lock-free, but a parallel lease/snapshot read can combine
  // the pre-save lease with a post-save ERP digest and report a false stale
  // conflict.  Accept only a lease that stayed identical around the snapshot.
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const leaseBefore = await selectLease(executor, scope);
    const snapshot = await readErpEditSnapshot(executor, scope);
    const leaseAfter = await selectLease(executor, scope);
    if (isStableStatusLease(leaseBefore, leaseAfter, scope)) {
      return {
        scope,
        lease: leaseAfter,
        stale: Boolean(leaseAfter?.baselineDigest && leaseAfter.baselineDigest !== snapshot.digest),
        snapshot,
      };
    }
  }
  throw editError(
    'ERP_EDIT_STATUS_CHECKING',
    '다른 작업의 저장이 끝나는지 확인 중입니다. 잠시 후 자동으로 다시 확인합니다.',
    503,
  );
}

function isStableStatusLease(before, after, scope) {
  if (!before || !after) return before === after;
  const fields = [
    'orderYear', 'orderWeek', 'custKey',
    'leaseToken', 'revision', 'baselineDigest',
    'ownerUserId', 'ownerName', 'clientId', 'pageCode',
  ];
  return fields.every((field) => before[field] === after[field])
    && before.orderYear === scope.orderYear
    && before.orderWeek === scope.orderWeek
    && before.custKey === scope.custKey;
}
...
export async function acquireErpEditLease(tQ, rawScope, user = {}, rawClient = {}) {
  ...
  const forceTakeover = rawClient.forceTakeover === true;
  if (!ownerUserId) throw editError('ERP_EDIT_OWNER_INVALID', '로그인한 사용자 정보가 없어 편집을 시작할 수 없습니다.', 401);
  if (forceTakeover && (!isAdminUser(user) || client.pageCode !== 'estimate' || rawClient.confirmed !== true)) {
    throw editError('ERP_EDIT_TAKEOVER_FORBIDDEN', '견적서 작업권 인계는 관리자 확인 후에만 가능합니다.', 403);
  }
  ...
  if (forceTakeover && (!existingActive || !rawClient.expectedLeaseStamp || rawClient.expectedLeaseStamp !== leaseStamp(existing))) {
    throw editError('ERP_EDIT_TAKEOVER_CHANGED', '작업권 또는 저장 상태가 변경되었습니다. 최신 내용을 확인한 뒤 다시 인계하세요.', 409, {
      lease: publicLease(existingActive ? existing : null, { userId: ownerUserId, clientId: client.clientId }),
    });
  }
  if (!forceTakeover && existingActive && (existing.ownerUserId !== ownerUserId || (existing.clientId !== client.clientId && !takeover))) {
  ...
  if (!forceTakeover && existingActive && existing.ownerUserId === ownerUserId && existing.clientId === client.clientId
  ...
  const baseline = takeover || forceTakeover
    ? snapshot.digest
...
export async function heartbeatErpEditLease(tQ, rawScope, user = {}, rawGuard = {}, options = {}) {
  const scope = normalizeEditScope(rawScope);
  ...
  if (options.renewOnly === true) {
    return { scope, lease: renewedLease, snapshot: null, stale: false };
  }
...
// Keep the lease transaction independent from the ERP status read.  A
// heartbeat must not hold its lease UPDLOCK while a save holds ERP row locks
// and later needs that same lease lock.
export async function renewThenReadErpEditStatus(dependencies = {}, rawScope, user = {}, rawGuard = {}) {
  const { withTransaction, query } = dependencies;
  if (typeof withTransaction !== 'function' || typeof query !== 'function') {
    throw new TypeError('renewThenReadErpEditStatus에는 withTransaction과 query가 필요합니다.');
  }
  await withTransaction((tQ) => heartbeatErpEditLease(tQ, rawScope, user, rawGuard, { renewOnly: true }));
  return getErpEditStatus(query, rawScope, {
    userId: user.userId,
    clientId: rawGuard?.clientId || rawGuard?.editGuard?.clientId,
  });
}
