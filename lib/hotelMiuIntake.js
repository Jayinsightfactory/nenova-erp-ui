...
export const HOTEL_MIU_FORWARD_MAJOR_COUNT = 9;
export const HOTEL_MIU_SUBWEEK_NUMBERS = Object.freeze([1, 2, 3, 4]);
...
function calendarMajorWeek(now = new Date()) {
  ...
  return { year, major: Math.min(52, Math.max(1, Math.ceil(dayOfYear / 7))) };
}

function shiftHotelMiuMajor(year, major, delta) {
...
/** 영업 발주는 달력 대차수보다 한 차수 앞서 입력하며, 기본부터 +9까지 고른다. */
export function hotelMiuMajorWeekOptions(now = new Date(), forwardMajorCount = HOTEL_MIU_FORWARD_MAJOR_COUNT) {
  const calendar = calendarMajorWeek(now);
  const base = shiftHotelMiuMajor(calendar.year, calendar.major, 1);
  const count = Math.min(20, Math.max(0, Number(forwardMajorCount) || 0));
  return Array.from({ length: count + 1 }, (_, offset) => {
    const shifted = shiftHotelMiuMajor(base.year, base.major, offset);
    return {
      year: String(shifted.year),
      major: shifted.major,
      majorText: String(shifted.major).padStart(2, '0'),
      label: `${shifted.year !== base.year ? `${String(shifted.year).slice(2)}년 ` : ''}${shifted.major}${offset === 0 ? ' 기본' : ` +${offset}`}`,
      offset,
      isDefault: offset === 0,
    };
  });
...
function hotelMiuWeekError(message, code = 'INVALID_HOTEL_MIU_WEEK') {
...
/**
 * 대차수/세부차수 직접입력을 ERP 숫자 차수로 정규화한다.
 * A/B는 입고 항공편 표시일 뿐 별도 주문 차수가 아니므로 제거한다.
 */
export function normalizeHotelMiuWeekSelection({
  year,
  major,
  detail = '',
  allowedMajors,
  ...
  if (!/^\d{4}$/.test(requestedYear)) {
    throw hotelMiuWeekError('차수 연도는 4자리로 선택해야 합니다.', 'INVALID_HOTEL_MIU_YEAR');
  }

  const majorRaw = String(major ?? '').trim().replace(/\s+/g, '').toUpperCase().replace(/차$/, '');
  const detailRaw = String(detail ?? '').trim().replace(/\s+/g, '').toUpperCase().replace(/차$/, '');
  const combined = majorRaw.includes('-') ? majorRaw : `${majorRaw}-${detailRaw}`;
  const match = combined.match(/^(?:(\d{4})-)?(\d{1,2})-(\d{1,2})([AB])?$/);
  if (!match) {
    throw hotelMiuWeekError('차수는 36과 2처럼 나누어 입력하거나 36-2, 36-1A처럼 입력하세요.');
  }
  const embeddedYear = match[1] || '';
  if (embeddedYear && embeddedYear !== requestedYear) {
    throw hotelMiuWeekError(`입력한 연도(${embeddedYear})와 선택 연도(${requestedYear})가 다릅니다.`, 'HOTEL_MIU_YEAR_MISMATCH');
  }
  const majorNumber = Number(match[2]);
  const detailNumber = Number(match[3]);
  if (!Number.isInteger(majorNumber) || majorNumber < 1 || majorNumber > 52) {
    throw hotelMiuWeekError('대차수는 1부터 52까지만 입력할 수 있습니다.');
  }
  if (!HOTEL_MIU_SUBWEEK_NUMBERS.includes(detailNumber)) {
    throw hotelMiuWeekError('세부차수는 1, 2, 3, 4만 입력할 수 있습니다.');
  }

  if (Array.isArray(allowedMajors) && allowedMajors.length) {
    const allowed = allowedMajors.some((item) => (
      String(item?.year) === requestedYear && Number(item?.major) === majorNumber
    ));
    if (!allowed) {
      throw hotelMiuWeekError('대차수는 화면에 표시된 기본 차수부터 +9차수 안에서 선택하세요.', 'HOTEL_MIU_WEEK_OUT_OF_RANGE');
    }
  }

  return {
    year: requestedYear,
    week: `${String(majorNumber).padStart(2, '0')}-${String(detailNumber).padStart(2, '0')}`,
    major: majorNumber,
    detail: detailNumber,
    flightSuffix: match[4] || '',
  };
}

/** 서버 조회·저장용: 연도를 추정하지 않고 정확한 숫자 차수로 정규화한다. */
export function requireHotelMiuWeekScope(year, week) {
  return normalizeHotelMiuWeekSelection({ year, major: week });
}
...
export function registeredHistoryView({ batches = [], snaps = [], productsByKey = {} } = {}) {
  ...
  const rows = summary.map((row) => {
    ...
    return { ...row, unit, byRound, beforeLabel, afterLabel, variety: historyVarietyLabel(product, row.prodName || prev?.prodName) };
  });
  return { batchNos, rows };
}

const VARIETY_NAME_PREFIX = /^(ROSE|CARNATION|HYDRANGEA|ORCHID|PEONY|LILY|TULIP|RANUNCULUS|ANEMONE|수국|장미|카네이션|호접란|작약|백합|튤립)/i;

export function historyVarietyLabel(product = {}, prodName = '') {
  const flower = String(product.FlowerName || product.flowerName || '').trim();
  if (flower) return flower;
  const countryFlower = String(product.CountryFlower || product.countryFlower || '').trim();
  if (countryFlower) return countryFlower;
  const name = String(prodName || product.ProdName || product.prodName || product.DisplayName || '').trim();
  const m = name.match(VARIETY_NAME_PREFIX);
  if (m) return m[1];
  const slash = name.split('/')[0].trim();
  if (slash && slash.length < 40 && slash !== name) return slash;
  return '기타';
}

export function attachVarietyToRows(rows = [], productsByKey = {}) {
  return (rows || []).map((row) => ({
    ...row,
    variety: historyVarietyLabel(productOf(productsByKey, row.prodKey), row.prodName),
  }));
}

export function historyVarietyButtons(rows = []) {
  const counts = new Map();
  for (const row of rows || []) {
    const v = row.variety || historyVarietyLabel({}, row.prodName);
...
export function toggleVarietyFilter(selected = [], variety) {
  const set = new Set((selected || []).filter(Boolean));
  const key = String(variety || '');
  if (!key) return [...set];
...
export function filterRowsByVarieties(rows = [], selected = []) {
  if (!selected?.length) return rows || [];
  const set = new Set(selected);
  return (rows || []).filter((row) => set.has(row.variety));
}

export function groupRowsByVariety(rows = []) {
  const order = [];
  const by = new Map();
  for (const row of rows || []) {
    const v = row.variety || '기타';
    if (!by.has(v)) {
      by.set(v, []);
      order.push(v);
    }
  ...
  return order.map((variety) => ({ variety, rows: by.get(variety) }));
}

export const HOTEL_MIU_VARIETY_FILTER_KEY = 'hotel-miu-variety-filter';

export function readVarietyFilter(custKey, storage = globalThis.localStorage) {
  try {
    if (!storage || !custKey) return [];
    const all = JSON.parse(storage.getItem(HOTEL_MIU_VARIETY_FILTER_KEY) || '{}');
    const v = all[String(custKey)];
...
export function writeVarietyFilter(custKey, selected = [], storage = globalThis.localStorage) {
  if (!storage || !custKey) return;
  let all = {};
  try { all = JSON.parse(storage.getItem(HOTEL_MIU_VARIETY_FILTER_KEY) || '{}'); } catch { all = {}; }
  all[String(custKey)] = (selected || []).map(String).filter(Boolean);
  storage.setItem(HOTEL_MIU_VARIETY_FILTER_KEY, JSON.stringify(all));
}
