...
import { ARRIVAL_DRIVE_COUNTRIES, selectArrivalDriveCandidates, scopeArrivalDriveRows, arrivalDriveTiming, resolveArrivalDriveSelection } from './arrivalDrivePolicy.js';
...
export async function runArrivalDriveAuto({ selection = null, user = null } = {}) {
  if (selection && !user?.userId) throw new Error('즉시 처리 담당자 확인 필요');
  if (global._arrivalDriveRunning) {
    if (selection) throw new Error('다른 원가 처리 중입니다. 상태를 새로고침한 뒤 다시 확인하세요');
    return;
  }
  global._arrivalDriveRunning = true;
  try {
    const config = arrivalDriveConfig();
    if (!config.enabled && !selection) return;
    const available = selectArrivalDriveCandidates(listVisible(actor), config);
    const candidates = selection ? [resolveArrivalDriveSelection(available, config, selection)] : available;
    for (const source of candidates) {
      const old = read(stateFile(source), {});
      if (old.status === 'complete') continue;
      if (!selection && !arrivalDriveTiming(source, old).ready) continue;
      const currentConfig = arrivalDriveConfig();
      if (!currentConfig.enabled || JSON.stringify(currentConfig) !== JSON.stringify(config)) break;
      const requestedBy = selection ? user.userId : actor.userId;
      const state = { filename: source.filename, week: source.week, country: source.country, at: new Date().toISOString(), status: 'processing', mode: selection ? 'manual-now' : 'automatic', requestedBy };
      ...
      try {
        ...
        if (!latest || ['id', 'sha', 'year', 'week', 'country'].some(k => latest[k] !== source[k]) || latest.reason) throw new Error('검증 중 최신 원본이 변경되었습니다');
        if (JSON.stringify(arrivalDriveConfig()) !== JSON.stringify(config)) throw new Error('검증 중 자동반영 설정이 변경되었습니다');
        if (selection) resolveArrivalDriveSelection([latest], config, selection);
        else if (!arrivalDriveTiming(latest).ready) continue;
        const saved = await createArrivalCostImport({ parsed, fileName: source.filename, user: selection ? user : actor, orderYear: source.year,
          driveSource: selection ? { ...source, executionMode: 'manual-now', requestedBy, requestedAt: state.at } : source });
