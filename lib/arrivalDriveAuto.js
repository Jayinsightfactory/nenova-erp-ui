...
import { ARRIVAL_DRIVE_COUNTRIES, selectArrivalDriveCandidates, scopeArrivalDriveRows } from './arrivalDrivePolicy.js';
...
export function arrivalDriveStatus() {
  ...
  const results = candidates.map(c => ({ ...c, ...read(stateFile(c), {}), reason: c.reason }));
...
export async function runArrivalDriveAuto() {
  ...
  try {
    ...
    for (const source of candidates) {
      ...
      if (old.status === 'complete' || old.status === 'review' || source.reason) continue;
      if (old.status === 'error' && Date.now() - Date.parse(old.at) < 300000) continue;
      ...
      try {
        ...
        if (!arrivalDriveConfig().enabled) break;
...
export function startArrivalDriveScheduler() {
  if (global._arrivalDriveTimer || process.env.NODE_ENV !== 'production' || !process.env.DB_SERVER) return;
  global._arrivalDriveTimer = setInterval(() => runArrivalDriveAuto().catch(e => console.warn('[arrival-drive]', e.message)), 60000);
  global._arrivalDriveTimer.unref?.();
}
