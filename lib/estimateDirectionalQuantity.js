...
export function evaluateDirectionalAvailability({ facts = {}, increase = 0, scope = {} } = {}) {
  ...
  if (hasInsufficientShipmentStock(availability.remainAfter)) {
    const error = directionalQuantityError(
      'STOCK_SHORTAGE',
      `품목 ${scope.prodKey} 가용재고=${availability.available}, 증가=${normalizedIncrease}, 반영후 잔량=${availability.remainAfter}입니다.`,
    );
    error.stockValidation = { availability: [result], postNative: [] };
    throw error;
  }
  return result;
}
