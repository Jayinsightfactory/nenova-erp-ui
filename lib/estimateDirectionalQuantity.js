...
/** Product.Stock already contains existing confirmed shipments; subtract only the new increase. */
export function evaluateDirectionalCurrentStock({ currentStock, increase = 0, scope = {} } = {}) {
  if (!Number.isFinite(Number(currentStock))) {
    throw directionalQuantityError('PRODUCT_STOCK_INVALID', '현재고를 잠그지 못해 저장하지 않았습니다.', 500);
  }
  const available = normalizeShipmentQty(currentStock);
  const normalizedIncrease = normalizeShipmentQty(increase);
  const remain = normalizeShipmentQty(available - normalizedIncrease);
  const result = { ...scope, available, increase: normalizedIncrease, remain, source: 'Product.Stock' };
  if (hasInsufficientShipmentStock(remain)) {
    const productLabel = formatDirectionalProductLabel(scope);
    const error = directionalQuantityError(
      'STOCK_SHORTAGE',
      `품목 ${productLabel} · 현재 가용재고=${available}, 증가=${normalizedIncrease}, 반영후 잔량=${remain}입니다.`,
    );
    error.stockValidation = { availability: [{ ...result, productLabel }], postNative: [] };
    throw error;
  }
...
export function futureStockShortageError({ scope = {}, negativeRow = {} } = {}) {
  const productLabel = formatDirectionalProductLabel(scope);
  const orderYear = String(negativeRow.OrderYear || scope.orderYear || '').trim();
  const orderWeek = String(negativeRow.OrderWeek || '').trim();
  const stock = normalizeShipmentQty(negativeRow.Stock);
  const weekLabel = [orderYear ? `${orderYear}년` : '', orderWeek ? `${orderWeek}차` : ''].filter(Boolean).join(' ');
  const error = directionalQuantityError(
    'FUTURE_STOCK_SHORTAGE',
    `품목 ${productLabel} · 재계산 후 ${weekLabel || '현재/후속 차수'} 재고=${stock}입니다. 전체 변경을 저장하지 않았습니다.`,
  );
  error.stockValidation = {
    availability: [],
    postNative: [{ ...scope, productLabel, orderYear, orderWeek, stock, negative: true }],
  };
  return error;
}
...
function isMssqlLockTimeout(error) {
  const numbers = [
    error?.number,
    error?.info?.number,
    ...
    error?.cause?.number,
    error?.cause?.info?.number,
    error?.cause?.originalError?.number,
    error?.cause?.originalError?.info?.number,
  ];
  return numbers.some(number => Number(number) === 1222);
}

function preWriteGateBusyError(message, cause) {
  const error = directionalQuantityError('STOCK_GATE_BUSY', message);
  error.preWriteGateBusy = true;
  if (cause) error.cause = cause;
  return error;
}

export function isRetryablePreWriteGateBusy(error) {
  return error?.code === 'STOCK_GATE_BUSY' && error.preWriteGateBusy === true;
}

export function preWriteStockGateAcknowledgement(error) {
  return isRetryablePreWriteGateBusy(error) ? { retryable: true, saved: false } : {};
}
...
export async function lockDirectionalGate(tQ) {
  ...
  try {
    ...
    if (isMssqlLockTimeout(cause)) {
      throw preWriteGateBusyError('재고 계산이 진행 중입니다. 잠시 후 다시 저장하세요.', cause);
    }
    throw cause;
  }
  if (Number(result?.recordset?.length || 0) !== 1) {
    throw preWriteGateBusyError('재고 계산 대기 작업이 있어 수량 변경을 저장하지 않았습니다.');
  }
}
