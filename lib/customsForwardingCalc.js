...
export function colombiaRatioMode(gw, cw) {
  ...
  if (g === 0 && c === 0) return { useWeight: true, label: 'GW/CW 없음 → 무게' };
  if (useWeight) {
    if (c > 0 && Math.abs(c - g) <= 0.01) return { useWeight: true, label: `CW ${c} = GW ${g} → 무게` };
    if (c === 0 || g === 0) return { useWeight: true, label: `GW ${g} / CW ${c} → 무게` };
    return { useWeight: true, label: `GW ${g} ≥ CW ${c} → 무게` };
  }
  return { useWeight: false, label: `CW ${c} > GW ${g} → CBM` };
}
