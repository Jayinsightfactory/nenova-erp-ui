...
/**
 * OutQuantity=0 유령 Detail + 연결 ShipmentDate/ShipmentFarm 삭제.
 * 농장행은 Detail FK를 막지 않기 위해 같이 지운다. ShipmentHistory 는 감사용으로 유지.
 * OrderDetail 은 건드리지 않는다.
 */
export async function purgeZeroOutShipmentDetail(tQ, sdetailKey, sql) {
  ...
  await tQ(`DELETE FROM ShipmentFarm WHERE SdetailKey=@dk`, { dk: { type: sql.Int, value: dk } });
