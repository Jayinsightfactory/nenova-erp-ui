...
// 기존 주문에 없던 붙여넣기 품목도 현재 분배 조회에서 빠지면 안 된다.
// 특히 CANCEL 미리보기는 이 조회 결과를 기준으로 실행 가능 여부를 표시한다.
export function pasteShipmentLookupProdKeys(order = {}, registeredOrder = {}) {
  return [...new Set([
    ...(order?.items || []).filter(item => !item?.skip).map(item => Number(item?.prodKey)),
    ...(registeredOrder?.items || []).map(item => Number(item?.prodKey)),
