...
async function build() {
  ...
  try {
    const r = await query(
      `WITH ActiveYear AS (
         SELECT MAX(TRY_CONVERT(INT, OrderYear)) AS OrderYear
           FROM OrderMaster
          WHERE ISNULL(isDeleted,0)=0 AND TRY_CONVERT(INT, OrderYear) IS NOT NULL
       )
       SELECT DISTINCT TOP 3 CAST(ay.OrderYear AS NVARCHAR(4)) AS OrderYear,
              LEFT(om.OrderWeek, 2) AS Major
         FROM OrderMaster om
         CROSS JOIN ActiveYear ay
        WHERE ISNULL(om.isDeleted,0)=0 AND TRY_CONVERT(INT, om.OrderYear)=ay.OrderYear
         ORDER BY Major DESC`
    );
    results.recentOrderYear = String(r.recordset[0]?.OrderYear || '');
    results.recentMajorWeeks = r.recordset.map(x => x.Major);
  } catch { results.recentOrderYear = ''; results.recentMajorWeeks = []; }

  const activeOrderYear = /^\d{4}$/.test(results.recentOrderYear) ? results.recentOrderYear : null;
  const yearScope = (alias) => activeOrderYear ? `AND ${alias}.OrderYear = '${activeOrderYear}'` : 'AND 1=0';
    ...
    const r = await query(
      `SELECT TOP 10 c.CustName,
        ...
        WHERE ISNULL(om.isDeleted,0)=0 AND ISNULL(od.isDeleted,0)=0
          AND om.OrderWeek LIKE '${latestMajor}-%' ${yearScope('om')}
        GROUP BY c.CustName
  ...
  try {
    ...
    const r = await query(
      `SELECT p.CounName,
         ...
         JOIN Product p ON od.ProdKey = p.ProdKey
        WHERE ISNULL(om.isDeleted,0)=0 AND ISNULL(od.isDeleted,0)=0
          AND om.OrderWeek LIKE '${latestMajor}-%' ${yearScope('om')} AND p.CounName IS NOT NULL
        GROUP BY p.CounName
        ORDER BY TotalQty DESC`
    );
  ...
  try {
    const r = await query(
      `SELECT TOP 5 om.OrderWeek,
         ...
         JOIN OrderDetail od ON od.OrderMasterKey = om.OrderMasterKey
         JOIN Product p ON od.ProdKey = p.ProdKey
        WHERE ISNULL(om.isDeleted,0)=0 AND ISNULL(od.isDeleted,0)=0 ${yearScope('om')}
    ...
    const r = await query(
      `SELECT
        ...
        WHERE ISNULL(sm.isDeleted,0)=0
          AND sm.OrderWeek LIKE '${latestMajor}-%' ${yearScope('sm')}`
    );
  ...
  try {
    const r = await query(
      `SELECT TOP 15 c.CustName, sm.OrderWeek,
         ...
         JOIN ShipmentDetail sd ON sd.ShipmentKey = sm.ShipmentKey
         JOIN Customer c ON sm.CustKey = c.CustKey
        WHERE ISNULL(sm.isDeleted,0)=0 AND sm.isFix=0 ${yearScope('sm')}
