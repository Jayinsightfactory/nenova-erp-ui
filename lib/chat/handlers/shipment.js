...
async function runCustomerShipmentItems(cust, week) {
  const rows = await query(
    `SELECT
        ...
        COUNT(*) AS RowCnt,
        MIN(sd.ShipmentDtm) AS FirstShipDtm,
        MAX(sd.ShipmentDtm) AS LastShipDtm,
  ...
  if (!list.length) {
    return { messages: [{ type: 'text', content: `${shipmentUnderstanding({ cust, week, mode: '출고 품목수량', rowCount: 0 })}\n📭 ${cust.CustName} ${week}차 출고 품목수량이 없습니다.` }] };
  }
  ...
  return {
    messages: [
      {
        type: 'text',
        content: `${shipmentUnderstanding({ cust, week, mode: '출고 품목수량', rowCount: list.length })}\n${week}차 ${cust.CustName} 출고 품목별 수량을 확인했습니다. ${isFix ? '확정 출고 기준' : '미확정/분배 기준'}이며 총 ${list.length}품목, 수량 합계 ${totalQty.toLocaleString('ko-KR')}입니다.`,
      },
      {
        type: 'card',
        card: {
          title: `${cust.CustName} · ${week} 출고 품목수량`,
          subtitle: isFix ? '확정 포함' : '미확정/분배 기준',
          rows: list.slice(0, 80).map(r => ({
            label: `${r.CounName || ''} ${r.FlowerName || ''} ${r.DisplayName || r.ProdName}`.trim(),
            value: `${Number(r.OutQty || 0).toLocaleString('ko-KR')} ${r.OutUnit || ''} · ${week}차 · ${fmtDate(r.FirstShipDtm)}${fmtDate(r.LastShipDtm) && fmtDate(r.LastShipDtm) !== fmtDate(r.FirstShipDtm) ? `~${fmtDate(r.LastShipDtm)}` : ''}`,
          })),
...
export async function handleShipmentLookup(text, user, payload = null) {
  ...
  const wantsItemQty = /(품목\s*수량|품목수량|품목별|수량|상세|내역)/.test(text) || payload?.mode === 'items';
  ...
  if (cust && week) {
    if (wantsItemQty) {
      return await runCustomerShipmentItems(cust, week);
    }
