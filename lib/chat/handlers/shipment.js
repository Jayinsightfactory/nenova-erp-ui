...
async function runCustomerShipmentItems(cust, week, payload = null) {
  const params = {
  ...
  if (!list.length) {
    return { messages: [{ type: 'text', content: `${shipmentUnderstanding({ cust, week, mode: targetName ? `출고 품목수량 (${targetName})` : '출고 품목수량', rowCount: 0 })}\n📭 ${cust.CustName} ${week}차 ${targetName ? `${targetName} ` : ''}출고 품목수량이 없습니다.` }] };
  }
  ...
  return {
    messages: [
      {
        type: 'text',
        content: `${shipmentUnderstanding({ cust, week, mode: targetName ? `출고 품목수량 (${targetName})` : '출고 품목수량', rowCount: list.length })}\n${week}차 ${cust.CustName} ${targetName ? `${targetName} ` : ''}출고 품목별 수량을 확인했습니다. ${isFix ? '확정 출고 기준' : '미확정/분배 기준'}이며 총 ${list.length}품목, 수량 합계 ${totalQty.toLocaleString('ko-KR')}입니다.`,
      },
      {
        type: 'card',
        card: {
          ...
          rows: list.slice(0, 80).map(r => ({
            ...
            value: `${Number(r.OutQty || 0).toLocaleString('ko-KR')} ${r.OutUnit || ''} · ${week}차`,
          })),
...
export async function handleShipmentLookup(text, user, payload = null) {
  ...
  if (cust && week) {
    if (wantsItemQty) {
      return await runCustomerShipmentItems(cust, week, payload);
    }
