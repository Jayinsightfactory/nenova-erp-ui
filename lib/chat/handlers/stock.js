  ...
  const r = await query(
    `SELECT TOP 1 p.ProdKey, p.ProdName, p.DisplayName, p.FlowerName, p.CounName, p.OutUnit
      ...
      WHERE ISNULL(p.isDeleted,0)=0
        AND ${where}
      ORDER BY
        CASE WHEN p.ProdName LIKE @t${tokens.length - 1} OR ISNULL(p.DisplayName,'') LIKE @t${tokens.length - 1} THEN 0 ELSE 1 END,
...
async function resolveWeekTarget(text, payload) {
  ...
  const flower = payloadProd || mappedProd || tokenProd ? null : (payloadFlower || await findFlowerInText(text));
...
async function runWeekIncomingFarmLookup(text, week, payload = null) {
  ...
  if (!rows.length) {
    return await buildStockTargetClarification(
      ...
      week,
      'incomingFarm',
      `${understoodStockText({ week, mode: 'incomingFarm', prod, flower, rowCount: 0 })}\n입고 농장 데이터가 바로 잡히지 않았습니다.`
    );
  }
  const total = rows.reduce((s, x) => s + Number(x.InQty || 0), 0);
  return {
    messages: [
      { type: 'text', content: `${understoodStockText({ week, mode: 'incomingFarm', prod, flower, rowCount: rows.length })}\n${week}차 ${prod?.ProdName || flower || '전체'} 입고 농장/수량입니다. 총 ${fmt(total)}입니다.` },
      {
        type: 'card',
        card: {
          title: `${week}차 입고 농장`,
...
async function runWeekStockStatusLookup(text, week, payload = null) {
  ...
  if (!rows.length) {
    return await buildStockTargetClarification(
      ...
      week,
      'weekStockStatus',
      `${understoodStockText({ week, mode: 'weekStockStatus', prod, flower, rowCount: 0 })}\n재고현황 데이터가 바로 잡히지 않았습니다.`
    );
  }
  ...
  return {
    messages: [
      {
        type: 'text',
        content: `${understoodStockText({ week, mode: 'weekStockStatus', prod, flower, rowCount: rows.length })}\n${week}차 ${titleName} 재고현황입니다. ${fixedRows.length === rows.length ? '확정 잔량 기준' : '계산 잔량 기준'}으로 ${fmt(summaryRemain)}입니다. 계산식은 전재고 ${fmt(totalPrev)} + 입고/조정 ${fmt(totalIn)} - 출고분배 ${fmt(totalOut)}입니다.`,
      },
      {
        type: 'card',
        card: {
