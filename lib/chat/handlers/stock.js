...
async function resolveWeekTarget(text, payload) {
  ...
  const flower = payloadProd || mappedProd ? null : (payloadFlower || await findFlowerInText(text));
  const prod = payloadProd || mappedProd || (flower ? null : await findProduct(text));
  return { prod, flower };
}
