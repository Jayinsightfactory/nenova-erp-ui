...
async function runWeekIncomingFarmLookup(text, week) {
  const flower = await findFlowerInText(text);
  const prod = flower ? null : await findProduct(text);
  ...
  if (!rows.length) {
    return { messages: [{ type: 'text', content: `${week}차 ${prod?.ProdName || flower || ''} 입고 농장 데이터가 없습니다.` }] };
  }
...
async function runWeekStockStatusLookup(text, week) {
  const flower = await findFlowerInText(text);
  const prod = flower ? null : await findProduct(text);
  if (!prod && !flower) {
    return { messages: [{ type: 'text', content: `어떤 품목/꽃종류의 ${week}차 재고현황을 볼까요? 예: "20-1차 카네이션 재고현황"` }] };
  }
  ...
  const rows = r.recordset || [];
  if (!rows.length) {
    return { messages: [{ type: 'text', content: `${week}차 ${prod?.ProdName || flower} 재고현황 데이터가 없습니다.` }] };
  }
...
export async function handleStockLookup(text, user, payload = null) {
  ...
  if (week && /(입고\s*농장|입고농장|농장|입고.*수량|수량.*입고)/.test(text)) {
    return await runWeekIncomingFarmLookup(text, week);
  }
  if (week && /(재고\s*현황|재고현황|잔량|재고)/.test(text)) {
    return await runWeekStockStatusLookup(text, week);
  }
