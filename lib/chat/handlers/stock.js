...
function understoodStockText({ week, mode, prod, flower, country, rowCount, extraPath = '' }) {
  ...
  const pathLine = extraPath || (
    mode === 'incomingFarm'
      ...
      : '검색 경로: 차수 정규화 → 품목/꽃종류 매칭 → 전재고 + 입고 - 출고분배 계산.'
  );
  return [
...
async function runWeekStockStatusLookup(text, week, payload = null) {
  ...
  const r = await query(
    `SELECT
        ...
        0 AS AdjustQty,
        ISNULL(wh.inQty, 0) AS InQty,
        ISNULL(ship.outQty, 0) AS OutQty,
        ISNULL(prev.prevStock, 0) + ISNULL(wh.inQty, 0) - ISNULL(ship.outQty, 0) AS CalcRemain,
      ...
      WHERE ISNULL(p.isDeleted,0)=0
        ...
        AND (
          ...
          OR ISNULL(ship.outQty,0) <> 0 OR fix.Stock IS NOT NULL
        )
      ...
      messages: [
        {
          ...
          content: `${understoodStockText({
            ...
            extraPath: '검색 경로: 전차수 ProductStock → 현차수 입고 → 현차수 출고분배 → 계산잔량 산출.',
