...
function shouldExcludeZero(text, payload = null) {
  return !!payload?.hideZero || /(?:^|\s)(?:0|영|제로)\s*(?:제외|빼고|말고)|0\s*(?:아닌|이\s*아닌)|(?:잔량|재고|수량)\s*(?:있는|남은)\s*(?:것|거)?만/.test(String(text || ''));
}

function stockRemainValue(row) {
  return row?.FixedRemain !== null && row?.FixedRemain !== undefined
    ? Number(row.FixedRemain || 0)
    : Number(row?.CalcRemain || 0);
}
...
async function runWeekStockStatusLookup(text, week, payload = null) {
  ...
  const fixedRows = visibleRows.filter(x => x.FixedRemain !== null && x.FixedRemain !== undefined);
      ...
      messages: [
        {
          ...
          content: `${understoodStockText({
            ...
            extraPath: '검색 경로: 차수피벗 재고 계산 → 품목/꽃종류 매칭 → 잔량만 표시.',
    ...
    messages: [
      {
        ...
        content: `${understoodStockText({
          ...
          extraPath: '검색 경로: 차수피벗 재고 계산 → 품목/꽃종류 매칭 → 잔량만 표시.',
        })}\n${week}차 ${titleName} 잔량은 ${fmt(totalRemain)}${unitText}입니다.${hideZero ? ' 0 잔량은 제외했습니다.' : ''}`,
      },
      {
        type: 'card',
        card: {
          title: `${week}차 ${titleName} 잔량`,
          subtitle: fixedRows.length ? `잔량만 표시 · 확정 ${fixedRows.length}/${visibleRows.length}건` : '잔량만 표시 · 계산값',
          rows: visibleRows.slice(0, 80).map(x => ({
            label: `${x.CounName || ''} ${x.FlowerName || ''} ${x.ProdName}`.trim(),
            value: `${fmt(stockRemainValue(x))} ${x.OutUnit || ''}`,
          })),
