...
async function runFinalLookup(cust, week, mode = 'byItem') {
  const rows = await query(
    `SELECT om.OrderWeek, p.ProdName, p.OutUnit,
            (ISNULL(od.BoxQuantity,0)+ISNULL(od.BunchQuantity,0)+ISNULL(od.SteamQuantity,0)) AS OrderQuantity
       FROM OrderMaster om
...
async function runOriginLookup(country, weekStr, majorWk, mode = 'total') {
  ...
  const rows = await query(
    `SELECT p.ProdName, p.CounName, p.FlowerName, p.OutUnit,
            SUM((ISNULL(od.BoxQuantity,0)+ISNULL(od.BunchQuantity,0)+ISNULL(od.SteamQuantity,0)))    AS Qty,
      ...
      ORDER BY SUM((ISNULL(od.BoxQuantity,0)+ISNULL(od.BunchQuantity,0)+ISNULL(od.SteamQuantity,0))) DESC`,
    params
  );
