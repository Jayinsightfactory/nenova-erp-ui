...
function inferOrderLookupMode(text) {
  const t = String(text || '').replace(/\s+/g, '');
  if (!t) return null;
  if (/(품목별|품목상세|상세|내역|목록|리스트)/.test(t)) return 'byItem';
  if (/(합계만|합계|총수량|총주문|총량|총)/.test(t)) return 'total';
  return null;
}
...
export async function handleOrderLookup(text, user, payload = null) {
  ...
  const mode     = payload?.mode || inferOrderLookupMode(text) || null;
      ...
      messages: [
        {
          type: 'text',
          content: [
            `✅ ${cust.CustName} · ${finalWeek}차 주문을 찾았습니다.`,
            '이렇게 보여드릴 수 있어요.',
            '- 품목별 상세: 품목명과 주문수량을 하나씩 표시',
            '- 합계만: 박스/단/송이 같은 단위별 총 주문수량만 표시',
            `예: "${cust.CustName} ${finalWeek} 주문 품목별"`,
            `예: "${cust.CustName} ${finalWeek} 주문 합계"`,
          ].join('\n'),
        },
        {
          type: 'actions',
...
async function runFinalLookup(cust, week, mode = 'byItem') {
  const rows = await query(
    // OrderDetail 에는 Box/Bunch/Steam 3개 컬럼이 모두 환산돼 저장됨.
    // 실제 주문 단위는 Product.OutUnit 이므로 해당 컬럼만 노출해야 정확.
    `SELECT om.OrderWeek, p.ProdName, p.OutUnit,
            CASE
              WHEN p.OutUnit IN (N'박스', 'BOX', 'Box', 'box') THEN ISNULL(od.BoxQuantity, 0)
              WHEN p.OutUnit IN (N'단',   'BUNCH', 'Bunch', 'bunch') THEN ISNULL(od.BunchQuantity, 0)
              WHEN p.OutUnit IN (N'송이', 'STEAM', 'Steam', 'steam', 'STEM', 'Stem') THEN ISNULL(od.SteamQuantity, 0)
              ELSE ISNULL(od.BoxQuantity, 0)
            END AS OrderQuantity
       FROM OrderMaster om
       JOIN OrderDetail od ON od.OrderMasterKey = om.OrderMasterKey
       JOIN Product p ON p.ProdKey = od.ProdKey
      WHERE om.CustKey=@ck AND ISNULL(om.isDeleted,0)=0 AND ISNULL(od.isDeleted,0)=0
        AND om.OrderWeek = @wk
      ORDER BY p.ProdName`,
    {
      ck: { type: sql.Int, value: cust.CustKey },
  ...
  return {
    messages: [
      { type: 'text', content: `📋 ${cust.CustName} · ${week}차 주문 (${rows.recordset.length}품목)` },
      {
        ...
        card: {
          title: `${cust.CustName} · ${week}`,
          // 전체 품목 표시 (truncate 제거 — 모바일에서도 스크롤로 볼 수 있게)
          rows: rows.recordset.map(r => ({
            label: r.ProdName,
            value: `${r.OrderQuantity} ${r.OutUnit || ''}`,
          })),
          footer: `총 ${rows.recordset.length}품목 · 합계 ${totalQty.toLocaleString()}`,
        },
      },
    ],
  };
...
async function runOriginLookup(country, weekStr, majorWk, mode = 'total') {
  ...
  if (finalWeek) {
    ...
    const wks = await query(
      `SELECT DISTINCT om.OrderWeek
         ...
         JOIN OrderDetail od ON od.OrderMasterKey = om.OrderMasterKey
         JOIN Product p      ON p.ProdKey  = od.ProdKey
        WHERE ISNULL(om.isDeleted,0)=0 AND ISNULL(od.isDeleted,0)=0 AND p.CounName=@co
  ...
  const rows = await query(
    // Box/Bunch/Steam 은 환산 저장이므로 OutUnit 기준 한 컬럼만 SUM.
    `SELECT p.ProdName, p.CounName, p.FlowerName, p.OutUnit,
       ...
       JOIN OrderDetail od ON od.OrderMasterKey = om.OrderMasterKey
       JOIN Product p      ON p.ProdKey  = od.ProdKey
      WHERE ISNULL(om.isDeleted,0)=0 AND ISNULL(od.isDeleted,0)=0 AND p.CounName=@co ${weekFilterClause}
