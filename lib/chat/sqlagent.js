...
// ── 2차 프롬프트: 결과 → 답변 포매팅
async function askFormatAnswer(client, model, userMessage, sql, rows, purpose, userId = null) {
...
AREAS: ${cat.areas.join(', ')}`;
  ...
  // ── 2차 시도: 답변 포매팅
  const formatted = await askFormatAnswer(client, MODEL_FAST, userMessage, finalSql, rows, finalPurpose, userId);
