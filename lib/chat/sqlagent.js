...
import { getPreferencePrompt } from './preferences';
...
// ── 1차 프롬프트: SQL 생성
async function askGenerateSQL(client, model, userMessage, { schemaText, catalogHint, codeHint = '', historyHint = '', previousError, userId = null }) {
...
// ── 2차 프롬프트: 결과 → 답변 포매팅
async function askFormatAnswer(client, model, userMessage, sql, rows, purpose, userId = null, preferenceHint = '') {
...
- 단, 사용자 질문이나 ANSWER PREFERENCES 가 0 제외를 요구하면 수량 0인 행만 제외할 수 있다.
- 숫자 값에 단위 꼭 포함 (원, 단, 송이, BOX, 건 등).
- ANSWER PREFERENCES 가 있으면 최종 답변의 정렬/묶음/표현 형식에 우선 반영한다.
- 금지: JSON 외 설명. 마크다운.
...
async function askClarification(client, userMessage, lastError, historyHint, context = {}) {
  const system = `너는 꽃 도매 ERP 챗봇이다. 방금 사용자 질문에 답을 찾지 못했다.
"이해하지 못했다" 라고 끝내지 말고, 사용자에게 **구체적인 역질문** 을 해서 의도를 명확히 해라.
너는 답을 찾아가는 과정을 사용자에게 짧게 알려야 한다. 즉, 네가 어떤 의도로 이해했고 어떤 조건으로 먼저 확인했는지 말한 뒤, 부족한 조건만 되묻는다.

반드시 JSON 으로만 출력:
{
  "text": "한국어 2~3문장. 1) 내가 이해한 의도 2) 먼저 확인한 조건/실패 이유 3) 다음에 필요한 확인 질문.",
...
내가 먼저 이해한 의도: ${context.purpose || '-'}
먼저 실행한 조회 SQL: ${context.sql || '-'}
조회 결과 행 수: ${context.rowCount ?? '-'}
...
AREAS: ${cat.areas.join(', ')}`;
    ...
    preferenceHint = await getPreferencePrompt(userId).catch(() => '');
    // API 사용량 기반 관심사 힌트
  ...
  let attempt = await askGenerateSQL(client, MODEL_FAST, userMessage, {
    schemaText, catalogHint, codeHint, historyHint, userId,
  });
  let lastError = null;
  let rows = null;
  let finalSql = null;
    ...
    try {
      ...
      if (i === 0) {
        attempt = await askGenerateSQL(client, MODEL_STRONG, userMessage, {
          schemaText, catalogHint, codeHint, historyHint, userId, previousError: lastError,
        });
        continue;
      }
      attempt = null;
  ...
  if (rows.length === 0) {
    const askBack = await askClarification(
      client,
      userMessage,
      'DB 조회는 성공했지만 해당 조건의 결과가 0건입니다.',
      historyHint,
      { purpose: finalPurpose, sql: finalSql, rowCount: rows.length }
    ).catch(() => null);
    if (askBack) {
      askBack._debug = { sql: finalSql, rowCount: rows.length, purpose: finalPurpose };
      return askBack;
    }
  }

  // ── 2차 시도: 답변 포매팅
  const formatted = await askFormatAnswer(client, MODEL_FAST, userMessage, finalSql, rows, finalPurpose, userId, preferenceHint);
  ...
  const processText = `찾아본 과정: "${finalPurpose || '질문'}"으로 이해하고 DB를 조회해서 ${rows.length}건을 확인했습니다.`;
  const msgs = [
    { type: 'text', content: processText },
    { type: 'text', content: formatted.text || `🔎 ${finalPurpose}` },
  ];
