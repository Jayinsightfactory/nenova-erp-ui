...
// ── 1차 프롬프트: SQL 생성
async function askGenerateSQL(client, model, userMessage, { schemaText, catalogHint, previousError }) {
...
${catalogHint}`;
  ...
  try {
    const resp = await client.messages.create(
      { model, max_tokens: 800, system, messages },
...
async function askFormatAnswer(client, model, userMessage, sql, rows, purpose) {
  const rowsSlice = rows.slice(0, 30);
  const preview = JSON.stringify(rowsSlice, null, 0).slice(0, 3000);
...
export async function handleSqlAgent(userMessage) {
...
AREAS: ${cat.areas.join(', ')}`;
  ...
  // ── 1차 시도: haiku
  let attempt = await askGenerateSQL(client, MODEL_FAST, userMessage, { schemaText, catalogHint });
  let lastError = null;
  let rows = null;
  let finalSql = null;
    ...
    try {
      ...
      if (i === 0) {
        attempt = await askGenerateSQL(client, MODEL_STRONG, userMessage, {
          schemaText, catalogHint, previousError: lastError,
        });
        continue;
      }
      attempt = null;
