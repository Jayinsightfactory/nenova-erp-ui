...
function choiceForCustomer(intent, cust, week) {
  ...
  if (intent === 'shipment') {
    return {
      label: `${weekText}${cust.CustName} 출고 품목수량`,
      sub: cust.CustArea || '',
      text: `${weekText}${cust.CustName} 출고 품목수량`,
      payload: { intent: 'shipment', mode: 'items', custKey: cust.CustKey, ...(week ? { week } : {}) },
    };
  }
...
export async function buildInvestigativeClarification(text, intent = 'unknown') {
  const requestedWeek = extractWeekHint(text);
  const [weeks, customers, products] = await Promise.all([
    requestedWeek ? Promise.resolve([]) : getWeekCandidates(5),
    searchCustomers(text, 5),
    searchProducts(text, 6),
  ]);
  const week = requestedWeek || weeks[0] || null;
  const choices = [];
  ...
  const found = [
    requestedWeek ? `차수: ${requestedWeek}` : (weeks.length ? `차수 후보: ${weeks.slice(0, 3).join(', ')}` : null),
