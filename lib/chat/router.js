...
import { handleKakaoSummaryLookup } from './handlers/kakaoSummary';
...
import { buildPreferenceAck, detectPreferenceFeedback, rememberPreference } from './preferences';
...
function wantsGroupedRows(text) {
  return /(묶어서|그룹|분류|정리|나눠서|품목\s*수량|품목별\s*수량|국가.*꽃|꽃.*국가)/.test(String(text || ''));
}
...
function rowGroupTitle(row) {
  const label = String(row?.label || '').trim();
  if (!label) return '';
  if (!String(row?.value || '').trim()) return label;
...
function rowItemLabel(row) {
  ...
  return group ? label.slice(group.length).trim() || label : label;
}
...
function groupCardRows(rows) {
  ...
  for (const row of rows || []) {
    const explicitGroup = !String(row?.value || '').trim();
    const group = explicitGroup ? String(row.label || '').trim() : rowGroupTitle(row);
    if (group && group !== lastGroup) {
      result.push({ label: group, value: '' });
      lastGroup = group;
    }
    if (!explicitGroup) {
      result.push({ ...row, label: rowItemLabel(row) });
    }
  }
  return result;
}
...
function filterPreviousCardRows(text, histories) {
  const zeroFilter = isZeroFilterFollowup(text);
  const groupRows = wantsGroupedRows(text);
  if (!zeroFilter && !groupRows) return null;
  ...
  for (const history of sources) {
    ...
    for (let i = history.length - 1; i >= 0; i -= 1) {
      ...
      let rows = source.rows.filter(row => {
        ...
        return !zeroFilter || (qty === null ? true : qty !== 0);
      });
      if (groupRows) rows = groupCardRows(rows);
      const conditionText = [
        zeroFilter ? '수량/잔량이 0인 항목 제외' : null,
        groupRows ? '국가/꽃종류별 묶음 정리' : null,
      ].filter(Boolean).join(' + ');
      return {
        messages: [
          {
            type: 'text',
            content: rows.length
              ? `제가 이해한 조건: 직전 조회 결과에서 ${conditionText}.\n검색 경로: 이전 챗봇 카드 결과 → 행별 수량 판독 → 재정렬.\n${rows.length}행입니다.`
              : `제가 이해한 조건: 직전 조회 결과에서 ${conditionText}.\n검색 경로: 이전 챗봇 카드 결과 → 행별 수량 판독 → 재정렬.\n표시할 항목이 없습니다.`,
          },
          {
            type: 'card',
            card: {
              ...source,
              title: `${source.title || '이전 조회 결과'} - 재정리`,
...
function classify(text) {
  ...
  // 입고 농장/농장별 수량 질문은 거래처명이 아니라 품목 입고 흐름으로 본다.
  if (/농장/.test(text) && !/(거래처|업체|고객|주문처)/.test(text)) {
    return { intent: 'stock' };
  }
  // 재고
...
function isPreferenceOnlyMessage(text) {
  const t = String(text || '');
  const correction = /(나와야|원하는\s*값|원하는\s*건|이런\s*식|이렇게|답변|정리\s*방식|고쳐|바꿔|아니|말고|제외후|제외\s*후)/.test(t);
  if (!correction) return false;

  const hasLookupAnchor = Boolean(extractWeek(t) || extractPeriod(t));
  const hasLookupCommand = /(조회|알려|보여|찾아|확인|정리해|몇|얼마|수량\s*(?:은|이|좀)?\??$)/.test(t);
  return !hasLookupAnchor || !hasLookupCommand;
}
...
export async function routeIntent(text, user, payload = null, options = {}) {
  ...
  const initialIntent = payload?.intent || classify(text).intent;

  if (!payload) {
    const preference = detectPreferenceFeedback(text);
    if (preference) {
      const saved = await rememberPreference(user, preference.rule, {
        scope: 'global',
        source: text,
      }).catch(e => {
        console.error('[chat.preferences.save]', e.message);
        return { saved: false, rule: preference.rule };
      });
      if (isPreferenceOnlyMessage(text)) {
        return buildPreferenceAck(saved.rule || preference.rule);
      }
    }
  }

  if (!payload && /(수입방|카톡|카카오).*(수량|합계|품목)|(?:수량|합계|품목).*(수입방|카톡|카카오)/.test(text)) {
    return await handleKakaoSummaryLookup(text, user);
  }
  const simpleShipmentQty =
    !payload &&
    initialIntent === 'shipment' &&
    extractWeek(text) &&
    /(출고\s*수량|출고수량|출고\s*물량|출고물량|분배\s*수량|분배수량|분배\s*물량|분배물량|출고\s*분배|출고분배|분배)/.test(text) &&
    !/(상위|하위|TOP|top|비율|분석|비교|증감|전년|전월|합계만|몇\s*개|리스트|전체)/.test(text);

  let simpleOrderLookup = false;
  if (
    !payload &&
    initialIntent === 'order' &&
    extractWeekDetail(text) &&
    /(품목\s*별|품목별|상세|내역|목록|리스트|합계만|합계|총\s*수량|총주문|총\s*주문)/.test(text)
  ) {
    const matchedCustomer = await findCustomer(text).catch(e => {
      console.error('[chat.router.simpleOrderLookup]', e.message);
      return null;
    });
    simpleOrderLookup = Boolean(matchedCustomer);
  }
  ...
  if (!payload && !simpleShipmentQty && !simpleOrderLookup && isComplexQuery(text)) {
