...
function filterPreviousCardRows(text, clientHistory) {
  if (!isZeroFilterFollowup(text) || !Array.isArray(clientHistory)) return null;
  for (let i = clientHistory.length - 1; i >= 0; i -= 1) {
    const msg = clientHistory[i];
      ...
      messages: [
        {
          type: 'text',
          content: rows.length
            ? `직전 조회 결과에서 0인 항목을 제외했습니다. ${rows.length}건입니다.`
            : '직전 조회 결과에서 0을 제외하면 표시할 항목이 없습니다.',
        },
        {
          type: 'card',
          card: {
...
export async function routeIntent(text, user, payload = null, options = {}) {
  ...
  if (!payload) {
    const filtered = filterPreviousCardRows(text, clientHistory);
  ...
  const hasDeicticFollowup = history.length > 0 && (
