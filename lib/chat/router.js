...
// ── 메인 라우터
export async function routeIntent(text, user) {
  const { intent } = classify(text);

  switch (intent) {
    case 'help':                   return await handleHelp(text, user);
    case 'order':                  return await handleOrderLookup(text, user);
    case 'stock':                  return await handleStockLookup(text, user);
    case 'shipment':               return await handleShipmentLookup(text, user);
    case 'sales':                  return await handleSalesLookup(text, user);
    case 'receivable':             return await handleReceivableLookup(text, user);
