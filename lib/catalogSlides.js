...
export function perPageSlotCount(perPage) {
  return catalogSlotCount(perPage);
}
...
export function collectPlacedLineIds(slides) {
  const ids = new Set();
  for (const sl of slides || []) {
    for (const id of sl.slots || []) {
      if (id) ids.add(id);
...
export function updateComposerSlide(slides, slideId, patch) {
  return (slides || []).map(sl => (sl.id === slideId ? { ...sl, ...patch } : sl));
}
...
function padSlots(slots, chunk) {
  const next = (slots || []).slice(0, chunk);
  while (next.length < chunk) next.push(null);
  return next;
}

function pageFromSlots({ titleBig, titleSmall, slots }) {
...
/** 슬라이드에 배치되지 않은 품목을 빈 칸·추가 슬라이드에 채움 */
function appendUnplacedLines(pages, unplaced, perPage) {
  if (!unplaced.length) return pages;
  const chunk = perPageSlotCount(perPage);
  ...
  for (const page of next) {
    while (queue.length) {
      const idx = page.slots.findIndex(s => !s);
      if (idx < 0) break;
      page.slots[idx] = queue.shift();
    }
    page.lines = page.slots.filter(Boolean);
  }

  while (queue.length) {
    const slots = Array(chunk).fill(null);
    for (let i = 0; i < chunk && queue.length; i += 1) {
      slots[i] = queue.shift();
    }
    const first = slots.find(Boolean);
    next.push(pageFromSlots({
...
/** composerSlides 우선, 없으면 lines 자동 페이징 — 슬롯 위치 보존 */
export function resolveCatalogPages({
  ...
  if (composerSlides?.length) {
    ...
    let pages = composerSlides
      .map(sl => pageFromSlots({
        titleBig: sl.titleBig,
        titleSmall: sl.titleSmall,
        slots: padSlots(
          (sl.slots || []).map(id => (id ? byId.get(id) || null : null)),
          chunk,
        ),
      }))
      .filter(p => p.slots.some(Boolean));

    const placed = new Set();
    pages.forEach(p => p.slots.forEach(l => { if (l) placed.add(l.id); }));
    const unplaced = (lines || []).filter(l => !placed.has(l.id));
    pages = appendUnplacedLines(pages, unplaced, perPage);
    return pages;
  }
  const sorted = sortLinesImageFirst(lines, imagesByProd);
  return buildCatalogExportPages(sorted, { perPage }).map(p => {
    const slots = padSlots(p.lines, chunk);
    return pageFromSlots({
      titleBig: p.titleBig,
      titleSmall: p.titleSmall,
      slots,
    });
  });
}
