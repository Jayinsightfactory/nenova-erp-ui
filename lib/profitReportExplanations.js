...
export const PROFIT_REPORT_EXPLANATIONS = [
  ...
  { key: 'E', label: '기초상품재고액', source: '같은 연도 직전 대차수(01차만 전년도 52차)의 마지막 EXE ProductStock와 동일 시점 VERIFIED 단가', formula: 'E = 전차수 마지막 ProductStock 환산수량 × 시점 단가 근거', confirmed: 'ProductStock 존재 + 숫자 세부차수 최신순; StockMaster.isFix는 재고 마감 조건으로 사용하지 않음', manual: '최종값 직접입력 불가' },
  { key: 'F', label: '기말상품재고액', source: '현재 대차수의 마지막 EXE ProductStock.Stock과 동일 OrderYear+OrderWeek의 VERIFIED 단가', formula: 'F = 마지막 ProductStock 환산수량 × 시점 단가 근거', confirmed: 'ProductStock 존재 + 숫자 세부차수 최신순; 최근원가·Product.Cost fallback 금지', manual: '최종값 직접입력 불가; 품목 단가 근거만 등록' },
