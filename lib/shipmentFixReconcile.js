...
export async function loadWeekOutboundProdKeys(q, orderWeek) {
  const result = await q(
    `SELECT DISTINCT sd.ProdKey
      ...
      ORDER BY sd.ProdKey`,
    { wk: { type: 'NVarChar', value: orderWeek } },
  );
...
export async function reconcileWeekAfterScopedOperation({
  ...
  const allWeekProdKeys = await loadWeekOutboundProdKeys(q, orderWeek);
