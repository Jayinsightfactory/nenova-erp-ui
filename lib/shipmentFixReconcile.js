...
/**
 * 확정 취소 후 스냅샷 재계산이 빠지면 전산 usp_ShipmentFix 가
 * 현재 ProductStock(이미 출고를 뺀 기말)에서 미확정 출고를 한 번 더 뺀다.
 * 주간 잔량(전차수+입고+조정-출고)은 부족하지 않은데 전산 검사만 큰 음수면 stale.
 */
export function isDoubleCountStaleSnapshot({ productStock = 0, unfixedOut = 0, weekRemain = 0 } = {}) {
  const exeRemain = Math.round(Number(productStock || 0) - Number(unfixedOut || 0));
