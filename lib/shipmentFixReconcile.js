 ...
 * - usp_StockCalculation 은 ProductStock·StockMaster 스냅샷을 갱신
 ...
 * StockMaster.isFix는 nenova.exe FormStockView가 읽거나 마감 판정에 쓰지 않는
 * 레거시/웹 마커다. 출고 확정 정합성을 이 값으로 차단하지 않는다.
...
export function deriveExeAlignedStatus(input = {}) {
  ...
  if (shipmentStatus === 'NO_SHIPMENT') {
    ...
    exeAligned = negativeLiveCount === 0
...
/**
 * SQL Server ROUND(n, 0): half away from zero.
 * JS Math.round(-0.5) === 0 이라 음수 잔량 판정에 쓰지 않는다.
 */
export function sqlRound0(n) {
  const x = Number(n) || 0;
  return x < 0 ? -Math.round(Math.abs(x)) : Math.round(x);
}

/**
 * usp_StockCalculation leftover − 이번 미확정 출고.
 * 2026-08-20 이후 usp_ShipmentFix 잔량 검사와 같다.
 * OrderYearWeek 결합키의 직전 StockMaster 만 이월로 쓴다(OrderWeek 단독 금지).
 */
export function shipmentFixWeekRemain({
  prevStock = 0,
  warehouseQty = 0,
  confirmedOut = 0,
  stockTypeAdj = 0,
  ...
  return Number(prevStock || 0)
    + Number(warehouseQty || 0)
    - Number(confirmedOut || 0)
    + Number(stockTypeAdj || 0)
    - Number(unfixedOut || 0);
}

export function isShipmentFixRemainNegative(input = {}) {
  return sqlRound0(shipmentFixWeekRemain(input)) < 0;
}

/**
 * 확정 취소 후 스냅샷 재계산이 빠지면, 패치 전 usp_ShipmentFix 가
 * 현재 ProductStock(이미 출고를 뺀 기말)에서 미확정 출고를 한 번 더 뺀다.
 * 주간 잔량은 부족하지 않은데 옛 검사만 큰 음수면 stale.
 * 2026-08-20 SP 패치 이후 EXE 확정 버튼은 이 경로로 막히지 않는다.
 */
export function isDoubleCountStaleSnapshot({ productStock = 0, unfixedOut = 0, weekRemain = 0 } = {}) {
  const exeRemain = sqlRound0(Number(productStock || 0) - Number(unfixedOut || 0));
  const week = Number(weekRemain || 0);
  return exeRemain < 0 && week >= -0.49;
}
...
export async function loadWeekOutboundProdKeys(q, orderYear, orderWeek, sqlTypes) {
    ...
    `SELECT COUNT(DISTINCT p.ProdKey) AS negativeLiveCount
      ...
      WHERE p.isDeleted=0 AND ISNULL(p.Stock,0) < 0
        AND EXISTS (
          ...
          WHERE sd.ProdKey=p.ProdKey AND sm.OrderYear=@yr AND sm.OrderWeek=@wk AND sm.isDeleted=0
            AND ISNULL(sd.OutQuantity,0)>0
        )`,
    params,
  );
...
export async function runStockCalculationSequential({
  ...
  // 2026-07-14: 품목이 많으면 exe 방식(ProdKey=0, 전 품목 단일 호출)으로 — nenova.exe 는
  // 분배/재고조정/주문추가/패킹리스트 전부 uspStockCalculation(yr, wk, 0) 한 번만 부른다
  // (디컴파일 확인). 품목별 순차 호출은 호출마다 이후 차수 전체를 cascade 재계산해서
  // 168품목 ≈ 3분+ 걸리던 것이 단일 set 연산 1회로 줄어든다. 결과의 prodKey=0 은
  // "전 품목 재계산됨" 마커 — reconcile 이 이를 보고 중복 재계산을 건너뛴다.
  if (uniqueKeys.length > 5) {
    try {
      ...
      const r = await q(
        ...
        {
          yr:  { type: sqlTypes.NVarChar, value: orderYear },
          wk:  { type: sqlTypes.NVarChar, value: orderWeek },
      ...
      if (Number(row.result || 0) === 0) {
        ...
        if (logFix) await logFix(`${logPrefix}_all_done`, `${orderYear}/${orderWeek} ${logLabel} 전품목 OK`);
        return { results, errors, total };
      }
      errors.push({ prodKey: 0, code: row.result, message: row.message || 'unknown' });
      if (logFix) await logFix(`${logPrefix}_all_error`, `${orderYear}/${orderWeek} ${logLabel} ${row.message || ''}`, true);
      return { results, errors, total };
    } catch (e) {
      errors.push({ prodKey: 0, code: -1, message: e.message });
      if (logFix) await logFix(`${logPrefix}_all_error`, `${orderYear}/${orderWeek} ${logLabel} ${e.message}`, true);
      return { results, errors, total };
    }
  }
...
export async function reconcileWeekAfterScopedOperation({
  ...
  const allWeekProdKeys = await loadWeekOutboundProdKeys(q, orderYear, orderWeek, sqlTypes);
  // prodKey=0 = 직전 단계에서 이미 전 품목 단일호출(exe 방식)로 재계산됨 → 중복 재계산 생략
  const calculatedAll = (alreadyCalculatedProdKeys || []).map(Number).includes(0);
  const keysToCalc = calculatedAll
    ? []
    : forceFullWeekRecalc
      ? allWeekProdKeys
