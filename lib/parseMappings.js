...
const FILE = path.join(process.cwd(), 'data', 'order-mappings.runtime.json');
const SEED_FILE = path.join(process.cwd(), 'data', 'order-mappings.json');
...
export function loadMappings(forceRefresh = false) {
  ...
  try {
    ensureDir();
    const source = fs.existsSync(FILE) ? FILE : SEED_FILE;
    if (fs.existsSync(source)) {
      _cache = JSON.parse(fs.readFileSync(source, 'utf8'));
    } else {
      _cache = {};
    }
  } catch (error) {
    _cache = null;
    throw new Error(`품목 매칭 저장소 읽기 실패: ${error.message}`);
  }
  return _cache;
}
...
const CONTEXT_TOKENS = new Set([
  '콜롬비아','colombia','중국','china','네덜란드','netherlands','에콰도르','ecuador','호주','australia','태국','thailand',
  '장미','rose','카네이션','carnation','수국','hydrangea','알스트로','alstro','alstromeria','alstroemeria',
  '루스커스','ruscus','박스','단','송이','개',
]);

const COUNTRY_PATTERNS = [
  ['colombia', /콜롬비아|colombia|(?:^|\s)콜(?:\s|$)/i],
  ['china', /중국|china/i],
  ['netherlands', /네덜란드|netherlands/i],
  ['ecuador', /에콰도르|ecuador/i],
  ['australia', /호주|australia/i],
  ['thailand', /태국|thailand/i],
];

function countryGroupsIn(text) {
  const s = ` ${String(text || '').toLowerCase().replace(/\s+/g, ' ')} `;
  const groups = new Set();
  for (const [group, pattern] of COUNTRY_PATTERNS) {
    if (pattern.test(s)) groups.add(group);
  }
  return groups;
}

function hasCountryConflictForFuzzy(inputName, key, value) {
  const keyCountries = countryGroupsIn(key);
  if (keyCountries.size <= 1) return false;

  const inputCountries = countryGroupsIn(inputName);
  const valueCountries = countryGroupsIn(value?.counName || '');
  if (valueCountries.size === 0) return false;

  for (const c of keyCountries) {
    if (!valueCountries.has(c) && !inputCountries.has(c)) return true;
  }
  return false;
}
...
function tokenize(s) {
  return String(s || '')
    ...
    .replace(/\d+\s*(박스|단|송이|개|대|ea|box|bunch|stem[s]?|steam|st|cm|kg|ml|팩|봉|병)/gi, ' ')
...
function tokenMatchesAny(inputToken, keyTokens) {
  if (keyTokens.some(k => tokensMatch(k, inputToken))) return true;
  const compactKey = compactTokens(keyTokens);
  // "유카리체리" ↔ "유카리 체리", "헤르메스오렌지" ↔ "헤르메스 오렌지"
  return inputToken.length >= 4 && compactKey.includes(inputToken);
}

function scoreCandidate(inputTokens, keyTokens) {
  const coreInputTokens = inputTokens.filter(t => !CONTEXT_TOKENS.has(t));
  const effectiveInputTokens = coreInputTokens.length ? coreInputTokens : inputTokens;
  const coreKeyTokens = keyTokens.filter(t => !CONTEXT_TOKENS.has(t));
  const effectiveKeyTokens = coreKeyTokens.length ? coreKeyTokens : keyTokens;
  const inputCompact = compactTokens(effectiveInputTokens);
  const keyCompact = compactTokens(effectiveKeyTokens);
  if (inputCompact && keyCompact === inputCompact) return 0.95;

  const matched = effectiveInputTokens.filter(t => tokenMatchesAny(t, effectiveKeyTokens)).length;
  if (!matched) return 0;
  const matchRatio = matched / effectiveInputTokens.length;
  const extraPenalty = Math.max(0, effectiveKeyTokens.length - matched) * 0.08;
  return Math.max(0.35, Math.min(0.95, matchRatio - extraPenalty));
}
...
export function findMappingFuzzy(inputName, mappings) {
  ...
  if (inTokens.every(t => CONTEXT_TOKENS.has(t))) return null;

  // 후보 수집: 입력 모든 토큰이 키에 매치되는 키들
  const candidates = [];
  for (const [key, value] of Object.entries(mappings)) {
    if (hasCountryConflictForFuzzy(inputName, key, value)) continue;
    const keyTokens = tokenize(key);
    if (keyTokens.length === 0) continue;
    const allIn = inTokens.every(t => tokenMatchesAny(t, keyTokens));
    if (allIn) {
      candidates.push({ key, value, keyLen: keyTokens.length, score: scoreCandidate(inTokens, keyTokens) });
    }
  }
  ...
  if (candidates.length === 1) {
    ...
    return { key: c.key, value: c.value, score: c.score || (inTokens.length / Math.max(c.keyLen, inTokens.length)), matchType: 'fuzzy' };
  }
  ...
  if (allProdKeys.size === 1) {
    candidates.sort((a, b) => (b.score || 0) - (a.score || 0) || Math.abs(a.keyLen - inTokens.length) - Math.abs(b.keyLen - inTokens.length));
    const c = candidates[0];
    return { key: c.key, value: c.value, score: c.score || (inTokens.length / Math.max(c.keyLen, inTokens.length)), matchType: 'fuzzy' };
  }
  ...
  if (colCandidates.length > 0) {
    ...
    colCandidates.sort((a, b) => (b.score || 0) - (a.score || 0) || Math.abs(a.keyLen - inTokens.length) - Math.abs(b.keyLen - inTokens.length));
    const c = colCandidates[0];
    return {
      key: c.key, value: c.value,
      score: c.score || (inTokens.length / Math.max(c.keyLen, inTokens.length)),
  ...
  return { key: top.key, value: top.value, score: top.score || (inTokens.length / Math.max(top.keyLen, inTokens.length)), matchType: 'fuzzy' };
}
...
// 매핑 삭제 — 저장된 키(정규화 토큰) 또는 입력 토큰 모두 허용
export function deleteMapping(keyOrToken) {
  const cache = { ...loadMappings(true) };
  let key = keyOrToken;
  if (!(key in cache)) {
    const norm = normalizeToken(keyOrToken);
...
export function normalizeToken(t) {
  return (t || '')
    ...
    .replace(/\d+(\.\d+)?\s*(박스|단|송이|개|대|box|bunch|stems|stem|steam|st|cm|ea)?/gi, ' ')
    .replace(/[|:：,\-→>]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
