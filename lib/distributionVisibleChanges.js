...
function visibleChanges(message, selectedWeek) {
  ...
  const result = [];
  let week = '', customer = '', context = '', action = '', titleAction = '';
  for (const line of lines) {
    ...
    if (scope) {
      ...
      titleAction=(context.match(/추가|취소/)||[])[0]||'';
      customer = ''; action = titleAction;
      continue;
    }
    const majorTitle=line.match(/^(?:(\d{4})[-년\s]+)?(\d{1,2})차\s+(.+)$/);
    if(majorTitle) {
      context=majorTitle[3].replace(/변경사항|변경 요청/g,'').trim();
      titleAction=(context.match(/추가|취소/)||[])[0]||'';
      customer='';action=titleAction;
      continue;
    }
    if (/^(추가|취소|삭제)$/.test(line)) { action = line; continue; }
    const hasQuantity = /\d+(?:\.\d+)?\s*(?:박스|단|송이|개|대|box|bunch|stem)|(?:^|\s)[+-]\s*\d+(?:\.\d+)?(?=\s|$)/i.test(line);
    if(result.length&&/^(?:->|→)?\s*총\s*\d/.test(line)&&!/추가|취소/.test(line))continue;
    if (hasQuantity) {
      const inline = line.match(/^(.+?)\s*[:：]\s*(.+)$/)||line.match(/^(.+?)\s+-\s+(?!\d)(.+)$/);
    ...
    customer = line.replace(/^[-•·]\s*/, ''); action=titleAction;
  }
