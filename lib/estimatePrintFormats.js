...
export const ESTIMATE_PRINT_FORMAT = {
  ESTIMATE: 'estimate',
  STATEMENT: 'statement',
  /** 부가세 미분류 견적서 — 분배단가를 공급가액으로 보고 부가세는 공급가의 10% */
  SUPPLY: 'supply',
};
...
export function isSupplyPrintFormat(printFormat) {
  return printFormat === ESTIMATE_PRINT_FORMAT.SUPPLY;
}

/** 견적서 양식(기존·부가세 미분류). 거래명세표가 아니면 true */
export function isEstimateSheetPrintFormat(printFormat) {
  return !isStatementPrintFormat(printFormat);
}
...
/**
 * 부가세 미분류 견적서
 * 저장 분배단가(Cost)는 부가세 포함값이지만, 이 양식에서는 그 숫자를 공급가 단가로 본다.
 * 공급가액 = 단가 × 수량, 부가세 = 공급가액 × 10%. DB Amount/Vat 는 쓰지 않는다.
 */
export function applySupplyPrintAmounts(row) {
  const qty = Number(row?.Quantity) || 0;
  const unitPrice = Number(row?.Cost) || 0;
  const amount = Math.round(unitPrice * qty);
  const vat = Math.round(amount * STATEMENT_VAT_RATE);
  return {
    ...row,
...
export function computePrintPreviewTotals(items, printFormat) {
  const list = items || [];
  if (isSupplyPrintFormat(printFormat)) {
    let supply = 0;
    let vat = 0;
    for (const item of list) {
      const row = applySupplyPrintAmounts(item);
...
export function getPrintFormatDocTitle(printFormat) {
  if (isStatementPrintFormat(printFormat)) return '거래명세표';
  if (isSupplyPrintFormat(printFormat)) return '견적서(부가세 미분류)';
  return '견적서';
}

export function getPrintFormatBigoSuffix(printFormat) {
  if (isStatementPrintFormat(printFormat)) return '종합거래명세표';
  if (isSupplyPrintFormat(printFormat)) return '부가세미분류 종합견적서';
  return '종합견적서';
}
