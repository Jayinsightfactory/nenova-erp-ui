...
export async function getPivotStatsRange({
  ...
  // 도착원가만 물량표 차수와 다른 차수 기준으로 불러올 때 사용 (read-only 조회 오버라이드).
  // 지정하지 않으면 기존과 동일하게 weekStart/weekEnd/orderYear를 그대로 쓴다.
  arrivalWeekStart,
  arrivalWeekEnd,
  arrivalOrderYear,
}) {
  ...
  try {
    arrivalMap = await getArrivalCostsForWeekRange({
      weekStart: arrivalWeekStart || weekStart,
      weekEnd: arrivalWeekEnd || arrivalWeekStart || weekEnd,
      orderYear: arrivalOrderYear || orderYear,
    });
  } catch (_) {
...
export async function getPivotStats({ weekStart, weekEnd, orderYear, arrivalWeekStart, arrivalWeekEnd }) {
  const weekStartNorm = normalizeOrderWeek(weekStart);
  const wEnd = normalizeOrderWeek(weekEnd || weekStart);
  const startYear = resolveActiveOrderYear(weekStart, orderYear);

  // 도착원가 별도차수 오버라이드 — 지정 시 물량표 차수(weekStart~weekEnd)와 무관하게
  // 이 차수 하나(또는 범위) 기준으로 모든 byWeek 버킷의 도착원가를 통일해서 가져온다.
  const arrival = arrivalWeekStart ? {
    weekStart: normalizeOrderWeek(arrivalWeekStart),
    weekEnd: normalizeOrderWeek(arrivalWeekEnd || arrivalWeekStart),
    orderYear: resolveActiveOrderYear(arrivalWeekStart, '', startYear),
  } : null;
  const arrivalOverride = arrival ? {
    arrivalWeekStart: arrival.weekStart,
    arrivalWeekEnd: arrival.weekEnd,
    arrivalOrderYear: arrival.orderYear,
  } : {};
  const arrivalWeekLabel = arrival
    ? `${arrival.orderYear}-${arrival.weekStart}${arrival.weekEnd !== arrival.weekStart ? `~${arrival.weekEnd}` : ''}`
    : null;

  const weeks = listOrderWeeksInRange(weekStartNorm, wEnd, startYear);
  const aggregated = await getPivotStatsRange({
    weekStart: weekStartNorm, weekEnd: wEnd, orderYear: startYear, ...arrivalOverride,
  });

  if (weeks.length <= 1) {
    return { ...aggregated, weeks, arrivalWeek: arrivalWeekLabel };
  }
  ...
  return { ...aggregated, weeks, byWeek, arrivalWeek: arrivalWeekLabel };
}
