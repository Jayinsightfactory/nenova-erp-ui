...
export async function saveManualCost({ year, week, deductionKey, cost, expectedRowVersionNo, expectedEstimateUnit, custKey, prodKey, user } = {}, transaction = withTransaction) {
  ...
  return transaction(async (q) => {
    ...
    if (text(expectedEstimateUnit, 30) !== estimateUnit) throw new Error('품목의 견적 단위가 변경되었습니다. 조회 후 단가를 다시 확인하세요.');
