...
export async function loadYearStockFlow(orderYear) {
  ...
  const arrivals = toMap(arrivalsQ.recordset);
  const shipments = toMap(shipmentsQ.recordset);
  const snapshots = toMap(snapshotsQ.recordset);
  ...
  // 품명/카테고리 붙이기 (+무게 placeholder·운송료성 품목 제외)
  const keys = [...new Set(flagged.map((f) => f.prodKey))];
  ...
  return flagged
    ...
    .sort((a, b) => Number(a.major) - Number(b.major) || a.impliedEnd - b.impliedEnd);
}
