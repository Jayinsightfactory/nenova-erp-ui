...
export async function exportCatalogPpt({
  ...
  const autoTxtH = estimateCatalogAutoTxtHcm(placedLines, fields, perPage, spacing, { cols: gridCols });
  const layout = computeCatalogLayout(perPage, spacing, { txtHcm: autoTxtH, cols: gridCols });
  ...
  for (const page of pages) {
    ...
    for (let i = 0; i < chunk; i += 1) {
      ...
      if (renderLine.imageUrl) {
        try {
          const data = await renderCatalogImageDataUri(absCatalogUrl(renderLine.imageUrl), renderLine);
          if (data) {
            slide.addImage({ data, x: ix, y: iy, w: isz, h: isz });
          }
        } catch { /* skip */ }
      }
