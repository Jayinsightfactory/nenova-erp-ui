  ...
  for (const page of pages) {
    ...
    for (let i = 0; i < chunk; i += 1) {
      ...
      if (renderLine.imageUrl) {
        try {
          const data = await renderCatalogImageDataUri(absCatalogUrl(renderLine.imageUrl), renderLine, 1200, cell.imgWcm / cell.imgHcm);
          if (data) {
      ...
      if (runs.length) {
        slide.addText(runs, { x: tx, y: ty, w: tw, h: th, margin: 0, paraSpaceAfter: 2.8, lineSpacingMultiple: 1.2, align: 'center', valign: 'top' });
      }
    }
  }
