...
import { renderCatalogImageDataUri } from './catalogImagePosition';
...
export async function exportCatalogPpt({
  ...
  for (const page of pages) {
    ...
    for (let i = 0; i < page.lines.length; i += 1) {
      ...
      if (line.imageUrl) {
        try {
          const data = await renderCatalogImageDataUri(absCatalogUrl(line.imageUrl), line)
            || await urlToDataUri(line.imageUrl);
          if (data) {
