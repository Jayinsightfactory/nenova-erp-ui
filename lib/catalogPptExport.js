...
import { absCatalogUrl, catalogLineNames } from './catalogUtils';
...
export async function exportCatalogPpt({
  ...
  const pages = buildCatalogExportPages(lines, { perPage });
  ...
  for (const page of pages) {
    ...
    for (let i = 0; i < page.lines.length; i += 1) {
      ...
      if (kor) {
        runs.push({
          text: kor,
          options: { fontSize: 14, bold: true, fontFace: 'Malgun Gothic', color: '222222' },
        });
      }
      if (!runs.length) {
        runs.push({
