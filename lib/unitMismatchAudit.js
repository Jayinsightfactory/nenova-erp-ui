...
/**
 * 단→박스 10배 저장(STORED_BUNCH_AS_BOX) 행의 보정 값 계산 — 순수 함수(DB 무의존).
 * 보정 원칙: OutUnit=박스 품목에서 OutQuantity 가 단(묶음)수로 들어가 BunchOf1Box 배 과대 →
 *   correctedOutQty = OutQuantity / BunchOf1Box 로 되돌리고, 나머지 환산필드는
 *   canonical distributeUnits(correctedOutQty, product) 로 재계산(13/14차 단일 공식 유지).
 * 주문(OrderDetail/orderBox)을 baseline 으로 한 detectStoredBunchAsBox 가 잡은 경우에만 보정.
 * @returns {object|null} { isMismatch, b1b, ratio, from, to } 또는 보정 대상 아님이면 null
 */
export function computeBunchAsBoxRepair(row, product = {}) {
  const warning = detectStoredBunchAsBox(row, product);
  if (!warning) return null;

  const b1b = asNumber(product.BunchOf1Box ?? row.BunchOf1Box);
  if (!(b1b > 1)) return null;

  const fromOutQty = asNumber(row.OutQuantity ?? row.outQty ?? row.uploadQty);
  if (!(fromOutQty > 0)) return null;

  // 박스 기준 보정값. 정수 박스가 정상이므로 반올림(부동소수 잔차 제거).
  const correctedBox = Math.round((fromOutQty / b1b) * 1e6) / 1e6;
  const units = distributeUnits(correctedBox, product);

  const from = {
    outQty: fromOutQty,
    boxQty: asNumber(row.BoxQuantity ?? row.boxQty),
    bunchQty: asNumber(row.BunchQuantity ?? row.bunchQty),
    steamQty: asNumber(row.SteamQuantity ?? row.steamQty),
    estQty: asNumber(row.EstQuantity ?? row.estQty),
  };
  const to = {
    outQty: units.outQty,
    boxQty: units.box,
    bunchQty: units.bunch,
    steamQty: units.steam,
    estQty: units.estQty,
  };
  ...
  return {
    isMismatch: true,
    b1b,
    ratio: to.outQty > 0 ? fromOutQty / to.outQty : b1b,
    from,
    to,
  };
}
