...
function buildExtractionPrompt(context) {
  ...
  return [
    ...
    'The tool input must be exactly {"requests":[{"sourceIdentity":"incoming identity","quote":"exact one-line raw substring","action":"ADD|CANCEL|SET","customerText":"raw text or null","productText":"raw text or null","qty":"finite number or null","unit":"박스|단|송이|null","week":"explicit WW-SS or null","shipmentDate":"explicit YYYY-MM-DD or null"}],"unresolved":[{"sourceIdentity":"incoming identity","quote":"exact one-line raw substring","reason":"required explanation"}]}. Every listed request and unresolved field is required; use null only where the schema says null is allowed.',
    ...
    'Return an explicit unresolved row with a required non-empty reason for every non-empty line not fully represented by a request or unresolved quote. The server also labels uncovered lines, but that safeguard does not guarantee semantic completeness inside an otherwise covered line.',
