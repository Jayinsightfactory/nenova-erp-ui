...
export function dutchPriceKey(entry) {
  if (isDutchIndividualPriceCustomer(entry?.customer)) return String(entry?.id || '');
...
export function dutchEntryPrice(entry, prices = {}) {
  return Number(prices?.[dutchPriceKey(entry)] || 0);
}
...
export function priceProgress(entries, prices) {
  const completed = (entries || []).filter(entry => dutchEntryPrice(entry, prices) > 0).length;
...
export function buildDutchPriceRows(entries, prices, currency = 'EUR') {
  return (entries || []).map(entry => {
    const unitPrice = dutchEntryPrice(entry, prices);
...
export function addDutchPriceColumns(XLSX, workbook, entries, prices, currency = 'EUR') {
  ...
  const priceByCell = new Map((entries || []).map(entry => [entry.id, dutchEntryPrice(entry, prices)]));
