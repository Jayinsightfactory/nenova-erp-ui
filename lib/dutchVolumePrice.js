...
export function dutchQuantityPriceNumberFormat(unitPrice, currency = 'EUR') {
  const price = Number(unitPrice || 0);
  if (!(price > 0)) return '';
  const safeCurrency = String(currency || 'EUR').replace(/"/g, '');
  return `#,##0.###"\n@ ${formatPrice(price)} ${safeCurrency}"`;
}

/**
 * Pivot 통계 물량표 원본의 열·행·병합·색상·수식 구조를 그대로 보존한다.
 * 단가는 새 열을 만들지 않고 수량 셀의 숫자 표시형식 두 번째 줄에 겹쳐 표시한다.
...
export function addDutchPriceColumns(XLSX, workbook, entries, prices, currency = 'EUR') {
  ...
  for (const sheetName of result.SheetNames) {
    ...
    for (const [id, price] of priceByCell) {
      ...
      const numberFormat = dutchQuantityPriceNumberFormat(price, currency);
      cell.z = numberFormat;
      cell.s = { ...(cell.s || {}), numFmt: numberFormat, alignment: { ...(cell.s?.alignment || {}), horizontal: 'center', vertical: 'center', wrapText: true } };
      const rowIndex = XLSX.utils.decode_cell(address).r;
      if (!target['!rows']) target['!rows'] = [];
      target['!rows'][rowIndex] = { ...(target['!rows'][rowIndex] || {}), hpt: Math.max(Number(target['!rows'][rowIndex]?.hpt || 0), 26) };
    }
