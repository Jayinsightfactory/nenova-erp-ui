...
export function dutchQuantityPriceNumberFormat(unitPrice) {
  ...
  if (!(price > 0)) return '';
  return `#,##0.###"   @ ${formatPrice(price)}"`;
}

/**
 * Pivot 통계 물량표 원본의 열·행·병합·색상·수식 구조를 그대로 보존한다.
 * 단가는 새 열을 만들지 않고 수량 셀의 숫자 오른쪽에 작은 보조값처럼 표시한다.
...
export function addDutchPriceColumns(XLSX, workbook, entries, prices, currency = 'EUR') {
  ...
  for (const sheetName of result.SheetNames) {
    ...
    for (const [id, price] of priceByCell) {
      ...
      const numberFormat = dutchQuantityPriceNumberFormat(price);
      cell.z = numberFormat;
      cell.s = { ...(cell.s || {}), numFmt: numberFormat, alignment: { ...(cell.s?.alignment || {}), horizontal: 'center', vertical: 'center', wrapText: false, shrinkToFit: true } };
      const position = XLSX.utils.decode_cell(address);
      const rowIndex = position.r;
      if (!target['!rows']) target['!rows'] = [];
      target['!rows'][rowIndex] = { ...(target['!rows'][rowIndex] || {}), hpt: Math.max(Number(target['!rows'][rowIndex]?.hpt || 0), 18) };
      if (!target['!cols']) target['!cols'] = [];
      target['!cols'][position.c] = { ...(target['!cols'][position.c] || {}), wch: Math.max(Number(target['!cols'][position.c]?.wch || 0), 13) };
    }
