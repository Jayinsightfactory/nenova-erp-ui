...
export function estimateCatalogAutoTxtHcm(lines, fields, perPage = 8, spacing = 'wide') {
  const list = (lines || []).filter(Boolean);
  if (!list.length) return 0;
  const base = computeCatalogLayout(perPage, spacing);
...
export function catalogPptImageSizeLabel(perPage = 8, spacing = 'wide') {
  const cm = computeCatalogLayout(perPage, spacing).imgSizeCm;
