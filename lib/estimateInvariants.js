...
/** nenova.exe FormPrintEstimate — 수량 0 행은 품목·단가·금액 모두 미출력 */
...
export function isOperationalEstimateDescr(text) {
  ...
  if (!s) return false;
  if (/수량\s*변동|분배\s*변동|붙여\s*넣기|단가\s*변경|차감단가/i.test(s)) return true;
