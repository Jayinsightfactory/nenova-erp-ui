...
/** 불량차감만 — 검역·단가차감·판매요청은 제외. 인쇄 적요 기본 미표시 대상 */
export function isDefectDeductionRow(row) {
  const t = String(row?.EstimateType || '').trim();
  if (/불량차감/i.test(t) || /^fee03$/i.test(t) || /^kr0010$/i.test(t)) return true;
  return /\[불량차감\]/.test(String(row?.ProdName || ''));
}
...
/** ShipmentDetail/Estimate Descr — 수량변동·단가변경 등 운영 로그(인쇄·견적 적요 제외) */
export function isOperationalEstimateDescr(text) {
  const s = String(text || '').trim();
  if (!s) return false;
  if (isSqlAuditDescrLine(s)) return true;
  if (/수량\s*변동|분배\s*변동|붙여\s*넣기|단가\s*변경|차감\s*단가|차감\s*수량|엑셀업로드|붙여넣기분배/i.test(s)) return true;
  if (/차감단가|차감수량/i.test(s)) return true;
  if (/\[\d{4}-\d{2}-\d{2}/.test(s)) return true;
  if (/\[\d{1,2}\/\d{1,2}\s+\d{1,2}:\d{2}/.test(s)) return true;
  // 트리거/무접두 수량요약: "20->10" / "20→10" / "20>10"
...
/** DB 트리거/SqlClient 감사 줄 — 분배·견적 적요에서 제거 */
export function isSqlAuditDescrLine(text) {
  const s = String(text || '');
  if (/SqlClient|Data Provider|node-mssql/i.test(s)) return true;
  if (/\[\d{4}-\d{2}-\d{2}[^\]]*\]\s*\[[^\]]*(?:SqlClient|Data Provider|node-mssql|\/\s*nenova)/i.test(s)) return true;
  // RIGHT() 잘림으로 앞이 잘린 잔여: "] [node-mssql / login] 1->2"
  if (/^\].*\[(?:node-mssql|SqlClient|\.Net)/i.test(s.trim())) return true;
  return false;
}

/**
 * 차감(Estimate) 행 비고 — 단가차감·검역차감은 Estimate.Descr을 보여 준다.
 * 불량차감 적요는 화면·인쇄 모두 showDeductionDescr===true일 때만 출력한다.
 * (SqlClient 감사줄은 제외)
 */
...
/**
 * 차감 행 — Estimate.Descr 없을 때 출고일 일자만 (예: 26일). 인쇄·화면 옵션으로만 표시.
 */
export function formatEstimateDeductionOutDay(row) {
  const m = String(row?.outDate || row?.EstimateDtm || '').match(/^\d{4}-\d{2}-(\d{2})/);
  return m ? `${parseInt(m[1], 10)}일` : '';
}

/** 견적 행 1건 — 화면·API·인쇄 공통 비고 */
export function formatEstimateDescrForRow(row, opts = {}) {
  const showDistribDesc = opts.showDistribDesc === true;
  const showDeductionOutDay = opts.showDeductionOutDay === true;
  const raw = String(row?.DescrRaw ?? row?.Descr ?? '').trim();

  if (isEstimateDeductionRow(row)) {
    const hideDefectDescr = isDefectDeductionRow(row) && opts.showDeductionDescr !== true;
    if (!hideDefectDescr) {
      const memo = formatEstimateDeductionDescr(raw);
      if (memo) return memo;
    }
    if (showDeductionOutDay) return formatEstimateDeductionOutDay(row);
    return '';
  }
...
/** 견적서 인쇄 적요 — 불량차감 적요는 기본 숨김, 옵션으로만 표시 */
export function formatEstimatePrintDescr(row, opts = {}) {
  return formatEstimateDescrForRow(row, { showDeductionDescr: false, ...opts });
}

/** 화면 비고 — 정상출고는 운영 로그 숨김, 불량차감 적요는 기본 숨김 */
export function sanitizeEstimateDescrForDisplay(row, opts = {}) {
  return formatEstimateDescrForRow(row, opts);
}
