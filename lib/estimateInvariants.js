...
/**
 * 차감(Estimate) 행 비고 — 화면은 Estimate.Descr을 보여 준다.
 * 인쇄의 불량차감 적요는 showDeductionDescr===true일 때만 출력한다.
 * (단가차감·검역차감 사용자 입력·차감내역 포함, SqlClient 감사줄만 제외)
 */
...
export function formatEstimateDescrForRow(row, opts = {}) {
  ...
  if (isEstimateDeductionRow(row)) {
    const hideDefectDescr = isDefectDeductionRow(row) && opts.showDeductionDescr === false;
    if (!hideDefectDescr) {
...
/** 화면 비고 — 정상출고는 운영 로그 숨김, 차감은 EXE Estimate.Descr + 표시 옵션 */
