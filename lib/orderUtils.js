...
export function normalizeOrderUnit(unit, fallback = '박스') {
  ...
  if (raw === '개' || raw === '대') return '송이';

  const lower = raw.toLowerCase();
  if (/^(?:st|ea)$/i.test(lower) || /(stem|steam|stems|송이|개|대|스팀|스템)/i.test(lower)) return '송이';
...
export function normalizeOrderYear(week, fallbackYear = new Date().getFullYear().toString()) {
  ...
  // 구형 NN-NN (붙여넣기 '이전 차수' 섹션) → 2025 레거시
  // ⚠ 이 규칙은 "과거 데이터 조회" 전용. 신규 저장/분배/재고 경로는 resolveActiveOrderYear 를 쓸 것
  //   (이 규칙을 탄 업로드가 2025 마스터·출고일을 생성한 사고: 2026-07-08, 28-01 연도분열)
  if (/^\d{2}-\d{2}$/.test(String(week).trim())) return '2025';
  return String(fallbackYear);
}

/** 현재 운영 데이터용 연도 해석 — normalizeOrderYear 의 "NN-NN→2025 레거시" 규칙을 타지 않는다.
 * 우선순위: 명시 연도(YYYY) → 주차에 박힌 연도(YYYY-NN-NN) → fallback(기본 현재 연도) */
export function resolveActiveOrderYear(week, explicitYear = '', fallbackYear = new Date().getFullYear().toString()) {
  const y = String(explicitYear ?? '').trim();
  if (/^\d{4}$/.test(y)) return y;
  const w = String(week ?? '').trim();
  if (w) {
    try {
      const parsed = validateOrderWeek(w);
      if (parsed.year) return String(parsed.year);
    } catch { /* 형식 오류는 각 API 의 week 검증이 따로 처리 — 연도는 fallback */ }
  }
  return String(fallbackYear);
}

/**
 * ERP 원장 조회·쓰기용 엄격 연도 해석.
 * - 요청의 명시 연도 또는 YYYY-NN-NN 차수에 포함된 연도만 허용한다.
 * - 둘 다 있으면 반드시 일치해야 한다.
 * - NN-NN만 받고 현재 연도로 추정하지 않는다. 사용자가 선택 연도를 보내야 한다.
 */
export function requireOrderYear(week, explicitYear = '') {
  const rawWeek = String(week ?? '').trim();
  const requested = String(explicitYear ?? '').trim();
  const parsed = validateOrderWeek(rawWeek);

  if (requested && !/^\d{4}$/.test(requested)) {
    const error = new Error(`OrderYear 형식 오류: 4자리 연도가 필요합니다 (받음: '${requested}').`);
    error.code = 'INVALID_ORDER_YEAR';
    error.statusCode = 400;
    throw error;
  }
  if (parsed.year && requested && String(parsed.year) !== requested) {
    const error = new Error(`선택 차수의 연도(${parsed.year})와 요청 연도(${requested})가 다릅니다.`);
    error.code = 'ORDER_YEAR_MISMATCH';
    error.statusCode = 400;
    throw error;
  }

  const orderYear = requested || String(parsed.year || '');
  if (!orderYear) {
    const error = new Error('연도를 확인할 수 없습니다. 연도를 선택한 뒤 다시 시도하세요.');
    error.code = 'ORDER_YEAR_REQUIRED';
    error.statusCode = 400;
    throw error;
  }
  return { orderYear, orderWeek: parsed.week };
}

/**
 * API 조회/저장용 { week: '29-01', year: '2026' }
 * - YYYY-WW-SS → 연도·차수 분리
 * - NN-NN 구형식 → 2025 레거시 (운영 DB OrderWeek 만 저장된 행)
 */
export function resolveOrderWeekQuery(week, fallbackYear = new Date().getFullYear().toString()) {
  const raw = String(week || '').trim();
  if (!raw) return { week: '', year: String(fallbackYear) };
  const v = validateOrderWeek(raw);
  return {
    week: v.week,
    year: v.year || normalizeOrderYear(raw, fallbackYear),
  };
}

/** 선택 등록차수와 DB 주문 row의 연도·차수 일치 여부 */
export function orderRowMatchesWeek(row, selectedWeek) {
  if (!row || !selectedWeek) return false;
  const want = resolveOrderWeekQuery(selectedWeek);
  let rowWeek = String(row.week || '');
  let rowYear = String(row.year || '');
  const m = rowWeek.match(/^(\d{4})-(\d{2}-\d{2})$/);
  if (m) {
    rowYear = rowYear || m[1];
    rowWeek = m[2];
  }
  if (rowWeek !== want.week) return false;
  if (!rowYear) return want.year === '2025';
  return rowYear === want.year;
}
