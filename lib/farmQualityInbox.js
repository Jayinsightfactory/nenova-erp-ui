...
export function qualityInboxMutationPolicy({action,user,item,reason,kind}){
 ...
 if(item?.excluded&&kind!=='REQUEST_EDIT')throw inboxError('INBOX_INVALID','제외된 피드백을 먼저 복원하세요.');
 if(!['COMMENT','REQUEST','REQUEST_EDIT'].includes(kind)&&!manage)throw inboxError('INBOX_FORBIDDEN','답변 및 개선 상태 기록은 수입부 권한이 필요합니다.');
