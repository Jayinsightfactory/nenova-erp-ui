...
export const COLUMN_GUIDE = [
  ...
  },
  {
    ...
    formula: '원본 양식과 같은 공식입니다. 이번 차수에 매입이 있으면 (매입액 + 그외통관비) ÷ 그 차수 매입수량 × 기말 재고수량. 이번 차수에 매입이 없으면 직전 차수 재고단가 × 기말 재고수량이라 첫 입고 시점 환율이 그대로 유지됩니다. 두 방법 모두 안 되면 검증된 품목별 시점단가 × 재고수량으로 보조합니다.',
    kind: 'auto',
    note: '재고수량을 웹이 다시 계산하지 않고 usp_StockCalculation 결과를 사용합니다. 새 입고가 있는 차수만 그 차수 환율로 재평가하고, 새 입고가 없으면 이전 환율이 그대로 남습니다(호주처럼 여러 차수에 걸쳐 재고로만 팔리는 경우). 자동식과 검증된 매입·취득원가 근거가 모두 없을 때만 INPUT_REQUIRED입니다. 판매·분배단가와 Product.Cost는 재고원가가 아니므로 후보나 자동값으로 사용하지 않습니다.',
