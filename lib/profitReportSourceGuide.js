...
export const COLUMN_GUIDE = [
  ...
  },
  {
    key: 'E',
    label: '기초상품재고액',
    source: '같은 연도의 직전 대차수(01차만 전년도 52차) 마지막 확정 ProductStock와 동일 시점의 확정 품목 단가를 사용합니다.',
    formula: '기초재고 = 전차수 확정 ProductStock 환산수량 × (확정 재고단가 또는 동일 차수·품목·단위의 사용자 확정 도착원가)',
    kind: 'auto',
    note: '01차만 전년도 52차를 전 차수로 봅니다. 확정 재고나 동일 시점 단가 근거가 없으면 INPUT_REQUIRED이며 전년도 동일차수나 보고서 수기 E/F로 대체하지 않습니다.',
    fields: 'StockMaster(isFix=1, 전 차수 OrderYear+OrderWeek) / ProductStock.Stock / WebStockPriceEvidence(VERIFIED) / WebArrivalCostLine+History(정확한 scope만)',
  },
  {
    key: 'F',
    label: '기말상품재고액',
    source: '이번 차수의 마지막 isFix=1 ProductStock와 그 정확한 OrderYear+OrderWeek의 확정 품목 단가 근거를 사용합니다.',
    formula: '기말재고 = 확정 ProductStock 환산수량 × (확정 재고단가 또는 동일 차수·품목·단위의 사용자 확정 도착원가)',
    kind: 'auto',
    note: '재고수량을 웹이 다시 계산하지 않고 usp_StockCalculation 결과를 사용합니다. 단가 근거가 하나라도 없으면 INPUT_REQUIRED이며 최근원가·Product.Cost·최종값 직접입력으로 대체하지 않습니다.',
    fields: 'StockMaster(isFix=1, OrderYear+OrderWeek) / ProductStock.Stock / WebStockPriceEvidence(VERIFIED) / WebArrivalCostLine+History(정확한 scope만)',
  },
  {
    key: 'G',
    label: '매입액(상품+포워딩)',
  ...
  },
  {
    key: 'H',
    label: '그외통관비',
    source: '[📦 그외통관비 입력] 화면의 값. 2026년 22~27차처럼 입력 이력이 없는 차수는 원본 “매출원가 양식” 엑셀의 구성요소를 그대로 자동 적용합니다(화면에 “원본 엑셀값” 배지). 무게(GW)는 입고관리에서 자동으로 가져옵니다.',
    ...
    kind: 'manualSource',
    note: '÷1.1은 부가세를 뺀 공급가만 원가로 잡기 위한 것입니다. 백상창고료·관세는 원래 공급가라 나누지 않고, 베트남 선율만 예외로 나누지 않습니다. 값의 출처는 저장값 > 원본 엑셀값 > 입고 GW 자동추천 순이고, 저장값이 하나라도 있으면 원본 엑셀값은 쓰지 않습니다. 입력이 없으면 0으로 계산되고 검증 목록에 뜹니다. 값이 없을 때 할 일: 통관사 청구서의 관세·선율 금액을 입력 화면에 넣으세요.',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    key: 'in-stockprice',
    label: '재고단가 근거 (🏷)',
    source: '확정 재고 스냅샷과 같은 OrderYear+OrderWeek에 연결된 외부 확정 단가 증거. 직접 저장 근거가 없으면 같은 품목·매입단위의 사용자 확정 도착원가를 읽습니다.',
    formula: '재고 평가액 = 확정 ProductStock 환산수량 × VERIFIED 시점 단가',
    kind: 'manualSource',
    note: 'sourceRef·기준일·확정자·확정시각이 모두 있어야 하며 다른 주차로 자동 상속하지 않습니다. 도착원가는 단순 업로드/자동매칭만으로 인정하지 않고 MATCH 또는 배분기준 저장 이력이 있어야 합니다.',
    fields: 'WebStockPriceEvidence(OrderYear, OrderWeek, ProdKey, EvidenceStatus=VERIFIED) / WebArrivalCostLine+Import+History',
  },
];
...
export const BOUNDARY_GUIDE = [
  {
    ...
    kind: 'auto',
    note: '“마지막 세부차수”는 isFix=1이고 ProductStock 자료가 실제로 있는 세부차수 중 번호가 가장 큰 것입니다. 미확정·NULL·시작재고 마커는 선택하지 않습니다.',
