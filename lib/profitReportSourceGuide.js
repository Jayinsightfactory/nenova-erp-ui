...
export const COLUMN_GUIDE = [
  ...
  },
  {
    key: 'E',
    label: '기초상품재고액',
    source: '전 차수(27차라면 26차)의 기말상품재고액을 그대로 이월합니다. 전 차수를 이미 “확정”했으면 그때 굳어진 값이 먼저입니다.',
    formula: '① 이번 차수에 직접 넣은 값 → ② 전 차수 확정 스냅샷의 기말재고 → ③ 전 차수 보고서에 저장한 기말재고 → ④ 전 차수 재고로 기말재고와 똑같은 규칙으로 자동계산',
    kind: 'autoManual',
    note: '01차만 전년도 52차를 전 차수로 봅니다. ②가 있으면 나중에 전산 재고가 바뀌어도 이월값은 바뀌지 않습니다. 전 차수 재고 스냅샷이 아예 없으면 자동계산이 안 되고 검증 목록에 뜹니다. 값이 없을 때 할 일: 전 차수 보고서를 열어 기말재고를 확인·확정하세요.',
    fields: 'ProductStock.Stock(전 차수 마지막 StockKey) / WebProfitReport.ColKey=E · pages/api/sales/profit-report.js',
  },
  {
    key: 'F',
    label: '기말상품재고액',
    source: '이번 차수의 마지막 확정 세부차수 재고 스냅샷(전산 재고현황의 마지막 잔량)에 품목별 재고평가단가를 곱합니다.',
    formula: '① 남은 재고수량 × 품목별 재고평가단가 ÷ 1.1([🏷 재고단가표] 기준) → ② 평가단가가 없으면 품목별 최근 매입단가 × 남은 수량 × 환율 → ③ 그것도 없으면 (구매금액 × 환율 + 포워딩 × 환율 + 그외통관비) ÷ 이번 차수 매입 총수량 × 남은 재고수량',
    kind: 'autoManual',
    note: '재고수량을 웹이 다시 계산하지 않고 전산 재고현황의 마지막 잔량을 그대로 씁니다. 재고 마감표시(isFix)는 스냅샷을 고르는 조건이 아니라 참고 표시입니다. 원본 엑셀 “재고잔량” 시트의 총합을 그대로 옮겨 적지 않습니다 — 그 시트에는 수동정정·오입력(예: 단가에 0이 하나 더 붙은 값)이 남아 있습니다. 보고서를 확정하면 그 시점 값으로 고정되어 이후 전산 재고가 바뀌어도 달라지지 않습니다. 마이너스 재고가 섞이면 검증 오류로 알려줍니다. 값이 없을 때 할 일: [🏷 재고단가표]에서 해당 품목 단가를 지정하세요.',
    fields: 'ProductStock.Stock(이번 차수 마지막 StockKey) × WebStockPrice/Product.Cost ÷ 1.1 / WarehouseDetail / WebProfitReport.ColKey=F',
  },
  {
    key: 'G',
    label: '매입액(상품+포워딩)',
