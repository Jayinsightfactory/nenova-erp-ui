...
export const COLUMN_GUIDE = [
  ...
  },
  {
    ...
    kind: 'auto',
    note: '재고수량을 웹이 다시 계산하지 않고 usp_StockCalculation 결과를 사용합니다. 자동식과 검증된 매입·취득원가 근거가 모두 없는 품목은 최근 확정 판매단가(×1.1) → 최근 매입 근사 순으로 자동 평가됩니다(2026-08-27 대표 확정 — 26~31차 백테스트에서 확정 엑셀 대비 오차율 7.3%로 검증). 자동평가는 화면·감사에 구분 표시되며, 실제 매입근거를 저장하면 항상 그 값이 우선합니다. Product.Cost는 사용하지 않습니다.',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    ...
    kind: 'autoManual',
    note: '신규 품목도 콜롬비아 5품종·베트남이면 카테고리 평균식에 자동 포함됩니다. 그 밖의 국가는 같은 품목·단위의 확정 도착원가가 있으면 자동 평가하고, 없으면 최근 확정 판매단가(×1.1) → 최근 매입 근사 순으로 자동 평가합니다(2026-08-27 대표 확정 자동처리). 직접 저장하는 근거는 sourceRef·기준일·확정자·확정시각이 모두 있어야 하며 다른 주차로 자동 상속하지 않고, 저장하면 자동평가를 항상 대체합니다. 2026년 22~28차만 원본 파일 SHA와 F셀 위치가 고정된 역사 증거를 보조수단으로 사용할 수 있습니다.',
