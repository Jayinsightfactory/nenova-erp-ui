...
export const COLUMN_GUIDE = [
  ...
  },
  {
    key: 'E',
    label: '기초상품재고액',
    source: '같은 연도의 직전 대차수(01차만 전년도 52차) 마지막 EXE ProductStock와 동일 시점의 확정 품목 단가를 사용합니다.',
    formula: '기초재고 = 전차수 마지막 EXE ProductStock 환산수량 × (확정 재고단가 또는 동일 차수·품목·단위의 사용자 확정 도착원가)',
    kind: 'auto',
    note: '01차만 전년도 52차를 전 차수로 봅니다. 확정 재고나 동일 시점 단가 근거가 없으면 INPUT_REQUIRED이며 전년도 동일차수나 보고서 수기 E/F로 대체하지 않습니다.',
    fields: 'StockMaster(전 차수 OrderYear+OrderWeek, ProductStock 존재) / ProductStock.Stock / WebStockPriceEvidence(VERIFIED) / WebArrivalCostLine+History(정확한 scope만)',
  },
  {
    key: 'F',
    label: '기말상품재고액',
    source: '이번 차수에서 ProductStock 자료가 있는 마지막 숫자 세부차수와 그 정확한 OrderYear+OrderWeek의 확정 품목 단가 근거를 사용합니다.',
    formula: '기말재고 = 마지막 EXE ProductStock 환산수량 × (확정 재고단가 또는 동일 차수·품목·단위의 사용자 확정 도착원가)',
    kind: 'auto',
    ...
    fields: 'StockMaster(OrderYear+OrderWeek, ProductStock 존재) / ProductStock.Stock / WebStockPriceEvidence(VERIFIED) / WebArrivalCostLine+History(정확한 scope만)',
  },
  {
    key: 'G',
    label: '매입액(상품+포워딩)',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    ...
    formula: '재고 평가액 = EXE ProductStock 환산수량 × VERIFIED 시점 단가',
    kind: 'manualSource',
