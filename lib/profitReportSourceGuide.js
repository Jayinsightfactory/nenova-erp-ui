...
export const COLUMN_GUIDE = [
  ...
  },
  {
    ...
    formula: '콜롬비아 5품종·베트남 = 기존재고는 전차수 금액(기존 환율)을 유지하고 이번 입고만 (매입액+그외통관비)÷매입수량으로 평가한 뒤, 판매는 기초부터 나갑니다. 호주는 입고 시점 구매단가×수량×선율과세환율을 입고층에 고정합니다. 그 밖은 검증된 품목별 시점단가 × 재고수량.',
    kind: 'auto',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    ...
    formula: '콜롬비아 5품종·베트남은 기존재고(기존 환율)와 이번 입고(새 환율)를 층으로 나눠 평가. 호주는 입고 시점 구매단가×수량×선율과세환율. 그 밖은 EXE ProductStock 환산수량×VERIFIED 시점 단가',
    kind: 'autoManual',
