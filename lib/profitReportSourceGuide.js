...
export const COLUMN_GUIDE = [
  ...
  },
  {
    key: 'E',
    label: '기초상품재고액',
    source: '같은 연도의 직전 대차수(01차만 전년도 52차) 마지막 확정 ProductStock와 동일 시점 VERIFIED 품목 단가를 사용합니다.',
    formula: '기초재고 = 전차수 확정 ProductStock 환산수량 × WebStockPriceEvidence 시점 단가',
    kind: 'auto',
    note: '01차만 전년도 52차를 전 차수로 봅니다. 확정 재고나 동일 시점 단가 근거가 없으면 INPUT_REQUIRED이며 전년도 동일차수나 보고서 수기 E/F로 대체하지 않습니다.',
    fields: 'StockMaster(isFix=1, 전 차수 OrderYear+OrderWeek) / ProductStock.Stock / WebStockPriceEvidence(VERIFIED)',
  },
  {
    key: 'F',
    label: '기말상품재고액',
    source: '이번 차수의 마지막 isFix=1 ProductStock와 그 정확한 OrderYear+OrderWeek의 VERIFIED 품목 단가 근거를 사용합니다.',
    formula: '기말재고 = 확정 ProductStock 환산수량 × WebStockPriceEvidence 시점 단가',
    kind: 'auto',
    ...
    fields: 'StockMaster(isFix=1, OrderYear+OrderWeek) / ProductStock.Stock / WebStockPriceEvidence(VERIFIED)',
  },
  {
    key: 'G',
    label: '매입액(상품+포워딩)',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    key: 'in-stockprice',
    label: '재고단가 근거 (🏷)',
    source: '확정 재고 스냅샷과 같은 OrderYear+OrderWeek에 연결된 외부 확정 단가 증거',
    formula: '재고 평가액 = 확정 ProductStock 환산수량 × VERIFIED 시점 단가',
    kind: 'manualSource',
    note: 'sourceRef·기준일·확정자·확정시각이 모두 있어야 하며 다른 주차로 자동 상속하지 않습니다.',
    fields: 'WebStockPriceEvidence(OrderYear, OrderWeek, ProdKey, EvidenceStatus=VERIFIED)',
  },
];
