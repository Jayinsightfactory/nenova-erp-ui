...
export const COLUMN_GUIDE = [
  ...
  },
  {
    key: 'R',
    label: '환율',
    source: '① 그 차수 입고별 과세환율 스냅샷(통관 신고 시점 관세청 과세환율, 호주 AUD 포함) → ② 29차부터는 전 차수 보고서에 저장된 과세환율 → ③ 통화마스터의 현재 환율',
    formula: '①은 그 차수 입고들의 과세환율을 구매금액으로 가중평균합니다. 구매현황에 남는 상업(환전) 환율과는 다른 값입니다. 세 가지가 다 없고 매입이나 포워딩 금액이 있으면 그 줄에 입력칸이 저절로 나타납니다.',
    kind: 'autoManual',
    note: '나라별 통화가 다릅니다 — 네덜란드 EUR, 호주 AUD, 중국 CNY, 일본 JPY, 나머지는 USD. 인보이스 과세환율과 다르면 직접 넣어 저장하세요. 넣은 값은 자동값이 아니라 저장된 입력값입니다.',
