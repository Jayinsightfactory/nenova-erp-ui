...
export const COLUMN_GUIDE = [
  ...
  },
  {
    ...
    formula: '기초재고 = 전차수 F. 모든 국가·품종 줄에서 이번 차수 기초재고 금액은 전차수 기말재고 금액과 같습니다.',
    kind: 'auto',
    note: '01차만 전년도 52차를 전 차수로 봅니다. 전차수가 확정되어 있으면 그 F를 가져오고, 아니면 전차수 F 공식으로 다시 계산합니다. 이스라엘·뉴질랜드·일본도 E는 전차수 F와 같으며, 매출원가에서 F를 빼지 않는 것만 다릅니다. 현재 차수 workbook E 리터럴은 자동 이월하지 않습니다.',
  ...
  },
  {
    ...
    formula: '모든 국가·품종에서 기존재고는 전차수 금액(기존 환율)을 유지하고 이번 입고만 새 환율로 평가한 뒤, 판매는 기초부터 나갑니다. 콜롬비아 5품종·베트남 신규입고는 (매입액+그외통관비)÷매입수량, 호주는 입고 시점 구매단가×수량×선율과세환율, 그 밖은 이번 매입액(구매×환율+포워딩×환율)입니다. 층별 평가가 안 될 때만 검증된 품목별 시점단가 × 재고수량으로 보조합니다.',
    kind: 'auto',
    note: '재고수량을 웹이 다시 계산하지 않고 usp_StockCalculation 결과를 사용합니다. 새로 들어오는 품목만 이번 차수 환율로 계산하고, 재고로 팔릴 때는 입고 당시 환율을 유지합니다. 자동식과 검증된 매입·취득원가 근거가 모두 없을 때만 INPUT_REQUIRED입니다. 판매·분배단가와 Product.Cost는 재고원가가 아니므로 후보나 자동값으로 사용하지 않습니다.',
  ...
  },
  {
    ...
    formula: '나라별 = 백상창고료((1차GW+2차GW) × 원/kg) + 관세1 + 관세2 + 선율 ÷ 1.1 + 월드운송료 ÷ 1.1 + 한국방역 ÷ 1.1. 콜롬비아 카네이션·장미·알스트로·루스커스는 세부차수 합계를 무게비율로 나누고, 과금중량이 총중량보다 크면 부피(CBM)비율로 나눕니다. APOLLO는 포워딩 업체이며 기본 배분은 콜카장알루입니다. 그외통관비에서 콜카장알루수국을 고르면 수국을 풀에 넣습니다.',
    kind: 'autoManual',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    ...
    formula: '통관비와 포워딩은 총중량과 과금중량이 같거나 과금중량이 더 작으면 무게비율, 과금중량이 더 크면 CBM비율로 나눕니다.',
    kind: 'autoManual',
    note: '카네이션·장미·알스트로·루스커스가 기본(콜카장알루)입니다. APOLLO는 포워딩 업체이고, 그외통관비 반차수에서 콜카장알루수국을 고르면 통관·트럭·항공료 풀에 수국을 함께 넣습니다. 수국만 따로 온 AWB는 나라 단위로 계산합니다.',
  ...
  },
  {
    ...
    formula: '모든 국가는 기존재고(기존 환율)와 이번 입고(새 환율)를 층으로 나눠 평가. 호주는 입고 시점 구매단가×수량×선율과세환율. 층별이 안 될 때만 EXE ProductStock 환산수량×VERIFIED 시점 단가',
    kind: 'autoManual',
