...
export const COLUMN_GUIDE = [
  ...
  },
  {
    key: 'E',
    label: '기초상품재고액',
    source: '같은 연도의 직전 대차수(01차만 전년도 52차) 기말상품재고액 원천과 마지막 EXE ProductStock를 사용합니다.',
    formula: '기초상품재고액 = 전차수 기말상품재고액. 모든 국가·품종 줄에서 이번 차수 기초재고 금액은 전차수 기말재고 금액과 같습니다.',
    kind: 'auto',
    note: '01차만 전년도 52차를 전 차수로 봅니다. 전차수가 확정되어 있으면 그 확정 기말재고를 가져오고, 아니면 전차수 기말재고를 같은 공식으로 다시 계산합니다. 이스라엘·뉴질랜드·일본도 기초재고는 전차수 기말재고와 같으며, 매출원가에서 기말재고를 빼지 않는 것만 다릅니다. 현재 차수 workbook 기초재고 리터럴은 자동 이월하지 않습니다.',
  ...
  },
  {
    ...
    formula: '원본 양식과 같은 공식입니다. 이번 차수에 매입이 있으면 (매입액 + 그외통관비) ÷ 그 차수 매입수량 × 기말 재고수량. 그외통관비에는 단당 통관비(엑셀 Q54)도 들어갑니다. 이번 차수에 매입이 없으면 직전 차수 재고단가 × 기말 재고수량이라 재고가 매입된 시점 환율이 그대로 유지됩니다. 재고수량이 없어도 기말상품재고액이 적혀 있으면 그 금액을 실제 재고로 넣습니다. 두 방법 모두 안 되면 검증된 품목별 시점단가 × 재고수량으로 보조합니다.',
    kind: 'auto',
    note: '재고수량을 웹이 다시 계산하지 않고 usp_StockCalculation 결과를 사용합니다. 새 입고가 있는 차수만 그 차수 매입(과세)환율로 재평가하고, 재고판매만 있는 차수는 재고가 매입됐을 때 환율을 유지합니다(호주). 재고잔량 수량이 0이어도 금액이 있으면 실제 재고입니다. 자동식과 검증된 매입·취득원가 근거가 모두 없을 때만 INPUT_REQUIRED입니다. 판매·분배단가와 Product.Cost는 재고원가가 아니므로 후보나 자동값으로 사용하지 않습니다.',
  ...
  },
  {
    ...
    formula: '나라별 = 백상창고료((1차GW+2차GW) × 원/kg) + 관세1 + 관세2 + 선율 ÷ 1.1 + 월드운송료 ÷ 1.1 + 한국방역 ÷ 1.1. 콜롬비아 카네이션·장미·알스트로·루스커스는 세부차수 합계를 항상 무게비율로 나눕니다(원본 양식과 동일). APOLLO는 포워딩 업체이며 기본 배분은 콜카장알루입니다. 그외통관비에서 콜카장알루수국을 고르면 수국을 풀에 넣습니다.',
    kind: 'autoManual',
  ...
  },
  {
    ...
    kind: 'calc',
    note: '이스라엘·뉴질랜드·일본은 기말재고를 다시 더합니다(매출액 − 매출원가 + 기말재고). 마이너스면 빨간색으로 보입니다. 엑셀에 베트남 이익을 ±4,576,000원 옮긴 숫자는 실제 이익률을 보기 위한 메모이며 보고서 매출이익에는 반영하지 않고 비고에만 남깁니다.',
  ...
  },
  {
    key: 'R',
    label: '환율',
    source: '관세청 과세환율(통관 신고 환율, 호주 AUD 포함). ① 그 차수 입고별 과세환율 스냅샷 → ② 그 차수에 저장·조회해 둔 과세환율 → ③ 2026년 22~27차는 원본 “매출원가 양식” 본표 환율. 세 가지 모두 “정확히 그 차수” 값입니다.',
    formula: '①은 그 차수 입고들의 과세환율을 구매금액으로 가중평균합니다. 구매현황에 남는 상업(환전) 환율과는 다른 값입니다. 셋 다 없고 매입이나 포워딩 금액이 있으면 그 줄에 입력칸이 저절로 나타납니다.',
    kind: 'autoManual',
    note: '나라별 통화가 다릅니다 — 네덜란드 EUR, 호주 AUD, 중국 CNY, 일본 JPY, 나머지는 USD. 같은 통화라도 통관 신고 주차가 다르면 값이 다릅니다(27차 USD: 콜롬비아 1,538.30 / 태국·에콰도르 1,548.52). 통화마스터의 현재 환율과 전 차수 환율은 참고 제안일 뿐 자동으로 들어가지 않습니다 — 과거 차수에 오늘 환율을 채우면 확정 손익이 조용히 바뀌기 때문입니다. 값이 없을 때 할 일: 관세청 고시 과세환율을 그 줄에 직접 입력해 저장하세요. 넣은 값은 자동값이 아니라 저장된 입력값입니다.',
  ...
  },
  {
    ...
    formula: '같은 청구서 안의 꽃 품목 나라로 국가를 판정하고, 못 찾으면 농장명·인보이스 표기로 판정합니다. 콜롬비아는 인보이스의 “수국” 표기로 수국과 나머지 4품목을 나누되, APOLLO 포워딩 배분은 기본 콜카장알루이고 그외통관비에서 콜카장알루수국을 고르면 수국을 풀에 합쳐 무게비율(CW≤GW) 또는 CBM비율(CW>GW)로 나눕니다.',
    kind: 'autoManual',
    note: '29차 이후에는 입고에 운송료 전표가 있다는 전제로 원천행 누락·미분류를 차단합니다. 수국/카네이션/장미/루스커스/네덜란드/호주/태국/중국/미국(레몬잎) 운송료 표기도 직접 분류합니다. 새 농장이라도 같은 BILL/AWB의 상품 국가를 먼저 사용하며, 그래도 못 잡히면 직접 입력한 값이 우선합니다.',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    ...
    formula: '추천은 1차+2차 무게를 먼저 합칩니다. 5t 묶음을 먼저 배정하고 남은 무게가 1t 이하면 1t 1대, 2.5t 이하면 2.5t 1대, 2.5t를 넘으면 5t 1대입니다. 예: 1,371kg → 2.5t 1대, 3,342kg → 5t 1대 275,000원, 6,000kg → 5t 1대+1t 1대입니다.',
    kind: 'autoManual',
  ...
  },
  {
    ...
    formula: '그외통관비는 항상 무게비율로 나눕니다. 항공료만 과금중량이 총중량과 다르면 부피(CBM)비율, 같으면 무게비율로 나눕니다(원본 양식 반차수 시트와 동일).',
    kind: 'autoManual',
    note: '카네이션·장미·알스트로·루스커스가 기본(콜카장알루)입니다. APOLLO는 포워딩 업체이고, 그외통관비 반차수에서 콜카장알루수국을 고르면 통관·트럭·항공료 풀에 수국을 함께 넣습니다. 수국 박스당무게 5.6kg·박스당CBM 7은 원본 반차수 시트 실측값입니다. 수국만 따로 온 AWB는 나라 단위로 계산합니다.',
  ...
  },
  {
    ...
    formula: '모든 국가·품종: 매입이 있는 차수는 (매입액+그외통관비)÷매입수량 × 기말수량, 매입이 없는 차수는 직전 단가 유지. 그래도 안 되면 EXE ProductStock 환산수량×VERIFIED 시점 단가',
    kind: 'autoManual',
...
export const BOUNDARY_GUIDE = [
  {
    ...
    source: '전산 재고 스냅샷',
    formula: '27차 기초재고는 26차의 마지막 세부차수 재고, 기말재고는 27차의 마지막 세부차수 재고를 씁니다. 01차일 때만 전년도 52차를 전 차수로 봅니다. 모든 국가·품종 줄에서 이번 차수 기초재고 금액은 전차수 기말재고 금액과 같습니다.',
    kind: 'auto',
  ...
  {
    key: 'bd-rate-source',
    label: '과세환율(R)은 그 차수 값만 자동 적용',
    source: '그 차수의 통관 신고 환율',
    formula: '① 그 차수 입고 통관 스냅샷 → ② 그 차수에 저장·조회해 둔 과세환율(나라별 지정값이 통화 기본값보다 먼저) → ③ 2026년 22~27차 원본 엑셀 본표 환율. 셋 다 없으면 “원천 없음”으로 두고 그 줄에 입력칸을 엽니다.',
    kind: 'auto',
    note: '전 차수 환율과 통화마스터 현재 환율은 자동으로 물려받지 않습니다 — 참고값으로만 보여 주고, 사람이 적용·저장해야 계산에 들어갑니다(과거 차수 손익이 조용히 바뀌는 것을 막기 위함). 관세청 조회 API 키가 없으면 자동 조회도 하지 않고 “원천 없음”으로 표시합니다.',
