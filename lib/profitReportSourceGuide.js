...
export const COLUMN_GUIDE = [
  ...
  },
  {
    ...
    kind: 'auto',
    note: '재고수량을 웹이 다시 계산하지 않고 usp_StockCalculation 결과를 사용합니다. 자동식과 검증 근거가 모두 없을 때만 INPUT_REQUIRED입니다. 비평균·비호주 신규 품목은 같은 연도·차수의 확정 분배에서 실제 VAT 포함 단가를 그대로 후보로 표시하고, 가장 많은 업체가 사용한 단가를 우선 추천한 뒤 /1.1합니다. 평균값을 만들지 않으며 사용자가 확인·저장하기 전에는 계산에 적용하지 않습니다.',
...
export const INPUT_GUIDE = [
  ...
  },
  {
    key: 'in-stockprice',
    label: '재고단가 근거 (🏷)',
    source: '원본 양식의 평균원가 자동식을 적용할 수 없는 국가·품종에서 쓰는 보조 단가 근거입니다. 같은 OrderYear+OrderWeek의 외부 확정 단가를 우선하고, 없으면 같은 품목·매입단위의 사용자 확정 도착원가를 읽습니다.',
    formula: '콜롬비아 5품종·베트남은 (G+H)÷매입수량×재고수량으로 자동 평가. 그 밖은 EXE ProductStock 환산수량×VERIFIED 시점 단가',
    kind: 'autoManual',
    note: '신규 품목도 콜롬비아 5품종·베트남이면 카테고리 평균식에 자동 포함됩니다. 그 밖의 국가는 같은 품목·단위의 확정 도착원가가 있으면 자동 평가하고, 없을 때만 그 품목의 단가 근거 등록을 요구합니다. sourceRef·기준일·확정자·확정시각이 모두 있어야 하며 다른 주차로 자동 상속하지 않습니다. 2026년 22~28차만 원본 파일 SHA와 F셀 위치가 고정된 역사 증거를 마지막 보조수단으로 사용할 수 있습니다.',
