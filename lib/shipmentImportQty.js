  ...
  for (const t of targets || []) {
    ...
    if (prior) {
      ...
      if (t.verifyOrder) {
        ...
        prior.intendedOrder = Number(prior.intendedOrder || 0) + Number(t.intendedOrder ?? intended);
      }
...
export function compareVerifyResult(targets, actualByKey) {
  ...
  for (const t of targets || []) {
    ...
    const orderMatches = !t.verifyOrder || sameQty(actualOrder, Number(t.intendedOrder ?? intended));
