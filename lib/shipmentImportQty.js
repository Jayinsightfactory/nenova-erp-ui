 ...
 * cleanupZeroOrder: 사용자가 명시적으로 켠 경우에만 엑셀=0 인 주문 잔재를 0 처리(soft delete).
 * 잔재를 남기면 전산(exe)이 주문 대비 "미분배"로 계속 표시함. */
export function resolveImportOrderSyncPlan({ orderQty, uploadQty, cleanupZeroOrder = false }) {
  ...
  if (cleanupZeroOrder && Number(orderQty || 0) > 0) {
    return { action: 'sync', allowOrderDelete: true };
  }
