...
/** 장미 물량표(업체별 열): 셀 값은 단, Product.OutUnit 은 박스 — 차수피벗(단위열)은 제외 */
export function isRoseAllocationImportRow(row, product = {}) {
  ...
  if (/미니\s*카네|mini\s*carnation|minicarnation|카네이션|carnation|수국|hydrangea|알스트로|alstroe?meria/i.test(text)) {
    return false;
  }
  if (row?.productFamily === 'rose') return true;
  return /장미|\brose\b/i.test(text);
}
...
export function normalizeUploadQtyForProduct(row, product = {}) {
  ...
  const outU = normalizeOrderUnit(product?.OutUnit, '박스');
  const b1b = Number(product.BunchOf1Box || 0);
  if (outU === '박스' && b1b > 0 && isRoseAllocationImportRow(row, product)) {
...
export function detectQtyWarnings(row, product = {}) {
  ...
  if (excelUnit) {
    ...
    b1b > 1 &&
    excelQty > 0 &&
    isRoseAllocationImportRow(row, product) &&
    !sameQty(upload, excelQty)
  ) {
    warnings.push({
      severity: 'info',
      code: 'ROSE_ALLOC_CONVERTED',
      message: `장미 물량표 ${excelQty}단 → ${upload}박스 환산 (박스당 ${b1b}단)`,
    });
