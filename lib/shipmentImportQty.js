...
/**
 * 최종값(SET) 업로드의 apply 직전 stale 판정.
 * preview 뒤 같은 목표값이 먼저 반영된 경우는 멱등 성공 후보로 인정하고,
 * preview 값도 목표값도 아닌 제3의 값만 예상 밖 동시 변경으로 차단한다.
 * uploadQty<=0 행의 주문은 계약상 보존 대상이므로 주문 변화로 막지 않는다.
 */
export function evaluateImportFinalStateStale({
  previewOrderQty,
  currentOrderQty,
  intendedOrderQty,
  preserveOrder = false,
  ...
  const orderChanged = previewOrderQty != null && !sameQty(previewOrderQty, currentOrderQty);
  const shipmentChanged = previewShipmentQty != null && !sameQty(previewShipmentQty, currentShipmentQty);
  return {
    orderBlocked: !preserveOrder && orderChanged && !sameQty(currentOrderQty, intendedOrderQty),
    shipmentBlocked: shipmentChanged && !sameQty(currentShipmentQty, intendedShipmentQty),
    orderAlreadyAtTarget: !preserveOrder && orderChanged && sameQty(currentOrderQty, intendedOrderQty),
    shipmentAlreadyAtTarget: shipmentChanged && sameQty(currentShipmentQty, intendedShipmentQty),
  };
}
...
/** 미매칭 매칭모달에서 override 를 저장할 때 쓰는 키 — 항상 "원본 라벨"(alias 정규화 전) 기준. */
export function importCustomerOverrideLabelKey(row) {
  return importMatchNormText(row?.customerLabel);
}

/** 미매칭 매칭모달 "제외(분배 안 함)" sentinel — custOverrides[label] 에 이 값이 들어가면 그 라벨은
 *  주문·분배 양쪽 모두 완전히 스킵한다(콜롬비아 농장 열 등 거래처가 아닌 라벨 처리용). */
export const IMPORT_IGNORE_CUSTOMER_VALUE = '__ignore__';

export function isImportIgnoreCustomerValue(value) {
  return String(value ?? '').trim() === IMPORT_IGNORE_CUSTOMER_VALUE;
}

/**
 * 엑셀 행의 (원본 customerLabel, alias 정규화된 normalizedLabel) 이 "제외" 표시된 라벨 집합에 속하는지 판정.
 * override 조회(resolveImportCustomerOverrideKey)와 동일하게 원본/정규화 라벨 둘 다 확인 —
 * 모달이 어느 쪽 라벨로 저장했든 항상 스킵되게 한다.
 * @param {string} rawCustomerLabel row.customerLabel (alias 정규화 전 원본)
 * @param {string} normalizedCustomerLabel normalizeCustomerLabel(row.customerLabel) 적용값
 * @param {Set<string>} ignoredLabelsSet key=normText(라벨)
 */
export function isRowFromIgnoredCustomer(rawCustomerLabel, normalizedCustomerLabel, ignoredLabelsSet) {
  if (!ignoredLabelsSet || typeof ignoredLabelsSet.has !== 'function') return false;
  return ignoredLabelsSet.has(importMatchNormText(rawCustomerLabel)) || ignoredLabelsSet.has(importMatchNormText(normalizedCustomerLabel));
}

/**
 * 엑셀 (원본 customerLabel, alias 정규화된 normalizedLabel) → 사용자가 override map 에 저장한 custKey 조회.
 * 미매칭 매칭모달은 원본 라벨 기준으로 override 를 저장하는데, 매칭 루프는 alias 정규화된 라벨을 쓰기
 * 때문에 "저장한 라벨"과 "조회하는 라벨"이 어긋나 override 가 무시되는 문제가 있었다(원본↔정규화 불일치).
 * 원본 라벨을 우선 조회(모달에서 실제 선택한 라벨) → 정규화 라벨도 보조로 조회.
 * @param {string} rawCustomerLabel row.customerLabel (alias 정규화 전 원본)
 * @param {string} normalizedCustomerLabel normalizeCustomerLabel(row.customerLabel) 적용값
 * @param {Map<string, number>} overrideMap key=normText(라벨) → custKey
 */
export function resolveImportCustomerOverrideKey(rawCustomerLabel, normalizedCustomerLabel, overrideMap) {
  if (!overrideMap || typeof overrideMap.get !== 'function') return 0;
  return overrideMap.get(importMatchNormText(rawCustomerLabel)) || overrideMap.get(importMatchNormText(normalizedCustomerLabel)) || 0;
}
  ...
  for (const t of targets || []) {
    ...
    if (prior) {
      ...
      byKey.set(key, {
        ...
        prodName: t.prodName,
        intended,
        verifyOrder: !!t.verifyOrder,
        intendedOrder: Number(t.intendedOrder ?? intended),
      });
    }
  }
  return [...byKey.values()];
...
export function compareVerifyResult(targets, actualByKey) {
  ...
  for (const t of targets || []) {
    ...
    const actualOrder = Number(actual.orderQty || 0);
    // 기존 주문은 수량 보존 대상이다. 주문이 없어서 이번 적용에서 신규 등록한 경우도
    // 정확한 수량 동기화가 아니라 EXE 조인에 필요한 양수 활성 주문행 존재만 확인한다.
    const orderMatches = !t.verifyOrder || actualOrder > 0;
    ...
    if (qtyMatches && orderMatches && dateSumMatches && dateIssueCount === 0) {
      matched += 1;
      continue;
    }
    mismatches.push({
...
/** 커밋 후 재조회까지 완료된 경우에만 화면에서 성공으로 표시한다. */
export function isShipmentImportVerificationSuccessful(verification = {}) {
  return !verification?.error && Number(verification?.mismatchCount || 0) === 0;
}

/**
 * (custKey, prodKey) DB 조합이 "이번 업로드가 다뤄야 했던 범위" 안인지 판정 — missingFromExcel 게이트.
 * 빈 셀은 파서가 실제 (업체,품목) 행으로 만들고, 품목 행 자체가 삭제된 경우에는 export 시점
 * _keymap 의 시트별 업체×품목 pair 로 범위를 복원한다. 전역 품목 집합만으로 판정하면 다른
 * 시트/업체의 기존 분배까지 0으로 만들 수 있으므로 exact pair 를 최우선으로 사용한다.
 ...
 */
export function isImportRowInUploadScope(
  custKey,
  prodKey,
  uploadedProductKeys,
  custKeysInScope,
  ...
  const pairKey = `${ck}|${pk}`;
  if (matchedPairKeys?.has?.(pairKey) || pairKeysInScope?.has?.(pairKey) || priorImportPairKeys?.has?.(pairKey)) return true;

  // 신규 파서는 exact pair 집합을 항상 넘긴다. 집합에 없는 조합은 전역 업체/품목이 각각
  // 존재하더라도 다른 시트 조합일 수 있으므로 여기서 즉시 제외한다.
  if (matchedPairKeys instanceof Set || pairKeysInScope instanceof Set) return false;

  // 구형 호출자 호환. 신규 삭제 판정은 위 exact pair 집합을 전달해야 한다.
  // keymap 이 없는 외부 파일은 실제로 읽힌 업체와 품목의 교집합만 허용한다.
  if (uploadedProductKeys?.has?.(pk) && custKeysInScope?.has?.(ck)) return true;
...
/**
 * 같은 차수·같은 시트의 이전 성공 업로드에서 다뤘고, 이번 시트에도 품목 자체는 남아 있는
 * 업체×품목 pair를 복원한다. 새 파일에서 업체 열이 사라져도 이전 최종값이 유령분배로
 * 남지 않게 하되, 다른 시트/다른 품목의 분배는 범위에 섞지 않는다.
 */
export function buildReplacementProductKeys(products, uploadedProductKeys) {
  const categories = new Set(products.filter(p => uploadedProductKeys.has(Number(p.ProdKey)))
    .map(p => String(p.CountryFlower || '').trim()).filter(Boolean));
  return new Set(products.filter(p => categories.has(String(p.CountryFlower || '').trim())).map(p => Number(p.ProdKey)));
}

export function buildPriorImportPairKeys(priorRows = [], currentSheetProductKeys = new Map()) {
  const result = new Set();
  for (const row of priorRows || []) {
    const custKey = Number(row?.CustKey || 0);
    const prodKey = Number(row?.ProdKey || 0);
    if (!custKey || !prodKey) continue;
    const sourceCells = String(row?.SourceCells || '');
    const sheets = sourceCells.split(',').map((cell) => cell.split('!')[0].trim()).filter(Boolean);
    if (sheets.some((sheet) => currentSheetProductKeys.get(sheet)?.has?.(prodKey))) {
      result.add(`${custKey}|${prodKey}`);
    }
  }
  return result;
}
...
export function resolveImportOrderSyncPlan({ orderQty, uploadQty, hasExistingOrder = Number(orderQty || 0) !== 0 }) {
  if (hasExistingOrder) {
    return { action: 'skip_keep_order', allowOrderDelete: false, preservesExistingOrder: true };
  }
  if (Number(uploadQty || 0) > 0) {
    return { action: 'create', allowOrderDelete: false, preservesExistingOrder: false };
  }
  return { action: 'skip_no_order', allowOrderDelete: false, preservesExistingOrder: false };
}

/**
 * 엑셀 물량표 적용의 쓰기 의도.
 *
 * 재고/입고/잔량 요약값은 참고용이고 실제 주문·분배 쓰기의 단일 수량 원천은
 * preview row 의 uploadQty(업체별 출고/분배 셀 합계)다. 이 규칙을 순수 함수로
 * 고정해 두어 재고 셀을 잘못 사용하거나 0수량으로 신규 분배를 만드는 회귀를 막는다.
 */
export function resolveImportWriteIntent({ uploadQty, hasExistingShipment = false, hasExistingOrder = false, shipmentOnly = false } = {}) {
  const qty = asNumber(uploadQty);
  const positive = qty > 0;
  return {
    source: 'uploadQty',
    stockQtyIgnored: true,
    shouldCreateShipment: positive && !hasExistingShipment,
    shouldUpdateShipment: positive && hasExistingShipment,
    shouldDeleteShipment: !positive && hasExistingShipment,
    shouldCreateOrUpdateOrder: !shipmentOnly && positive && !hasExistingOrder,
  };
}
