...
/** 라움+초이문 공통 매입단가 저장 요청 정규화. pnlKey는 요청에 없다 — 서버가 연도+대차수로 양쪽을 다시 잠근다. */
export function normalizeSharedRaumPurchaseCostUpdates(updates) {
  if (!Array.isArray(updates) || updates.length === 0) throw scopedError('저장할 단가 변경이 없습니다.');
  if (updates.length > MAX_UPDATES) throw scopedError(`한 번에 최대 ${MAX_UPDATES}개 단가만 저장할 수 있습니다.`);
  const seen = new Set();
  return updates.map((raw, index) => {
    const major = Number(raw?.major);
    const identity = String(raw?.identity || '').trim();
    if (!Number.isInteger(major) || major <= 0 || !identity) {
      throw scopedError(`${index + 1}번째 변경의 차수·품목 정보가 올바르지 않습니다.`);
    }
    const unique = `${major}|${identity}`;
    if (seen.has(unique)) throw scopedError(`${major}차의 같은 품목이 중복 요청되었습니다.`);
    seen.add(unique);
    const expected = raumPnlSharedCostSnapshot(raw?.expected || []);
    if (!expected.length) throw scopedError(`${major}차 품목의 화면 기준값이 없어 저장할 수 없습니다.`);
    return { major, identity, expected, costPrice: parseRaumPurchaseCost(raw?.costPrice) };
  });
}

/** partnerCode 생략 시 라움+초이문 union 연도를 반환한다(공통 매입단가 화면 전용). */
export async function loadRaumPnlPurchaseCostYears(partnerCode) {
  const partners = partnerCode ? [String(partnerCode).trim().toLowerCase()] : SHARED_PARTNERS;
  const params = Object.fromEntries(partners.map((code, i) => [`pc${i}`, { type: sql.NVarChar, value: code }]));
  const result = await query(
    `SELECT DISTINCT m.OrderYear
       FROM WebRaumPnl AS m
      WHERE ISNULL(m.isDeleted, 0)=0 AND m.PartnerCode IN (${partners.map((_, i) => `@pc${i}`).join(',')})
...
const LOCKED_SHARED_MASTERS_SQL = `
  SELECT m.PnlKey, m.OrderYear, m.MajorWeek, m.PartnerCode
    FROM WebRaumPnl AS m WITH (UPDLOCK, HOLDLOCK)
   WHERE m.OrderYear=@yr
     AND m.MajorWeek=@major
     AND m.PartnerCode IN ('raum', 'choimun')
     AND ISNULL(m.isDeleted, 0)=0
   ORDER BY m.PartnerCode, m.PnlKey`;

export const RAUM_PNL_SHARED_PURCHASE_COST_WRITE_SQL = {
  masters: LOCKED_SHARED_MASTERS_SQL,
  items: LOCKED_ITEMS_SQL,
  item: RAUM_PNL_PURCHASE_COST_WRITE_SQL.item,
  parent: RAUM_PNL_PURCHASE_COST_WRITE_SQL.parent,
};

/**
 * 라움+초이문 공통 매입단가 저장. 같은 연도·대차수의 활성 라움+초이문 결산을 함께 잠근 뒤
 * PartnerCode+PnlKey+ItemKey+CostPrice 전체 snapshot을 대조하고, 존재하는 양쪽 매칭행만 한 트랜잭션에서 수정한다.
 * 한쪽 거래처에 결산이나 품목행이 없으면 새로 만들지 않고 존재하는 쪽만 수정한다.
 */
export async function saveRaumSharedPurchaseCosts({ orderYear, updates, actor }) {
  const year = String(orderYear || '').trim();
  if (!/^\d{4}$/.test(year)) throw scopedError('유효한 연도를 선택해 주세요.');
  const normalized = normalizeSharedRaumPurchaseCostUpdates(updates)
    .sort((a, b) => a.major - b.major || a.identity.localeCompare(b.identity));
  const safeActor = String(actor || 'user').slice(0, 100);

  return withTransaction(async tQuery => {
    const touchedMasters = new Map();
    const mastersByMajor = new Map();
    let changedRows = 0;

    for (const update of normalized) {
      let masters = mastersByMajor.get(update.major);
      if (!masters) {
        const result = await tQuery(LOCKED_SHARED_MASTERS_SQL, {
          yr: { type: sql.NVarChar, value: year },
          major: { type: sql.Int, value: update.major },
        });
        masters = result.recordset || [];
        mastersByMajor.set(update.major, masters);
      }
      if (!masters.length) {
        throw scopedError(`${update.major}차 손익계산서를 찾을 수 없습니다. 새로고침 후 다시 확인해 주세요.`, 409, 'STALE_SCOPE');
      }

      const matching = [];
      for (const master of masters) {
        const pnlKey = Number(master.PnlKey);
        const locked = await tQuery(LOCKED_ITEMS_SQL, { pnlKey: { type: sql.Int, value: pnlKey } });
        for (const row of (locked.recordset || [])) {
          if (raumPnlCostIdentity(row) === update.identity) {
            matching.push({ ...row, pnlKey, partnerCode: String(master.PartnerCode || '').trim().toLowerCase() });
          }
        }
      }
      if (!matching.length || !sameRaumPnlSharedCostSnapshot(matching, update.expected)) {
        throw scopedError(`${update.major}차 품목 단가가 다른 화면에서 변경되었습니다. 입력값을 보존한 뒤 새로고침해 다시 확인해 주세요.`, 409, 'STALE_COST');
      }
      for (const row of matching) {
        await tQuery(RAUM_PNL_PURCHASE_COST_WRITE_SQL.item, {
          cost: { type: sql.Float, value: update.costPrice },
          pnlKey: { type: sql.Int, value: row.pnlKey },
          itemKey: { type: sql.Int, value: Number(row.ItemKey) },
        });
        changedRows += 1;
        if (!touchedMasters.has(row.pnlKey)) {
          const master = masters.find(m => Number(m.PnlKey) === row.pnlKey);
          touchedMasters.set(row.pnlKey, {
            pnlKey: { type: sql.Int, value: row.pnlKey },
            yr: { type: sql.NVarChar, value: year },
            major: { type: sql.Int, value: update.major },
            pc: { type: sql.NVarChar, value: String(master.PartnerCode) },
          });
        }
      }
    }
