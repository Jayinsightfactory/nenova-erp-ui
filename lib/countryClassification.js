...
//
// CASE_CATEGORY(SQL)는 업무 규칙이 검증된 동작이라 재작성하지 않는다 — 이 모듈은 그 SQL이 만들어내는
// "카테고리 키 이름" 집합(PROFIT_REPORT_CATEGORY_KEYS)이 코드 쪽(profitReportClassification.js)과
// 정확히 같은지 테스트로만 보증하는 참조용 상수를 제공한다.
...
export function baseCountry(value) {
  ...
  for (const { key, tokens } of COUNTRY_TOKEN_MAP) {
    if (tokens.some((t) => token.includes(normalizeToken(t)))) return key;
  }
  return null;
}
