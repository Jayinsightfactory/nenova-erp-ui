...
export async function planEstimateOverflow(tQ, sql, originalPlan) {
  ...
  for (const group of [...plan].sort((a,b)=>a.row.OrderWeek.localeCompare(b.row.OrderWeek) || a.row.SdetailKey-b.row.SdetailKey)) {
    for (const c of group.changes) {
      ...
      if (prev && Number(prev.source.DetailCost)!==Number(row.DetailCost)) throw error('같은 다음 차수 품목으로 보내는 원본 단가가 다릅니다. 각각 확인해 주세요.');
      transfer.set(key,{source:row,week:next.OrderWeek,out:qty((prev?.out||0)+split.nextOut)});
      rows.push({sdateKey:row.SdateKey,prodKey:row.ProdKey,prodName:row.ProdName,unit:row.EstUnit,fromWeek:row.OrderWeek,toWeek:next.OrderWeek,oldQuantity:Number(row.DateEstQuantity),requestedQuantity:Number(c.item.quantity),currentQuantity:c.newDateEstQuantity,currentIncrease:split.currentIncrease,nextIncrease:split.nextIncrease,targetKey:key});
    }
    finalizeGroup(group);
  }
  ...
  for (const [key,t] of transfer) {
    ...
    targets.push(target);
    for (const row of rows.filter(r=>r.targetKey===key)) Object.assign(row,{shipmentDate:stamp(target.row.ShipmentDtm).slice(0,10),cost:Number(target.row.DetailCost),newShipment:target.newDetail});
  }
  const preview = {required:true,rows,planHash:hash({facts:[...facts],consumed,plan:plan.map(g=>({row:g.row,changes:g.changes})),targets})};
