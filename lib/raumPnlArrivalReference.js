...
// Exact scope: OrderYear + MajorWeek prefix + ProdKey. No fallback to another year/week.
...
export function buildRaumPnlArrivalReferences(items = [], rows = []) {
  ...
  for (const [itemKey, target] of wanted) {
    ...
    for (const row of byProd.get(target.prodKey) || []) {
      ...
      const candidate = { week, cost: converted, unit: target.unit || rawUnit || '', rawCost, rawUnit };
...
export async function loadRaumPnlArrivalReferences({ orderYear, major, items }, tQuery = query) {
  ...
  return buildRaumPnlArrivalReferences(items, response.recordset || []);
}
