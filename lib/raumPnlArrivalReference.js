...
export function buildRaumPnlArrivalReferences(items = [], rows = [], requestedMajor) {
  ...
  for (const [itemKey, target] of wanted) {
    ...
    for (const row of selectedRows) {
      ...
      const sourceStemCost = canonicalUnit(rawUnit) === '단' && sourceCount > 0 ? rawCost / sourceCount : null;
      ...
      const converted = target.unit
        ? (rawUnit ? (sourceStemCost != null && canonicalUnit(target.unit) !== '단'
          ? convertArrivalUnitCost(sourceStemCost, '송이', canonicalUnit(target.unit), product)
          : convertArrivalUnitCost(rawCost, canonicalUnit(rawUnit), canonicalUnit(target.unit), product)) : null)
        : rawCost;
      if (!week) continue;
      const candidate = {
        week,
