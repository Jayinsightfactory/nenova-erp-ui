...
export function buildSummary({ channel = null, mappings = null } = {}) {
  ...
  // cells[canonical] = { weeks: { [week]: { [year]: {total, supply, vat, count} } }, sourceNames:Set, status }
  const cells = new Map();
  const ensureRow = (canonical) => {
    if (!cells.has(canonical)) {
      cells.set(canonical, { canonicalName: canonical, weeks: {}, sourceNames: new Set(), status: '' });
    }
    return cells.get(canonical);
  };
  // 기본 업체 행을 먼저 깐다.
  ...
  for (const batch of batches) {
    const week = batch.orderWeek;
    const year = batch.salesYear;
    weeksSeen.add(week);
    yearsSeen.add(year);
    for (const r of batch.raw || []) {
      ...
      const cell = (row.weeks[week][year] = row.weeks[week][year] || { total: 0, supply: 0, vat: 0, count: 0 });
      ...
      // 전체 합계 집계
      totals.rawTotal += r.totalAmount;
      if (m.mappingStatus === '미매칭') {
        totals.unmatchedAmount += r.totalAmount;
        totals.unmatchedCount += 1;
      } else if (m.mappingStatus === '후보') {
        totals.candidateAmount += r.totalAmount;
        totals.candidateCount += 1;
      } else {
        totals.matchedAmount += r.totalAmount;
  ...
  const customers = Array.from(cells.values())
    .map(row => ({
  ...
  return {
    channel: channel || '전체',
    weeks: COMPARE_WEEKS,
    availableWeeks: Array.from(weeksSeen).sort(),
