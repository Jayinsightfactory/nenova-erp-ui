...
function romFuzzyHit(rom, p) {
  ...
  return latinWordsOf(p).some(w => {
    ...
    // 자모(자음모음) 기준 근사 — 0.62 는 carnation 같은 긴 일반단어까지 쓸어담아 0.75 유지
    return l / Math.max(rq.length, wq.length) >= 0.75;
  });
}

// 품목 이름 전체(한글+영문)를 발음열로 — 키입력마다 재계산하지 않게 캐시
const _hayRomCache = new WeakMap();
function hayRomOf(p) {
  if (_hayRomCache.has(p)) return _hayRomCache.get(p);
  const hay = [p.ProdName, p.DisplayName, p.catalogKorName, p.mappingKorName, p.catalogMatchKorName]
    .filter(Boolean).join(' ').toLowerCase();
  const v = phoneticSquash(romanizeKo(hay));
  _hayRomCache.set(p, v);
  return v;
}

/** 텍스트 편집 등에서 쓰는 느슨한 한글 검색 매칭 — 자모/발음열 기준, 단어 경계 무시 */
export function catalogKorLooseMatch(query, texts) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return true;
  const list = (texts || []).filter(Boolean).map(s => String(s));
  const hay = list.join(' ').toLowerCase();
  if (hay.includes(q) || hay.replace(/\s+/g, '').includes(q)) return true;
  if (list.some(t => jamoMatch(q, t))) return true;
  const rq = phoneticSquash(romanizeKo(q));
  if (rq.length < 3) return false;
  const hr = phoneticSquash(romanizeKo(hay));
  if (rq.length >= 4 && hr.includes(rq)) return true;
  // 단어·인접단어쌍 LCS 근사 (스카이라인→Sky Line)
  const words = hay.match(/[a-z0-9]{3,}/g) || [];
  const joined = words.map((w, i) => (i < words.length - 1 ? w + words[i + 1] : null)).filter(Boolean);
  return [...words, ...joined].some(w => {
    const wq = phoneticSquash(w);
    if (rq.length >= 4 && wq.includes(rq)) return true;
    return lcsLen(rq, wq) / Math.max(rq.length, wq.length) >= 0.75;
  });
}
...
export function filterProducts(products, { flower, countryFlower, search }) {
  ...
  return list.filter(p => {
    ...
    return tokens.every(tok => {
      ...
      // 자모(자음모음) 근사 — 단어·인접단어쌍(스카이라인→Sky Line) 발음열 매칭
      const rom = romanizeKo(tok);
      if (romFuzzyHit(rom, p)) return true;
      // 이름 전체 발음열 연속 포함 (붙여쓴 여러 단어)
      const rq = phoneticSquash(rom);
      return rq.length >= 4 && hayRomOf(p).includes(rq);
    });
  });
}
