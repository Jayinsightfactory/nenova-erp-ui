...
export function normalizeDetectedSalesPasteWeek(value = '') {
  ...
  const match = canonicalMatch || source.match(/(?:^|\s)(\d{4})\s*년\s*(\d{1,2})\s*-\s*(\d{1,2})(?:\s*차)?(?:\s|$)/);
  const yearSingleMatch = source.match(/(?:^|\s)(\d{4})\s*년\s*(\d{1,2})\s*차(?:\s|$)/);
  const shortMatch = source.match(/(?:^|\s)(\d{1,2})\s*-\s*(\d{1,2})(?:\s*차)?(?:\s|$)/);
  const singleMatch = source.match(/(?:^|\s)(\d{1,2})\s*차(?:\s|$)/);
