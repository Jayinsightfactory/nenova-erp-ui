...
export function buildSalesPasteWeekChoices(now = new Date()) {
  ...
  // 수입방 원문에는 네덜란드 3차 발주도 존재한다. DB OrderWeek의 동일한
  // WW-SS 형식을 사용하되 기존 01/02 뒤에 03을 명시적으로 노출한다.
  return result.flatMap((choice) => {
    if (!String(choice.week).endsWith('-02')) return [choice];
    const major = String(choice.week).split('-')[0];
    return [choice, {
      ...choice,
      week: `${major}-03`,
      label: `${Number(major)}-3차`,
      default: false,
    }];
  });
}
...
export function normalizeDetectedSalesPasteWeek(value = '') {
  ...
  const canonicalMatch = source.match(/(?:^|\s)(\d{4})\s*-\s*(\d{1,2})\s*-\s*(\d{1,2})(?:\s|$)/);
  const match = canonicalMatch || source.match(/(?:^|\s)(\d{4})\s*년\s*(\d{1,2})\s*-\s*(\d{1,2})\s*(?:차|치)?\s*[AB]?(?:\s|$)/i);
  const yearSingleMatch = source.match(/(?:^|\s)(\d{4})\s*년\s*(\d{1,2})\s*(?:차|치)(?:\s|$)/);
  const shortMatch = source.match(/(?:^|\s)(\d{1,2})\s*-\s*(\d{1,2})\s*(?:차|치)?\s*[AB]?(?:\s|$)/i);
  const singleMatch = source.match(/(?:^|\s)(\d{1,2})\s*(?:차|치)(?:\s|$)/);
  const parts = match || yearSingleMatch || shortMatch || singleMatch;
  if (!parts) return '';
  const year = match || yearSingleMatch ? Number(parts[1]) : null;
  const major = Number(match || yearSingleMatch ? parts[2] : parts[1]);
  const minor = Number(match ? parts[3] : (shortMatch && !yearSingleMatch ? parts[2] : 1));
  if (!Number.isInteger(major) || major < 1 || major > 52 || ![1, 2, 3].includes(minor)) return '';
  const short = `${String(major).padStart(2, '0')}-${String(minor).padStart(2, '0')}`;
  return year >= 2020 && year <= 2099 ? `${year}-${short}` : short;
}
...
export function resolveDetectedSalesPasteScope(detectedWeek, choices = []) {
  ...
  if (!week) return null;
  const explicitYear = /^\d{4}-/.test(week) ? week.slice(0, 4) : '';
  const shortWeek = explicitYear ? week.slice(5) : week;
  const match = choices.find(choice => choice.week === shortWeek && (!explicitYear || String(choice.year) === explicitYear));
...
export function salesPasteCountryContext(line = '', previous = '') {
  ...
  if (/베트남|vietnam/i.test(text)) return '베트남';
  if (/태국|thailand/i.test(text)) return '태국';
  if (/호주|australia/i.test(text)) return '호주';
  return previous;
}
...
export function countSalesPasteQuantityLines(text = '') {
  return normalizeSalesPasteInputText(text)
    .split('\n')
    .filter((line) => /-?\d+(?:\.\d+)?\s*(?:박\s*스|boxes?|box|bx|단|bunch(?:es)?|bun|송\s*이|개|대|스\s*팀|스\s*템|st|stems?|steam|ea)\s*(?:추가|취소)?\s*$/i.test(line.trim()))
    .length;
}
...
export function salesPasteUnitOptions() {
  return [...SALES_PASTE_UNITS];
}
