...
//
// 역할:
//   1. aggregateArrivalCosts(records)  — 순수함수, DB 없음, 다중 농장 MAX 집계
...
// ── 다중 AWB/농장 → 최고가 농장 기준 ────────────────────────────────────────
//
//   동일 ProdKey 가 여러 AWB·농장에서 입고된 경우:
//     arrivalCost = MAX(displayArrivalKRW)  (카탈로그·피벗은 품목당 1개 표시)
//   inQty = WarehouseDetail.OutQuantity — inQty=0 행은 제외.
//
// ── Read-only — DB 쓰기 없음 ─────────────────────────────────────────────────
...
import { buildOrderYearWeek, normalizeOrderWeek, resolveActiveOrderYear } from './orderUtils';
