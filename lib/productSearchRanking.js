...
import { getDisplayName, getEnglishOf, tokenizeForMatch } from './displayName.js';
...
function productText(product) {
  return [
    ...
    product?.sub,
    product?.ProdCode,
    ...(Array.isArray(product?.MappingAliases) ? product.MappingAliases : []),
  ].filter(Boolean).join(' ');
}

// 품목 객체는 검색어가 바뀌어도 그대로 재사용되므로 품목쪽 파생 문자열은 한 번만 만든다.
const compactProductTextCache = new WeakMap();
function compactProductText(product) {
  if (!product || typeof product !== 'object') return compact(productText(product));
  const hit = compactProductTextCache.get(product);
  if (hit !== undefined) return hit;
  const value = compact(productText(product));
  compactProductTextCache.set(product, value);
  return value;
}

const compactDisplayNameCache = new WeakMap();
function compactDisplayName(product) {
  if (!product || typeof product !== 'object') return compact(getDisplayName(product));
  const hit = compactDisplayNameCache.get(product);
  if (hit !== undefined) return hit;
  const value = compact(getDisplayName(product));
  compactDisplayNameCache.set(product, value);
  return value;
}
...
function directMatch(query, product) {
  ...
  return compactProductText(product).includes(wanted);
}
...
// 별칭 그룹은 검색어에만 의존한다. 품목마다 다시 계산하지 않는다.
let aliasGroupsQuery = null;
let aliasGroupsCache = null;
function getAliasGroups(query) {
  const key = String(query ?? '');
  if (aliasGroupsQuery === key) return aliasGroupsCache;
  const groups = tokenizeForMatch(key)
  ...
  aliasGroupsQuery = key;
  aliasGroupsCache = { groups, required: meaningful.length ? meaningful : groups };
  return aliasGroupsCache;
}

function hasDirectAlias(query, product) {
  const { groups, required } = getAliasGroups(query);
  if (!groups.length) return true;
  const target = compactProductText(product);
...
export function scoreProductSearchOptions(query, options = [], {
  ...
  const naturalScores = keyword ? new Map(scoreNaturalLanguageProducts(keyword, (options || []).map((option) => option?.__product || option), { limit: options.length || 200 }).allCandidates.map((item) => [item.source, item])) : new Map();
  const scored = (options || []).map((option, index) => {
    const product = option?.__product || option;
    const usage = getProductUsageRank(product);
    const natural = naturalScores.get(product);
    const matchScore = keyword ? Number(natural?.score || 0) * 100 : 0;
    const isDirect = keyword ? directMatch(keyword, product) : true;
    const popularity = Math.min(30, Math.log10(usage.rank + 1) * 10);
    const exactMatchBonus = keyword && compactDisplayName(product) === compactKeyword ? exactBonus : 0;
