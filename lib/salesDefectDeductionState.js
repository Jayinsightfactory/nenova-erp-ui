...
export function assertIncomingConfirmed(row = {}) {
  ...
  if (Boolean(row.ImportReviewRequired ?? row.importReviewRequired)) {
    throw new Error('수입부 보완 필요 상태를 먼저 해결해야 합니다.');
  }
  return true;
}
...
export function planDeductionRegistration({
  ...
  customerExists = true, productSalesRowExists = false, shipmentKey = null, cost = 0,
} = {}) {
  ...
  if (!productSalesRowExists || !(Number(shipmentKey) > 0)) {
    throw new Error('대상 차수에 해당 업체·품목의 EXE 확정 판매행이 없습니다.');
  }
