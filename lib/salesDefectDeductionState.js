...
export function explainDefectLedgerRegisterMiss(row = {}, { key, estimateKey, applyQuantity } = {}) {
  const deductionKey = Number(key || row.DeductionKey || row.deductionKey || 0);
  if (!row || Number(row.IsDeleted ?? row.isDeleted) === 1) {
    return `차감 행 ${deductionKey}를 찾을 수 없습니다. 새로고침 후 다시 확인하세요.`;
  }
  if (Number(row.EstimateKey ?? row.estimateKey ?? 0) === Number(estimateKey)) return '';
  if (!Boolean(row.ImportConfirmed ?? row.importConfirmed)) {
    return `차감 행 ${deductionKey}의 수입부 확정이 해제되어 등록하지 못했습니다. 새로고침 후 다시 확인하세요.`;
  }
  const remaining = Number(row.RemainingQuantity ?? row.remainingQuantity ?? row.Quantity ?? row.quantity ?? 0);
  if (Boolean(row.IsCarryoverLedger ?? row.isCarryoverLedger) && remaining + 0.0001 < Number(applyQuantity || 0)) {
    return `차감 행 ${deductionKey}의 잔여수량이 부족합니다. 현재 잔여 ${remaining}`;
  }
  return `차감 행 ${deductionKey}의 원장을 갱신하지 못했습니다. 화면을 새로고침한 뒤 다시 등록하세요.`;
}
