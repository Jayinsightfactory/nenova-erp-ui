...
export function buildRaumPnlSharedPurchaseCostMatrix(rows = [], scope = {}) {
  ...
  // The loader supplies newest settlement first and each settlement in its saved
  // estimate row order (Seq, ItemKey). Map insertion order therefore preserves
  // the existing estimate layout; do not alphabetically re-sort this shared view.
  const items = [...itemMap.values()].map(item => ({
    ...
    cells: weeks.map(week => {
      ...
      return {
        ...
        arrivalReferences: cellRows[0]?.arrivalReferences || [],
        arrivalReferenceError: cellRows[0]?.arrivalReferenceError || '',
...
/**
 * Display-only join for purchase-cost management. The returned shape is
 * `{ weeks, items }`; each `items[].cells` entry is either null (neither side
 * has that week) or `{ key, major, shared, shilla }`. `shared` is the existing
 * Raum+Choimun cell and `shilla` is the existing Shilla-only cell, so their
 * snapshots and save scopes never mix.
 */
export function buildRaumPnlCombinedPurchaseCostMatrix(sharedRows = [], shillaRows = [], scope = {}) {
  const orderYear = String(scope.orderYear ?? '').trim();
  const isolatedPartnerCode = String(scope.partnerCode || 'shilla').trim().toLowerCase();
  const isolatedPartnerLabel = String(scope.partnerLabel || '신라').trim();
  if (!/^\d{4}$/.test(orderYear)) return { weeks: [], items: [], conflicts: [] };

  const shared = buildRaumPnlSharedPurchaseCostMatrix(sharedRows, { orderYear });
  const scopedShillaRows = (Array.isArray(shillaRows) ? shillaRows : []).filter(row =>
    String(row?.orderYear ?? row?.OrderYear ?? '').trim() === orderYear &&
    String(row?.partnerCode ?? row?.PartnerCode ?? '').trim().toLowerCase() === isolatedPartnerCode &&
    positiveProdKey(row?.pnlKey ?? row?.PnlKey) != null &&
    positiveProdKey(row?.itemKey ?? row?.ItemKey) != null &&
    Number.isInteger(Number(row?.major ?? row?.MajorWeek)) && Number(row?.major ?? row?.MajorWeek) > 0
  );
  const pnlKeysByMajor = new Map();
  for (const row of scopedShillaRows) {
    const major = Number(row.major ?? row.MajorWeek);
    if (!pnlKeysByMajor.has(major)) pnlKeysByMajor.set(major, new Set());
    pnlKeysByMajor.get(major).add(positiveProdKey(row.pnlKey ?? row.PnlKey));
  }
  const ambiguousMajors = new Set([...pnlKeysByMajor].filter(([, pnlKeys]) => pnlKeys.size > 1).map(([major]) => major));
  const conflicts = [...ambiguousMajors].sort((a, b) => b - a)
    .map(major => `${major}차 ${isolatedPartnerLabel} 결산이 여러 건입니다. ${isolatedPartnerLabel} 단가를 표시하거나 저장할 수 없습니다.`);
  const usableShillaRows = scopedShillaRows.filter(row => !ambiguousMajors.has(Number(row.major ?? row.MajorWeek)));
  const shilla = buildRaumPnlPurchaseCostMatrix(usableShillaRows, { orderYear, partnerCode: isolatedPartnerCode });
  const majorSet = new Set([...shared.weeks, ...shilla.weeks].map(week => Number(week.major)).filter(Number.isInteger));
  const weeks = [...majorSet].sort((a, b) => b - a).map(major => ({ key: String(major), major, label: `${major}차` }));

  const shillaByIdentity = new Map();
  for (const item of shilla.items) {
    const combinedIdentity = item.prodKey != null ? item.identity : `${isolatedPartnerCode}:${item.identity}`;
    shillaByIdentity.set(combinedIdentity, { ...item, identity: combinedIdentity });
  }
  const items = [];
  const seen = new Set();
  for (const item of shared.items) {
    seen.add(item.identity);
    items.push({ ...item, shared: item, shilla: shillaByIdentity.get(item.identity) || null });
  }

  // `buildRaumPnlPurchaseCostMatrix` alphabetizes its standalone display. For the
  // combined view, append Shilla-only identities in incoming row order instead.
  for (const row of usableShillaRows) {
    const baseIdentity = identity(row);
    if (!baseIdentity) continue;
    const combinedIdentity = positiveProdKey(row?.prodKey ?? row?.ProdKey) != null ? baseIdentity : `${isolatedPartnerCode}:${baseIdentity}`;
    const item = shillaByIdentity.get(combinedIdentity);
    if (item && !seen.has(combinedIdentity)) {
      seen.add(combinedIdentity);
      items.push({ ...item, shared: null, shilla: item });
    }
  }
  for (const [combinedIdentity, item] of shillaByIdentity) {
    if (!seen.has(combinedIdentity)) items.push({ ...item, shared: null, shilla: item });
  }
  ...
  return {
    ...
    items: items.map(item => {
      const sharedByMajor = new Map((item.shared?.cells || []).filter(Boolean).map(cell => [cell.major, cell]));
      const shillaByMajor = new Map((item.shilla?.cells || []).filter(Boolean).map(cell => [cell.major, cell]));
      return {
        identity: item.identity,
        name: item.name,
        prodKey: item.prodKey,
        prodName: item.prodName,
        shillaName: item.shilla?.name || null,
        shillaProdName: item.shilla?.prodName || null,
        unit: item.unit,
        isCustom: item.isCustom,
        cells: weeks.map(week => {
          const sharedCell = sharedByMajor.get(week.major) || null;
          const shillaCell = shillaByMajor.get(week.major) || null;
          return sharedCell || shillaCell
            ? { key: `${week.key}|${item.identity}`, major: week.major, shared: sharedCell, shilla: shillaCell }
            : null;
        }),
      };
    }),
