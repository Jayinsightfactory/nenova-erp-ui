...
export function buildRaumPnlCombinedPurchaseCostMatrix(sharedRows = [], shillaRows = [], scope = {}) {
  ...
  const scopedShillaRows = (Array.isArray(shillaRows) ? shillaRows : []).filter(row =>
    String(row?.orderYear ?? row?.OrderYear ?? '').trim() === orderYear &&
    String(row?.partnerCode ?? row?.PartnerCode ?? '').trim().toLowerCase() === 'shilla' &&
  ...
  const conflicts = [...ambiguousMajors].sort((a, b) => b - a)
    .map(major => `${major}차 신라 결산이 여러 건입니다. 신라 단가를 표시하거나 저장할 수 없습니다.`);
  const usableShillaRows = scopedShillaRows.filter(row => !ambiguousMajors.has(Number(row.major ?? row.MajorWeek)));
  const shilla = buildRaumPnlPurchaseCostMatrix(usableShillaRows, { orderYear, partnerCode: 'shilla' });
  ...
  for (const item of shilla.items) {
    const combinedIdentity = item.prodKey != null ? item.identity : `shilla:${item.identity}`;
  ...
  for (const row of usableShillaRows) {
    ...
    const combinedIdentity = positiveProdKey(row?.prodKey ?? row?.ProdKey) != null ? baseIdentity : `shilla:${baseIdentity}`;
