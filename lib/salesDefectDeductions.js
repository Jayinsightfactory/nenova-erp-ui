...
export function matchSalesDefectRows(rows, {
  ...
  return (rows || []).map((raw, index) => {
    ...
    return {
      ...
      matchedProductDbName: resolvedProdKey ? (product?.prodName || '') : '',
...
async function loadManagerOptions() {
  ...
  return [...byId.values()].sort((a, b) => {
    const ai = DEFAULT_MANAGER_OPTIONS.indexOf(a.managerName);
    const bi = DEFAULT_MANAGER_OPTIONS.indexOf(b.managerName);
...
export async function saveManagerOption({ managerId = '', managerName = '', user } = {}) {
  ...
  await withTransaction(async (tQuery) => {
    const existing = await tQuery(
      `SELECT TOP 1 ManagerKey FROM WebSalesDefectManager WHERE ManagerId=@id AND IsDeleted=0`,
