...
export async function resolveEstimateContext({ year, week, custKey, prodKey }, q = query) {
  ...
  return {
    ...
    targetIsFix: Number(targetRow?.DetailFix || 0) === 1,
...
async function enrichSupportDistributionCosts(rows, { year, week } = {}) {
  ...
  const customerTargets = new Map(await Promise.all(customerKeys.map(async (custKey) => {
    ...
    return [custKey, Boolean(selectExeEstimateTargetCandidate(target.recordset || []))];
  })));
    ...
    for (const raw of rows) {
      ...
      if (key) {
        ...
        const importReset = before && (
          Number(before.custKey || 0) !== Number(item.custKey || 0)
          || Number(before.prodKey || 0) !== Number(item.prodKey || 0)
          || Number(before.quantity || 0) !== Number(item.quantity || 0)
          || String(before.sourceUnit || '') !== String(item.sourceUnit || '')
