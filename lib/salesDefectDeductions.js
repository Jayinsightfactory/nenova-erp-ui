...
async function enrichSupportDistributionCosts(rows, { year, week } = {}) {
  ...
  return (rows || []).map((row) => {
    ...
    const enriched = {
      ...
      customerTargetRegistered: Boolean(context?.shipmentKey),
      carryoverClassification: context?.shipmentKey ? 'REGISTERED_CUSTOMER' : 'SURPLUS',
    ...
    const eligibility = evaluateDefectRegistrationEligibility({ row: enriched, context, exactExistingEstimate: Boolean(exactExisting) });
    return {
      ...enriched,
      ...
      targetShipmentKey: context?.shipmentKey || null,
