...
import { buildProductSuggestions, matchImportRows } from './orderImportMatch.js';
import { loadMappings } from './parseMappings.js';
import { loadImportUnits } from './orderImportUnits.js';
import { loadImportCustomerMappings, resolveImportCustomer } from './orderImportCustomerMatch.js';
import {
...
function scoreCustomerLookup(input, customer) {
  ...
  if (!wanted) return 0;
  const fields = [customer.CustName, customer.CustCode, customer.OrderCode]
    .map(normalizeSearch).filter(Boolean);
  if (fields.some((field) => field === wanted)) return 100;
  if (fields.some((field) => field.includes(wanted) || wanted.includes(field))) return 85;
...
function rankCustomerLookup(input, customers, limit = 20) {
  return (customers || [])
    .map((customer) => ({
      customer,
    ...
    .sort((a, b) => b.score - a.score || String(a.customer.CustName).localeCompare(String(b.customer.CustName)))
    .slice(0, limit)
    .map(({ customer, score }) => ({ ...customer, MatchScore: score }));
}

/**
 * 불량차감 업로드도 붙여넣기/수입주문과 같은 품목 매칭 엔진을 사용한다.
 *
 * 공유되는 데이터와 규칙:
 * - Product + OrderDetail 단위 이력(prodUnitMap)
 * - data/order-mappings.json 학습 매핑
 * - data/order-import-units.json 단위 학습
 * - 국가/장미 길이/믹스박스/운임 품목 예외 및 후보 모호성 가드
 */
export function matchSalesDefectRows(rows, {
  ...
  savedMappings = null,
  unitCatalog = null,
  savedCustomerMappings = null,
} = {}) {
  const productContext = {
    allProducts: allProducts || [],
    productByKey,
    prodUnitMap,
    savedMappings: savedMappings || loadMappings(true),
    unitCatalog: unitCatalog || loadImportUnits(true),
  };
  const customerMappings = savedCustomerMappings || loadImportCustomerMappings(true);
  return (rows || []).map((raw, index) => {
    ...
    const customer = resolveImportCustomer(row.customerName, customers, {
      ...
      savedMappings: customerMappings,
    });
    const inputName = [row.productName, row.colorName].filter(Boolean).join(' ').trim();
    const product = matchImportRows([{
    ...
    const resolvedProdKey = row.prodKey || product?.prodKey || null;
    const resolvedProductName = product?.displayName || product?.prodName || row.productName;
    return {
      ...row,
      rowNo: raw.sourceRowNo || raw.rowNo || index + 1,
      ...
      matchedProductName: resolvedProdKey ? resolvedProductName : '',
      unit: resolvedProdKey ? (product?.unit || row.sourceUnit || '') : (row.sourceUnit || ''),
      ...
      productConfidence: product?.confidence || (resolvedProdKey ? 1 : 0),
      productConfidenceLabel: product?.confidenceLabel || (resolvedProdKey ? 'manual' : 'none'),
      customerFromMapping: customer.fromMapping,
      customerMappingKey: customer.mappingKey,
      customerSuggestions: customer.suggestions,
      productSuggestions: product?.suggestedProducts || [],
      unitSource: product?.unitSource || '',
      unitMatchType: product?.unitMatchType || '',
      mappingMatchType: product?.mappingMatchType || null,
      mappingMatchKey: product?.mappingMatchKey || null,
      fromMapping: Boolean(product?.fromMapping),
      fallbackSuspect: Boolean(product?.fallbackSuspect),
      ambiguityReason: product?.ambiguityReason || null,
      needsReview: !customer.custKey || !resolvedProdKey || !row.quantity || Boolean(product?.ambiguousCountry),
    };
  });
}
...
export async function loadLookupData({ q = '', kind = '' } = {}) {
  ...
  if (kind === 'customer') {
    ...
    if (r.recordset.length > 0 || !keyword) return { customers: r.recordset };
    const all = await query(
      `SELECT TOP 1000 CustKey, CustCode, CustName, OrderCode, Manager
         FROM Customer
        WHERE ISNULL(isDeleted,0)=0
        ORDER BY CustName`,
      {},
    );
    return { customers: rankCustomerLookup(keyword, all.recordset) };
  }
  if (kind === 'product') {
    const r = await query(
    ...
    if (r.recordset.length > 0 || !keyword) return { products: r.recordset };
    const all = await query(
      `SELECT TOP 2000 p.ProdKey, p.ProdCode, p.ProdName, p.DisplayName,
              p.FlowerName, p.CounName, p.OutUnit, p.EstUnit
         FROM Product p
        WHERE ISNULL(p.isDeleted,0)=0
        ORDER BY p.CounName, p.FlowerName, p.ProdName`,
    ...
    const suggestions = buildProductSuggestions(keyword, all.recordset, { limit: 20, minScore: 35 });
    return {
      products: suggestions.map((suggestion) => byKey.get(Number(suggestion.prodKey))).filter(Boolean),
    };
  }
  const r = await query(
...
export async function loadMatchContext() {
  const [customerResult, productResult, unitResult, farmResult] = await Promise.all([
    query(`SELECT CustKey, CustCode, CustName, OrderCode FROM Customer WHERE ISNULL(isDeleted,0)=0 ORDER BY CustName`, {}),
    query(`SELECT ProdKey, ProdCode, ProdName, DisplayName, FlowerName, CounName, OutUnit, EstUnit
    ...
    query(`SELECT ProdKey,
             ...
             FROM OrderDetail
            WHERE ISNULL(isDeleted,0)=0 AND ProdKey IS NOT NULL
            GROUP BY ProdKey`, {}),
  ...
  const prodUnitMap = {};
  for (const row of unitResult.recordset || []) {
    const box = Number(row.TotalBox || 0);
    const bunch = Number(row.TotalBunch || 0);
    const steam = Number(row.TotalSteam || 0);
    if (box === 0 && bunch === 0 && steam === 0) continue;
    if (bunch >= box && bunch >= steam) prodUnitMap[row.ProdKey] = '단';
    else if (steam >= box && steam >= bunch) prodUnitMap[row.ProdKey] = '송이';
  ...
  return {
    ...
    savedCustomerMappings: loadImportCustomerMappings(true),
...
function changeSummary(before, after, action) {
  ...
  if (before.productName !== after.productName || Number(before.prodKey || 0) !== Number(after.prodKey || 0)) {
    changes.push(`품목 매칭 ${before.productName || '-'} (#${before.prodKey || '-'} ) → ${after.productName || '-'} (#${after.prodKey || '-'} )`);
  }
...
export async function listDeductions({ year, week, manager = '', includeDeleted = false, history = false } = {}) {
  ...
  const where = `d.OrderYear=@year AND d.OrderWeek=@week
                ...
                AND (@manager=N'' OR COALESCE(NULLIF(d.CreatedBy,N''), NULLIF(d.UpdatedBy,N''), N'')=@manager)`;
  ...
  const managers = await query(
    `SELECT DISTINCT
            COALESCE(NULLIF(d.CreatedBy,N''), NULLIF(d.UpdatedBy,N''), N'') AS ManagerId,
            COALESCE(NULLIF(d.CreatedByName,N''), NULLIF(d.UpdatedByName,N''),
                     COALESCE(NULLIF(d.CreatedBy,N''), NULLIF(d.UpdatedBy,N''), N'')) AS ManagerName
       FROM WebSalesDefectDeduction d
      WHERE d.OrderYear=@year AND d.OrderWeek=@week AND d.IsDeleted=0
        AND COALESCE(NULLIF(d.CreatedBy,N''), NULLIF(d.UpdatedBy,N''), N'')<>N''
      ORDER BY ManagerName, ManagerId`, params,
  );
  const result = {
    rows: rows.recordset.map(snapshot),
    managerOptions: managers.recordset.map((item) => ({
      managerId: String(item.ManagerId || ''),
      managerName: String(item.ManagerName || item.ManagerId || ''),
    })),
  };
  if (history) {
