...
export async function loadLookupData({ q = '', kind = '' } = {}) {
  ...
  if (kind === 'customer') {
    const r = await query(
      `SELECT TOP 80 CustKey, CustCode, CustName, CustArea, OrderCode, Manager
         FROM Customer
    ...
    if (r.recordset.length > 0 || !keyword) {
      return { customers: keyword ? rankCustomerLookup(keyword, r.recordset, 80) : r.recordset };
    }
  ...
  if (kind === 'product') {
    ...
    if (r.recordset.length > 0 || !keyword) {
      ...
      const suggestions = buildProductSuggestions(keyword, r.recordset, { limit: 40, minScore: 20 });
      const byKey = new Map(r.recordset.map((item) => [Number(item.ProdKey), item]));
      const ranked = suggestions.map((suggestion) => byKey.get(Number(suggestion.prodKey))).filter(Boolean);
...
function changeSummary(before, after, action) {
  if (!before) {
    ...
    if (after.prodKey) {
      created.push(`품목 매칭 ${after.matchedProductName || after.productName || '-'} (#${after.prodKey})`);
...
async function getStoredSnapshot(q, key, { lock = false } = {}) {
  const result = await q(
    `SELECT d.*,
       ...
       FROM WebSalesDefectDeduction ${lock ? 'WITH (UPDLOCK,HOLDLOCK) ' : ''}AS d
