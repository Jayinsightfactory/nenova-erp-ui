...
/**
 * 불량차감 업로드는 과거 엑셀/붙여넣기 학습값을 품목 매칭에 사용하지 않는다.
 * 업로드된 품종·품명은 원문으로만 표시하고, Product DB 후보는 화면 검색에서
 * 사용자가 직접 선택한 경우에만 ProdKey를 부여한다. 기존 선택값(ProdKey)은
 * 저장된 원장 재편집/재매칭 시 보존한다.
 */
export function matchSalesDefectRows(rows, {
  ...
  return (rows || []).map((raw, index) => {
    ...
    const customer = resolveImportCustomer(row.customerName, customers, {
      ...
      // 이전 엑셀 업로드로 저장된 거래처 매핑도 이 화면에서는 사용하지 않는다.
      savedMappings: {},
    });
