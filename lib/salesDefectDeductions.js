...
export async function cancelIncomingDeductions({ year, week, rows, user } = {}) {
  ...
  await withTransaction(async (tQuery) => {
    for (const key of keys) {
      ...
      await tQuery(
        `UPDATE WebSalesDefectDeduction
            SET ImportConfirmed=0, ImportConfirmedBy=NULL, ImportConfirmedByName=NULL,
...
export async function registerDeductions({ year, week, ids, deductionType = '불량차감', user, overrides = {} } = {}) {
  ...
  await withTransaction(async (tQuery) => {
    ...
    for (const { key, dbRow, ctx, typeCode } of preflight) {
      ...
      registered.push({
        deductionKey: key, estimateKey, estimateAction, quantity: qty, unit: ctx.unit || normalizeUnit(dbRow.SourceUnit) || '단',
