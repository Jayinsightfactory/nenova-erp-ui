...
async function refreshSalesFeed({fetchImpl=fetch,from,to,maxPages=DEFAULT_MAX_PAGES,signal,timeoutMs=8_000,setTimeoutImpl=setTimeout,clearTimeoutImpl=clearTimeout}) {
  ...
  for(let page=0;page<maxPages;page++) {
    ...
    if(!data.hasMore) return {messages,complete:true,pages:page+1};
  ...
  return {messages,complete:false,pages:maxPages};
}

function startBoundedAutoRefresh({run,isEligible=()=>true,intervalMs=REFRESH_INTERVAL_MS,maxBackoffMs=60_000,now=()=>Date.now(),setIntervalImpl=setInterval,clearIntervalImpl=clearInterval}) {
...
module.exports={REFRESH_INTERVAL_MS,DEFAULT_MAX_PAGES,kstCalendarDate,messageIdentity,readSalesFeedPage,refreshSalesFeed,startBoundedAutoRefresh};
