...
function recentSalesPeriod(now=new Date()) {
  const to=kstCalendarDate(now);
  const from=kstCalendarDate(new Date(Date.parse(`${to}T00:00:00+09:00`)-6*86400000));
  return {from,to};
}
...
module.exports={REFRESH_INTERVAL_MS,DEFAULT_MAX_PAGES,kstCalendarDate,recentSalesPeriod,messageIdentity,validInboxScope,validKstPeriod,isAutoRefreshEligible,isCurrentRefresh,shouldBufferIncoming,readSalesFeedPage,refreshSalesFeed,startBoundedAutoRefresh};
