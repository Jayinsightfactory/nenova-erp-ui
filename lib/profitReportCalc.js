...
// 원본 매출원가 양식 본표에서 기말상품재고액 수식이 실제로 박혀 있는 카테고리들이다.
// 기말상품재고액 = (매입액 + 그외통관비) / 해당 카테고리 그 차수 매입수량 * 마지막 ProductStock 환산수량.
// 이 목록은 원본 workbook 재현 검증(22~28차 fixture) 전용이며, 운영 계산은 아래
// weeklyAverageInventoryUnitCost()가 모든 국가·품종에 같은 공식을 적용한다.
...
export const AUSTRALIA_INVENTORY_CATEGORY = '호주';

/** 공제·기타(미분류)는 재고 원장이 없다. 그 밖 모든 국가·품종이 재고 평가 대상이다. */
export const NON_INVENTORY_REPORT_CATEGORIES = Object.freeze(['공제', '기타(미분류)']);

export function usesWeeklyAverageInventoryCategory(category) {
  const key = String(category || '');
  return !!key && !NON_INVENTORY_REPORT_CATEGORIES.includes(key);
}

/** 01차는 전년도 52차, 그 외는 같은 연도 직전 대차수. */
export function previousMajorOf(orderYear, major) {
  const year = String(orderYear || '');
  const currentMajor = Number(major);
  if (!/^\d{4}$/.test(year) || !Number.isFinite(currentMajor) || currentMajor < 1) return null;
  if (currentMajor <= 1) return { orderYear: String(Number(year) - 1), major: '52' };
  return { orderYear: year, major: String(currentMajor - 1).padStart(2, '0') };
}

/**
 * 그 차수 매입 평균원가 단가 — 원본 매출원가 양식의 기말상품재고액 수식과 같다.
 *
 *   단가 = (매입액 + 그외통관비) ÷ 그 차수 매입수량
 *        = (구매금액(외화)×과세환율 + 포워딩(USD)×과세환율 + 그외통관비) ÷ 매입수량
 *
 * 외화 금액이 있으면 그 차수 과세환율이 반드시 있어야 하고, 매입수량이 없으면 계산하지 않는다(null).
 * 카테고리 제한이 없다 — 모든 국가·품종에 같은 공식을 쓴다.
 */
export function weeklyAverageInventoryUnitCost({
  purchaseForeign = 0, forwardingForeign = 0, taxableRate, customsCost = 0, purchaseQty,
} = {}) {
  const qty = Number(purchaseQty);
  if (!(qty > 0)) return null;
  const goods = Number(purchaseForeign || 0);
  const forwarding = Number(forwardingForeign || 0);
  const customs = Number(customsCost || 0);
  const rate = taxableRate == null ? null : Number(taxableRate);
  if ((Math.abs(goods) > 0.000001 || Math.abs(forwarding) > 0.000001)
    && !(Number.isFinite(rate) && rate > 0)) return null;
  const unitCost = ((goods + forwarding) * Number(rate || 0) + customs) / qty;
  return Number.isFinite(unitCost) ? unitCost : null;
}

/** 그 차수 매입이 없을 때 쓰는 직전 차수 재고 단가 — 첫 입고 시점 환율이 그대로 유지된다. */
export function carriedInventoryUnitCost({ beginValue, beginQty } = {}) {
  const qty = Number(beginQty);
  const value = beginValue == null ? null : Number(beginValue);
  if (!(qty > 0) || value == null || !Number.isFinite(value)) return null;
  return value / qty;
}

/**
 * 직전 단가 이어쓰기 없이 그 차수 원천만으로 기말상품재고액을 만든다.
 * 그 차수 매입 평균원가 → 그 차수 품목별 검증 단가 → 원본 workbook 증거 순이다.
 */
export function resolveNonLayeredInventoryClosing({
  category, purchaseForeign, forwardingForeign, taxableRate, customsCost, purchaseQty, stockQty,
  conversionMissing, unitMismatch, directValue, historicalValue, directStatus, historicalStatus, directSources, historicalSources,
  declaredValue, declaredStatus, declaredSources,
} = {}) {
  const ending = Number(stockQty);
  const declared = declaredValue == null ? null : Number(declaredValue);
  const hasDeclared = declared != null && Number.isFinite(declared);
  if (usesWeeklyAverageInventoryCategory(category) && !conversionMissing && !unitMismatch && Number.isFinite(ending)) {
    if (ending === 0) {
      if (hasDeclared) {
        return {
          value: declared,
          status: declaredStatus || 'VERIFIED_DECLARED_INVENTORY',
          method: 'declared_inventory_amount',
          sources: declaredSources || ['declared-inventory-amount'],
        };
      }
      return { value: 0, status: 'VERIFIED_CATEGORY_AVERAGE', method: 'no_stock', sources: ['no-stock'] };
    }
    const unitCost = weeklyAverageInventoryUnitCost({
      purchaseForeign, forwardingForeign, taxableRate, customsCost, purchaseQty,
    });
    if (unitCost != null) {
      return {
        value: unitCost * ending,
        status: 'VERIFIED_CATEGORY_AVERAGE',
        method: 'weekly_average_cost',
        sources: ['weekly-average-cost:(매입액+그외통관비)/매입수량*ProductStock'],
      };
    }
  }
  if (hasDeclared && !(Number.isFinite(ending) && ending > 0)) {
    return {
      value: declared,
      status: declaredStatus || 'VERIFIED_DECLARED_INVENTORY',
      method: 'declared_inventory_amount',
  ...
  if (directValue != null && Number.isFinite(Number(directValue))) {
    return {
      value: Number(directValue),
      status: directStatus || 'VERIFIED',
      method: 'item_evidence',
      sources: directSources || [],
    };
  }
  if (historicalValue != null && Number.isFinite(Number(historicalValue))) {
    return {
      value: Number(historicalValue),
      status: historicalStatus || 'VERIFIED_HISTORICAL_WORKBOOK',
      method: 'historical',
...
/**
 * 한 차수의 기말상품재고액 — 원본 매출원가 양식 공식을 그대로 쓴다.
 *
 *   1) 그 차수 매입이 있으면  : 그 차수 매입 평균원가(그외통관비 포함) × 기말 재고수량
 *                             — 매입환율은 그 차수 과세환율. 재고판매만 있는 차수는 바꾸지 않는다.
 *   2) 그 차수 매입이 없으면  : 직전 차수 재고 단가 × 기말 재고수량
 *                             (재고가 매입됐을 때 환율·통관단가가 그대로 유지된다)
 *   3) 재고수량이 0이거나 없어도 기말상품재고액이 적혀 있으면 그 금액(실제 재고)
 *   4) 둘 다 계산 불가면      : 그 차수 품목별 검증 단가 → 원본 workbook 증거
 *
 * 2)가 있기 때문에 새 입고가 없는 차수에는 그때 환율이 다시 적용되지 않는다.
 */
export function resolveInventoryClosing({
  category, beginQty, beginValue, purchaseQty, purchaseForeign, forwardingForeign, taxableRate, customsCost,
  endQty, conversionMissing, unitMismatch, directValue, historicalValue, directStatus, historicalStatus,
  directSources, historicalSources, declaredValue, declaredStatus, declaredSources,
} = {}) {
  const ending = Number(endQty);
  const evaluable = usesWeeklyAverageInventoryCategory(category) && !conversionMissing && !unitMismatch;
  const declared = declaredValue == null ? null : Number(declaredValue);
  const hasDeclared = declared != null && Number.isFinite(declared);
  if (evaluable && Number.isFinite(ending) && ending > 0) {
    const weeklyUnit = weeklyAverageInventoryUnitCost({
      purchaseForeign, forwardingForeign, taxableRate, customsCost, purchaseQty,
    });
    if (weeklyUnit != null) {
      return {
        value: weeklyUnit * ending,
        status: 'VERIFIED_CATEGORY_AVERAGE',
        method: 'weekly_average_cost',
        sources: ['weekly-average-cost:(매입액+그외통관비)/매입수량*ProductStock'],
      };
    }
    const carriedUnit = carriedInventoryUnitCost({ beginValue, beginQty });
    if (carriedUnit != null) {
      return {
        value: carriedUnit * ending,
        status: 'VERIFIED_CARRIED_UNIT_COST',
        method: 'carried_unit_cost',
        sources: ['carried-unit-cost:전차수 재고단가 유지(이번 차수 매입 없음)'],
      };
    }
  }
  // 수량이 없어도 금액이 있으면 실제 재고다. 수량이 있으면 위 공식이 우선이다.
  if (hasDeclared && !(Number.isFinite(ending) && ending > 0)) {
    return {
      value: declared,
      status: declaredStatus || 'VERIFIED_DECLARED_INVENTORY',
  ...
  return resolveNonLayeredInventoryClosing({
    category, purchaseForeign, forwardingForeign, taxableRate, customsCost, purchaseQty,
    stockQty: endQty, conversionMissing, unitMismatch,
    directValue, historicalValue, directStatus, historicalStatus, directSources, historicalSources,
  });
}

/**
 * 이번 차수 기초상품재고액 = 전차수 기말상품재고액.
 * 전차수가 확정되어 있으면 그 확정 기말재고를 쓰고, 아니면 전차수 기말재고를 같은 공식으로 재현한다.
 * 전차수에 매입이 없었다면 전전차수 재고 단가가 그대로 이어진다.
 */
export function reconstructPreviousClosing({
  category,
  confirmedPreviousF,
  confirmedPreviousSources,
  prevPrevClosing,
  prevBeginQty,
  prevPurchaseQty,
  prevPurchaseForeign,
  prevForwardingForeign,
  prevTaxableRate,
  prevCustomsCost,
  prevEndQty,
  prevConversionMissing,
  prevUnitMismatch,
  prevDirectValue,
  prevHistoricalValue,
  prevDirectStatus,
  prevHistoricalStatus,
  prevDirectSources,
  prevHistoricalSources,
  prevDeclaredValue,
  prevDeclaredStatus,
  prevDeclaredSources,
} = {}) {
  if (confirmedPreviousF != null && Number.isFinite(Number(confirmedPreviousF))) {
    return {
      value: Number(confirmedPreviousF),
      status: 'VERIFIED_CONFIRM_SNAPSHOT',
      method: 'confirm_snapshot',
      sources: confirmedPreviousSources || ['confirm-snapshot:previous-F'],
    };
  }
  return resolveInventoryClosing({
    category,
    beginQty: prevBeginQty,
    beginValue: prevPrevClosing?.value ?? null,
    purchaseQty: prevPurchaseQty,
    purchaseForeign: prevPurchaseForeign,
    forwardingForeign: prevForwardingForeign,
    taxableRate: prevTaxableRate,
    customsCost: prevCustomsCost,
    endQty: prevEndQty,
    conversionMissing: prevConversionMissing,
    unitMismatch: prevUnitMismatch,
    directValue: prevDirectValue,
    historicalValue: prevHistoricalValue,
    directStatus: prevDirectStatus,
    historicalStatus: prevHistoricalStatus,
    directSources: prevDirectSources,
    historicalSources: prevHistoricalSources,
    declaredValue: prevDeclaredValue,
    declaredStatus: prevDeclaredStatus,
    declaredSources: prevDeclaredSources,
  });
}
...
export function computeCategoryAverageInventoryValue(input = {}) {
  if (!CATEGORY_AVERAGE_INVENTORY_KEYS.includes(String(input.category || ''))) return null;
...
export function isDeclaredEndingInventory(stock) {
  return String(stock?.priceEvidenceStatus || '') === 'VERIFIED_DECLARED_INVENTORY'
    && stock?.evidenceValue != null
    && Number.isFinite(Number(stock.evidenceValue));
}

/** 재고수량이 없으면 기말상품재고액 선언 금액을 받을 수 있다. 수량이 있으면 공식이 우선이다. */
export function allowsDeclaredEndingInventoryAmount(stock) {
  return n0(stock?.endQty) === 0;
}
 ...
 * 수량이 없어도 VERIFIED_DECLARED_INVENTORY 금액이 있으면 실제 재고로 채택한다. */
export function computeAutoEndingStock(stock) {
  if (!stock) return null;
  if (isDeclaredEndingInventory(stock)) return Number(stock.evidenceValue);
...
export function endingStockSourceKind(stock) {
  if (!stock) return 'missing_stock_snapshot';
  if (isDeclaredEndingInventory(stock)) return 'verified_declared_inventory';
  if (stock.snapshotConfirmed !== true) return 'missing_stock_snapshot';
  ...
  if (stock.priceEvidenceStatus === 'VERIFIED_CARRIED_UNIT_COST') return 'verified_carried_unit_cost';
  if (stock.priceEvidenceStatus === 'VERIFIED_CONFIRM_SNAPSHOT') return 'verified_confirm_snapshot';
  ...
  if (stock.priceEvidenceStatus === 'VERIFIED_DECLARED_INVENTORY') return 'verified_declared_inventory';
...
export function computeProfitRow(row, edits = {}) {
  ...
  // CurrencyMaster 현재값과 전차수 값은 참고 제안일 뿐 자동 계산에 사용하지 않는다.
  ...
  // E는 서버 자동값만 허용. F는 재고수량이 없을 때만 선언 금액(수기/엑셀 기말상품재고액)을 받는다.
  const E = row.auto.E ?? null;
  const autoF = computeAutoEndingStock(row.stock) ?? row.auto.F ?? null;
  const F = allowsDeclaredEndingInventoryAmount(row.stock) ? pick('F', autoF) : autoF;
  const C = N + L + O;
  const P = Q * n0(R);
  const T = n0(S) * n0(R);
  const G = P + T;
