...
export function usesLayeredInventoryCategory(category) {
  return CATEGORY_AVERAGE_INVENTORY_KEYS.includes(String(category || ''))
    || String(category || '') === AUSTRALIA_INVENTORY_CATEGORY;
}
...
/** 이번 입고분 원화. 호주는 Q×R, 콜롬비아 5품종·베트남은 (Q×R+S×R+H). 매입이 없으면 0. */
...
/**
 * 한 차수의 기말 재고금액. 층별 대상은 기초금액(전차수 기말)을 유지한 채 이번 입고만 새 원가로 평가한다.
 * 그 밖은 그 차수 검증 단가·카테고리 평균·역사 workbook 순이다.
 */
 ...
 * 층별 대상은 전전차수 비층별 기말을 전차수 기초로 두고 전차수 입고만 새 원가로 쌓는다.
 */
