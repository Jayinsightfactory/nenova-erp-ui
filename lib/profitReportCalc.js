...
/** 공제·미분류는 재고 환율 층이 없다. 그 밖 모든 국가·품종은 층별 환율(FIFO)을 쓴다. */
export const NON_LAYERED_INVENTORY_CATEGORIES = Object.freeze(['공제', '기타(미분류)']);

export function usesLayeredInventoryCategory(category) {
  const key = String(category || '');
  return !!key && !NON_LAYERED_INVENTORY_CATEGORIES.includes(key);
}
...
/** 이번 입고분 원화. 호주는 Q×R, 콜롬비아 5품종·베트남은 (Q×R+S×R+H), 그 밖은 Q×R+S×R. 매입이 없으면 0. */
export function incomingInventoryPurchaseValue({
  category, purchaseForeign = 0, forwardingForeign = 0, taxableRate, customsCost = 0, purchaseQty,
} = {}) {
  const qty = Number(purchaseQty) || 0;
  if (!(qty > 0)) return 0;
  if (String(category || '') === AUSTRALIA_INVENTORY_CATEGORY) {
    if (!Number.isFinite(Number(purchaseForeign)) || !Number.isFinite(Number(taxableRate)) || !(Number(taxableRate) > 0)) return null;
    return Number(purchaseForeign) * Number(taxableRate);
  }
  if (CATEGORY_AVERAGE_INVENTORY_KEYS.includes(String(category || ''))) {
    ...
    return average ? average.unitCost * qty : null;
  }
  ...
  if (Math.abs(goods) > 0.000001 || Math.abs(forwarding) > 0.000001) {
    if (!(Number.isFinite(Number(taxableRate)) && Number(taxableRate) > 0)) return null;
    return (goods + forwarding) * Number(taxableRate);
  }
  return 0;
}
...
export function resolveNonLayeredInventoryClosing({
  ...
  if (!conversionMissing && !unitMismatch) {
    const average = computeCategoryAverageInventoryValue({
      category, purchaseForeign, forwardingForeign, taxableRate, customsCost, purchaseQty, stockQty,
    });
    if (average) {
      return {
        ...
        sources: ['category-average:(G+H)/purchaseQty*ProductStock'],
      };
    }
  }
...
/**
 * 한 차수의 기말 재고금액. 모든 국가는 기초금액(전차수 기말)을 유지한 채 이번 입고만 새 원가로 평가한다.
 * 층별이 안 되면 그 차수 검증 단가·카테고리 평균·역사 workbook 순이다.
 */
export function resolveInventoryClosing({
  ...
  directSources, historicalSources,
} = {}) {
  if (usesLayeredInventoryCategory(category) && !conversionMissing && !unitMismatch) {
    const purchaseValue = incomingInventoryPurchaseValue({
      category, purchaseForeign, forwardingForeign, taxableRate, customsCost, purchaseQty,
    });
    const layered = computeLayeredInventoryValue({
      beginQty, beginValue, purchaseQty, purchaseValue, endQty,
    });
    if (layered) {
      return {
        value: layered.value,
        ...
        sources: [`layered-inventory:${layered.method}`],
      };
    }
  }
...
/**
 * 이번 차수 기초재고 E = 전차수 기말재고 F.
 * 전차수가 확정되어 있으면 그 F를 쓰고, 아니면 전차수 F 공식으로 재현한다.
 * 층별 국가는 전전차수 비층별 기말 또는 확정 F를 전차수 기초로 두고 전차수 입고만 새 원가로 쌓는다.
 */
...
/**
 * 기존재고는 전차수 금액(기존 환율)을 유지하고, 이번 차수 입고분만 새 원가로 평가한다.
 * 판매는 기초 재고부터 나간다(FIFO).
 */
export function computeLayeredInventoryValue({
  beginQty, beginValue, purchaseQty, purchaseValue, endQty,
} = {}) {
  const endingQty = Number(endQty);
  if (!Number.isFinite(endingQty)) return null;
  if (endingQty === 0) return { value: 0, status: 'VERIFIED_LAYERED_INVENTORY', method: 'no_stock' };
  const openingQty = Math.max(0, Number(beginQty) || 0);
  const incomingQty = Math.max(0, Number(purchaseQty) || 0);
  const openingValue = beginValue == null ? null : Number(beginValue);
  const incomingValue = purchaseValue == null ? null : Number(purchaseValue);
  const openingUnit = openingQty > 0 && openingValue != null && Number.isFinite(openingValue)
    ? openingValue / openingQty : null;
  const incomingUnit = incomingQty > 0 && incomingValue != null && Number.isFinite(incomingValue)
    ? incomingValue / incomingQty : null;
  const soldQty = openingQty + incomingQty - endingQty;
  let remainingBegin = Math.max(0, openingQty - Math.max(0, soldQty));
  if (soldQty < -0.000001) remainingBegin = openingQty;
  const remainingNew = endingQty - remainingBegin;
  if (remainingBegin > 0.000001 && openingUnit == null) return null;
  if (remainingNew > 0.000001 && incomingUnit == null) {
    if (incomingQty <= 0 && openingUnit != null) {
      return {
        value: endingQty * openingUnit,
        status: 'VERIFIED_LAYERED_INVENTORY',
        method: 'carried_only',
        remainingBegin: endingQty,
        remainingNew: 0,
        beginUnit: openingUnit,
        newUnit: null,
      };
    }
    return null;
  }
  const value = (remainingBegin > 0 ? remainingBegin * openingUnit : 0)
    + (remainingNew > 0 ? remainingNew * incomingUnit : 0);
  if (!Number.isFinite(value)) return null;
  return {
    value,
    status: 'VERIFIED_LAYERED_INVENTORY',
    method: remainingNew > 0 && remainingBegin > 0 ? 'mixed' : remainingNew > 0 ? 'new_receipts' : 'carried_only',
    remainingBegin,
    remainingNew,
    beginUnit: openingUnit,
    newUnit: incomingUnit,
  };
}
...
export function endingStockSourceKind(stock) {
  ...
  if (stock.priceEvidenceStatus === 'VERIFIED_LAYERED_INVENTORY') return 'verified_layered_inventory';
