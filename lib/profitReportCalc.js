...
/**
 * 원본 재고잔량 시트의 비평균·비호주 품목은 부가세 포함 원화 표시단가를 /1.1해
 * 평가한다. Product.Cost는 EXE 품목관리의 "출고단가"이므로 자동 확정 근거가 아니라
 * 사용자가 차수별 재고단가로 승격할 수 있는 추천값으로만 제공한다.
 */
export function productMasterStockPriceSuggestion(input = {}) {
  const category = String(input.category || '');
  if (!category || category === '호주' || CATEGORY_AVERAGE_INVENTORY_KEYS.includes(category)) return null;
  const grossCost = Number(input.cost);
  if (!(grossCost > 0) || !Number.isFinite(grossCost)) return null;
  return {
    price: grossCost / 1.1,
    grossCost,
    basis: 'PRODUCT_MASTER_OUT_PRICE_SUGGESTION_ONLY',
  };
}
