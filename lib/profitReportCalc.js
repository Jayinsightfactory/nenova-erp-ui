...
/**
 * 비평균 신규 품목의 재고단가 추천값.
 * 원본 재고잔량 시트는 품목별 VAT 포함 단가 하나를 수량에 곱한 뒤 국가 합계를 /1.1한다.
 * 따라서 당차수 확정 분배단가를 평균내지 않고, 가장 많은 업체가 사용한 실제 단가를
 * 대표값으로 제안한다. 업체 수가 같으면 그 단가의 EstQuantity 합계가 큰 쪽을 우선한다.
 * 반환값은 추천일 뿐이며 사용자가 확인·저장하기 전에는 E/F 계산 근거가 아니다.
 */
export function distributionMajorityStockPriceSuggestion(input = {}) {
  const category = String(input.category || '');
  if (!category || (input.allowCategoryAverage !== true && CATEGORY_AVERAGE_INVENTORY_KEYS.includes(category))) return null;
  const rows = Array.isArray(input.rows) ? input.rows : [];
  const validRows = rows.filter((row) => {
    const estQty = Number(row?.estQty);
    const grossCost = Number(row?.grossCost);
    return Number.isFinite(estQty) && estQty > 0
      && Number.isFinite(grossCost) && grossCost > 0;
  });
  if (!validRows.length) return null;
  const grouped = new Map();
  for (const row of validRows) {
    const grossCost = Math.round(Number(row.grossCost) * 100) / 100;
    const key = String(grossCost);
    if (!grouped.has(key)) {
      grouped.set(key, { grossCost, customers: new Set(), totalEstQuantity: 0, distributionCount: 0 });
    }
    const group = grouped.get(key);
    if (row.custKey != null) group.customers.add(String(row.custKey));
    group.totalEstQuantity += Number(row.estQty);
    group.distributionCount += Number(row.distributionCount || 1);
  }
  const candidates = [...grouped.values()].map((group) => ({
    grossCost: group.grossCost,
    price: group.grossCost / 1.1,
    customerCount: group.customers.size,
    totalEstQuantity: group.totalEstQuantity,
    distributionCount: group.distributionCount,
  })).sort((a, b) => b.customerCount - a.customerCount
    || b.totalEstQuantity - a.totalEstQuantity
    || b.grossCost - a.grossCost);
  const selected = candidates[0];
  return {
    ...selected,
    candidates,
    basis: 'CURRENT_WEEK_CONFIRMED_DISTRIBUTION_MAJORITY_CUSTOMER_PRICE',
  };
}
...
export function sampleInventoryAveragePriceSuggestions(rows = []) {
  ...
  const peers = normalized.filter((row) => !row.isSample && row.scopeKey && row.unit && row.qty > 0 && row.price != null);
