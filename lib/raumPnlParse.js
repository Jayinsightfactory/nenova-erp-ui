...
import { RAUM_GANGNAM_MULTISHEET } from './raumPnlImportReview.js';
...
function detectBranch(sheetName, aoa) {
  ...
  const gangnamInName = name.includes('강남');
  ...
  // 행사명이 지점명 뒤에 붙은 강남 시트도 기존 강남 지점에 합산한다.
  // 건대는 기존의 엄격한 이름 규칙을 유지한다.
  if (branch?.includes('강남') || gangnamInName) branch = '강남';
  if (!major && gangnamInName) {
    const majorMatch = name.match(/(?:^|[^\d])(\d{1,2})\s*차/);
    if (majorMatch) major = majorMatch[1].padStart(2, '0');
  }
...
export function parseRaumQuoteWorkbook(XLSX, workbook) {
  ...
  for (const c of verification) {
    if (!c.ok) warnings.push(formatVerificationWarning(c));
  }
...
export function parseRaumQuoteWorkbookGroups(XLSX, workbook, options = {}) {
  // Parsing remains pure. API code supplies a descriptor it just read from the
  // active registry; client fields never create a custom-hotel capability.
  const requestedCode = String(options.partnerCode == null ? '' : options.partnerCode).trim().toLowerCase();
  if (options.partner && requestedCode && String(options.partner.code || '').trim().toLowerCase() !== requestedCode) {
    throw new Error('선택한 호텔 정보가 달라졌습니다. 호텔을 다시 선택한 뒤 파일을 올려주세요.');
  }
  const partner = resolvePnlPartner(options.partnerCode, options.partner);
  const warnings = [];
  ...
  const batches = [...byMajor.entries()]
    ...
    .map(([major, sheets]) => {
      ...
      for (const [branch, count] of branchCounts) if (count > 1) {
        const sheetNames = sheets.filter(sheet => sheet.branch === branch).map(sheet => sheet.sheetName);
        if (partner.code === 'raum' && branch === '강남') verification.push({
          code: RAUM_GANGNAM_MULTISHEET, group: branch, label: '동일 차수·강남 시트',
          sheetVal: count, parsedVal: count, diff: 0, ok: true, requiresConfirmation: true, sheetNames,
          info: `${Number(major)}차 강남 시트 ${count}개를 합산했습니다. 원본 시트 목록과 합산 수량·금액을 확인한 뒤에만 저장할 수 있습니다.`,
        });
        else verification.push({
          group: branch, label: '동일 차수·지점 시트', sheetVal: count, parsedVal: count,
          diff: 0, ok: false, sheetNames, info: `${Number(major)}차 ${branch} 시트가 ${count}개라 중복 합산 위험이 있습니다.`,
        });
      }
      const batchWarnings = verification.filter(c => !c.ok).map(formatVerificationWarning);
      return {
...
function formatVerificationNumber(value) {
  return value == null || !Number.isFinite(Number(value)) ? '없음' : Number(value).toLocaleString();
}

function formatVerificationWarning(check) {
  const diff = check?.diff == null ? '' : ` (차이 ${formatVerificationNumber(check.diff)})`;
  return `검증 실패 — [${check?.group || '-'}] ${check?.label || '검증'}: 견적서 ${formatVerificationNumber(check?.sheetVal)} vs 파싱 ${formatVerificationNumber(check?.parsedVal)}${diff}`;
}
