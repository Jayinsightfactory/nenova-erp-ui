...
const sourceIdentity = value => typeof value === 'string' ? value.slice(0, 512) : '';
...
export function parsePasteOperation(row) {
  ...
  const entries = (Array.isArray(payload.entries) ? payload.entries : []).map(entry => ({
    ...
    qty: number(entry?.qty), unit: text(entry?.unit), sourceIdentity: sourceIdentity(entry?.sourceIdentity),
  }));
