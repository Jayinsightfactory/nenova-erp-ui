...
import { actionLogPayload } from './pasteOperationAudit.js';
...
/** 응답 구조가 달라도 실제 쓰기 결과 건수를 잃지 않도록 우선순위를 고정한다. */
export function extractActionLogAffectedCount(data, fallback = 0) {
  const candidates = [
    data?.appliedCount,
    data?.committedCount,
    data?.shipmentChangedCount,
    data?.orderChangedCount,
    data?.pushed,
    data?.count,
    data?.affected,
    data?.saved,
  ...
  return finiteCount(fallback) ?? 0;
}

export function buildActionLogResultDesc(data) {
  const summary = data?.auditSummary || {};
  const parts = [];
  if (data?.week) parts.push(`week=${data.week}`);
  if (data?.auditBatchKey) parts.push(`audit=${data.auditBatchKey}`);
  if (data?.appliedCount != null) parts.push(`applied=${data.appliedCount}`);
  if (data?.committedCount != null) parts.push(`committed=${data.committedCount}`);
  if (data?.verifiedCount != null) parts.push(`verified=${data.verifiedCount}`);
  if (data?.orderChangedCount != null) parts.push(`orderChanged=${data.orderChangedCount}`);
  if (data?.shipmentChangedCount != null) parts.push(`shipmentChanged=${data.shipmentChangedCount}`);
  if (summary.verificationChecked != null) parts.push(`verify=${summary.verificationChecked}`);
  if (summary.verificationMismatchCount != null) parts.push(`mismatch=${summary.verificationMismatchCount}`);
  ...
  try {
    ...
    await query(
         ...
         @payload, @result, @resultDesc, @riskLevel, @ip, @ua)`,
      {
        ...
        payload:       { type: sql.NVarChar, value: ['SHIPMENT_ADJUST_BATCH', 'SHIPMENT_ADJUST_BATCH_UNDO'].includes(data.actionType) ? (data.payload || '') : (data.payload || '').slice(0, 4000) },
...
export function withActionLog(handler, opts = {}) {
  return async (req, res) => {
    ...
    // Actor 판별 — userName(사람이름) 우선, 없으면 userId
    ...
    res.json = (data) => {
      ...
      try {
        if (data?.success === false || data?.error) {
          ...
          // 결과에서 건수 추출 시도
          affectedCount = extractActionLogAffectedCount(data, affectedCount);
          capturedDesc = buildActionLogResultDesc(data);
        }
      } catch (_) {}
    ...
    try {
      ...
      await writeLog({
        actor,
        sessionId:     claudeSession || userName,
        actionType:    opts.actionType || actionType,
        method,
        endpoint,
        affectedTable: opts.affectedTable || affectedTable,
        affectedCount,
        payload:       actionLogPayload(opts.actionType || actionType, body, capturedResponse),
