...
function addWeekSheet(wb, record) {
  ...
  const sum = (c) => ({ formula: `SUM(${L(c)}${first}:${L(c)}${tr - 1})` });
  branches.forEach((b, bi) => { trow.getCell(col.branch0 + bi).value = sum(col.branch0 + bi); });
  for (const c of [col.qty, col.costAmt, col.saleAmt, col.profit, col.nen, col.miu]) trow.getCell(c).value = sum(c);
  trow.getCell(col.rate).value = { formula: `IFERROR(${L(col.profit)}${tr}/${L(col.saleAmt)}${tr},0)` };
  ...
  return { sheetName, cols: { costAmt: L(col.costAmt), saleAmt: L(col.saleAmt), profit: L(col.profit), nen: L(col.nen), miu: L(col.miu) } };
}
...
function addSummarySheet(wb, records, refs) {
  ...
  records.forEach((rec, i) => {
    ...
    const idx = (colLetter) => `INDEX(${sn}!${colLetter}:${colLetter},MATCH("합계",${sn}!A:A,0))`;
    const row = ws.getRow(r);
    row.getCell(1).value = ref.sheetName;
    row.getCell(2).value = dateStr(rec.master.QuoteDate);
    row.getCell(3).value = { formula: idx(ref.cols.costAmt) };
    row.getCell(4).value = { formula: idx(ref.cols.saleAmt) };
    row.getCell(5).value = { formula: `D${r}-C${r}` };
    row.getCell(6).value = { formula: `IFERROR(E${r}/D${r},0)` };
    row.getCell(7).value = { formula: idx(ref.cols.nen) };
    row.getCell(8).value = { formula: idx(ref.cols.miu) };
    for (let c = 1; c <= 8; c += 1) {
  ...
  for (const c of [3, 4, 5, 7, 8]) {
    trow.getCell(c).value = { formula: `SUM(${ws.getColumn(c).letter}${first}:${ws.getColumn(c).letter}${tr - 1})` };
    trow.getCell(c).numFmt = FMT_MONEY;
  }
  trow.getCell(6).value = { formula: `IFERROR(E${tr}/D${tr},0)` };
