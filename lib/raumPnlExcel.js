...
function addWeekSheet(wb, record) {
  ...
  return { sheetName, cols: { costAmt: L(col.costAmt), saleAmt: L(col.saleAmt), profit: L(col.profit), rate: L(col.rate), nen: L(col.nen), miu: L(col.miu) } };
}
...
function addSummarySheet(wb, records, refs) {
  ...
  records.forEach((rec, i) => {
    ...
    row.getCell(5).value = { formula: idx(ref.cols.profit) };
    row.getCell(6).value = { formula: idx(ref.cols.rate) };
  ...
  trow.getCell(6).value = { formula: `IFERROR(E${tr}/(C${tr}+E${tr}),0)` };
