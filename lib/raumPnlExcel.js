...
import { resolvePnlPartner } from './raumPnlPartner.js';
...
// 차수 손익 합계 — 사입도 매입단가 입력 시 손익에 포함 (웹 computeTotals 와 동일 정의).
// 모든 수식 셀에 이 값을 result 캐시로 심는다 — 재계산 안 하는 뷰어(미리보기·한셀 등)에서도 값이 보이게.
function recordTotals(items, pct) {
  const t = { qty: 0, sale: 0, consignedSale: 0, pnlSale: 0, cost: 0, profit: 0, branchQty: {} };
  for (const it of items) {
    ...
    if (it.consigned) t.consignedSale += sale;
    // 매입단가가 비어 있는 행은 손익 대상 매출에 넣지 않는다.
    // 그래야 사입/신규 이미지 행이 단가 미입력 상태에서 이익으로 과대계상되지 않는다.
    if (it.costPrice != null && it.costPrice !== '' && Number.isFinite(Number(it.costPrice))) {
...
function partnerLabelOf(records, override) {
  if (override) return override;
  try { return resolvePnlPartner(records?.[0]?.master?.PartnerCode).label; } catch { return '라움'; }
}

// 한 차수 시트 — 반환: 결산 시트가 참조할 시트명
function addWeekSheet(wb, record, partnerLabel) {
  const { master, items } = record;
  const major = Number(master.MajorWeek);
  const sheetName = wb.getWorksheet(`${major}차`) ? `${master.OrderYear}-${major}차` : `${major}차`;
  ...
  title.value = `${partnerLabel} ${major}차 (${dateStr(master.QuoteDate)}) — VAT별도`;
  ...
  // 2행: 헤더
  const heads = ['품목명', '단위', ...branches, '수량계', '1개당 매입단가\n(원/VAT별도)', '매입액', '매출단가', '매출액', '이익', '이익율',
  ...
  items.forEach((it, i) => {
    ...
    const qty = Number(it.qty || 0);
    const cost = it.costPrice != null && it.costPrice !== '' && Number.isFinite(Number(it.costPrice)) ? Number(it.costPrice) : null;
    ...
    const hasCost = `${L(col.cost)}${r}<>""`;
    row.getCell(col.costAmt).value = { formula: `IF(${hasCost},${L(col.qty)}${r}*${L(col.cost)}${r},0)`, result: cost != null ? qty * cost : 0 };
    row.getCell(col.profit).value = { formula: `IF(${hasCost},${L(col.saleAmt)}${r}-${L(col.costAmt)}${r},0)`, result: cost != null ? qty * sale - qty * cost : 0 };
    row.getCell(col.rate).value = { formula: `IF(${hasCost},IFERROR(${L(col.profit)}${r}/${L(col.saleAmt)}${r},0),0)`, result: cost != null && qty * sale ? (qty * sale - qty * cost) / (qty * sale) : 0 };
    row.getCell(col.rate).numFmt = FMT_PCT;
    row.getCell(col.nen).value = { formula: `IF(${hasCost},${L(col.profit)}${r}*${PCT_REF},0)`, result: (cost != null ? qty * sale - qty * cost : 0) * pct };
    row.getCell(col.miu).value = { formula: `IF(${hasCost},${L(col.profit)}${r}*(1-${PCT_REF}),0)`, result: (cost != null ? qty * sale - qty * cost : 0) * (1 - pct) };
    row.getCell(col.remark).value = it.consigned ? `사입${it.remark ? ` · ${it.remark}` : ''}` : (it.remark || '');
  ...
  // 이익율 분모 = 매입단가가 입력된 손익 대상 매출.
  trow.getCell(col.rate).value = {
    formula: `IFERROR(${L(col.profit)}${tr}/${L(col.saleAmt)}${tr},0)`,
    result: totals.rate,
  };
  ...
  ws.getColumn(col.cost).width = 16;
  ws.getColumn(col.sale).width = 10;
...
function addSummarySheet(wb, records, refs, partnerLabel) {
  const ws = wb.addWorksheet('결산');
  const year = records[0]?.master?.OrderYear || '';
  ws.mergeCells('A1:H1');
  ws.getCell('A1').value = `${year} ${partnerLabel} 손익 결산`;
  ...
  records.forEach((rec, i) => {
    ...
    // 총이익·이익율은 각 차수 시트 합계행 참조 — 매입단가 입력행만 손익에 반영.
  ...
  // 손익대상 매출은 단가 입력행 기준이며 차수 시트의 이익율과 동일한 정의를 사용한다.
  trow.getCell(6).value = {
    formula: `IFERROR(E${tr}/(C${tr}+E${tr}),0)`,
    result: refs.reduce((a, ref) => a + (ref.totals?.pnlSale || 0), 0) > 0
      ? grand.profit / refs.reduce((a, ref) => a + (ref.totals?.pnlSale || 0), 0)
      : 0,
  };
...
export async function buildRaumPnlWorkbook(records, options = {}) {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'nenovaweb';
  wb.calcProperties.fullCalcOnLoad = true; // 열 때 전체 재계산 — 수식 결과 최신 보장
  const partnerLabel = partnerLabelOf(records, options.partnerLabel);
  const refs = records.map(rec => addWeekSheet(wb, rec, partnerLabel));
  addSummarySheet(wb, records, refs, partnerLabel);
