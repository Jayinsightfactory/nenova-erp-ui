...
// 그래서 다음주 수요일 00:00(=화요일 마감 직후)에 차수 N 의 TUE_CLOSE(판매등록 마감) 스냅샷을 고정한다.
// 예: 28차(기준수요일 07-08) → 마감 스냅샷은 07-15(수) 00:00 = 07-14(화)까지 수정분 확정.
//
...
const LOCKED_TYPES = ['TUE_FINAL', 'WED_CHECK', 'TUE_CLOSE'];
      ...
      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name='WebSalesSnapshot')
       BEGIN
         ...
         CREATE UNIQUE INDEX UX_WebSalesSnapshot_Fixed
           ...
           WHERE SnapshotType IN (N'TUE_FINAL', N'WED_CHECK', N'TUE_CLOSE');
...
async function schedulerTick(state) {
  ...
  for (const w of weeks) {
    ...
    // (고정 스냅샷 화17:00/수16:00/차주수00:00 만 자동)
  }

  // 전차수 판매등록 마감(TUE_CLOSE) — 차수 N 판매등록은 다음주 화요일까지 수정 가능, 수요일부터 수정금지.
  // 새 사이클 수요일 00:00부터(=전차수 화요일 마감 직후) 전차수 마감 스냅샷을 보장한다 (지각 생성 포함).
  // 예: 28차(기준수요일 07-08) 마감은 07-15(수) 00:00 — 07-14(화)까지의 판매등록이 확정본.
  if (day >= 3 && prevMajor) {
    const prevWeeks = await weeksWithData(prevYear, prevMajor);
    for (const w of prevWeeks) {
      const onTime = day === 3 && hour === 0 && min < 5;
      await takeSnapshot({
        week: w, orderYear: prevYear, type: 'TUE_CLOSE', actor: 'scheduler',
        note: onTime
          ? '판매등록 마감 — 차주 화요일까지 수정분 확정(수요일부터 수정금지)'
          : '지각 생성(예정시각에 서버 미가동 등) — 화요일 마감 이후 상태일 수 있음',
      });
    }
  }
}
...
export function startSalesSnapshotScheduler() {
  ...
  console.log('[salesSnapshot] 스케줄러 시작 — 화17:00 TUE_FINAL / 수16:00 WED_CHECK / 차주수00:00 TUE_CLOSE(판매등록 마감)');
}
