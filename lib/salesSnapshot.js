...
export async function captureCurrentRows(week, orderYear = '') {
  const year = String(orderYear || '').trim();
  const yearSql = year ? `AND ${salesCaptureYearPredicateSql()}` : '';
  const params = { week: { type: sql.NVarChar, value: week } };
  if (year) params.yr = { type: sql.NVarChar, value: year };
  const r = await query(
...
export async function takeSnapshot({ week, orderYear, type, actor = 'system', note = '' }) {
  ...
  const rows = await captureCurrentRows(week, orderYear);
...
export async function detectAndRecordChange(week, orderYear, actor = 'watchdog') {
  ...
  const [baseRows, currRows] = await Promise.all([getSnapshotRows(last.SnapshotKey), captureCurrentRows(week, orderYear)]);
