...
/**
 * R 원천 선택(순수 함수, DB 의존 없음 — 서버/클라이언트 공용).
 *
 ...
 *          previousWeekRate:?number, previousWeekLabel:?string}} input
 */
 ...
 * 관세청 과세환율 공식 조회.
 *
 * API 키는 환경변수 `KCS_EXCHANGE_RATE_API_KEY`로만 주입한다(코드/저장소에 절대 넣지 않는다 —
 * 이름만 .env.local.example에 있다). 키가 없으면 조회를 시도하지 않고 "원천 없음"으로 보고해,
 * 담당자가 고시 환율을 직접 입력하도록 한다. 조회에 성공하면 호출부가 saveTaxableRate(...,
 * rateSource='kcs_api')로 캐시한다.
 */
export async function fetchKcsTaxableRate({ currency, declarationDate }) {
  const apiKey = process.env.KCS_EXCHANGE_RATE_API_KEY;
  if (!apiKey) {
    return {
      available: false,
      reason: 'missing_api_key',
      message: '관세청 과세환율 API 키(KCS_EXCHANGE_RATE_API_KEY)가 설정되어 있지 않아 공식 조회를 할 수 없습니다. 고시 환율을 직접 입력하세요.',
    };
  }
  ...
  return {
    ...
    message: 'API 키는 설정되어 있으나 관세청 과세환율 연동은 아직 사용하지 않습니다. 고시 환율을 직접 입력하세요.',
  };
}
