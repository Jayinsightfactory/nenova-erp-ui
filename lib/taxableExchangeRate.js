 ...
 *   3. (2026년 28차 이후에만 값이 들어옴) 관세청 공식 과세환율 API(KCS) — 실제 입고일자별
 *      (WarehouseMaster.InputDate→ArrivalDtm→UploadDtm) 환율을 wd.TPrice로 가중평균한다
 ...
 * 포함) 기본 활성 — 2026년 28차 이후는 시크릿 없이도 자동 조회를 시도한다. 조회에 성공하면
...
export function isKcsRateEligibleWeek(orderYear, major) {
  return String(orderYear) === KCS_RATE_YEAR && Number(major) >= KCS_RATE_MIN_MAJOR;
}

/**
 * 카테고리별 관세청 공식 과세환율(KCS) — resolveTaxableRate()의 kcsRate/kcsDetail 입력을 만드는
 * 오케스트레이션. 2026년 28차 이후에만 동작하고(isKcsRateEligibleWeek), 그 범위 밖에서는 DB
 * 조회도 네트워크 호출도 하지 않고 빈 객체를 반환해 22~27차를 그대로 보존한다.
 *
 * lib/kcsRateDateWeights.js#loadWarehouseDateWeights(WarehouseMaster InputDate→ArrivalDtm→
 * UploadDtm 우선순위)로 카테고리별 실제 입고 날짜 전부와 그 날짜의 TPrice 가중치를 받아, 서로
...
export async function kcsRatesByCategory(major, orderYear) {
  ...
  for (const p of points) {
    ...
    byCategoryPoints.get(p.category).points.push({ date: p.date, weight: p.weight });
  }

  const resByKey = new Map();
  await Promise.all([...new Set(points.map((p) => `${p.currency}::${p.date}`))].map(async (key) => {
  ...
  for (const [category, { currency, points: datePoints }] of byCategoryPoints) {
    ...
    for (const dp of datePoints) {
      ...
      if (res?.available) {
        ...
        usedDates.push({ date: dp.date, weight: dp.weight, rate: Number(res.rate) });
      } else {
