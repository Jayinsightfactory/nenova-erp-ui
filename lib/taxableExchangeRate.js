...
// 적용했고, 29차 이후에는 매입 여부와 무관하게 전차수 R을 자동 상속까지 했다). 이제 CurrencyMaster는
// **제안(suggestion)** 으로만 화면에 뜬다. 전차수 값도 이 exact-week resolver에서는 제안으로만
// 반환하며, 별도 이월 정책은 현재 차수에 재고화 대상 매입이 전혀 없을 때만 최근 exact R을 사용한다.
//
// ## 키
 ...
 * 어느 것도 없으면 `missing`이다. 이 함수 자체는 **CurrencyMaster 현재 환율과 전차수 R을 자동
 * 적용하지 않고** suggestions로만 돌려준다. 현재 차수에 재고화 대상 매입이 전혀 없을 때의 제한적
 * 이월은 `lib/profitReportTaxableRateCarry.js`가 별도로 수행하며 DB에 자동 저장하지 않는다.
 *
