...
// List label only: retain original names for identity, search and detail views.
export function compactQualityProductName(value){
 const original=String(value??'').trim();
 return original.replace(/^(?:ROSE|CARNATION|HYDRANGEA)(?:\s*\/\s*|\s+)/i,'').trim()||original;
}
