...
// ── 유출 이력(egress ledger) ───────────────────────────────────────────────────
// 직원 PC 데몬(egress-monitor)이 보내는 "파일이 어디로 나갔나" 기록. 차단·알림 없음, 이력만. 관리자만 열람.
//   kind: copy(USB·클라우드 동기화 폴더로 복사) | print(인쇄) | email(Outlook 첨부) | webupload(웹메일·웹업로드, 파일명만) | kakao(카톡 파일 전송) | download(드라이브 내려받기, downloadLog와 별개 뷰)
//   sha가 드라이브 색인의 파일과 맞으면 fileId를 붙여 "그 파일의 흐름"으로 묶인다(파일명만 맞으면 fileNameMatch).
export const EGRESS_KINDS = Object.freeze(['copy', 'print', 'email', 'webupload', 'kakao', 'download']);
const egressFile = () => path.join(root(), 'egress.jsonl');
function readEgress() { try { return fs.readFileSync(egressFile(), 'utf8').split('\n').filter(Boolean).map((l) => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean); } catch { return []; } }
export function recordEgress(evt) {
  const kind = String(evt.kind || '').toLowerCase(); if (!EGRESS_KINDS.includes(kind)) return { ok: false, error: 'kind' };
  const filename = String(evt.filename || '').slice(0, 300); if (!filename) return { ok: false, error: 'filename' };
  const sha = String(evt.sha || '').toLowerCase().slice(0, 64);
  const rows = fileRows(); const latest = new Map(); for (const r of rows) latest.set(r.id, r);
  let match = null, how = '';
  if (sha) { match = [...latest.values()].find((r) => r.sha === sha && !r.deleted) || null; if (match) how = 'sha'; }
  if (!match) { const nm = filename.toLowerCase(); match = [...latest.values()].find((r) => !r.deleted && String(r.filename || '').toLowerCase() === nm) || null; if (match) how = 'name'; }
  const who = resolveUploader({ orbitUserId: evt.orbitUserId, userName: evt.userName, hostname: evt.hostname });
  const row = { id: `eg-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`, type: 'egress', kind, filename, sha, size: Number(evt.size) || 0,
    fileId: match ? match.id : '', matchHow: how, fileStage: match ? match.stage : '', fileSensitive: match ? !!match.sensitive : false,
    userName: who.name || String(evt.userName || ''), dept: who.dept || '', hostname: String(evt.hostname || '').slice(0, 80), orbitUserId: String(evt.orbitUserId || '').slice(0, 40),
    dest: String(evt.dest || '').slice(0, 400), destKind: String(evt.destKind || '').slice(0, 40), app: String(evt.app || '').slice(0, 80), detail: String(evt.detail || '').slice(0, 500),
    at: /^\d{4}-\d{2}-\d{2}T/.test(String(evt.at || '')) ? String(evt.at) : new Date().toISOString(), receivedAt: new Date().toISOString(), dedupKey: String(evt.dedupKey || '').slice(0, 120) };
  if (row.dedupKey && readEgress().some((r) => r.dedupKey === row.dedupKey)) return { ok: true, dup: true };
  fs.mkdirSync(root(), { recursive: true }); fs.appendFileSync(egressFile(), JSON.stringify(row) + '\n');
  return { ok: true, row };
}
export function listEgress(user, { days = 30, kind = '', who = '', fileId = '', q = '' } = {}) {
  if (!canAdmin(user)) return [];
  const since = Date.now() - Math.min(365, Math.max(1, Number(days) || 30)) * 86400e3;
  const ql = String(q || '').toLowerCase();
  return readEgress().filter((r) => new Date(r.at).getTime() >= since && (!kind || r.kind === kind) && (!who || r.userName === who) && (!fileId || r.fileId === fileId)
    && (!ql || r.filename.toLowerCase().includes(ql) || String(r.dest || '').toLowerCase().includes(ql) || String(r.detail || '').toLowerCase().includes(ql)))
    .sort((a, b) => (b.at > a.at ? 1 : -1)).slice(0, 5000);
}
// 파일 하나의 전체 흐름: 업로드 → 내려받기 → 유출 이력 (관리자만)
export function fileTimeline(user, id) {
  if (!canAdmin(user)) return [];
  const ups = readIndex().filter((r) => r.id === id && r.type !== 'download').map((r) => ({ kind: 'upload', at: r.correctedAt || r.uploadedAt, userName: r.uploaderName, detail: r.correctedAt ? `분류 교정 (${r.correctedBy})` : `업로드 v${r.version} · ${r.hostname}` }));
  const dls = readIndex().filter((r) => r.type === 'download' && r.fileId === id).map((r) => ({ kind: 'download', at: r.at, userName: r.byName || r.by, detail: '드라이브에서 내려받기' }));
  const eg = readEgress().filter((r) => r.fileId === id).map((r) => ({ kind: r.kind, at: r.at, userName: r.userName, detail: [r.destKind, r.dest, r.app, r.detail].filter(Boolean).join(' · ') }));
  return [...ups, ...dls, ...eg].sort((a, b) => (a.at > b.at ? 1 : -1));
}
