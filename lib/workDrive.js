...
export function extractCycle(name) {
  ...
  if (!m) m = s.match(/(?:^|[^\d])(\d{2})0?(\d)(?=차|_|\s|\.|$)/);          // 3802·3801 (세부차수 0 접두 허용, 뒤에 차/_/공백/확장자)
...
const STAGE_RULES = [
  ['송금·경영', /지출결의|결의서|송금|외화|수입비교|면장|세금계산|계산서|정산서|급여대장|예산|결산|매출이익|손익|보고서|위임장|월말재고|증빙/i],
  ['품질', /불량|quality|claim|클레임|품질|하자|이슈|issue|defect|reclamo/i],
  ['원가·운임', /원가|운임|freight|arrival|도착원가|통관|관세|customs|물류비/i],
  ['견적·거래처', /견적|estimate|quotation|거래명세|명세서|invoice to|청구|판매현황|판매/i],
  ['입고', /입고|proforma|packing|awb|phyto|b\/l|bl\b|shipping|holex|선적|인보이스|invoice|\bci\b|\bco\b|콜카장|tiba/i],
  ['출고', /출고|분배|shipment|배송|납품/i],
...
export function isSensitive(name, stage) {
  return stage === '송금·경영' || /계약|contrato|contract|인보이스|invoice|proforma|단가|원가|급여|연봉|주민|여권|passport|통장|계좌/i.test(String(name || ''));
}
...
export function resolveUploader({ orbitUserId, userName }) {
  const nm = USER_ID_ALIAS[String(orbitUserId || '')] || userName;
  return deptOfName(nm);
}
export function classify({ filename, orbitUserId, userName }) {
  const cycle = extractCycle(filename);
  const stage = classifyStage(filename);
  const who = resolveUploader({ orbitUserId, userName });
...
export function ingestFile({ buffer, filename, orbitUserId, userName, hostname, dir, mtime, eventType }) {
  ...
  const c = classify({ filename: fn, orbitUserId, userName });
  const idx = fileRows();
...
export function reclassifyAll(user) {
  ...
  for (const r of latest.values()) {
    if (r.deleted || r.correctedBy) continue; // 사람이 손으로 고친 건 건드리지 않는다
    scanned++;
    const c = classify({ filename: r.filename, orbitUserId: r.orbitUserId, userName: r.uploaderName });
    if (c.cycle === (r.cycle || '') && c.stage === r.stage && !!c.sensitive === !!r.sensitive) continue;
    appendIndex({ ...r, cycle: c.cycle, stage: c.stage, sensitive: c.sensitive, confidence: c.confidence, correctedAt: new Date().toISOString(), correctedBy: 'auto-reclassify' });
    changed++;
  }
...
// ── 접근 ─────────────────────────────────────────────────────────────────────
const canAdmin = (user) => isOrbitReportViewer(user);
