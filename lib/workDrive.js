...
const STAGE_RULES = [
  ['송금·경영', /지출결의|결의서|송금|외화|수입비교|면장|세금계산|계산서|정산서|급여대장|예산|결산/i],
  ['품질', /불량|quality|claim|클레임|품질|하자|이슈|issue/i],
  ['원가·운임', /원가|운임|freight|arrival|도착원가|통관|관세|customs|물류비/i],
  ['견적·거래처', /견적|estimate|quotation|거래명세|명세서|invoice to|청구/i],
  ['입고', /입고|proforma|packing|awb|phyto|b\/l|bl\b|shipping|holex|선적|인보이스|invoice|\bci\b|\bco\b/i],
  ['출고', /출고|분배|shipment|배송|납품/i],
  ['발주', /발주|order|pedido|주문|물량표|취합|cloud|farm|농장|수국|장미|카네이션|알스트로|콜롬비아|에콰도르|ecua|colombia/i], // 3802_콜롬비아수국 같은 농장 발주(품종·국가만 있는 이름)
];
