...
export function checkAutomationAuth(req) {
  // ECOUNT_INGEST_TOKEN: ECOUNT 스크래퍼 데몬 전용(2026-08-07) — 기존 두 토큰과 독립 회전 가능
  const tokens = [process.env.MOYI_API_TOKEN, process.env.AUTOMATION_API_TOKEN, process.env.ECOUNT_INGEST_TOKEN].filter(Boolean);
...
/** MOYI가 Nenova 조회 브리지를 호출할 때만 사용. 수집기 전용 토큰은 허용하지 않는다. */
export function checkMoyiAutomationAuth(req) {
