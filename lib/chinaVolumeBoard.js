...
// 확정 출고(row.outOrders)가 아직 없으면 확정 전 전산 주문(row.orders)까지 내려가서 본다.
// 확정되면 더 정확한 확정 출고 수량을 우선한다.
export function chinaSystemOrderQuantity(row, custName) {
  const confirmed = Number(row?.outOrders?.[custName] || 0);
  if (confirmed > 0) return confirmed;
  return Number(row?.orders?.[custName] || 0);
}

// 박스번호를 오름차순 정리한 뒤, 바로 다음 숫자로 3개 이상 끊김없이 이어지는 구간만
// "시작~끝"으로 압축한다. 정확히 2개만 연속이면 그대로 콤마 나열을 유지한다.
export function formatChinaBoxNumberList(boxNumbers = []) {
  const cleaned = (boxNumbers || []).map(value => String(value ?? '').trim()).filter(Boolean);
  const numeric = [];
  const nonNumeric = [];
  for (const value of cleaned) {
    if (/^\d+$/.test(value)) numeric.push(Number(value));
    else nonNumeric.push(value);
  }
  ...
  while (i < numeric.length) {
    let j = i;
    while (j + 1 < numeric.length && numeric[j + 1] === numeric[j] + 1) j += 1;
    const runLength = j - i + 1;
    if (runLength >= 3) {
      parts.push(`${numeric[i]}~${numeric[j]}`);
    } else {
      for (let k = i; k <= j; k += 1) parts.push(String(numeric[k]));
    }
    i = j + 1;
  }
...
export function chinaVolumeCellText(quantity, allocations = []) {
  ...
  const boxNumbers = allocations.map(item => String(item?.boxNo || '').trim()).filter(Boolean);
  const boxText = formatChinaBoxNumberList(boxNumbers);
  return boxText ? `${quantityText} (${boxText})` : (qty > 0 ? quantityText : '');
}

export function buildChinaVolumeWorkbookRows({ year, week, rows = [], customers = [], cells = {}, appliedOnly = false, customerDays = {} }) {
  const includedCustomers = appliedOnly
    ? customers.filter(customer => rows.some(row => Number(cells[`${customer.custKey}:${row.prodKey}`]?.quantity || 0) > 0))
    : customers;
  const includedRows = appliedOnly
    ? rows.filter(row => includedCustomers.some(customer => Number(cells[`${customer.custKey}:${row.prodKey}`]?.quantity || 0) > 0))
    : rows;
  return [
    [`${year}년 ${week} 중국 물량표`, ...includedCustomers.map(customer => customer.area || '기타'), '', '', '', ''],
    ['출고요일', ...includedCustomers.map(customer => customerDays[customer.custKey] || ''), '', '', '', ''],
    ['품목', ...includedCustomers.map(customer => `${customer.custName}${customer.orderCode ? `\n${customer.orderCode}` : ''}`), '주문', '입고', '잔량', '품목'],
    ...includedRows.map(row => {
      const productLabel = chinaVolumeProductLabel(row.prodName);
      let orderTotal = 0;
      let incomingTotal = 0;
      const customerCells = includedCustomers.map(customer => {
        const saved = cells[`${customer.custKey}:${row.prodKey}`];
        const pivotQuantity = chinaSystemOrderQuantity(row, customer.custName);
        const orderQuantity = Number(saved?.orderQuantity ?? pivotQuantity);
        const incomingQuantity = appliedOnly ? Number(saved?.quantity || 0) : (saved?.quantity ?? pivotQuantity);
        orderTotal += orderQuantity;
        incomingTotal += incomingQuantity;
        return chinaVolumeCellText(incomingQuantity, saved?.allocations || []);
      });
      const remainTotal = Math.round((orderTotal - incomingTotal) * 1000) / 1000;
      return [productLabel, ...customerCells, orderTotal || '', incomingTotal || '', remainTotal || '', productLabel];
    }),
  ];
}

export function stepChinaOrderWeek(value, delta) {
  const match = String(value || '').trim().match(/^(\d{1,2})(?:-(\d{1,2}))?$/);
  const major = Math.min(52, Math.max(1, Number(match?.[1] || 35) + Number(delta || 0)));
  const detail = Math.min(99, Math.max(1, Number(match?.[2] || 1)));
  return `${String(major).padStart(2, '0')}-${String(detail).padStart(2, '0')}`;
}

export function canApplyChinaPackingRows(rows) {
  return Array.isArray(rows) && rows.length > 0 && rows.every(row => {
    if (row?.mappingStatus !== 'MATCHED') return false;
    const check = validateChinaPackingDistribution(row);
    return check.valid;
  });
}

export function chinaPackingDistributions(row) {
  if (Array.isArray(row?.distributions) && row.distributions.length) {
    return row.distributions.map(item => ({ cellKey: String(item?.cellKey || ''), quantity: Number(item?.quantity || 0) }));
  }
  return row?.mappingStatus === 'MATCHED' && row?.cellKey
    ? [{ cellKey: String(row.cellKey), quantity: Number(row.quantity || 0) }]
    : [];
}

export function validateChinaPackingDistribution(row) {
  const sourceQuantity = Math.round(Number(row?.quantity || 0) * 1000) / 1000;
  const distributions = chinaPackingDistributions(row);
  const distributedQuantity = Math.round(distributions.reduce((sum, item) => sum + Number(item.quantity || 0), 0) * 1000) / 1000;
  const remainingQuantity = Math.round((sourceQuantity - distributedQuantity) * 1000) / 1000;
  return {
    sourceQuantity,
    distributedQuantity,
    remainingQuantity,
    valid: sourceQuantity > 0 && distributions.length > 0
      && distributions.every(item => /^\d+:\d+$/.test(item.cellKey) && Number(item.quantity || 0) > 0)
      && Math.abs(remainingQuantity) < 0.001,
  };
}

export function setChinaPackingRowDistributions(rows, sourceRow, distributions) {
  return (rows || []).map(row => Number(row?.sourceRow) === Number(sourceRow)
    ? { ...row, distributions: (distributions || []).map(item => ({ cellKey: String(item.cellKey || ''), quantity: Number(item.quantity || 0) })) }
    : row);
}

export function distributeChinaBoxAllocations(allocations, distributions) {
  const boxes = (allocations || []).map(item => ({ boxNo: String(item.boxNo || ''), remaining: Number(item.quantity || 0) }));
  return (distributions || []).map(distribution => {
    let needed = Number(distribution.quantity || 0);
    const assigned = [];
    for (const box of boxes) {
      if (needed <= 0) break;
      if (box.remaining <= 0) continue;
      const quantity = Math.round(Math.min(needed, box.remaining) * 1000) / 1000;
      if (quantity > 0) assigned.push({ boxNo: box.boxNo, quantity });
      box.remaining = Math.round((box.remaining - quantity) * 1000) / 1000;
      needed = Math.round((needed - quantity) * 1000) / 1000;
    }
    return { ...distribution, allocations: assigned };
  });
}

export function applyChinaPackingCustomerMatch(rows, sourceRow, customer) {
  return (rows || []).map(row => {
    if (Number(row?.sourceRow) !== Number(sourceRow)) return row;
    const product = row.product || null;
    return {
      ...row,
      customer,
      mappingStatus: product ? 'MATCHED' : 'PRODUCT_UNMATCHED',
      cellKey: product ? `${customer.custKey}:${product.prodKey}` : '',
    };
  });
}

export function rematchChinaPackingRow(rows, sourceRow, customer, product) {
  const custKey = Number(customer?.custKey || 0);
  const prodKey = Number(product?.prodKey || 0);
  if (!custKey || !prodKey) return rows || [];
  return (rows || []).map(row => Number(row?.sourceRow) === Number(sourceRow)
    ? {
        ...row,
        customer,
        ...
        cellKey: `${custKey}:${prodKey}`,
        distributions: [{ cellKey: `${custKey}:${prodKey}`, quantity: Number(row.quantity || 0) }],
      }
    : row);
}
...
export function planChinaBoxNeighborAreas({ rows = [], customers = [], cells = {} }) {
  ...
  const isEmpty = (rowIndex, columnIndex) => {
    ...
    const quantity = saved?.quantity ?? chinaSystemOrderQuantity(rows[rowIndex], customers[columnIndex]?.custName);
...
export function mergeChinaCellAllocations(matchedRows) {
  ...
  for (const row of matchedRows || []) {
    ...
    const distributions = distributeChinaBoxAllocations(row.allocations, chinaPackingDistributions(row));
    for (const distribution of distributions) {
      if (!distribution.cellKey || !(Number(distribution.quantity || 0) > 0)) continue;
      const current = cells[distribution.cellKey] || { quantity: 0, allocations: [], sourceRows: [] };
      current.quantity += Number(distribution.quantity || 0);
      current.allocations.push(...distribution.allocations);
      current.sourceRows.push(row.sourceRow);
      cells[distribution.cellKey] = current;
    }
  }
  return cells;
}
...
export function mergeChinaPackingIntoPivotCells(matchedRows, pivotData) {
  ...
  return Object.fromEntries(Object.entries(automatic).map(([cellKey, cell]) => {
    ...
    const pivotQuantity = chinaSystemOrderQuantity(product, customer?.custName);
    return [cellKey, {
      ...cell,
      packingQuantity: cell.quantity,
      orderQuantity: pivotQuantity,
      quantity: cell.quantity,
    }];
  }));
}

export function restoreChinaPackingCells(savedCells, matchedRows, pivotData) {
  const automatic = mergeChinaPackingIntoPivotCells(matchedRows, pivotData);
  return Object.fromEntries(Object.entries(automatic).map(([key, cell]) => [key, {
    ...cell,
    ...(savedCells?.[key] || {}),
    quantity: cell.packingQuantity,
    packingQuantity: cell.packingQuantity,
    orderQuantity: cell.orderQuantity,
    allocations: savedCells?.[key]?.allocations || cell.allocations,
  }]));
}
...
export function summarizeChinaVolumeTotals({ pivotData, packingRows = [], cells = {} } = {}) {
  ...
  for (const row of rows) {
    for (const customer of customers) {
      ...
      const quantity = Number(raw || 0) || Number(row?.orders?.[customer.custName] || 0);
  ...
  for (const packingRow of packingRows || []) {
    ...
    if (packingRow?.mappingStatus === 'MATCHED' && packingRow?.cellKey) {
      ...
      for (const distribution of chinaPackingDistributions(packingRow)) {
        if (!distribution.cellKey) continue;
        packingByCell[distribution.cellKey] = roundedChinaTotal((packingByCell[distribution.cellKey] || 0) + Number(distribution.quantity || 0));
      }
    } else {
      unmatchedRowCount += 1;
  ...
  for (const cellKey of cellKeys) {
    ...
    const invoiceDifference = roundedChinaTotal(packingQuantity - (pivotByCell[cellKey] || 0));
    ...
    if (Math.abs(invoiceDifference) >= CHINA_TOTAL_EPSILON) {
      invoiceMismatches.push(comparison);
    }
