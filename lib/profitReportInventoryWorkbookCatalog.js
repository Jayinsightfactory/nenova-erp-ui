 ...
 * - 이 catalog는 정확히 2026년 28차 재고잔량 원본의 단가 근거다. 22~27차는 각 workbook F 셀의
 *   역사 parity 경로를 유지하고, 29차 이후는 해당 차수의 확정 도착원가/품목 단가 근거가 필요하다.
...
export function inventoryWorkbookPriceEvidenceByProduct(orderYear, orderWeek) {
  ...
  for (const entry of catalog.entries || []) {
    ...
    result[String(entry.prodKey)] = {
      ...
      // 기존 읽기 전용 소비자의 표시 호환성을 유지하되, 적용 범위는 effectiveScope로
      // 반드시 함께 해석한다. 이 필드는 28차 이후 계속 유효하다는 뜻이 아니다.
      effectiveFrom: `${catalog.source.orderYear}-${String(catalog.source.majorWeek).padStart(2, '0')}`,
      effectiveScope: `${catalog.source.orderYear}-${String(catalog.source.majorWeek).padStart(2, '0')} only`,
    };
  }
  return result;
}
