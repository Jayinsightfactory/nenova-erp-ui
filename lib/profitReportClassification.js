...
export const DOMESTIC_SAMPLE_CATEGORY = { key: '국내', coun: '국내', flower: /왁스/, product: /샘플/ };

/** 원본 workbook의 역사 행: 22~27차는 공제, 28차부터는 국내 샘플 행이다. */
export function profitReportCategoriesForWeek(majorWeek) {
  const major = Number(majorWeek);
  if (major >= 28) return [...CATEGORIES.slice(0, -1), { ...DOMESTIC_SAMPLE_CATEGORY }];
  return CATEGORIES.map(category => ({ ...category }));
}

export const EXTRA_CATEGORY = '기타(미분류)';

// CNF(C&F, 운임포함) 거래 카테고리 — 사장님 확인(2026-08-26): 호주·베트남은 항공운임이
// 매입단가에 이미 포함되어 별도 운송료 전표가 존재하지 않는 것이 정상 구조다.
// 포워딩(S)=0이 결함이 아니므로 항공료 누락 감사와 기대범위 판정에서 제외한다.
// 매입액(Q)에 운임이 들어있어 재고평가 공식((Q×R+S×R+H)÷매입수량)도 그대로 정확하다.
export const CNF_CATEGORIES = Object.freeze(['호주', '베트남']);
...
export function classifyCategory(counName, flowerName, productName = '') {
  ...
  if (/수국\s*운송료/.test(product)) return '콜롬비아 수국';
  ...
  if (/호주\s*운송료/.test(product)) return '호주';
  if (/태국\s*운송료/.test(product)) return '태국';
  if (/중국\s*운송료/.test(product)) return '중국';
  if (/레몬잎\s*운송료/.test(product)) return '미국';
  // 경영지원 placeholder가 국내/왁스로 저장되어도 품명의 명시 국가 단서를 우선한다.
  if (/CHINA|중국/i.test(product)) return '중국';
  if (coun.includes('국내') && flower.includes('왁스') && /샘플/.test(product)) return '국내';
