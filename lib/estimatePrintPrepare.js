...
export function prepareEstimatePrintRows(rawRows, {
  ...
  const descrOpts = {
    showDistribDesc,
    showDeductionOutDay,
    showDeductionDescr: showDeductionDescr === true,
  };
  const statementFormat = isStatementPrintFormat(printFormat);
  const supplyFormat = isSupplyPrintFormat(printFormat);
  const fmtN = (n) => Number(n || 0).toLocaleString();
  const descLabel = (r) => formatEstimatePrintDescr(r, descrOpts);

  // FormEstimateView.GetPrintDetail은 SQL에서 이미
  // (ProdKey, Cost) 단위로 정상출고를 합산하고, Estimate 차감행은
  // 행별로 UNION 한다. 이 결과를 다시 웹 규칙으로 합치거나
  // 국가/화종 우선순위로 정렬하면 nenova.exe와 다른 출력이 된다.
  // API가 _exePrint를 붙인 경우에는 DB 반환 순서·수량·금액·적요를
  // 그대로 보존하고, 수량 0 행만 EXE와 같은 규칙으로 제외한다.
  // 부가세 미분류 양식만 Cost(분배단가)를 공급가 단가로 다시 계산한다.
  const exeLayout = !statementFormat
    && Array.isArray(rawRows)
    && rawRows.length > 0
    && rawRows.every((row) => row?._exePrint === true);
  ...
  if (exeLayout) {
    let rows = rawRows
      ...
      .map((row) => ({
        ...row,
        // EXE ReportEstimate는 SQL의 UnitQuantity를 그대로 표시한다.
        UnitQuantity: row.UnitQuantity || `${fmtN(row.Quantity)}${row.Unit || ''}`,
        // ReportEstimate.Descr 바인딩은 DB 원문이다. 화면용 운영 로그
        // 정리 규칙을 정상출고·검역·단가차감에 적용하지 않는다.
        // 불량차감 적요만 웹 인쇄 옵션(기본 미표시)을 따른다.
        DescrRaw: row.Descr ?? '',
        Descr: row.Descr ?? '',
      }));
    if (supplyFormat) rows = rows.map(applySupplyPrintAmounts);
    ...
    return {
      ...
      descLabel: (r) => {
        if (isDefectDeductionRow(r) && descrOpts.showDeductionDescr !== true) {
          return descrOpts.showDeductionOutDay ? formatEstimateDeductionOutDay(r) : '';
        }
        return r.Descr ?? '';
      },
      estimateTypeLabel,
      exePrintParity: !supplyFormat,
    };
  }
  ...
  rows.forEach((r) => {
    ...
    if (!groups[key]) {
      groups[key] = {
        ...r, Quantity: 0, BoxQty: 0, Amount: 0, Vat: 0, _breakdown: {}, _outDates: new Set(), _descrParts: [],
      };
    }
    const g = groups[key];
    ...
    g._descrParts.push(r.DescrRaw ?? r.Descr ?? '');
  ...
  rows = Object.values(groups).map((g) => {
    ...
    // 부가세 미분류는 저장된 분배단가(Cost)를 공급가 단가로 쓰므로
    // Amount+Vat로 부가세포함 단가를 다시 만들지 않는다.
    if (!supplyFormat && qty > 0 && (supply + vat) > 0) g.Cost = Math.round((supply + vat) / qty);
    const mergedRaw = mergeEstimateDescrRaw(...(g._descrParts || []));
    g.DescrRaw = mergedRaw;
    g.Descr = isEstimateDeductionRow(g)
      ? formatEstimateDeductionDescr(mergedRaw)
      : sanitizeDescrTextForPrint(mergedRaw);
    delete g._descrParts;
  ...
  else if (supplyFormat) rows = rows.map(applySupplyPrintAmounts);
