...
export function prepareEstimatePrintRows(rawRows, {
  ...
  // 화면·인쇄 공통 — 차감은 EXE Estimate.Descr, 정상출고는 운영로그 제외
  const descLabel = (r) => {
    if (showDistribDesc && !isEstimateDeductionRow(r) && r?._distribDesc) return r._distribDesc;
    const memo = sanitizeEstimateDescrForDisplay(r);
    if (memo) return memo;
    if (isEstimateDeductionRow(r)) return formatEstimatePrintDescr(r, { showDistribDesc });
    return '';
  };
