...
// 기록 대상: 로그인한 직원 계정 전부. 사장님·관리용 계정(NO_RECORD_USER_IDS)은 제외 — 본인 업무는 이미 자동화돼 있어 관찰 가치가 없다.
// 열람은 nenovaSS3 만.
// 저장: data/replay/<userId>/<sessionId>.jsonl (이벤트 1줄 1건) + <sessionId>.meta.json (목록·단계 요약)
// 상한: 세션 8MB · 사용자 150MB · 14일 — 넘으면 오래된 세션부터 지운다(서버 디스크 보호).
import fs from 'fs';
import path from 'path';
import { isOrbitReportViewer } from './orbitReportAccess';

export const NO_RECORD_USER_IDS = Object.freeze(['nenovaSS3', 'admin']);
const NO_RECORD_SET = new Set(NO_RECORD_USER_IDS.map((x) => x.toLowerCase()));
const SESSION_MAX = 8 * 1024 * 1024, USER_MAX = 150 * 1024 * 1024, KEEP_DAYS = 14, STEP_MAX = 800; // 계정 ~12개 × 150MB = 최대 1.8GB
...
export function canRecord(user) { const id = uidOf(user).toLowerCase(); return id !== '' && !NO_RECORD_SET.has(id); }
