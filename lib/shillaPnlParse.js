...
function sourceValueText(value) {
  ...
  return normSpace(value) || '빈칸';
}

function sourceRowSummary(values) {
  if (!values) return '';
  return [
    ['품명', values.name], ['칼라', values.color], ['입고수량', values.qty],
    ['매입단가', values.buyPrice], ['매입액', values.buyAmount],
    ['매출단가', values.sellPrice], ['매출액', values.sellAmount],
  ].map(([label, value]) => `${label}=${sourceValueText(value)}`).join(', ');
}
...
function parseSheet(sheetName, aoa, meta, orderYear) {
  ...
  for (let r = header.row + 1; r < aoa.length; r += 1) {
    ...
    const sourceValues = {
      ...
      qty: header.qty >= 0 ? row[header.qty] : null,
      buyPrice: header.buyPrice >= 0 ? row[header.buyPrice] : null,
      buyAmount: header.buyAmount >= 0 ? row[header.buyAmount] : null,
      sellPrice: header.sellPrice >= 0 ? row[header.sellPrice] : null,
      sellAmount: header.sellAmount >= 0 ? row[header.sellAmount] : null,
    };
    items.push({
      seq: items.length + 1,
      name,
  ...
  for (const it of items) {
    if (!it.ok) {
      verification.push({
        ...
        diff: null,
        ok: false,
        sourceRow: it.source?.row ?? null,
        sourceValues: it.sourceValues,
        info: `원본 행 값: ${sourceRowSummary(it.sourceValues)} — ${it.name}: ${it.issues.join(' / ')}`,
      });
    }
  }
...
function warningText(check) {
  ...
  if (check.sheetVal == null && check.parsedVal == null && check.info) {
    return `검증 실패 — [${check.group}] ${check.label}${infoText}`;
  }
