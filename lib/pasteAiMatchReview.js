...
import { getEnglishOf, tokenizeForMatch } from './displayName.js';
...
function customerExactLabels(name) {
  const source=String(name||'').replace(/（/g,'(').replace(/）/g,')');
  const labels=[source];
  for(const match of source.matchAll(/\(([^()]+)\)/g))labels.push(match[1]);
  return [...new Set(labels.map(normalizeCustomerToken).filter(Boolean))];
}
...
export function customerReviewCandidates(input, customers, mappings = {}, proposedKey) {
  ...
  const exact = customers.filter(c => customerExactLabels(c.CustName).includes(normalized));
  ...
  return orders.flatMap((order, oi) => {
    ...
    return [{ id: `c${oi}`, kind: 'customer', input: order.custName, ...customer },
      ...order.items.map((item, ii) => {
        const matchName = item.matchName || item.inputName;
        // The common ranker's direct-alias filter may discard valid varieties when
        // a glued header token (콜장미) is treated as part of the cultivar name.
        // Independently retrieve actual names using translated cultivar tokens.
        const tokens = tokenizeForMatch(matchName).filter(t => !/^(콜)?(장미|카네이션|수국|알스트로)$/.test(t));
        const needles = tokens.map(t => [t, getEnglishOf(t)].filter(Boolean).map(s => s.toLowerCase().replace(/\s+/g, '')));
        const lexical = products.filter(p => productReviewAllowed(matchName, p)).map(p => {
          const text = `${p.ProdName} ${p.DisplayName || ''}`.toLowerCase().replace(/\s+/g, '');
          return { key: p.ProdKey, score: needles.reduce((score, group) => score + (group.some(n => text.includes(n)) ? Math.max(...group.map(n => n.length)) : 0), 0) };
        }).filter(p => p.score > 0).sort((a, b) => b.score - a.score || a.key - b.key).slice(0, 12);
        const keys = [item.prodKey, item.proposedProdKey, ...lexical.map(p => p.key), ...(item.suggestedProducts || []).map(p => p.ProdKey)];
        const candidates = [...new Set(keys.map(Number))].map(key => productByKey.get(key))
          .filter(p => productReviewAllowed(matchName, p)).map(p => ({
            key: Number(p.ProdKey), name: p.ProdName, displayName: p.DisplayName,
            country: p.CounName, flower: p.FlowerName,
          }));
        return { id: `p${oi}:${ii}`, kind: 'product', input: item.inputName, matchContext: matchName, customer: order.custName,
