...
export const EVENT_TYPES = {
  ...
  // 2026-07-08: 신 스키마 AI분류 실측 코드(실 시트 값 확인분만 추가, 전체 taxonomy는 미확인 — 매칭 안 되면 원본 코드 그대로 표시되므로 안전)
  COMM_INFO: '정보 공유',
};
...
export function stageOfRoom(room) {
  const r = String(room || '');
  // 방 정보가 없는 이벤트(예: 비즈니스이벤트 탭엔 room 컬럼이 없음)를 빈 문자열로 넣으면
  // ''.includes(r)가 항상 true라 무조건 첫 단계(IMPORT)로 오분류되던 버그 — 빈 값은 먼저 컷.
  if (!r) return '';
