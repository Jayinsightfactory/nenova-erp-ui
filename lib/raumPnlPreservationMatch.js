...
export function matchRaumPnlPreservationRows(incomingItems, existingRows, partnerCode) {
  ...
  function hasCompleteUniqueContentBijection(indices, groupCandidates) {
    if (indices.length !== groupCandidates.length || indices.length === 0) return false;
    const incomingContents = indices.map(index => contentKey(incoming[index]));
    const existingContents = groupCandidates.map(candidate => candidate.content);
    if (new Set(incomingContents).size !== incomingContents.length) return false;
    if (new Set(existingContents).size !== existingContents.length) return false;
    const existingSet = new Set(existingContents);
    return incomingContents.every(signature => existingSet.has(signature));
  }
  ...
  function safelyMatch(indices, index, keyForItem, viaFallback) {
    ...
    for (const [key, groupIndices] of groups) {
      ...
      // Stable exact groups may cross source rows only for a complete bijection:
      // every qty/supply signature is unique on both sides and both sets agree.
      // A partial match (for example B,C,C against A,B,C) must leave all source
      // rows for stable-source matching rather than consume B first.
      const stableExactContentBijection = stableSources && !viaFallback;
      const allowContentStage = !stableExactContentBijection || hasCompleteUniqueContentBijection(groupIndices, groupCandidates);
      if (allowContentStage) {
