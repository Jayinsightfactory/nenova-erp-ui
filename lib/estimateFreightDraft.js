...
export function combineCarnationFreight(drafts, sources, parentWeek, rounding, enabled = true) {
  ...
  const dates = [...new Set(sources.filter(row => row.OrderWeek === target && !row.scopeError).map(row=>row.outDate).filter(Boolean))];
...
export function validateFreightDraft(rows, { year, parentWeek, custKey, products, existing = [] }) {
  ...
  return rows.map(row => {
    ...
    if (row.combined && existing.some(item=>row.sourceWeeks.includes(item.OrderWeek) && Number(item.ProdKey)===Number(row.prodKey))) throw new Error(`${row.name}: 1·2차에 기존 운임이 있습니다. 중복 등록하지 않습니다. 기존 운임을 먼저 확인하세요.`);
