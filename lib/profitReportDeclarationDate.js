// lib/profitReportDeclarationDate.js — 과거 날짜 우선순위 진단용 SELECT 전용 모듈(현재 계산 미사용).
//
// ## 목적
// 과거에는 InputDate가 비어 있을 때 ArrivalDtm/UploadDtm으로 대신하는 방안을 진단하기 위해 만들었다.
// 현재 매출이익보고서 계산은 이 모듈을 호출하지 않는다. 실제 KCS 기준일은 사용자가 입고에 명시한
// WarehouseMaster.InputDate가 최우선이며, 누락 시 ArrivalDtm/UploadDtm으로 대체하지 않는다.
// 원본 반복 규칙과 사용자 확정이 있는 호주 AUD 일정 보완은 lib/kcsRateDateWeights.js의 명시적
// registry가 담당하고, 그 밖의 누락은 입력 필요로 남긴다.
// 실제 카테고리별 TPrice 가중 날짜/가중치 집계는 lib/kcsRateDateWeights.js가 직접 수행한다.
// 이 모듈은 날짜 하나를 미리
...
// 이 동적 우선순위는 과거 진단을 보존하기 위한 것이며 실제 KCS 계산 원천이 아니다.
