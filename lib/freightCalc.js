...
export function computeFreightCost({ master, basis = 'AUTO', customs = {}, details = [], productMeta, flowerMeta }) {
  ...
  const rowsResolved = details.map(d => {
    ...
    return { ...d, steamQty, stemsPerBunch: effStemsPerBunch, _fnKey: fnKey, _boxWeight: boxWeight, _boxCBM: boxCBM, _stemsPerBox: stemsPerBox, _tariffRate: tariffRate, _steamQtySource: steamQtySource, _stemsPerBunchSource: stemsPerBunchSource, _outUnit: (d.outUnit || '').trim() };
  });
  ...
  const rows = rowsResolved.map(r => {
    ...
    // F는 DB UPrice 그대로(=display 단위 단가). G/L/M 만 카테고리에서 가져와 환산.
    ...
    else displayQty = E;  // 송이
    const displayFreightUSD = Number(b.freightPerDisplayUnit) || 0;     // 카테고리 displayUnit 당 운임
    const displayCustomsKRW = Number(b.customsPerDisplayUnit) || 0;     // 카테고리 displayUnit 당 통관
    const displayCnfUSD = F + displayFreightUSD;                         // displayUnit 당 CNF USD
    ...
    return {
      ...
      displayFobUSD: F,                                  // DB UPrice = display 단위 단가
