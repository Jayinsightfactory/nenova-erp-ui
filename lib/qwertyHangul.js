...
/**
 * 한글 입력 모드에서 필드 전체가 현재 QWERTY 버퍼의 변환 결과일 때만 편집한다.
 * 선택된 거래처명·IME 한글·커서 중간 편집은 가로채지 않아 기존 값을 보존한다.
 */
export function editHangulSearchBuffer({
  display = '',
  buffer = '',
  key = '',
  selectionStart,
  ...
  const ownsDisplay = text === qwertyToHangul(raw);
  const cursorAtEnd = start === text.length && end === text.length;

  if (!ownsDisplay || !cursorAtEnd) {
    return { handled: false, display: text, buffer: raw };
  }

  if (key === 'Backspace') {
    if (!raw) return { handled: false, display: text, buffer: raw };
    const nextBuffer = raw.slice(0, -1);
    return {
      handled: true,
      display: nextBuffer ? qwertyToHangul(nextBuffer) : '',
      buffer: nextBuffer,
    };
  }

  if (String(key).length !== 1 || !/[a-zA-Z0-9 ._-]/.test(key)) {
    return { handled: false, display: text, buffer: raw };
  }

  const nextBuffer = raw + key;
  return {
    handled: true,
    display: qwertyToHangul(nextBuffer),
    buffer: nextBuffer,
  };
}
