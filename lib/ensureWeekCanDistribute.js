...
import { findFixStatusWeek, resolveFixStatusOrderYear } from './fixStatusYearScope';

/** 확정 차수/품목군이면 출고분배·주문등록+분배 차단 (paste.js 와 동일) */
export async function ensureWeekCanDistribute(targetWeek, prodKeys = [], selectedYear = '') {
  if (!targetWeek) {
    alert('차수를 선택하세요.');
    return false;
  }
  try {
    const orderYear = resolveFixStatusOrderYear(selectedYear, targetWeek);
    ...
    const fixedInfo = findFixStatusWeek(d.weeks || [], { orderYear, orderWeek: targetWeek });
