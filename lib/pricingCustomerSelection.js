...
// query starts with all returned products selected; picker filtering never
// removes hidden or manually deselected products.
export function filterPricingProducts(products, search) {
  const rows = Array.isArray(products) ? products : [];
  const term = String(search || '').trim().toLocaleLowerCase();
  if (!term) return rows;
  return rows.filter((p) => [p.ProdName, p.DisplayName, p.FlowerName, p.CounName, p.ProdCode]
