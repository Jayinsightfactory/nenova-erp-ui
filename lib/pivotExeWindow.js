...
/** Converts page-relative table geometry to body coordinates; pass headerHeight: 0 to the row window. */
export function getPivotExePageViewport({ tableTop = 0, headerHeight = 0, viewportHeight = 0 } = {}) {
  const bodyTop = tableTop + headerHeight;
  return {
    scrollTop: Math.max(0, -bodyTop),
    clientHeight: Math.max(0, viewportHeight - Math.max(0, bodyTop)),
  };
}
