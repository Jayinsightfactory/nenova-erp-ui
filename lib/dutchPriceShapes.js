...
function shapeXml(shape, index) {
  ...
  return `<xdr:twoCellAnchor editAs="oneCell"><xdr:from><xdr:col>${shape.col}</xdr:col><xdr:colOff>70000</xdr:colOff><xdr:row>${shape.row}</xdr:row><xdr:rowOff>50000</xdr:rowOff></xdr:from><xdr:to><xdr:col>${shape.col + 1}</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>${shape.row + 1}</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:to><xdr:sp macro=""><xdr:nvSpPr><xdr:cNvPr id="${index + 2}" name="단가 ${index + 1}"/><xdr:cNvSpPr txBox="1"/></xdr:nvSpPr><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:solidFill><a:srgbClr val="FFFDF8"/></a:solidFill><a:ln w="12700"><a:solidFill><a:srgbClr val="C0504D"/></a:solidFill></a:ln></xdr:spPr><xdr:txBody><a:bodyPr wrap="none" anchor="ctr" lIns="20000" rIns="20000" tIns="0" bIns="0"/><a:lstStyle/><a:p><a:pPr algn="ctr"/><a:r><a:rPr lang="ko-KR" sz="700" b="1"/><a:t>${esc(price)}</a:t></a:r><a:endParaRPr lang="ko-KR" sz="700"/></a:p></xdr:txBody></xdr:sp><xdr:clientData/></xdr:twoCellAnchor>`;
}
...
export async function addDutchPriceShapesToXlsx(XLSX, buffer, workbook, entries, prices) {
  ...
  for (const [sheetName, sheetPath] of paths) {
    ...
    const shapes = (entries || []).filter(entry => entry.sheetName === sheetName && Number(prices?.[entry.id]) > 0).map(entry => ({
      row: XLSX.utils.decode_cell(entry.cellAddress).r,
      col: pickOverlayColumn(XLSX, sheet, entry.cellAddress, occupied),
      price: Number(prices[entry.id]),
    }));
