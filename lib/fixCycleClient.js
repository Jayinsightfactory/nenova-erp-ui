...
export async function runShipmentFixAction(week, action, countryFlowers = [], stockProdKeys = [], selectedYear = '') {
  const orderYear = resolveFixStatusOrderYear(selectedYear, week);
  const { data: d } = await postShipmentFix({ week, orderYear, action, force: true, countryFlowers, stockProdKeys });
  if (!d.success) {
...
export async function runEditWithFixCycle({ weeks, orderYear = '', countryFlowers = [], stockProdKeys = [], progress, apply }) {
  ...
  try {
    for (const wk of sortWeeksDesc(targetWeeks)) {
      progress?.(`${wk} 확정해제 중`);
      await runShipmentFixAction(wk, 'unfix', countryFlowers, stockProdKeys, orderYear);
    ...
    for (const wk of sortWeeksAsc(unfixedWeeks)) {
      ...
      await runShipmentFixAction(wk, 'fix', countryFlowers, stockProdKeys, orderYear);
    }
    throw err;
  }
