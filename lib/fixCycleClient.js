...
import { buildFixStatusQuery, findFixStatusWeek, resolveFixStatusOrderYear } from './fixStatusYearScope';
...
export async function fetchWeekFixStatus(week, selectedYear) {
  const orderYear = resolveFixStatusOrderYear(selectedYear, week);
  const res = await fetch(
    `/api/shipment/fix-status?${buildFixStatusQuery({ orderYear, fromWeek: week, toWeek: week })}`,
    { credentials: 'same-origin' },
  );
...
export async function runShipmentFixAction(week, action, countryFlowers = [], stockProdKeys = [], selectedYear) {
  const orderYear = resolveFixStatusOrderYear(selectedYear, week);
  // 자동 편집 사이클은 뒤 차수 확정 경고를 우회하지 않는다. 대상 구간 산출이
  // 빠졌다면 서버가 중단시켜야 downstream 재고 스냅샷이 오래된 채 남지 않는다.
  const { data: d } = await postShipmentFix({ week, orderYear, action, force: false, countryFlowers, stockProdKeys });
  if (!d.success) {
...
export async function runEditWithFixCycle({ weeks, orderYear, countryFlowers = [], stockProdKeys = [], progress, apply }) {
  ...
  try {
    for (const wk of sortWeeksDesc(targetWeeks)) {
      progress?.(`${wk} 확정해제 중`);
      await runShipmentFixAction(wk, 'unfix', countryFlowers, stockProdKeys, selectedYear);
    ...
    for (const wk of sortWeeksAsc(unfixedWeeks)) {
      ...
      await runShipmentFixAction(wk, 'fix', countryFlowers, stockProdKeys, selectedYear);
    }
    throw err;
  }
