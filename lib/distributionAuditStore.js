...
function createDistributionAuditStore({ root = DEFAULT_ROOT, now = () => new Date(), randomUUID = () => crypto.randomUUID() } = {}) {
  ...
  async function listAuditReports({ year, week } = {}) {
    const scope = validateScope({ year, week });
    let names;
    try {
      names = await fs.promises.readdir(storageRoot);
  ...
  return { root: storageRoot, saveAudit, listAudits, listAuditReports, getAudit };
}
...
module.exports = {
  ...
  listAuditReports: (input, options) => storeFor(options).listAuditReports(input),
