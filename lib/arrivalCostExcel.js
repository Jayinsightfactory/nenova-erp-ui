...
function findTableHeader(aoa) {
  ...
  for (let r = 0; r < Math.min(aoa.length, 80); r += 1) {
    ...
    const qty = findIndex(headers, (k) => /^(수량|quantity|qty)$/.test(k));
...
function buildProductLookups(products) {
  const list = (products || []).map((p) => ({
    ...p,
    _names: [p.ProdName, p.DisplayName].map(normalizeArrivalText).filter(Boolean),
  }));
  return list;
}
...
function matchProduct(rawName, products, mappings = {}) {
  ...
  if (learnedKey > 0) {
    const learnedProduct = products.find((product) => Number(product.ProdKey) === learnedKey);
...
function matchFarm(rawName, farms) {
  const query = normalizeArrivalText(rawName);
  if (!query) return null;
  return (farms || []).find((farm) => {
    const name = normalizeArrivalText(farm.FarmName);
...
function findArrivalColumns(headers) {
  ...
  return {
    ...
    quantity: findIndex(headers, (k) => /^(수량|quantity|qty)$/.test(k)),
...
function inferMeta({ aoa, sheetName, fileName, defaultOrderYear, defaultOrderWeek }) {
  ...
  const source = `${sheetName} ${headText} ${fileName}`;
  const orderWeek = inferOrderWeek(source) || text(defaultOrderWeek || '');
  ...
  const grossWeight = parseArrivalNumber(firstMatchingValue(aoa, /gross\s*weigh|grossweight|^gw$/i));
  const chargeableWeight = parseArrivalNumber(firstMatchingValue(aoa, /chargeable|chargeableweight|^cw$|vracht/i));
...
export function parseArrivalCostWorkbook(input, {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    if (productIdx >= 0 && quantityIdx >= 0 && !isNumeric(probe[quantityIdx]) && isNumeric(probe[quantityIdx + 1])) {
      ...
      productIdx += 1;
      quantityIdx += 1;
      for (const key of ['fob', 'freightPerUnit', 'customsPerUnit', 'otherPerUnit', 'unitCount', 'stemCost', 'unitCost', 'vatCost']) {
    ...
    for (let r = table.rowIndex + 1; r < aoa.length; r += 1) {
      ...
      if (!rawName || !(quantity > 0) || /^(total|합계|grand total|소계)$/i.test(rawName)) continue;
      const product = matchProduct(rawName, productsList, mappings);
      const farmNameRaw = shiftFarm >= 0 ? text(row[shiftFarm]) : farmRawDefault;
      const farm = matchFarm(farmNameRaw, farms);
      const sourceStem = parseArrivalNumber(row[columns.stemCost]);
      const sourceUnit = parseArrivalNumber(row[columns.unitCost]);
      const sourceArrivalCost = sourceUnit ?? sourceStem;
      ...
      const item = {
        ...
        farmNameRaw,
        prodKey: product?.ProdKey || null,
        farmKey: farm?.FarmKey || null,
        unit: product?.OutUnit || (unitCount && unitCount > 1 ? '단' : '단'),
        quantity,
        fobUSD: parseArrivalNumber(row[columns.fob]),
        freightPerUnitUSD: parseArrivalNumber(row[columns.freightPerUnit]),
        customsPerUnitKRW: parseArrivalNumber(row[columns.customsPerUnit]),
        otherPerUnitKRW: parseArrivalNumber(row[columns.otherPerUnit]),
        ...
        sheetName,
        sourceRow: r + 1,
        rawJson: JSON.stringify({ cells: row, headers: table.headers, meta }),
      };
