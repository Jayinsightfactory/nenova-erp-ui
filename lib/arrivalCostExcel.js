...
import { isArrivalExampleSheet } from './arrivalImportPolicy.js';
...
// 현재 네덜란드 원가자료는 2026년 차수 원장이다. 업로드 화면에 다른 연도가
// 남아 있어도 NL 파일을 전년도 동일 차수에 저장하지 않는다.
export const DUTCH_ARRIVAL_ORDER_YEAR = '2026';

const FLOWER_ALIASES = [
  ['수국', /hydrangea|수국/i],
  ['장미', /\broses?\b|자연장미|장미/i],
  ['백합', /\blily\b|\blilies\b|\broselily\b|백합/i],
...
// Identity comparison must keep origin/brand/grade tokens discarded by search.
export function normalizeArrivalProductIdentity(value) {
  return text(value).normalize('NFKC').toLowerCase().replace(/\s+/g, ' ').trim();
}

function compatibleArrivalSpecification(rawName, productName) {
  const specs = value => {
    const name = normalizeArrivalProductIdentity(value);
    return [
      (name.match(/\b(?:xxl|xl|l|m|s)\s*$/i) || [''])[0].trim(),
      (name.match(/\b\d+\s*cm\b/i) || [''])[0].replace(/\s/g, ''),
      /\bmix\s*box\b/i.test(name) ? 'mixbox' : '',
      (name.match(/\[(?:mel|ez)\]/i) || [''])[0],
      /\bmc\b/i.test(name) ? 'mc' : '',
    ];
  };
  const source = specs(rawName), candidate = specs(productName);
  return source.every((value, index) => value === candidate[index]);
}
...
// 16-1B처럼 머리글에 COLOMBIA가 없는 시트는 같은 차수·같은 파일의 국가를 이어받는다.
export function fillMissingArrivalCountries(rows = []) {
  const named = [...new Set(rows.map((row) => text(row.countryName)).filter(Boolean))];
  const workbookCountry = named.length === 1 ? named[0] : '';
  const byWeek = new Map();
  for (const row of rows) {
    const country = text(row.countryName);
    if (!country) continue;
    const week = String(row.orderWeek || '');
    const found = byWeek.get(week) || new Set();
    found.add(country);
    byWeek.set(week, found);
  }
  for (const row of rows) {
    if (text(row.countryName)) continue;
    const weekCountries = byWeek.get(String(row.orderWeek || ''));
    if (weekCountries && weekCountries.size === 1) row.countryName = [...weekCountries][0];
    else if (workbookCountry) row.countryName = workbookCountry;
  }
  return rows;
}
...
function inferOrderWeek(source) {
  // Thai source cells use `.38-1`; downloaded filenames use `(38-1) (1)`.
  // Keep token boundaries so a date/identifier such as 2026-09-18 is not a week.
  const match = text(source).match(/(?:^|[\s([|:：]|차수\s*)\.?\s*(\d{1,2})\s*[-_]\s*(\d{1,2})(?=$|[\s)\]|,;]|차)/i);
  return match ? `${Number(match[1])}-${Number(match[2])}` : '';
}
...
function matchProduct(rawName, products, mappings = {}) {
  ...
  const identity = normalizeArrivalProductIdentity(rawName);
  const strict = products.filter(product => normalizeArrivalProductIdentity(product.ProdName) === identity);
  if (strict.length === 1) return strict[0];
  if (strict.length > 1) return null;
  // An exact name is stronger evidence than a partial learned alias or a boosted
  // substring score. Never let product iteration order break an exact-name tie.
  const compatibleSpec = product => compatibleArrivalSpecification(rawName, product.ProdName);
  const exact = products.filter(product => compatibleSpec(product) && product._names.includes(query));
  if (exact.length === 1) return exact[0];
  if (exact.length > 1) return null;
  const flower = inferArrivalFlower(rawName);
  const compatibleFlower = product => !flower || !inferArrivalFlower(product.FlowerName)
    || flower === inferArrivalFlower(product.FlowerName);
  ...
  if (learnedKey > 0) {
    ...
    if (learnedProduct && compatibleFlower(learnedProduct) && compatibleSpec(learnedProduct)) return learnedProduct;
  }
  let best = null;
  let bestScore = 0;
  for (const product of products) {
    if (!compatibleFlower(product) || !compatibleSpec(product)) continue;
...
function inferMeta({ aoa, sheetName, fileName, defaultOrderYear, defaultOrderWeek }) {
  ...
  const fallbackWeek = inferOrderWeek(fileName) || text(defaultOrderWeek || '');
  // A numbered historical sheet must not inherit another week's filename.
  const compatibleFallback = !/^\d{1,2}$/.test(text(sheetName))
    || Number(sheetName) === Number(fallbackWeek.split('-')[0]);
  const orderWeek = inferSheetOrderWeek(sheetName)
    || inferOrderWeek(`${sheetName} ${headText}`)
    || (compatibleFallback ? fallbackWeek : '');
  ...
  const orderYear = country === '네덜란드'
    ? DUTCH_ARRIVAL_ORDER_YEAR
    : String(defaultOrderYear || new Date().getFullYear()).slice(0, 4);
...
export function parseArrivalCostWorkbook(input, {
  ...
  for (const sheetName of workbook.SheetNames) {
    if (isArrivalExampleSheet(sheetName)) {
      skippedSheets.push({ sheetName, reason: '예시·견본 시트 제외' });
      continue;
    }
    hydrateSheetFormulas(workbook.Sheets[sheetName]);
    expandMergedCells(workbook.Sheets[sheetName]);
    const ws = workbook.Sheets[sheetName];
    // Formatting-only !ref can extend to XFC across hundreds of rows. Read the
    // actual value/formula extent, without truncating any populated column.
    let end = null;
    for (const [address, cell] of Object.entries(ws)) {
      if (!/^[A-Z]+\d+$/.test(address) || (!cell?.f && (cell?.v == null || cell.v === ''))) continue;
      const at = XLSX.utils.decode_cell(address);
      end = end ? { r: Math.max(end.r, at.r), c: Math.max(end.c, at.c) } : at;
    }
    if (!end) continue;
    const aoa = XLSX.utils.sheet_to_json(ws, {
      header: 1, blankrows: true, defval: null, range: { s: { r: 0, c: 0 }, e: end },
    });
    ...
    for (let r = table.rowIndex + 1; r < aoa.length; r += 1) {
      ...
      if (!(sourceArrivalCost > 0)) {
        rejectedRows.push({ orderYear: meta.orderYear, orderWeek: meta.orderWeek, sheetName, sourceRow: r + 1, productNameRaw: rawName });
        continue;
      }
  ...
  fillMissingArrivalCountries(rows);
  for (const stat of sheetStats) {
    if (stat.country) continue;
    const sample = rows.find((row) => row.sheetName === stat.sheetName && text(row.countryName));
    if (sample) stat.country = sample.countryName;
  }
  return {
