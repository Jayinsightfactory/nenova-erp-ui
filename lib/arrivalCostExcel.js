...
function matchProduct(rawName, products, mappings = {}) {
  ...
  const exact = products.filter(product => product._names.includes(query));
  ...
  if (learnedKey > 0) {
    ...
    if (learnedProduct && compatibleFlower(learnedProduct)) return learnedProduct;
  }
  let best = null;
  let bestScore = 0;
  for (const product of products) {
    if (!compatibleFlower(product)) continue;
