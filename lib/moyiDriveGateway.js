...
export async function loadMoyiDrive(req) {
  ...
  if (!upstream.ok || !Array.isArray(data)) {
    ...
    return { status: upstream.status >= 500 ? 503 : upstream.status, body: pendingDriveModel(reason, 'MOYI_DRIVE_UPSTREAM_REJECTED') };
  }
  ...
  return {
    ...
    body: {
      ...
      capabilities: { ...model.capabilities, listItems: true },
    },
  };
}
