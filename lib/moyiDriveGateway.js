...
export function hasExistingFilesScopeInput(body = {}) {
  return hasCrossWorkspaceInput(body)
    || [
      ...
      'file', 'files', 'file_id', 'fileId', 'file_ids', 'fileIds', 'id', 'ids',
    ]
      .some((key) => Object.prototype.hasOwnProperty.call(body, key));
}
...
export async function previewExistingMoyiFiles(req) {
  ...
  return {
    ...
    body: {
      ...
      preview: normalizeExistingFilesPreview(data),
    },
  };
}

export async function applyExistingMoyiFiles(req) {
  const token = tokenFrom(req);
  if (!token) {
    return { status: 503, body: pendingDriveModel('MOYI 보고 연동을 먼저 연결해 주세요.', 'MOYI_CONNECTION_TOKEN_MISSING') };
  }
  let upstream;
  try {
    // confirm 외에 회사·폴더·파일 식별값은 보내지 않는다. 서버가 연결표에서
    // 현재 회사와 루트 폴더를 다시 계산한다.
