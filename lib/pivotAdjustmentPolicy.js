...
// - AUTO_CANCEL + 활성 분배 있음: 분배만 취소
// - AUTO_CANCEL + 활성 분배 없음: 주문만 취소
...
export function resolvePivotAdjustmentPolicy({ mode, type, hasActiveOrder, hasActiveShipment = false }) {
  ...
  if (autoCancel) {
    ...
    return {
      ...
      mutateOrder: !hasActiveShipment,
      mutateShipment: hasActiveShipment,
      reason: hasActiveShipment ? 'auto_cancel_distribution_only' : 'auto_cancel_order_only',
    };
  }
