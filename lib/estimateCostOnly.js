...
export async function executeEstimateCostOnly(tQ, body, { sql, user, assertEditGuard, advanceEditGuard, validateOnly = false }) {
  ...
  // Internal composition option, never read from the HTTP body. Preview uses
  // the exact same locks, scope, baselines and extra-price checks as writing.
  if (validateOnly === true) return {
    validated: true,
    scope,
    detailCosts: plans.filter(plan => !plan.item.estimateKey).map(plan => ({
      sdetailKey: plan.item.sdetailKey, cost: plan.item.cost,
    })),
    intent: { requestedYear, custKey, mode, week, items },
  };
