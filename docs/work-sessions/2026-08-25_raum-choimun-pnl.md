...
- 차수 상세에 **전산 매칭** 열을 보여 도착원가 매입단가 기준 품목을 확인한다. 저장본은 `WebRaumPnlItem.ProdKey`로 `Product`를 읽어 품목명을 붙인다.
...
### 3. 2026-08-25 — 품목 매칭이 안 보임

**Q.** 라움도 초이문도 품목 매칭 결과가 안 보인다. 보여야 매입단가가 맞는지 안다.

**A.** 저장본을 다시 열면 `ProdKey`만 있고 전산 품목명이 없어서 전부 미매칭처럼 보였다. 매칭 이름도 툴팁에만 있었다. 상세에 `전산 매칭` 열을 넣고, 저장본은 Product JOIN으로 품목명을 붙였다. 업로드 직후에는 첫 차수 상세를 열어 매칭·매입단가를 바로 보게 했다.

**결과.** `lib/raumPnlMatchDisplay.js`, `loadRaumPnlDetail` JOIN Product, 목록에 미매칭 건수.

## 미완 / 다음 세션

- Cafe24 배포 후 라움/초이문 차수 상세에서 전산 매칭 열·매입단가 확인.
