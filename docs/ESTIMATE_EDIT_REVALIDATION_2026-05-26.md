...
- `nenova.exe` 작업 방식처럼 확정된 차수는 먼저 확정취소한 뒤 수정해야 한다.
- 단가 변경 API는 확정 차수를 만나면 `FIXED_WEEK`로 차단한다.
- 사용자는 확정현황에서 필요한 구간을 먼저 확정취소하고, 단가/수량 수정 후 낮은 차수부터 다시 확정해야 한다.
- 미확정 차수에서는 `ShipmentDetail.Cost`, `Amount`, `Vat`만 단일 트랜잭션으로 수정한다.
