...
-- 2026-08-20 dump BEFORE week-remain remain-check patch.
-- Restore only by replacing the body of dbo.usp_ShipmentFix (do not CREATE if it already exists).
-- =============================================
-- Author:		<우리비엔씨 이청솔>
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CheckCnt = 0;
		WITH stock
		 AS ( SELECT ps.ProdKey,
					 ps.Stock,
					 sm.OrderYearWeek
				FROM StockMaster sm
				JOIN   ProductStock ps
				  ON sm.StockKey = ps.StockKey )
	   SELECT @CheckCnt = COUNT(*)
		 FROM (select OrderYearWeek2, ProdKey, Sum(OutQuantity) OutQuantity from #ShipmentList
		 group by OrderYearWeek2, ProdKey) sl
		 JOIN      stock ns
		   ON sl.OrderYearWeek2 = ns.OrderYearWeek
			  AND sl.ProdKey = ns.ProdKey
