...
### 2026-06-03 영업매출관리 페이지 기획

- 요청: 네노바웹에 `영업매출관리` 메뉴를 추가해 이카운트 API에서 판매현황 데이터를 가져오고, 네노바웹 비교용 DB에 입력/저장한 뒤 업체별/차수별 매출을 2024년, 2025년, 2026년으로 비교할 수 있게 기획.
- 확인한 샘플:
  - `C:/Users/USER/Documents/카카오톡 받은 파일/25년 24차_양재동.xlsx`
  - `C:/Users/USER/Documents/카카오톡 받은 파일/매출비교.xlsx`
- 확인 내용:
  - 샘플 엑셀은 API 연동 전 화면/컬럼/매칭 기준을 파악하기 위한 참고자료이며, 운영 기본 원본은 이카운트 API 조회 결과.
  - 이카운트 판매현황 원본 구조는 `거래처명`, `품목명`, `수량`, `단가(vat포함)`, `공급가액`, `부가세`, `합계`, `적요`.
  - `매출비교.xlsx`는 네노바 통용 업체명 기준으로 차수별/연도별 매출을 비교하는 실무표 참고 구조.
  - 이카운트 사업자명과 실제 통용명이 달라서 원본 매출을 바로 비교하지 않고 거래처 매칭/분배 단계를 둬야 함.
- 기획:
  - 메뉴 위치는 `채권관리 > 판매현황` 바로 아래 `영업매출관리`, 경로는 `/sales/revenue-management`.
  - 1단계부터 `이카운트 API 조회 및 저장` 기반으로 구현한다.
  - 화면 로딩 시 자동 호출하지 않고, 사용자가 버튼을 누를 때만 read-only API를 호출한다.
  - 같은 연도/차수/기간은 저장된 Batch를 우선 보여주고, 강제 재조회는 별도 버튼으로 분리한다.
  - 매칭은 기존 주문등록 업체 매칭 구조를 재사용하되, 이카운트 거래처명 기준 `N:1`, `1:1`, 필요 시 수동 `1:N 분배`를 지원.
  - 자동 매칭은 후보 제시까지만 하고, 확정 저장은 사용자가 선택한 뒤에만 한다.
  - 이카운트 API 데이터를 네노바웹 비교용 테이블에 입력/저장하지만, 이카운트 원본 쓰기/변경/push는 하지 않는다.
- 결과 문서:
  - `docs/SALES_REVENUE_MANAGEMENT_PLAN_2026-06-03.md`

### 2026-06-03 영업매출관리 메뉴/페이지 1차 추가

- 요청: 웹에 배포가 안 되는 것 같다고 확인 요청.
- 확인:
  - 운영 `/api/dev/git-log?type=log`가 최신 문서 커밋 `277799c`를 보고 있어 배포 자체는 정상.
  - 화면에 안 보인 이유는 이전 작업이 문서 기획만 수정했고 실제 메뉴/페이지 코드를 추가하지 않았기 때문.
- 조치:
  - `채권관리` 그룹에서 `판매현황` 바로 아래 `영업매출관리` 메뉴 추가.
  - `/sales/revenue-management` 페이지 추가.
  - 페이지는 `이카운트 API 조회 및 저장 -> 원본 저장 -> 업체 매칭 -> 매출 비교` 흐름을 표시.
  - 현재 1차 화면은 운영 데이터 쓰기 없음. 이카운트 read-only API 조회/저장 API 연결은 다음 구현 단계.
- 검증:
  - 번들 Node로 `next build` 통과.

### 2026-06-03 영업매출관리 엑셀형 업체 목록/매칭 저장 보정

- 요청:
  - 화면이 `매출비교.xlsx`처럼 보여야 하는데 업체가 너무 소수로 나옴.
  - 미매칭 업체는 검색해서 최초 매칭해두면 이후 동일하게 적용되게 해야 함.
- 확인:
  - 기존 1차 화면은 배포 확인용 샘플 4개 업체만 하드코딩되어 있어 실무표와 다름.
  - `매출비교.xlsx` 6월 시트는 업체 행이 `미우`, `소재2호`, `그린`, `꽃길`, `알파`, `미카엘`, `송우`, `스타일`, `녹색` 등 전체 업체 목록 기준.
- 조치:
  - `/sales/revenue-management` 표를 엑셀형 전체 업체 목록 기준으로 재구성.
  - 22차/23차는 `매출비교.xlsx` 샘플 구조를 반영하고, 24차는 `25년 24차_양재동.xlsx` 판매현황 원본 샘플의 23개 이카운트 거래처 금액을 반영.
  - 이카운트 원본 거래처명별 매칭 상태 패널 추가.
  - 미매칭/후보 업체 선택 -> 네노바 거래처 검색 -> 통용명 입력 -> `이 매칭 확정 저장` 흐름 추가.
  - 새 API `/api/sales/revenue-customer-mappings` 추가.
  - 새 유틸 `lib/salesRevenueMappings.js` 추가.
  - 저장 위치는 `data/sales-revenue-customer-mappings.json`. 주문/출고/재고/ECOUNT 원본 DB는 수정하지 않음.
- 기준:
  - 저장된 이카운트 거래처명 매칭은 다음 조회부터 자동 적용.
  - 내장 후보보다 사용자가 확정 저장한 매칭을 우선 적용.
- 검증:
  - 번들 Node로 `next build` 통과.

### 2026-06-03 영업매출관리 Claude 인계서 생성

- 요청: 이 작업을 Claude에서 이어서 할 수 있게 준비.
- 조치:
  - `docs/CLAUDE_HANDOFF_SALES_REVENUE_MANAGEMENT_2026-06-03.md` 생성.
  - 현재 운영 커밋, 배포 상태, 관련 파일, 샘플 엑셀 구조, 매칭 저장 구조, 다음 구현 단계, ECOUNT 금지 작업을 정리.
  - Claude에 바로 붙여넣을 수 있는 프롬프트 포함.
- 핵심 기준:
  - 영업매출관리는 엑셀 업로드 기반이 아니라 ECOUNT API read-only 조회 기반.
...
---
# 2026-08-31 네덜란드 물량표 엑셀 PIVOT 양식 일치

- 네덜란드 물량표의 단가 엑셀 저장에서 별도 `NL_단가표` 요약 시트 생성을 제거.
- `PIVOT 통계 → 물량표 엑셀 다운로드`의 네덜란드 원본 workbook을 그대로 사용하고 각 업체 수량 열 오른쪽에 단가 열 하나만 추가.
- 원본 품목·칼라·업체·요약·농장 열, 셀 서식, 행 높이, 열 너비를 보존하고 주문 합계 수식에서는 새 단가 열을 제외.
- 직접 조회도 `/api/stats/pivot-volume-excel`에서 동일 원본 workbook을 받아 단가를 입력하며 ERP 원장은 읽기 전용으로 보존.
