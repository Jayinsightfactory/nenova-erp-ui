...
- #526 자동검사48초 성공, 운영 ec13ef34. Cafe24 34083045020 전체 성공(2분35초, 기본 브라우저 검사 포함).
- 새 로그인 화면에서 그라시오사 → Anthurium Graciosa 15cm #181 등 실제 후보 반환 확인. 수정 전 동일 입력0건이 수정 후 표시됐다. 영문 Graciosa도 동일 품목 후보 정상.
- console error/warn 0건. CSS viewport1920×1080에서 검색창 모달620×720, x650/y180으로 화면 안에 표시. 기존 브라우저 확대율80%를 보정해 CSS크기를 맞췄으며 100% 확대 자체를 확인했다고 주장하지 않는다. 임시viewport 원복.
- 한글 IME 조합 이벤트는 실행형 코드검사로 확인했으며 실제 OS 한글 조합키 입력 재현은 수행하지 않았다. 브라우저 시험은 확정된 문자열 입력+검색 버튼 사용.
- 검색만 수행했다. 실제 연결·해제·단가저장 및 ERP원장 쓰기 없음.
