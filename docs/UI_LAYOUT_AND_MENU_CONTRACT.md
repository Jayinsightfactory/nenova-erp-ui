...
- 이 순서는 일부 메뉴의 예외가 아니라 `MENU_ITEMS`의 모든 메뉴에 적용합니다. 페이지가 전용 내부 복귀 이벤트를 제공하지 않아도 공통 화면틀이 사용자 클릭·입력·선택 이후의 첫 뒤로가기를 같은 메뉴의 새 초기 상태로 다시 열고, 그 다음 뒤로가기부터 메뉴 이동 이력을 사용합니다. URL로 상세 상태가 전달된 경우 `popup=1`만 보존하고 상세 쿼리를 제거합니다.
