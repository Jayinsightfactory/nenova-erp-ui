...
- 팝업/자식창에서 다른 메뉴를 누르면 새 창을 다시 만들지 않고 현재 창에서 이동하며, 메뉴 이동 기록을 남깁니다.
- 뒤로가기는 페이지 내부의 상세·입력 단계가 열려 있으면 먼저 해당 메뉴의 초기 목록·선택 화면으로 돌아갑니다. 페이지가 이미 초기 상태일 때만 같은 창의 직전 메뉴로 돌아갑니다. 기록이 없는 자식창도 닫지 않고 대시보드로 이동합니다. 일반 창은 브라우저 이력을 우선 사용합니다. 저장 중인 페이지의 기존 `beforeunload` 확인은 그대로 존중합니다.
- 내부 복귀가 필요한 페이지는 취소 가능한 `nenova:menu-back-request` 이벤트를 수신하고 `preventDefault()`한 뒤 자체 상태만 초기화합니다. 진행 중인 저장은 막고, 저장하지 않은 상세·미리보기는 확인 후 닫으며, 이 동작 자체는 API나 ERP 원장을 변경하지 않습니다.
