...
동일 호텔 자동 연결 옵션(`applySameHotel=true`)은 선택 연도 신라의 정확한 원본 품목명·단위(공백만 정규화) 그룹으로 범위를 확대한다. 다른 호텔·연도·단위·수동행은 제외하며, 다른 기존 ProdKey 충돌은 전체 취소한다. 한 행 재선택도 그룹의 미연결 행을 채운다. 그룹 해제는 같은 그룹의 연결을 함께 비운다. 이후 신라 업로드는 해당 연도 호텔에서 유일한 활성 품목 연결만 재사용하고 미리보기와 저장에서 동일 결과·지문을 다시 검증한다. 신규 테이블/전역 별칭/매입단가 학습은 추가하지 않는다. 옵션 생략/false는 기존 단일행 계약이다.
