...
## 2026-05-26 붙여넣기 주문등록 / 차수피벗 보완

요청:

- 붙여넣기 주문등록에서 `21차 호주 추가 / 소재2호 / 반커부쉬 5박스 / 코알라펀 1박스` 입력 시 고객명이 `소재2호`로 잡혀야 한다.
- `/shipment/week-pivot?popup=1`, `/shipment/week-pivot?weekFrom=2026-21-01&weekTo=2026-21-01`에서 차수 입력값이 잘리지 않아야 한다.
- 차수피벗 엑셀 다운로드는 저장 위치 선택창 없이 바로 다운로드되게 한다.

확인/수정:

- `pages/api/orders/parse-paste.js`의 서버 파서가 `21-1` 형식뿐 아니라 `21차` 형식도 `21-01` 차수로 인식하도록 보완했다.
- `21차 호주 추가` 같은 줄은 품목 주문 줄이 아니라 섹션 헤더로 처리되게 하여, 다음 일반 텍스트 줄 `소재2호`가 고객명으로 유지되도록 했다.
- `pages/orders/paste.js`의 클라이언트 미리보기 파서도 같은 규칙을 적용했다. 호주/루스커스/레몬잎/호접/덴파레/리시안셔스/튤립 문맥도 클라이언트에서 인식하도록 맞췄다.
- `pages/shipment/week-pivot.js`의 차수 입력 폭을 `2026-21-01` 전체가 보이도록 116px로 확대했다.
- 차수피벗 엑셀 생성은 `showSaveFilePicker`를 사용하지 않고 `XLSX.writeFile`로 즉시 다운로드되게 변경했다. 웹 브라우저 보안상 로컬 Excel 앱 강제 실행은 불가하므로, 다운로드 후 자동 열림은 브라우저/OS 설정을 따른다.

검증:

- 정규식 샘플 검증: `21차 호주 추가` -> `21-01`, 헤더 처리 true.
- `next build` 성공.

## 2026-05-26 엑셀 출력 전 자연어 매칭 정리

문제:

- 차수피벗 엑셀에서 보이는 품목 오류는 엑셀 생성 자체보다, 붙여넣기 주문등록 단계에서 카톡식 자연어가 잘못 품목 매칭된 결과가 누적되어 발생한다.
- `data/order-mappings.json`에 과거 자동학습으로 저장된 카톡 메모성 문장들이 남아 있었다. 예: `오늘 출고`, `총`, `재고`, `창고`, `<>`, `->`, 복수 국가명이 섞인 키.

수정:

- 수동 매핑은 유지하고, `auto=true`인 noisy 매핑 108건을 `data/order-mappings-fallback-suspects.json`으로 격리했다.
- 남은 정상 매핑은 320건, 격리 후보는 279건이다.
- `lib/parseMappings.js`에서 fuzzy 매칭 시 국가 문맥이 충돌하는 저장 키를 후보에서 제외하도록 보강했다.
- `콜롬비아 태국`처럼 국가/품목군 문맥만 남은 입력은 품목으로 자동 매칭하지 않도록 했다.
...
## 2026-05-26 숫자 포함 업체명 파싱 보완

문제:

- `21차 호주 추가 / 소재2호 / 반커부쉬 5박스 / 코알라펀 1박스` 입력에서 `소재2호`가 고객명으로 분류되지 않았다.
- 원인은 고객명 판별 조건이 숫자를 포함한 줄을 고객명에서 제외했기 때문이다. 실제 업체명에는 `소재2호`, `2층미우`, `친구5125`처럼 숫자가 포함될 수 있다.

수정:

- 서버 파서 `parseNaturalSectionOrders`와 클라이언트 미리보기 파서 모두 `isNaturalCustomerLine`을 추가했다.
- 숫자가 있어도 `수량+단위`가 없고, `추가/취소`, 괄호/화살표, 차수 단독 줄이 아니면 고객명으로 인정한다.

검증:

- `21차 호주 추가` -> 차수 헤더, 고객명 아님
- `소재2호` -> 고객명
- `반커부쉬 5박스` -> 품목/수량 줄
- `코알라펀 1박스` -> 품목/수량 줄
- `2층미우`, `친구5125` -> 고객명

## 2026-05-26 차수피벗 세부 품목 매칭 보완

문제:

- 차수피벗/붙여넣기 주문등록에서 “품목 매칭”은 느슨한 품종명이 아니라 실제 ERP `Product`의 세부 품목이어야 한다.
- 예를 들어 `진다스위트 피치 L` 입력은 `진다 스윗트` 같은 일반 별칭이나 `SPRAY ROSE Sweety` 계열이 아니라 `Den. Jinda Sweet (피치) L`처럼 품종, 색상, 규격까지 맞아야 한다.
- 기존 토큰화는 `L`, `M` 같은 1글자 규격 토큰을 버려서 `L/M/XL` 구분이 약했다.

수정:

- `lib/displayName.js`에 `진다스위트`, `진다 스위트`, `진다스윗`, `진다 스윗트` 계열을 `JINDA SWEET`로 연결했다.
- `L/M/S/XL/2L/LL` 규격 토큰은 1글자여도 매칭 토큰으로 유지한다.
- 입력에 명시 규격이 있으면, 후보 품목명에 다른 규격이 명시된 품목은 점수 0으로 제외한다. 예: `피치 L`은 `피치 XL`, `피치 M`을 제외.
- 입력에 `진다/Jinda`가 있으면 후보 품목에도 `Jinda/진다`가 있어야 하므로, `Sweety` 같은 다른 품목의 유사도 오매칭을 차단한다.
- `pages/api/orders/parse-paste.js` 사전 필터에도 `태국`, `진다스위트` 계열 키워드를 추가했다.
...
- 비고/수정내역에 내부 드라이버명 `.Net SqlClient Data Provider`가 노출되던 기존 데이터는 화면/엑셀 표시 단계에서 해당 출고 상세/마스터의 사용자명 또는 계정명으로 치환한다.
- 새로 생성되는 출고수량 수정 비고는 `[시간] [사용자명] 내용` 형식으로 저장되게 했다.
- 차수피벗 셀 편집으로 생성되는 비고는 `[사용자명]1>2(+1)`처럼 시간/내부문구 없이 변경 전후 수량과 증감만 저장한다.
