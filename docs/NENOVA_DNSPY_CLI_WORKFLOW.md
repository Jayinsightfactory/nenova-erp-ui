...
## 2-1. 후보 조회와 저장 검증의 범위 일치

dnSpy에서 후보 목록을 만드는 SQL과 저장 이벤트 내부의 유효성 검사를 각각
확인한다. `FormShipmentDistribution` 농장 후보는 `ViewWarehouse` 전체 이력에서
`ProdKey`만 제한하므로, 웹의 모달 GET과 POST/트랜잭션 검증도
`lib/shipmentFarmCandidates.js`의 동일 scope를 사용해야 한다. 한쪽에만
`OrderYear/OrderWeek`를 추가하면 화면에는 보이는 후보가 저장 단계에서 거부된다.

후보 범위가 넓더라도 실제 출고 대상 `ShipmentMaster/ShipmentDetail`을 찾는
업무키에는 여전히 `OrderYear + OrderWeek + CustKey + ProdKey`를 적용한다.

## 2-2. downstream read-only 검증

주문·분배 변경 뒤에는 저장 테이블만 보지 않고 EXE가 읽는 결과를 확인한다.

- 견적: `ViewShipment + ViewOrder + ShipmentDate + PeriodDay + DetailFix=1` 실제 조인과 확정 조건
- 매출: `ShipmentDetail.Amount/Vat`, `isFix=1`, 대상 차수 집계
- farm-only: `ShipmentFarm`/`ShipmentDetail.Descr` 외 `OutQuantity`, 금액, 날짜,
  `Estimate`, `WebProfitReport` 보존

결과는 작업 보고서에 남기고, 소스 계약은 `__tests__/shipmentDownstreamImpactContract.test.js`
로 자동검사한다.
