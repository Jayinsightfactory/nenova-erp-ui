...
- 확정 SP가 실패하면 `skipStockCalc`여도 재계산 후 재시도한다. 확정 취소 뒤 스냅샷이
  기말(출고를 이미 뺀 값)으로 남은 채 옛 `usp_ShipmentFix`가 출고를 한 번 더 빼던 경우를
  웹에서 복구한다. 2026-08-20부터는 전산 SP 잔량 검사 자체가 leftover−미확정 출고라서
  nenova.exe 확정 버튼도 같은 이중차감으로 막히지 않는다.
