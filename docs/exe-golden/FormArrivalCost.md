...
- 시트에 적힌 GW(Gross Weight)·CW(Chargeable Weight)를 행에 저장한다. 목록 특별기준은 콜롬비아에만 적용한다. 콜롬비아에서 CW>GW이면 장미만이고 안내는 `콜롬비아장미 면 활성화`, CW≤GW이면 카네이션·알스트로를 보여 주고 안내는 `콜롬비아카네이션·알스트로 면 활성화`이며 장미는 숨긴다. 품종 버튼에도 같은 활성화를 붙인다. 다른 국가는 CW/GW와 관계없이 그대로 보여 준다. 웹 원장 표시 필터이며 원본 행을 지우지 않는다.
- 업로드 저장은 같은 연도·차수·국가만 `IsCurrent=0`으로 바꾸고(집합 SUPERSEDE), 같은 차수의 국가 공란 현재본도 함께 내린다. 새 행은 40건씩 `WebArrivalCostLine`에 INSERT한다. 행마다 전체 JSON 이력을 넣지 않는다. 매칭·배분기준 변경 이력은 기존처럼 행 단위다.
- 시트 머리에 COLOMBIA가 없는 `16-1B` 같은 분할 시트는 같은 차수·같은 파일의 국가를 이어받는다.
