...
- E/F 수량은 EXE가 계산한 마지막 ProductStock를 사용한다. 모든 국가·품종의 기말상품재고액은 `(Q×R + S×R + H) ÷ 당주 매입수량 × ProductStock 재고수량`이며 H에는 단당 통관비(엑셀 Q54)가 들어간다. 그 차수 매입이 없으면 직전 재고단가×기말수량으로 재고가 매입된 시점 환율을 유지한다. 재고수량이 0이거나 재고잔량에 없어도 기말상품재고액이 적혀 있으면 그 금액을 실제 재고로 넣는다. 근거가 없으면 `INPUT_REQUIRED`/`UNVERIFIED`이며 `Product.Cost`, 최근 입고단가, 수국 하드코딩, E 최종값 직접입력으로 대체하지 않는다. H/R/S는 입고 GW/CW·BILL 시점 환율·포워딩 원천에서 자동 계산하고, 외부 인보이스 확정값만 sourceRef·기준일·확정자·확정시각과 함께 웹 전용 원장에 저장한다. 수량 없는 F 선언 금액도 같은 웹 전용 경로다. 비고도 같은 웹 전용 보고서 저장 경로를 사용한다. 엑셀 베트남 ±4,576,000원 이익 이동은 메모이며 보고서 J에 반영하지 않는다.
...
- 기초재고 E는 현재 workbook의 E 셀을 복사하지 않고 직전 대차수 F를 위와 같은 공식·원천으로 다시 계산한다. 모든 국가·품종 줄에서 이번 차수 E는 전차수 F와 같아야 한다(01차는 전년도 52차). 전차수가 확정되어 있으면 그 확정 F를 쓰고, 아니면 전차수 F를 동일 공식으로 재현한다. 따라서 원본 26차 F와 27차 E 사이의 베트남 3,658,862.88875원 불연속은 `WORKBOOK_ANOMALY`로 기록하지만 웹 계산에는 자동 이월하지 않는다.
...
- **추천값**(`lib/colombiaTruck.js deriveTruckPlan()`): 1차+2차 Gross Weight를 합산한다. 5t 묶음을 먼저 배정하고 잔여가 1t 이하면 1t 1대, 2.5t 이하면 2.5t 1대, 2.5t 초과~5t면 5t 1대다. 예: 1,371kg → 2.5t 1대, 3,342kg → 5t 1대 275,000원, 6,000kg → 5t 1대+1t 1대다. 중량이 없으면 추천값을 만들지 않는다.
...
2. **차량 자동추천 = 합산 중량의 차량 등급, 실제값이 항상 우선**: `lib/colombiaTruck.js deriveTruckPlan()`이
   5t 묶음 뒤 잔여를 1t/2.5t/5t 등급으로 올린다(1.371t=2.5t, 3.342t=5t 275,000원). 저장된 실제 차량·비용(국가별
...
4. **기말상품재고액(F) 평가**: 모든 국가·품종에 원본 양식 공식을 쓴다. 그 차수 매입이 있으면
   `(매입액 + 그외통관비) ÷ 그 차수 매입수량 × 기말 재고수량`, 그 차수 매입이 없으면
   `직전 차수 재고단가 × 기말 재고수량`이라 첫 입고 시점 환율이 유지된다. 두 방법 모두 불가할 때만 정확한
   ...
   최종값 직접입력은 E에 금지하고, F는 재고수량이 없을 때만 선언 금액으로 받는다. 근거가 없으면 `INPUT_REQUIRED`/`UNVERIFIED`로 남긴다.
...
## 2026-08-19 운영 규칙 정렬 — 차량 등급·APOLLO 수국·재고 환율 고정

웹 전용 매출이익 보고서 계산만 바꾼다. `OrderDetail`/`ShipmentDetail`/`Estimate`/`ProductStock` ERP 원장은 쓰지 않는다.

1. **차량 추천 3.3t → 5T 275,000원**: `deriveTruckPlan()`이 2.5t 초과~5t 잔여를 5t 1대로 올린다. 과거 차수 저장 실제값·22~27차 엑셀 청구액은 그대로 우선한다.
2. **CW>GW이면 H도 CBM**: 콜롬비아 반차수 H/S가 같은 비율을 쓴다. APOLLO는 포워딩 업체이며 배분 기본값은 콜카장알루다. 그외통관비 반차수에서 콜카장알루수국을 고른 경우에만 수국 박스를 풀에 넣는다. 수국 전용 AWB는 나라 단위로 남긴다.
3. **기존재고 환율 고정**: 모든 국가·품종의 기말상품재고액은 원본 양식 공식 `(매입액+그외통관비)÷그 차수 매입수량 × 기말수량`이다. 그외통관비에는 단당 통관비(Q54)가 포함된다. **그 차수 매입이 없으면 직전 차수 재고단가를 그대로 쓴다** — 그래서 호주처럼 첫 입고 뒤 여러 차수에 걸쳐 재고로만 팔리는 경우 재고가 매입된 시점 환율이 유지되고, 새 입고가 생긴 차수에만 그때 환율로 재평가된다. 엑셀 29차가 매입이 있는데도 27차 1,068.23을 고정하거나, 31차가 매입 없이 O37을 바꾸는 것은 엑셀 오류이며 웹은 따르지 않는다. 이번 차수 기초재고는 전차수 기말재고와 같다.
4. **콜롬비아 배분 기준(2026-08-19 원본 대조 정정)**: 그외통관비는 **항상 무게비율**(원본 `H21=$C$17*M37`). 항공료만 원본 `F21=IF(CW=GW, 항공료×M37, 항공료×R37)`을 따라 과금중량이 총중량과 정확히 같을 때만 무게비율이고 다르면 CBM비율이다. 수국 박스당무게 5.6kg·박스당CBM 7은 2026 31차 원본 콜롬비아 1차 시트 `K41`/`P41` 실측값이다.

## 2026-08-20 사용자 확정 — 호주 환율·Q54·베트남 메모·수량 없는 F

웹 전용 계산만 바꾼다. ERP 원장은 쓰지 않는다.

1. **호주 환율**: 매입이 있는 차수만 그 차수 매입(과세)환율로 바꾼다. 재고판매만 있는 차수는 재고가 매입된 시점 환율을 유지한다. 엑셀 29차 O37을 27차 1,068.23에 고정하거나 31차 매입 없이 환율을 바꾼 값은 엑셀 오류다.
2. **Q54 단당 통관비**: 기말상품재고액 공식의 그외통관비(H)에 포함한다. 28·29차 엑셀이 빼 둔 것은 실수다.
3. **베트남 ±4,576,000원**: 실제 이익률을 보기 위한 엑셀 메모다. 보고서 매출이익(J)에는 이동하지 않고 비고에만 남긴다.
4. **수량 없는 기말상품재고액**: 재고잔량 수량이 없어도 금액이 있으면 실제 재고다. 2026년 31차 네덜란드 F12=2,923,273.166, 태국 F14=1,149,859.79, 중국 F15=1,813,712.94, 에콰도르 F16=3,485,942(파일 SHA `56a5fdd4…9c9556`).
