...
3. **기존재고 환율 고정**: 모든 국가·품종의 기말상품재고액은 원본 양식 공식 `(매입액+그외통관비)÷그 차수 매입수량 × 기말수량`이다. **그 차수 매입이 없으면 직전 차수 재고단가를 그대로 쓴다** — 그래서 호주처럼 첫 입고 뒤 여러 차수에 걸쳐 재고로만 팔리는 경우 첫 입고 시점 환율이 유지되고, 새 입고가 생긴 차수에만 그때 환율로 재평가된다. 이번 차수 기초재고는 전차수 기말재고와 같다.
