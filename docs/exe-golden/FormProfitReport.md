...
4. **기말재고(F) 층별 평가**: 모든 국가·품종은 기존재고를 전차수 금액(기존 환율)으로
   유지하고 이번 입고만 새 원가로 평가한다(FIFO). 콜롬비아·베트남 신규입고 원가는
   `(Q×R+S×R+H)÷매입수량`, 호주는 입고 시점 `구매단가×수량×선율과세환율(Q×R)`, 그 밖은
   이번 매입액 `Q×R+S×R`이다. 층별이 안 될 때만 정확한
...
## 2026-08-19 운영 규칙 정렬 — 차량 등급·APOLLO 수국·층별 재고환율
...
3. **기존재고 환율 고정**: 모든 국가·품종은 전차수 F 금액(기존 환율)을 기초로 가져오고, 이번 입고만 새 환율로 평가한 뒤 FIFO로 기말을 만든다. 콜롬비아 5품종·베트남 신규입고는 `(Q×R+S×R+H)÷매입수량`, 호주는 `Q×R`, 그 밖은 `Q×R+S×R`이다. 카테고리 평균으로 기말 전체 수량을 재평가하는 식은 콜롬비아·베트남 입고층에만 쓴다.
