...
4. **기말재고(F) 층별 평가**: 콜롬비아 5품종과 베트남, 호주는 기존재고를 전차수 금액(기존 환율)으로
   유지하고 이번 입고만 새 원가로 평가한다(FIFO). 콜롬비아·베트남 신규입고 원가는
   `(Q×R+S×R+H)÷매입수량`, 호주는 입고 시점 `구매단가×수량×선율과세환율(Q×R)`이다. 그 밖의 카테고리는 정확한
...
3. **기존재고 환율 고정**: 콜롬비아 5품종·베트남·호주는 전차수 F 금액(기존 환율)을 기초로 가져오고, 이번 입고만 새 환율로 평가한 뒤 FIFO로 기말을 만든다. 호주 신규입고는 `Q×R`(구매단가×수량×선율과세환율)이며 재고 판매 차수에 현재 환율을 다시 적용하지 않는다.
