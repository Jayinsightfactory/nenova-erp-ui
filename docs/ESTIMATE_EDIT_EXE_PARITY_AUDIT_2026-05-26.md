...
이 방식은 기존 카네이션/장미류와 맞는 경우가 많지만, 최근 확인된 알스트로처럼 실제 견적 표시 단위가 `스팀`이어야 하는 품목은 금액 계산 기준도 단가 단위와 함께 다시 검증해야 한다.

이번 작업에서는 출력 표시를 스팀 기준으로 맞췄지만, DB 단가가 스팀당 단가인지 단/묶음당 단가인지까지 확인하지 못했으므로 계산식을 바꾸지는 않았다.
...
2. 알스트로 단가가 스팀당인지, 박스/단 기준인지 운영 실제 견적서와 전산 화면으로 확인해야 한다.
