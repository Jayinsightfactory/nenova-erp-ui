...
### 2. 견적서 단가 수정은 확정 차수에서 차단한다

초기 구현은 관련 `ShipmentMaster`가 확정이면 트랜잭션 안에서 `isFix=0`으로 바꾼 뒤 `ShipmentDetail.Cost`, `Amount`, `Vat`를 직접 수정하고 다시 `isFix=1`로 돌렸다.

추가 검증 결과, 웹이 단가 수정 API 안에서 자동으로 `isFix`를 내렸다 올리는 방식은 전산의 구간 확정취소/재확정 절차와 맞지 않는다. 과거 차수 수정은 사용자가 필요한 구간을 먼저 확정취소하고, 수정 후 낮은 차수부터 다시 확정하는 흐름으로 진행해야 한다.

조치:

- `/api/estimate/update-cost`는 `ShipmentMaster`를 `UPDLOCK/HOLDLOCK`으로 확인한다.
- 확정 차수가 포함되어 있으면 `FIXED_WEEK`로 차단하고, 확정취소 후 다시 시도하라고 안내한다.
- 미확정 차수에서만 `ShipmentDetail.Cost`, `Amount`, `Vat`를 단일 트랜잭션으로 수정한다.
- 단가 수정 자체는 수량/재고/출고일분배를 변경하지 않는다.
