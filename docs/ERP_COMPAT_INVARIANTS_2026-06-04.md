...
- ✅ **update-cost ShipmentDetail.Descr 단가로그 누적** — **수정**. 분배 비고(ShipmentDetail.Descr)는 "담당자+수량변경 최신2"(lib/shipmentDescr) 전용. 단가로그 append 제거. 분배 비고 오염/무한증가 차단.
- ✅ **update-cost Estimate.Descr 차감단가 append** — **수정(2026-06-24)**. `Estimate.Descr` 에 단가 로그 미기록. 이력은 API `changes[]` + 조회 시 `sanitizeDescrTextForPrint`.
- 🚫 **`repair-negative-product-stock.js --apply`** — **운영 금지(2026-06-25)**. 유령재고 사고. → [STOCK_INTEGRITY_DESIGN.md](STOCK_INTEGRITY_DESIGN.md)
...
- ⏸ **Estimate.Descr 무한 append** (`update-quantity` 차감수량 등) — 화면/API sanitize 로 운영 로그는 숨김. DB append 는 잔존(추후 길이 cap 검토).
