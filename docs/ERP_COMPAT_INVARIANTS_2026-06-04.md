...
## 별도 검토 대상 (견적 도메인 — 이번 미수정, 추후)
- `pages/api/estimate/update-cost.js:273-286`: 단가 수정 시 ShipmentDate.EstQuantity fallback 이 `sdt.ShipmentQuantity` 까지 내려가 환산품목에서 Est=Out 강제 효과 가능(불변식 5). 견적 금액 로직이라 신중 검토 필요.
- `update-cost.js:181-271`, `update-quantity.js:110-117`: Estimate/ShipmentDetail.Descr 변경로그 무한 append(트리거 TR_ShipmentDetail_OutQty_Log 와 별개). 길이 cap 또는 ShipmentHistory 전용 기록 권장.
- `pages/api/orders/index.js:566` updateOrder: 클라이언트 manager 무검증 UPDATE — UI 가 UserName 보내면 위험. UserInfo 검증 권장.
- `pivotStats.js:90` StockMaster raw OrderYearWeek `< @yws`(full) — StockMaster 포맷에 따라 경계 불일치 가능(기존 이슈, 별개).
