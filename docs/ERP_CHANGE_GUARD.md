...
구두점 차수 경계를 허용하되 날짜/식별자 부분일치는 금지한다. 각 시트 차수를
우선하며 과거 숫자명 시트에 최신 파일명 차수를 덮지 않는다. Plantilla 템플릿 제외,
실제 값/수식 범위 추출과 SourceRow 보존을 arrivalThaiWeek.test.js로 검증한다.
원본 원가와 기존 저장 검증/ERP 원장 경계는 변경하지 않는다.

## 2026-09-22 선택 원가파일 즉시 처리

24시간 자동 예약은 유지한다. 관리자 확인 POST만 정확한 id/sha/year/week/country의 최신 후보 한 개를 즉시 실행한다. 파일 등록시각을 조작하거나 일반 업로드로 전체 과거 시트를 넣지 않는다. 같은 공통 scope/저장 코어에서 수동 현재본과 수정 이력·다른 파일군을 보호하며 중복은 기존 감사키를 반환한다. 응답 불명은 성공/실패로 단정하지 않고 상태를 조회한다. 실행형 회귀 `arrivalDriveRunNow.test.js` 및 기존 잠금/롤백 fixture 참조.

## 2026-09-22 호텔 원가 연결·복합단위

도착원가 정확명 비교에는 MEL/EZ/MC/국가/규격을 보존한다. 단-5스팀 등 명시 포장단위는 원본 단당 송이수로 참조 환산하며 단으로 단순 치환하지 않는다. 기존 원가/매입단가/ERP 원장은 보존. 운영 연결 보정은 원본 셀·유일 활성 품목·국가·수동 이력 부재를 확인한 현재연도 대상만 배포 후 감사와 함께 수정한다. 근거 불명 미연결은 자동 추정하지 않는다.

작성일: 2026-07-20

## 2026-09-22 Roselily Aisha 오매칭
부분일치 ROSE가 품종가점으로 정확한 백합 품명과 동점이 되어 배열 순서로 선택됐다. 도착원가 파서는 유일한 정확명을 먼저 찾고 중복 정확명은 보류한다. rose 단어 경계와 백합 힌트를 분리하고, 알려진 상이 품종의 fuzzy 후보를 제외한다. `arrivalCostExcel.test.js`에서 후보 순서 반전·정확 후보 누락·중복명 fixture를 실행한다. 보정은 배포 후 확인된 웹 원가11행의 품목키/품종만 감사와 함께 정정하고 수량·원가·농장 및 ERP 원장은 보존한다.
