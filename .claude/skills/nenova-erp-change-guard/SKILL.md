...
6. 기존 정책 함수를 재사용하되, 결합 API는 `mode`를 명시한다.
7. 기존 주문 있음/없음 × ADD/CANCEL과 전년도 동일 차수 fixture를 테스트한다.
8. `npm run test:nenova-dnspy-evidence`와 `npm run test:erp-contract`를 실행한다.
9. `npm run guard:erp-writes -- --changed-from <base-sha>`를 실행한다.
10. `npm run build`를 실행한다.
