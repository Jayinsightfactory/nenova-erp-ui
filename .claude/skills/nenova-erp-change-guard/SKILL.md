...
1. 사용자 동작을 만들기·증가·감소·보존 단위로 분해한다.
2. Order/Shipment/Stock 부작용 행렬을 작성한다.
3. `OrderYear + OrderWeek + CustKey + ProdKey`가 필요한 모든 조회·잠금·쓰기 경로를 추적한다.
4. 기존 정책 함수를 재사용하되, 결합 API는 `mode`를 명시한다.
5. 기존 주문 있음/없음 × ADD/CANCEL과 전년도 동일 차수 fixture를 테스트한다.
6. `npm run test:erp-contract`를 실행한다.
7. `npm run guard:erp-writes -- --changed-from <base-sha>`를 실행한다.
...
파일과 줄, 사용자 동작, 예상 부작용, 실제 부작용, 실행한 명령과 결과를 남긴다. 변경 범위 검사와 저장소 전체 기존 위반을 구분해 보고한다. 이 스킬은 운영 데이터 보정을 승인하지 않으며, 코드 검증과 데이터 보정은 별도 단계로 유지한다.
