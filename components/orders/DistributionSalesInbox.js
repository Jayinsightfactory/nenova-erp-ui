...
export default function DistributionSalesInbox({year,week,disabled,onLoadText}) {
  ...
  const [liveHistory,setLiveHistory]=useState({}),[liveHistoryStatus,setLiveHistoryStatus]=useState({loading:false,error:'',asOf:'',loaded:false,warnings:[]}),[liveHistoryPage,setLiveHistoryPage]=useState(0);
  ...
  async function refreshLiveHistory(scope=liveScope,messages=liveBatch) {
    ...
    try {
      ...
      if(!response.ok||data?.success!==true||data?.advisoryOnly!==true||data?.erpAction!=='NONE'||!validScope||!Array.isArray(data?.items)||!data.items.every(validItem)||!completeUniqueIdentitySet||!Array.isArray(data?.warnings)) throw new Error(liveHistoryError(data,'최신 전산 이력 응답 범위 또는 식별값이 올바르지 않습니다.'));
  ...
  function liveHistoryPanel(row) {
    ...
    if(!item) return <aside className="live-history-panel" aria-label="원문과 인접한 최신 전산 이력"><strong>최신 전산 이력</strong><p>{liveHistoryStatus.loading?'원문별 최신 전산 이력을 확인하는 중입니다.':liveHistoryStatus.error?'최신 전산 이력을 읽지 못했습니다. 기존 이력은 유지합니다.':liveHistoryStatus.loaded?'대응 이력 미확인':'원문을 불러오면 최신 전산 이력과 대조합니다.'}</p></aside>;
    return <aside className="live-history-panel" aria-label="원문과 인접한 최신 전산 이력"><div className="live-history-head"><strong>최신 전산 이력</strong><span className={`live-status live-status-${item.status||'UNKNOWN'}`}>{liveHistoryLabel(item)}</span></div><small>{item.reason||'읽기 전용 대조 결과입니다.'}{liveHistoryStatus.asOf?` · 기준 ${shortKstTime(liveHistoryStatus.asOf)}`:''}</small>{Array.isArray(item.requests)&&item.requests.length?item.requests.map((request,index)=><section className="live-request" key={`${request.id||'request'}:${index}`}><strong>{request.customerText||'업체 확인 필요'} · {request.productText||'품목 확인 필요'} · {request.qty??'?'} {request.unit||''}</strong>{request.quote&&<blockquote>{request.quote}</blockquote>}<small>{request.reason||item.reason||'연결 사유 확인 필요'}</small>{renderEvents('주문 변경 이력',request.orderEvents,'order')}{renderEvents('분배·출고 이력',request.shipmentEvents,'shipment')}</section>):<p>대응 이력 미확인</p>}</aside>;
  }
