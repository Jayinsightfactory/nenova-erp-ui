...
export default function DistributionSalesInbox({year,week,disabled,onLoadText}) {
  ...
  return <section className="sales-inbox" aria-label="영업방 대화 수신함">
    ...
    <div hidden={!open}>
      ...
      <div className="list compact-match-list"><div className="compact-match-column-head" aria-hidden="true"><span>카톡 변경 요청</span><span>대응 작업</span><span>매칭 상태</span></div>{visibleDisplayRows.map(compactMessageRow)}{!rows.length&&<p>{refreshStatus.loading?'영업방 대화를 확인하는 중입니다.':refreshStatus.error?'영업방 자동 확인에 실패했습니다. 다시 확인할 수 있습니다.':!loadedPeriod?'영업방 자동 확인을 기다리는 중입니다.':'현재 기간에 영업방 대화가 없습니다.'}</p>}{rows.length>0&&!visibleDisplayRows.length&&<p>이 분류의 대화가 없습니다.</p>}</div>
