...
export default function DistributionSalesInbox({year,week,disabled,onLoadText,evidenceMessages=[],evidenceOrders=[]}) {
  ...
  function balanceComparisonPanel(identity) {
    if(!hasAcceptedLiveHistoryScope)return null;
    const comparison=comparisonForIdentity(liveBalanceComparison,identity,{includeConsistent:true});
  ...
  function liveHistoryPanel(row) {
    const item=hasAcceptedLiveHistoryScope?liveHistory[row.identity]:null;
    ...
    if(!liveBatchIdentities.has(row.identity)) return <aside className="live-history-panel" aria-label="원문과 인접한 최신 전산 이력"><strong>최신 전산 이력</strong><p>아직 대조하지 않은 원문입니다.</p></aside>;
  ...
  return <section className="sales-inbox" aria-label="영업방 대화 수신함">
    ...
    <div hidden={!open}>
      ...
      <div className="list compact-match-list"><div className="compact-match-column-head" aria-hidden="true"><span>차수</span><span>업체</span><span>변경내역</span></div>{visibleDisplayRows.map(compactMessageRow)}{!rows.length&&<p>{refreshStatus.loading?'영업방 대화를 확인하는 중입니다.':refreshStatus.error?'영업방 자동 확인에 실패했습니다. 다시 확인할 수 있습니다.':!loadedPeriod?'영업방 자동 확인을 기다리는 중입니다.':'현재 기간에 영업방 대화가 없습니다.'}</p>}{rows.length>0&&!visibleDisplayRows.length&&<p>이 분류의 대화가 없습니다.</p>}</div>
    ...
    <style jsx global>{`.sales-inbox .compact-match-tabs{position:static}.sales-inbox .compact-match-column-head,.sales-inbox .compact-match-summary{grid-template-columns:minmax(0,1.45fr) minmax(0,1fr) minmax(120px,.58fr)}.sales-inbox .compact-match-column-head{display:grid;gap:7px;padding:3px 7px;background:#edf4f9;color:#526b82;font-size:10px;font-weight:600}.sales-inbox .compact-match-summary{gap:7px}.sales-inbox .compact-row-meta{display:block;margin-bottom:1px;color:#62798f}.sales-inbox .compact-message-summary,.sales-inbox .compact-operation-summary{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;white-space:normal;line-height:1.25}.sales-inbox .compact-match-status{min-width:0}.sales-inbox .compact-status-badges{display:flex;gap:3px;flex-wrap:wrap}.sales-inbox .compact-match-status .compact-automatic-badge,.sales-inbox .compact-match-status .compact-manual-badge{font-size:9px}@media(max-width:900px){.sales-inbox .compact-match-column-head,.sales-inbox .compact-match-summary{grid-template-columns:minmax(0,1.1fr) minmax(0,.85fr) minmax(94px,.6fr)}.sales-inbox .compact-operation-summary{display:-webkit-box}.sales-inbox .compact-match-summary{gap:4px;padding:4px 5px}.sales-inbox .compact-match-column-head{gap:4px;padding:3px 5px}}`}</style>
    <style jsx global>{`.sales-inbox .compact-message-summary{display:block;white-space:normal}.sales-inbox .compact-request-text{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;line-height:1.25}`}</style>
