...
export default function ShillaPurchaseCostInput({ item, cell, draft, onChange, disabled, unavailable, unavailableMessage, label = '신라' }) {
  if (!cell) return <div style={{ minWidth: 165, flex: '1 1 165px', padding: 4, color: unavailable ? '#b91c1c' : '#94a3b8', fontSize: 10 }}>{unavailable ? (unavailableMessage || `${label} 조회 실패`) : `${label} 자료 없음`}</div>;
