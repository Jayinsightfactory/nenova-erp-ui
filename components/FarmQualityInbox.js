...
export default function FarmQualityInbox({data,year,from,to,search='',focus,onDirty,onBusy,refresh}){
 ...
 return <section className="inbox" aria-label="통합 피드백 목록">
  ...
  <div className={`workspace ${selected?'expanded':''}`}><div className="rows"><div className="row-labels" aria-hidden="true"><span>농장 · 진행 상태 · 최신 코멘트</span><span>주요 품목 · 수량 / 비중 · 차수별 발생</span></div>{rows.map(item=>{const summary=summarizeQualityInbox(item);return <button disabled={busy} aria-pressed={selected?.key===item.key} className={`row ${selected?.key===item.key?'selected':''}`} key={item.key} onClick={()=>openItem(item)}>
  ...
  <style jsx>{`
   ...
   @media(max-width:760px){.workspace,.workspace.expanded{height:auto;grid-template-columns:minmax(0,1fr);min-height:0}.rows{max-height:420px}.row,.product-line{grid-template-columns:minmax(0,1fr)}aside{max-height:none}.filters label{margin-left:0;width:100%}.events{max-height:260px}}
  `}</style>
 </section>;
}
