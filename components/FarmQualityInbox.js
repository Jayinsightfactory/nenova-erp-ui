...
import {summarizeQualityInbox,feedbackStatusLabel,feedbackNeedsRequest,feedbackPriority} from '../lib/farmQualityInboxSummary';
...
export default function FarmQualityInbox({data,year,from,to,search='',focus,onDirty,onBusy,refresh}){
 ...
 return <section className="inbox" aria-label="통합 피드백 목록">
  ...
  <div className={`workspace ${selected?'expanded':''}`}><div className="rows"><div className="row-labels" aria-hidden="true"><span>농장 · 불량 요약</span><span>주요 품목 · 불량수량 비중</span><span>진행 상태 · 최신 코멘트</span></div>{rows.map(item=>{const summary=summarizeQualityInbox(item);return <button disabled={busy} aria-pressed={selected?.key===item.key} className={`row ${selected?.key===item.key?'selected':''}`} key={item.key} onClick={()=>openItem(item)}>
   <span className="summary-column"><strong>{summary.title}</strong><span>{summary.overview}</span><small>{summary.totalsText}</small><span className="badges">{summary.newCount>0&&<b>새 불량 {summary.newCount}</b>}{summary.reviewRequired&&<b>원본 확인 필요</b>}{!inboxActive(item)&&<b>제외됨</b>}</span></span>
   <span className="products-column">{summary.topGroups.map(group=><span className="product-group" key={group.unit}>{group.products.map(product=><span className="product-line" key={product.key}><span><b>{product.name}</b> · {productQuantity({...product,unit:group.unit})}{product.share!=null&&<small> · {count(product.share)}% (불량수량 비중)</small>}</span><small>{product.weekText}</small></span>)}{group.remaining>0&&<small>외 {group.remaining}개 품목 · 상세에서 전체 보기</small>}</span>)}{!summary.topGroups.length&&<small>현재 집계할 품목 없음 · 상세 이력 확인</small>}</span>
   <span className="progress-column"><span className="badges">{summary.statuses.map(state=><b className={`status-${state.code}`} key={state.code}>{state.label} {state.count}건</b>)}</span>{summary.latest?<span className="comment"><span className="latest-comment" title={summary.latest.Body}>{summary.latest.Body}</span><small>{summary.latest.AuthorName||'작성자 미상'}{commentDate(summary.latest.CreatedAt)&&` · ${commentDate(summary.latest.CreatedAt)}`}</small></span>:<small>아직 코멘트 없음</small>}</span>
