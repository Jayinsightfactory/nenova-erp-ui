...
import { createPivotPreferenceWriter, createPivotResizeSession, normalizeCollectivePivotWidths, pivotResizePreferenceKey, withCollectivePivotWidth } from '../lib/pivotExeInteraction';
...
const DEFAULT_ZONES = { rows: ['CounName', 'FlowerName', 'ProdName'], cols: ['OrderYear', 'OrderWeek', 'ListType', 'CustName'], values: [{ id: 'Quantity', aggregation: 'sum' }], filters: ['CountryFlower', 'CustArea', 'ShipmentDtm', 'UPrice', 'TPrice', 'OrderNo', 'CustDescr'] };
...
function FieldMenu({ field, aggregation, sort, popupRef, position, onAggregation, onClose, onMove, onHide, onSort, onBestFit, onFilter, onReorder }) {
  ...
  return <div ref={popupRef} data-testid="pivot-exe-field-menu" role="dialog" aria-label={`${field.label} 메뉴`} style={{...menuStyle,...position,visibility:position?'visible':'hidden'}} onClick={(event) => event.stopPropagation()}>
    ...
    {aggregation && <label style={{display:'block',fontSize:11,marginTop:7}}>집계 <select value={aggregation} onChange={(event)=>onAggregation(field.id,event.target.value)}>{field.numeric && <><option value="sum">합계</option><option value="avg">평균</option><option value="min">최소</option><option value="max">최대</option></>}<option value="count">개수</option></select></label>}
...
export default function PivotExePanel() {
  ...
  const applyView = useCallback((input) => {
    ...
    setWidths(normalizeCollectivePivotWidths(view.widths)); setRowHeight(view.rowHeight); setSorts(view.sorts);
  ...
  const currentView = useMemo(() => ({schemaVersion:1,zones,hidden,decimals,previousNonzeroDecimals,zeroVisible,widths,rowHeight,sorts,selections,ast,filterActive,showRowTotals,showColumnTotals,showGrandTotals}), [zones,hidden,decimals,previousNonzeroDecimals,zeroVisible,widths,rowHeight,sorts,selections,ast,filterActive,showRowTotals,showColumnTotals,showGrandTotals]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setRows(Array.isArray(payload.rows) ? payload.rows : []); setSuccessRange(payload.range || {...range}); setSource(payload.source || 'nenova.exe FormQuantityPivot');
  ...
  const zoneChip = (id, zone) => {
    ...
    return <span key={`${zone}-${id}`} draggable onDragStart={(event)=>{event.dataTransfer.setData('application/x-nenova-pivot-field',id);event.dataTransfer.effectAllowed='move';setDraggedField(id);}} onDragEnd={()=>setDraggedField(null)} title="끌어서 이동하거나 ⇄ 버튼으로 세로 행·가로 열·값·필터를 선택" style={{...chipStyle,cursor:'grab'}}><button data-testid={`pivot-exe-field-${id}`} data-zone={zone} type="button" onClick={(event) => openFieldMenu(id, event)}>{BY_ID[id].label}{sorts[id] === 'asc' ? ' ▲' : sorts[id] === 'desc' ? ' ▼' : ''}</button><button type="button" aria-label={`${BY_ID[id].label} 영역 이동`} title="영역 이동" style={moveChipButton} onClick={(event)=>openFieldMenu(id,event)}>⇄</button><button data-testid={`pivot-exe-filter-${id}`} type="button" title={`${BY_ID[id].label} 값 필터${active ? ' 적용됨' : ''}`} aria-label={`${BY_ID[id].label} 값 필터`} style={{...filterChipButton,...(active ? filterChipActive : null)}} onClick={(event) => openValueFilter(id, event)}>{active ? '●' : '⌄'}</button><small>{zone === 'values' ? zones.values.find((value)=>value.id===id)?.aggregation : ''}</small></span>;
  };
  ...
  return <main style={{padding:'8px 12px 14px',minWidth:0}}>
    ...
    <div style={{fontSize:11,color:'#52647a',margin:'6px 0 3px'}}>필드 버튼을 끌거나 <b>⇄</b>를 눌러 <b>세로 행 · 가로 열 · 값 · 필터</b> 사이에서 이동할 수 있습니다.</div>
