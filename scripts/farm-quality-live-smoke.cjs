...
(async()=>{
 ...
 assert.equal(r.status,200,'quality API');const data=await r.json();assert(data.success&&Array.isArray(data.groups)&&Array.isArray(data.cases)&&Array.isArray(data.farmTrends)&&Array.isArray(data.issueCandidates)&&Array.isArray(data.signals));
 assert(data.coverage&&Array.isArray(data.coverage.rows),'coverage rows');
 for(const field of ['sourceTotal','activeTotal','inRangeCount','outOfRangeCount','invalidWeekCount','deletedCount','eligibleCount','incompleteCount'])assert(Number.isFinite(Number(data.coverage[field])),`coverage ${field}`);
 assert.equal(Number(data.coverage.sourceTotal),Number(data.coverage.activeTotal)+Number(data.coverage.deletedCount),'coverage source conservation');
 assert.equal(Number(data.coverage.activeTotal),Number(data.coverage.inRangeCount)+Number(data.coverage.outOfRangeCount)+Number(data.coverage.invalidWeekCount),'coverage active conservation');
 assert(!JSON.stringify({coverage:data.coverage,signals:data.signals}).match(/CustKey|customerKey|customerIdentity/),'customer identifier leaked through quality API');
 console.log(JSON.stringify({liveRead:'passed',groups:data.groups.length,cases:data.cases.length,farmTrends:data.farmTrends.length,issueCandidates:data.issueCandidates.length,signals:data.signals.length,activeSources:data.coverage.activeTotal}));
 ...
 try{
  ...
  assert.equal(await page.$eval('main nav button[aria-pressed=true]',e=>e.textContent),'불량 분석');
  assert.equal(await page.$eval('.analysis-source-count',e=>e.textContent.trim()),`기존 불량 분석 · 농장·품목 ${data.groups.length}개`);
  assert(await page.$('[aria-label="불량 원본 범위"]'));
