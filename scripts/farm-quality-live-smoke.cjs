...
(async()=>{
 ...
 assert(!JSON.stringify(data.signals).match(/CustKey|CustName|customerIdentity/),'customer identity leaked through signal API');
 console.log(JSON.stringify({liveRead:'passed',groups:data.groups.length,cases:data.cases.length,farmTrends:data.farmTrends.length,issueCandidates:data.issueCandidates.length,signals:data.signals.length}));
