...
const files=fs.readdirSync(dir).filter(f=>/^distribution(?:Baseline|Change|Checklist|SalesInbox|Audit|ManualApplication|MessageApplicationStatus).*\.test\.js$/.test(f)||f==='pasteFourColumnLayout.test.js').sort();
